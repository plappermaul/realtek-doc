...
int32 do_rtdrv_get_ctl_time(int cmd, void *user, rtdrv_union_t *buf)
{
    ...
    /** TIME **/
        ...
        case RTDRV_TIME_PORTPTPFULLSEC_GET:
            rtk_copy_from(&buf->time_cfg, user, sizeof(rtdrv_timeCfg_t));
            ret = rtk_time_portPtpFullSec_get(buf->time_cfg.unit, buf->time_cfg.port, buf->time_cfg.partialSec, buf->time_cfg.bitLen, &buf->time_cfg.fullSec);
            rtk_copy_to(user, &buf->time_cfg, sizeof(rtdrv_timeCfg_t));
            break;
