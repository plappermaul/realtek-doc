...
int32 rt_util_flashEnv_get(char *name, char *pValue, int buffLen)
{
    ...
    kfree(env);
 #else
    if (RT_ERR_OK != (ret = rt_util_flashVar_get((char *)(SPI_FLASH_BASE + rtk_flash_partition_offset_get(FLASH_INDEX_LOADER_ENV)), rtk_flash_partition_size_get(FLASH_INDEX_LOADER_ENV), name, pValue, buffLen)))
        return ret;
 #endif
 #else
    if (RT_ERR_OK != (ret = rt_util_flashVar_mtd_get(CONFIG_ENV_MTD_INDEX, name, pValue, buffLen)))
        return ret;
 #endif
#else
    /* non-linux */
