...
static uint32 _enableType[] = {
    DISABLED,
    ENABLED,
    };
static uint32 _modeURPFCheck[] = {
    RTK_L3_URPF_MODE_LOOSE,
    RTK_L3_URPF_MODE_STRICT,
    };

static uint32 _modeIPMCBridge[] = {
    VLAN_BRIDGE_MODE_MAC_BASED,
...
do {                                                                                    \
    ...
    {                                                                                   \
        RT_ERR(_retval, (MOD_VLAN|MOD_DAL), _errMsg);                                     \
        goto _errHandle;                                                                \
    }                                                                                   \
} while(0)
...
dal_longan_vlan_profile_get(uint32 unit, uint32 idx, rtk_vlan_profile_t *pProfile)
{
    ...
    RT_ERR_HDL(reg_array_field_read(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_L2_LRN_ENf, &temp_var), errHandle, ret);
    ...
    /*routing fields */
    RT_ERR_HDL(reg_array_field_read(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP4_UC_RT_ENf, &temp_var), errHandle, ret);
    VLAN_VALUE_TO_ACTION(_enableType, pProfile->ip4_uRoute, temp_var, "", errHandle, ret);
    RT_ERR_HDL(reg_array_field_read(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP6_UC_RT_ENf, &temp_var), errHandle, ret);
    VLAN_VALUE_TO_ACTION(_enableType, pProfile->ip6_uRoute, temp_var, "", errHandle, ret);

    RT_ERR_HDL(reg_array_field_read(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP4_MC_RT_ENf, &temp_var), errHandle, ret);
    VLAN_VALUE_TO_ACTION(_enableType, pProfile->ip4_mRoute, temp_var, "", errHandle, ret);

    RT_ERR_HDL(reg_array_field_read(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP6_MC_RT_ENf, &temp_var), errHandle, ret);
    VLAN_VALUE_TO_ACTION(_enableType, pProfile->ip6_mRoute, temp_var, "", errHandle, ret);

    RT_ERR_HDL(reg_array_field_read(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP4_URPF_CHK_ENf, &temp_var), errHandle, ret);
    VLAN_VALUE_TO_ACTION(_enableType, pProfile->ip4_uRPF_chk, temp_var, "", errHandle, ret);

    RT_ERR_HDL(reg_array_field_read(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP6_URPF_CHK_ENf, &temp_var), errHandle, ret);
    VLAN_VALUE_TO_ACTION(_enableType, pProfile->ip6_uRPF_chk, temp_var, "", errHandle, ret);

    RT_ERR_HDL(reg_array_field_read(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP4_URPF_DFLT_ROUTE_ENf, &temp_var), errHandle, ret);
    VLAN_VALUE_TO_ACTION(_enableType, pProfile->ip4_uRPF_dfltRoute, temp_var, "", errHandle, ret);

    RT_ERR_HDL(reg_array_field_read(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP6_URPF_DFLT_ROUTE_ENf, &temp_var), errHandle, ret);
    VLAN_VALUE_TO_ACTION(_enableType, pProfile->ip6_uRPF_dfltRoute, temp_var, "", errHandle, ret);

    RT_ERR_HDL(reg_array_field_read(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP4_URPF_CHK_MODEf, &temp_var), errHandle, ret);
    VLAN_VALUE_TO_ACTION(_modeURPFCheck, pProfile->ip4_uRPF_chkMode, temp_var, "", errHandle, ret);

    RT_ERR_HDL(reg_array_field_read(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP6_URPF_CHK_MODEf, &temp_var), errHandle, ret);
    VLAN_VALUE_TO_ACTION(_modeURPFCheck, pProfile->ip6_uRPF_chkMode, temp_var, "", errHandle, ret);
    ...
    /*L2 Bridge Action*/
    RT_ERR_HDL(reg_array_field_read(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP4_MC_BDG_MODEf, &temp_var), errHandle, ret);
    VLAN_VALUE_TO_ACTION(_modeIPMCBridge, pProfile->ipmcBdgMode, temp_var, "", errHandle, ret);

    RT_ERR_HDL(reg_array_field_read(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP6_MC_BDG_MODEf, &temp_var), errHandle, ret);
    VLAN_VALUE_TO_ACTION(_modeIPMCBridge, pProfile->ip6mcBdgMode, temp_var, "", errHandle, ret);
...
dal_longan_vlan_profile_set(uint32 unit, uint32 idx, rtk_vlan_profile_t *pProfile)
{
    ...
    RT_ERR_HDL(reg_array_field_write(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_L2_NEW_SA_ACTf, &temp_var), errHandle, ret);
    /*routing fields */
    VLAN_ACTION_TO_VALUE(_enableType, temp_var, pProfile->ip4_uRoute, "", errHandle, ret);
    RT_ERR_HDL(reg_array_field_write(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP4_UC_RT_ENf, &temp_var), errHandle, ret);
    VLAN_ACTION_TO_VALUE(_enableType, temp_var, pProfile->ip6_uRoute, "", errHandle, ret);
    RT_ERR_HDL(reg_array_field_write(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP6_UC_RT_ENf, &temp_var), errHandle, ret);
    VLAN_ACTION_TO_VALUE(_enableType, temp_var, pProfile->ip4_mRoute, "", errHandle, ret);
    RT_ERR_HDL(reg_array_field_write(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP4_MC_RT_ENf, &temp_var), errHandle, ret);
    VLAN_ACTION_TO_VALUE(_enableType, temp_var, pProfile->ip6_mRoute, "", errHandle, ret);
    RT_ERR_HDL(reg_array_field_write(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP6_MC_RT_ENf, &temp_var), errHandle, ret);
    VLAN_ACTION_TO_VALUE(_enableType, temp_var, pProfile->ip4_uRPF_chk, "", errHandle, ret);
    RT_ERR_HDL(reg_array_field_write(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP4_URPF_CHK_ENf, &temp_var), errHandle, ret);
    VLAN_ACTION_TO_VALUE(_enableType, temp_var, pProfile->ip6_uRPF_chk, "", errHandle, ret);
    RT_ERR_HDL(reg_array_field_write(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP6_URPF_CHK_ENf, &temp_var), errHandle, ret);
    VLAN_ACTION_TO_VALUE(_enableType, temp_var, pProfile->ip4_uRPF_dfltRoute, "", errHandle, ret);
    RT_ERR_HDL(reg_array_field_write(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP4_URPF_DFLT_ROUTE_ENf, &temp_var), errHandle, ret);
    VLAN_ACTION_TO_VALUE(_enableType, temp_var, pProfile->ip6_uRPF_dfltRoute, "", errHandle, ret);
    RT_ERR_HDL(reg_array_field_write(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP6_URPF_DFLT_ROUTE_ENf, &temp_var), errHandle, ret);
    VLAN_ACTION_TO_VALUE(_modeURPFCheck, temp_var, pProfile->ip4_uRPF_chkMode, "", errHandle, ret);
    RT_ERR_HDL(reg_array_field_write(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP4_URPF_CHK_MODEf, &temp_var), errHandle, ret);
    VLAN_ACTION_TO_VALUE(_modeURPFCheck, temp_var, pProfile->ip6_uRPF_chkMode, "", errHandle, ret);
    RT_ERR_HDL(reg_array_field_write(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP6_URPF_CHK_MODEf, &temp_var), errHandle, ret);
    ...
    /*L2 Bridge Action*/
    VLAN_ACTION_TO_VALUE(_modeIPMCBridge, temp_var, pProfile->ipmcBdgMode, "", errHandle, ret);
    RT_ERR_HDL(reg_array_field_write(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP4_MC_BDG_MODEf, &temp_var), errHandle, ret);
    VLAN_ACTION_TO_VALUE(_modeIPMCBridge, temp_var, pProfile->ip6mcBdgMode, "", errHandle, ret);
    RT_ERR_HDL(reg_array_field_write(unit, LONGAN_VLAN_PROFILE_SETr, REG_ARRAY_INDEX_NONE, idx,\
                          LONGAN_IP6_MC_BDG_MODEf, &temp_var), errHandle, ret);
