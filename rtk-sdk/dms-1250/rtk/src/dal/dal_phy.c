...
dal_phy_portPtpTxTimestampFifo_get(uint32 unit, rtk_port_t port, rtk_time_txTimeEntry_t *pTimeEntry)
{
    ...
    if ((ret = dal_phy_time_portPtpFullSec_get(unit, port, pTimeEntry->txTime.sec, 8, &(pTimeEntry->txTime.sec))) != RT_ERR_OK)
    {
        RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PHY), "Error Code: 0x%X", ret);
        return ret;
    }
    ...
    uint32 bitLen, uint64 *pFullSec)
{
    ...
    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PHY), "unit=%d,port=%d,partialSec=%d,bitLen=%d", unit, port, partialSec, bitLen);
    ...
    /* function body */
    PHY_SEM_LOCK(unit);
    if ((ret = phy_ptpRefTime_get(unit, port, &curTime)) != RT_ERR_OK)
    {
        PHY_SEM_UNLOCK(unit);
        RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PHY), "Error Code: 0x%X", ret);
        return ret;
