...
int eth_initialize(bd_t *bis)
{
	...
	if (board_eth_init != __def_eth_init) {
		if (board_eth_init(bis) < 0)
			CAMEO_SIMPLE_DISPLAY_PRINTF(("Board Net Initialization Failed\n"));
	} else if (cpu_eth_init != __def_eth_init) {
		if (cpu_eth_init(bis) < 0)
			CAMEO_SIMPLE_DISPLAY_PRINTF(("CPU Net Initialization Failed\n"));
	} else
		CAMEO_SIMPLE_DISPLAY_PRINTF(("Net Initialization Skipped\n"));
	...
	if (!eth_devices) {
		...
		do {
			...
			CAMEO_SIMPLE_DISPLAY_PRINTF(("%s", dev->name));
