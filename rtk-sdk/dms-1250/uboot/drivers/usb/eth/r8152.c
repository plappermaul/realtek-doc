...
static int r8152_send_common(struct ueth_data *ueth, volatile void *packet, int length)
{
	struct usb_device *udev = ueth->pusb_dev;
	u32 opts1, opts2 = 0;
	int err;
	int actual_len;
	static ALLOC_CACHE_ALIGN_BUFFER(uint8_t, msg,
...
static int r8152_send(struct eth_device *eth, volatile void *packet, int length)
{
	struct ueth_data *dev = (struct ueth_data *)eth->priv;
...
{
	struct ueth_data *dev = (struct ueth_data *)eth->priv;
	static ALLOC_CACHE_ALIGN_BUFFER(uint8_t, recv_buf, RTL8152_AGG_BUF_SZ);
				...
				struct eth_device *eth)
{
	...
	/* Get the MAC address */
	if (r8152_read_mac(tp, eth->enetaddr) < 0)
		return 0;
