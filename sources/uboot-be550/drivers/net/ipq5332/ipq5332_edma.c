...
#include "../rtl8221b/rtl8226_typedef.h"
#include "../rtl8221b/nic_rtl8226b_init.h"
#include "../rtl8221b/nic_rtl8226b.h"
#endif

#ifdef CONFIG_TP_EXT_SWITCH
#ifdef CONFIG_TP_EXT_SWITCH_RTL8367_ALL
#include "../rtl8367_common_V1_4_2/chip.h"
#include "../rtl8367_common_V1_4_2/port.h"
#include "../rtl8367_common_V1_4_2/rtk_switch.h"
#include "../rtl8367_common_V1_4_2/vlan.h"
#include "../rtl8367_common_V1_4_2/dal/rtl8367d/dal_rtl8367d_port.h"
#include "../rtl8367_common_V1_4_2/dal/rtl8367d/rtl8367d_smi.h"
#include "../rtl8367_common_V1_4_2/dal/rtl8367d/rtl8367d_asicdrv.h"

switch_chip_t g_switch_chip = CHIP_END;

#endif
#ifdef CONFIG_TP_EXT_SWITCH_RTL8372
#include "../rtl8372/rtk_types.h"
#include "../rtl8372/rtk_error.h"
#include "../rtl8372/rtk_switch.h"
#include "../rtl8372/port.h"
#include "../rtl8372/dal/rtl8373/dal_rtl8373_switch.h"
#include "../rtl8372/dal/rtl8373/dal_rtl8373_port.h"
#include "../rtl8372/dal/rtl8373/dal_rtl8373_drv.h"
#endif
#endif

#ifdef CONFIG_TP_EXT_PHY_RTL8251B
#include "../rtl8251b/rtl8251b_typedef.h"
#include "../rtl8251b/nic_rtl8251b_init.h"
#include "../rtl8251b/nic_rtl8251b.h"
#endif
...
#define ERR_PRINT(ret) \
	do {	\
		if ( ret != SUCCESS ) printf("[rtl8221b] %s[%d]: error, return: %d. \n", __FUNCTION__, __LINE__, ret); \
	} while(0)

extern int ipq_mdio_read(int mii_id, int regnum, unsigned short *data);
extern int ipq_mdio_write(int mii_id, int regnum, unsigned short value);

int g_mdio_num = 7;

static int rtl8221b_mdio_write(unsigned short devad, int regnum, unsigned short val)
{
	int ret = 0;
	regnum = MII_ADDR_C45 | ((devad & 0x1f) << 16) | (regnum & 0xffff);

	ret = ipq_mdio_write(g_mdio_num, regnum, val);
	if (ret == -ETIMEDOUT)
	{
		printf("ipq_mdio_write ETIMEDOUT\n");
		return FAILURE;
	}
	return SUCCESS;
}

static int rtl8221b_mdio_read(unsigned short devad, int regnum, unsigned short *data)
{
	int ret = 0;
	regnum = MII_ADDR_C45 | (devad << 16) | (regnum & 0xffff);

	ret = ipq_mdio_read(g_mdio_num, regnum, data);
	if (ret == -ETIMEDOUT)
	{
		printf("ipq_mdio_read ETIMEDOUT\n");
		return FAILURE;
	}
	return SUCCESS;
}
...
{
	return rtl8221b_mdio_read(dev, (int)addr, data);
}

BOOLEAN
MmdPhyWrite( IN HANDLE hDevice, IN UINT16 dev, IN UINT16 addr, IN UINT16 data)
{
	return rtl8221b_mdio_write(dev, (int)addr, data);
}

static void rtl8221b_gpio_reset(int gpio_num)
{
...
static int status_get(void)
{
	UINT16 phydata = 0;
	int ret = FAILURE;

	ret = MmdPhyRead(g_handle, MMD_VEND2, 0xA434, &phydata);
	if (ret != SUCCESS)
	{
		printf("get rtl8221b phy 0xA434 failed\n");
	}
	else
	{
		printf("get rtl8221b phy 0xA434 phydata=0x%x\n", phydata);
	}

	ret = MmdPhyRead(g_handle, MMD_VEND1, 0x7580, &phydata);
	if (ret != SUCCESS)
	{
		printf("get rtl8221b phy 0x7580 failed\n");
	}
	else
	{
		printf("get rtl8221b phy 0x7580 phydata=0x%x\n", phydata);
	}

	ret = MmdPhyRead(g_handle, MMD_VEND1, 0x7582, &phydata);
	if (ret != SUCCESS)
	{
		printf("get rtl8221b phy 0x7582 failed\n");
	}
	else
	{
		printf("get rtl8221b phy 0x7582 phydata=0x%x\n", phydata);
	}

	ret = MmdPhyRead(g_handle, MMD_VEND1, 0x758D, &phydata);
	if (ret != SUCCESS)
	{
		printf("get rtl8221b phy 0x758D failed\n");
	}
	else
	{
		printf("get rtl8221b phy 0x758D phydata=0x%x\n", phydata);
	}

	ret = MmdPhyRead(g_handle, MMD_VEND1, 0x758C, &phydata);
	if (ret != SUCCESS)
	{
		printf("get rtl8221b phy 0x758C failed\n");
	}
	else
	{
		printf("get rtl8221b phy 0x758C phydata=0x%x\n", phydata);
	}

	ret = MmdPhyRead(g_handle, MMD_VEND1, 0x697A, &phydata);
	if (ret != SUCCESS)
	{
		printf("get rtl8221b phy 0x697A failed\n");
	}
	else
	{
		printf("get rtl8221b phy 0x697A phydata=0x%x\n", phydata);
	}

	return ret;
}

static int rtl8221b_init_common(void)
{
	BOOL ret;
	PHY_LINK_ABILITY phy_ability;
	PHY_EEE_ENABLE eee_enable;

	struct device_node *mdio;
	struct mii_bus *mdio_bus;

	ret = Rtl8226b_phy_init(g_handle, &phy_ability, TRUE);
	ERR_PRINT(ret);

	ret = Rtl8226b_serdes_autoNego_set(g_handle, FALSE);
	ERR_PRINT(ret);

	ret = Rtl8226b_serdes_option_set(g_handle, PHY_SERDES_OPTION_HiSGMII);
	ERR_PRINT(ret);

	ret = Rtl8226b_phy_reset(g_handle);
	ERR_PRINT(ret);
	...
	phy_ability.adv_2_5G = 1;
	phy_ability.FC = 1;
	phy_ability.AsyFC = 1;
	ret = Rtl8226b_autoNegoAbility_set(g_handle, &phy_ability);
	ERR_PRINT(ret);
	
	ret = Rtl8226b_autoNegoEnable_set(g_handle, TRUE);
	ERR_PRINT(ret);

	eee_enable.EEE_100 = 0;
	eee_enable.EEE_1000 = 0;
	eee_enable.EEE_2_5G = 0;
	ret = Rtl8226b_eeeEnable_set(g_handle, &eee_enable);
	ERR_PRINT(ret);

	ret = Rtl8226b_greenEnable_set(g_handle, FALSE);
	ERR_PRINT(ret);
	
	Rtl8226b_enable_set(g_handle, TRUE);
...
#ifdef CONFIG_TP_EXT_PHY_RTL8221B_LAN
static int rtl8221b_lan_init(int mdio_num)
{
	g_mdio_num = mdio_num;
	rtl8221b_init_common();
	return 0;
}
#endif

static int rtl8221b_wan_init(int mdio_num)
{
	g_mdio_num = mdio_num;
	rtl8221b_init_common();
	return 0;
...
static int rtl8221b_init(void)
{
#ifdef CONFIG_TP_EXT_PHY_RTL8221B_LAN
	rtl8221b_gpio_reset(43); 	//lan hardware reset
	printf("Init RTL8221, phy_reset_gpio 43 \n");
	rtl8221b_lan_init(6);
#endif

	rtl8221b_gpio_reset(39); 	//wan hardware reset
	printf("Init RTL8221, phy_reset_gpio 39 \n");
	rtl8221b_wan_init(7);

	return 0;
}

static int ipq_rtl8221_Link_Update(int mdio_num)
{
	BOOL status = FAILURE;
	BOOL linkOK = FALSE;
	...
	status = Rtl8226b_is_link(g_handle, &linkOK);
    if (status != 1)
    {
		printf("Rtl8226b_is_link error\n");
		return 1;
	}
	...
	if (!linkOK)
	{
		if (mdio_num == 7)
		{
			printf("RTL8221B WAN not link!!!\n");
		}
		else
		{
			printf("RTL8221B LAN not link!!!\n");
		}
		return 1;
	}
	...
	if (mdio_num == 7)
	{
		printf("RTL8221B WAN link!!!\n");
	}
	else
	{
		printf("RTL8221B LAN link!!!\n");
	}
...
#define RTL8211_STATUS_LINK_MASK	0x0004
#define RTL8211_STATUS_DUPLEX_MASK	0x0008
#define RTL8211_STATUS_SPEED_MASK	0x2040
#define RTL8211_STATUS_FULL_DUPLEX	0x2000

#define RTL8251B_LAN_MDIO_NUM	2
#define RTL8251B_WAN_MDIO_NUM	3
#define RTL8251B_LAN_RST_GPIO	45
#define RTL8251B_WAN_RST_GPIO	39
#define MII_ADDR_C45			(1<<30)

#define PROC_DIR_LAN			"rtl8251b_lan"
#define PROC_DIR_WAN			"rtl8251b_wan"

#define LOG_ERR(fmt, ...) \
	do { \
		printf("[rtl8251b] %s[%d]: "fmt, __func__, __LINE__, ##__VA_ARGS__); \
	} while(0)
#define ERR_PRINT(ret) \
	do {	\
		if ( ret != SUCCESS ) printf("[rtl8251b] %s[%d]: error, return. ", __FUNCTION__, __LINE__); \
	} while(0)
...
static int lw_flip = 0;
static int g_mdio_num = 0;

static int rtl8251b_mdio_write(unsigned short devad, int regnum, unsigned short val)
{
	int ret = 0;
	regnum = MII_ADDR_C45 | ((devad & 0x1f) << 16) | (regnum & 0xffff);
...
static int rtl8251b_mdio_read(unsigned short devad, int regnum, unsigned short *data)
{
	int ret = 0;
	regnum = MII_ADDR_C45 | (devad << 16) | (regnum & 0xffff);
...
static int rtl8211_phy_read_by_switch(uint32_t dev_id, uint32_t phy_id, uint16_t reg_id, uint16_t *reg_val)
{
	rtk_uint32 pvalue = 0;

	if (RT_ERR_OK != dal_rtl8373_phy_regbits_read(phy_id, dev_id, reg_id, 0xffff, &pvalue))
	{
		printf("rtk_rtl8373_phy_regbits_read failed!\n");
		return FAILURE;
	}

	*reg_val = (uint16_t)(pvalue & 0xffff);

	return SUCCESS;
}

static int rtl8211_phy_write_by_switch(uint32_t dev_id, uint32_t phy_id, uint16_t reg_id, uint16_t reg_val)
{
	if (RT_ERR_OK != dal_rtl8373_phy_regbits_write(1<<phy_id, dev_id, reg_id, 0xffff, reg_val))
	{
		printf("rtk_rtl8373_phy_regbits_write failed!\n");
		return FAILURE;
	}
...
#ifdef GE550V2_RTL8372_RTL8251B
	if (g_mdio_num == RTL8251B_WAN_MDIO_NUM)
	{
		ret = rtl8251b_mdio_read(dev, (int)addr, data);
	}
	else
	{
		if (SUCCESS != rtl8211_phy_read_by_switch(dev, 8, addr, data))
		{
			ret = FALSE;
		}
		else
...
MmdPhyWrite( IN HANDLE hDevice, IN UINT16 dev, IN UINT16 addr, IN UINT16 data)
{
	...
	if (g_mdio_num == RTL8251B_WAN_MDIO_NUM)
	{
		ret = rtl8251b_mdio_write(dev, addr, data);
	}
	else
	{
		if (SUCCESS != rtl8211_phy_write_by_switch(dev, 8, addr, data))
		{
			ret = FALSE;
		}
		else
...
static void rtl8251b_gpio_reset(int gpio_num)
{
	// int ret;
	// gpio_num += 447;
	// ret = gpio_request(gpio_num , "phy8251_reset");
	// if(ret < 0)
	// {
	// 	printf("gpio request failed, ret:%d\n", ret);
	// }
	gpio_direction_output(gpio_num, 1);
	mdelay(200);
	gpio_set_value(gpio_num, 0);
	...
	gpio_set_value(gpio_num, 1);
	mdelay(200);
	printf("GPIO%d reset switch done\n", gpio_num);
	// gpio_free(gpio_num);
	return;
}
...
static int
rtl8251_phy_get_status(uint32_t dev_id, uint32_t phy_id,
		struct port_phy_status *phy_status)
{
	...
	MmdPhyRead(g_handle, MMD_VEND2, 0xA434, &phydata);
	phy_status->link_status = (phydata & RTL8211_STATUS_LINK_MASK)?TRUE:FALSE;
	phy_status->duplex = (phydata & RTL8211_STATUS_DUPLEX_MASK)?FAL_FULL_DUPLEX:FAL_HALF_DUPLEX;
    if (phy_status->link_status)
    {
        ...
        case 0:
        {
            ...
            case 2:
                ...
                break;
            case 3:
                phy_status->speed = FAL_SPEED_100;	// 1G lite
                break;
        ...
        case 1:
        {
            ...
            case 1:
                ...
                break;
            case 2:
                phy_status->speed = FAL_SPEED_5000;  //5G
                break;
            case 3:
                phy_status->speed = FAL_SPEED_1000;   // 2.5G lite
                break;
            default:
                phy_status->speed = FAL_SPEED_10;
                break;
	...
	// printf("get rtl8251 %u linkstatus: %u, speed: %d, duplex: %d\n", phy_id, phy_status->link_status, phy_status->speed, phy_status->duplex);
	return 0;
}


u8 rtl8251_phy_get_link_status(uint32_t dev_id, uint32_t phy_id)
{
	struct port_phy_status phy_status;
	rtl8251_phy_get_status( dev_id, phy_id, &phy_status);
	if (phy_status.link_status)
	{
		return 0; //up
	}
...
u32 rtl8251_phy_get_speed(uint32_t dev_id, uint32_t phy_id,
...
u32 rtl8251_phy_get_duplex(uint32_t dev_id, uint32_t phy_id,
...
static int _status_get(void)
{
	UINT16 phydata = 0;
	int ret = -1;

	ret = MmdPhyRead(g_handle, MMD_VEND2, 0xA434, &phydata);
	LOG_ERR("get rtl8251b phy 0xA434 phydata=0x%x\n", phydata);

	ret = MmdPhyRead(g_handle, MMD_VEND1, 0x7580, &phydata);
	LOG_ERR("get rtl8251b phy 0x7580 phydata=0x%x\n", phydata);

	ret = MmdPhyRead(g_handle, MMD_VEND1, 0x758D, &phydata);
	LOG_ERR("get rtl8251b phy 0x758D phydata=0x%x\n", phydata);

	ret = MmdPhyRead(g_handle, MMD_VEND1, 0x6973, &phydata);
	LOG_ERR("get rtl8251b phy 0x6973 phydata=0x%x\n", phydata);
...
int rtl8251b_init_common(a_uint32_t dev_id, a_uint32_t port_bmp)
{
	BOOL ret;
	PHY_LINK_ABILITY phy_ability;
	PHY_EEE_ENABLE eee_enable;
...
#ifdef GE550V2_RTL8372_RTL8251B
	ret = Rtl8251b_phy_init(g_handle, &phy_ability, TRUE);
	ERR_PRINT(ret);

	if (g_mdio_num == RTL8251B_WAN_MDIO_NUM)
	{
		ret = Rtl8251b_serdes_autoNego_set(g_handle, TRUE);
		ERR_PRINT(ret);

		// ret = Rtl8251b_serdes_option_set(g_handle, PHY_SERDES_SPEED_USXGMII);
		// ERR_PRINT(ret);
	}
	else
	{
		ret = Rtl8251b_serdes_autoNego_set(g_handle, FALSE);
		ERR_PRINT(ret);
	...
	ret = Rtl8251b_phy_reset(g_handle);
	ERR_PRINT(ret);
#else
	ret = Rtl8251b_phy_init(g_handle, &phy_ability, TRUE);
	ERR_PRINT(ret);
	...
	phy_ability.Full_1000 = 1;
	phy_ability.adv_2_5G = 1;
	phy_ability.adv_5G = 1;
	phy_ability.FC = 1;
	phy_ability.AsyFC = 1;
	ret = Rtl8251b_autoNegoAbility_set(g_handle, &phy_ability);
	ERR_PRINT(ret);
	
	ret = Rtl8251b_autoNegoEnable_set(g_handle, TRUE);
	ERR_PRINT(ret);
	...
	eee_enable.EEE_2_5G = 0;
	eee_enable.EEE_5G = 0;
	ret = Rtl8251b_eeeEnable_set(g_handle, &eee_enable);
	ERR_PRINT(ret);

	ret = Rtl8251b_greenEnable_set(g_handle, FALSE);
	ERR_PRINT(ret);
	
	Rtl8251b_enable_set(g_handle, TRUE);
...
int rtl8251b_lan_init(a_uint32_t dev_id, a_uint32_t port_bmp, int mdio_num)
{
	g_mdio_num = mdio_num;
	rtl8251b_init_common(dev_id,port_bmp);
	return 0;
}
int rtl8251b_wan_init(a_uint32_t dev_id, a_uint32_t port_bmp, int mdio_num)
{
	g_mdio_num = mdio_num;
	rtl8251b_init_common(dev_id,port_bmp);
	return 0;
...
{
	static int rtl8251_is_init = 0;
	struct phy_ops *rtl8251_phy_ops;
	rtl8251_phy_ops = (struct phy_ops *)malloc(sizeof(struct phy_ops));
	if (!rtl8251_phy_ops)
	   return -ENOMEM;
	rtl8251_phy_ops->phy_get_link_status = rtl8251_phy_get_link_status;
	rtl8251_phy_ops->phy_get_speed = rtl8251_phy_get_speed;
	rtl8251_phy_ops->phy_get_duplex = rtl8251_phy_get_duplex;
	*ops = rtl8251_phy_ops;

	if (rtl8251_is_init == 0)
	{
		rtl8251b_gpio_reset(RTL8251B_LAN_RST_GPIO); 	//LAN hardware reset
		rtl8251b_lan_init(0,0, RTL8251B_LAN_MDIO_NUM);
		rtl8251b_gpio_reset(RTL8251B_WAN_RST_GPIO); 	//LAN hardware reset
		rtl8251b_lan_init(0,0, RTL8251B_WAN_MDIO_NUM);
		rtl8251_is_init = 1;
	}
...
#endif /* CONFIG_TP_EXT_PHY_RTL8251B */
...
#ifdef CONFIG_TP_EXT_SWITCH
#ifdef CONFIG_TP_EXT_SWITCH_RTL8367_ALL
static int ipq_rtl8367_Link_Update(void)
{
	rtk_port_linkStatus_t linkStatus = 0;
	rtk_port_speed_t speed = 0;
	rtk_port_duplex_t duplex = 0;
	int status = 1;

	for(int i = UTP_PORT0; i < UTP_PORT5; ++i)
	{
		if (0 != rtk_port_phyStatus_get(i, &linkStatus, &speed, &duplex))
		{
			printf("rtk_port_phyStatus_get error");
			continue;
		}
		
		if (!linkStatus)
		{
			printf("RTL8367sc port%d not link!!!\n", i);
			continue;
		}
		status = 0;
		printf("RTL8367SSC Port%d %s \n", i, LINK_RTL8367(portStatus.link));
	}
	return status;
}
#endif
#ifdef CONFIG_TP_EXT_SWITCH_RTL8372
static int ipq_rtl8372_Link_Update(void)
{
	rtk_port_status_t port_status;
	int status = 1;
#ifdef GE550V2_RTL8372_RTL8251B
	for(int i = UTP_PORT4; i <= UTP_PORT8; ++i)
#else
	for(int i = UTP_PORT4; i <= UTP_PORT7; ++i)
#endif
	{
		if (0 != dal_rtl8373_portStatus_get(i, &port_status))
		{
			printf("dal_rtl8373_portStatus_get error");
			continue;
		}
		
		if (!(port_status.link))
		{
			printf("RTL8372 port%d not link!!!\n", i);
			continue;
		}
		status = 0;
		printf("RTL8372 Port%d %s \n", i, LINK_RTL8372(ability.link));
	}
	return status;
}
#endif
...
#ifdef CONFIG_TP_EXT_SWITCH
#ifdef CONFIG_TP_EXT_SWITCH_RTL8367_ALL
		else if ((i == 1) && (priv->rtl8367_switch_enable == 1))
		{
			if (!ipq_rtl8367_Link_Update())
				linkup++;
			continue;
		}
#endif
#ifdef CONFIG_TP_EXT_SWITCH_RTL8372
		else if ((i == 1) && (priv->rtl8372_switch_enable == 1))
		{
			if (!ipq_rtl8372_Link_Update())
				linkup++;
			continue;
		}
#endif
#endif
#ifdef CONFIG_TP_EXT_PHY_RTL8221B
		else if ((i == 0) && (priv->rtl8221_enable == 1))
		{
#ifdef CONFIG_TP_EXT_PHY_RTL8221B_LAN
			if (!ipq_rtl8221_Link_Update(6))
			{
				linkup++;
				status_get();
			}
#endif	
			if (!ipq_rtl8221_Link_Update(7))
			{
				linkup++;
				status_get();
			}
...
#ifdef CONFIG_TP_EXT_PHY_RTL8251B
		if (phy_info->phy_type == AQ_PHY_TYPE || phy_info->phy_type == RTL8251B_PHY_TYPE){
#else
...
static rtk_api_ret_t rtk_phyPatch_set(rtk_uint32 opt)
{
#define RTL8367C_PHY_CONTROL                 0
	...
	for (port = 0; port <= 4; port++) {
		...
		/*According to rtk AE, these ops have been included in 1.4.2 dirver
		if (opt == 0) {
...
#endif
#ifdef CONFIG_TP_EXT_SWITCH_RTL8367_ALL
static void reset_rtl8367s_switch_gpio(int gpio)
{
...
static void rtl8367_sw_init(void)
{
	...
	reset_rtl8367s_switch_gpio(CONFIG_SWITCH_RESET);

	ret = rtk_switch_init();
	printf("rtk_switch_init ret = %d!!!!!!!!!!!!\n", ret);
	mdelay(500);
	
	/*if (g_switch_chip == CHIP_RTL8367C)
	{
		rtk_phyPatch_set(0);
		printf("phyPatch set Done.\n");
	}*/
	...
	printf("rtk_port_macForceLinkExt_set port [%d] ret = %d!!!!!!!!!!!!\n", EXT_PORT0, ret);

	ret = rtk_port_sgmiiNway_set(EXT_PORT0, DISABLED);
	printf("rtk_port_sgmiiNway_set port [%d] ret = %d!!!!!!!!!!!!\n", EXT_PORT0, ret);

	ret = rtk_port_macForceLinkExt_set(EXT_PORT1, mode,&mac_cfg);
	printf("rtk_port_macForceLinkExt_set port [%d] ret = %d!!!!!!!!!!!!\n", EXT_PORT1, ret);

	ret = rtk_port_sgmiiNway_set(EXT_PORT1, DISABLED);
	printf("rtk_port_sgmiiNway_set port [%d] ret = %d!!!!!!!!!!!!\n", EXT_PORT1, ret);
}
#endif

#ifdef CONFIG_TP_EXT_SWITCH_RTL8372
static void reset_rtl8372_switch_gpio(int gpio_num)
{	
	gpio_direction_output(gpio_num, 1);
	mdelay(500);
	gpio_set_value(gpio_num, 0);
...
static void rtl8372_sw_init(void)
{
	int ret = 0;
	rtk_port_ability_t ability;

	reset_rtl8372_switch_gpio(CONFIG_SWITCH_RESET);

	if ((ret = rtk_switch_init()) != RT_ERR_OK) {
		printf("!!!rtk_switch_init failed...ret=%d\n", ret);
	}
	
	if ((ret = rtk_sdsMode_set(SERDES_ID0, SERDES_10GR) != RT_ERR_OK)) {
		printf("!!!sdsMode set failed...ret=%d\n", ret);
	}

	if ((ret = dal_rtl8373_port_sdsNway_set(SERDES_ID0, SERDES_10GR, 0)) != RT_ERR_OK)
	{
		printf("!!!sdsNway set failed...ret=%d\n", ret);
	}

#ifdef GE550V2_RTL8372_RTL8251B
	if (RT_ERR_OK != dal_rtl8373_port_extphyid_set(SERDES_ID1, 0x2))
	{
		printf("!!!dal_rtl8373_port_extphyid_set set failed...ret=%d\n", ret);
	}

	if ((ret = rtk_sdsMode_set(SERDES_ID1, SERDES_10GUSXG)) != RT_ERR_OK) {
		printf("!!!sdsMode set 1 failed...ret=%d\n", ret);
	}

	if (dal_rtl8373_port_sdsNway_set(SERDES_ID1, SERDES_10GUSXG, 0) != RT_ERR_OK)
	{
		printf("!!!sdsNway set failed...ret=%d\n", ret);
	}
#endif
	/*
	if(RT_ERR_OK != dal_rtl8373_portFrcAbility_get(UTP_PORT3, &ability))
	{
		printf("dal_rtl8373_portFrcAbility_get error!!\n");
	}

	ability.forcemode = ENABLED;
	ability.link = PORT_LINKUP;
	ability.duplex = PORT_FULL_DUPLEX;
	ability.speed = PORT_SPEED_10G;
	ability.txpause = ENABLED;
	ability.rxpause = ENABLED;

	if(RT_ERR_OK != dal_rtl8373_portFrcAbility_set(UTP_PORT3, &ability))
	{
		printf("dal_rtl8373_portFrcAbility_set error!!\n");
	}
	*/
...
int ipq5332_edma_init(void *edma_board_cfg)
{
	...
	phy_info_t *phy_info;
	int phy_id;
	uint32_t phy_chip_id = 0, phy_chip_id1, phy_chip_id2;
	int clk[4] = {0};
...
#ifdef CONFIG_TP_EXT_SWITCH
#ifdef CONFIG_TP_EXT_SWITCH_RTL8367_ALL
	int rtl8367_switch_enable = 0;
#endif
#ifdef CONFIG_TP_EXT_SWITCH_RTL8372
	int rtl8372_switch_enable = 0;
#endif
#endif
#ifdef CONFIG_TP_EXT_PHY_RTL8221B
	int rtl8221_enable = 0;
#endif
...
#ifdef CONFIG_TP_EXT_SWITCH
#ifdef CONFIG_TP_EXT_SWITCH_RTL8367_ALL
	rtl8367_switch_enable = fdtdec_get_uint(gd->fdt_blob, node, "rtl8367_switch_enable", 0);
	printf("rtl8367_switch_enable:%d\n", rtl8367_switch_enable);
#endif
#ifdef CONFIG_TP_EXT_SWITCH_RTL8372
	rtl8372_switch_enable = fdtdec_get_uint(gd->fdt_blob, node, "rtl8372_switch_enable", 0);
	printf("rtl8372_switch_enable:%d\n", rtl8372_switch_enable);
#endif
#endif
#ifdef CONFIG_TP_EXT_PHY_RTL8221B
	rtl8221_enable = fdtdec_get_uint(gd->fdt_blob, node, "rtl8221_enable", 0);
	printf("rtl8221_enable:%d\n", rtl8221_enable);
#endif
...
#ifdef CONFIG_TP_EXT_SWITCH
#ifdef CONFIG_TP_EXT_SWITCH_RTL8367_ALL
		ipq5332_edma_dev[i]->rtl8367_switch_enable = rtl8367_switch_enable;
		printf("ipq5332_edma_dev[%d]->rtl8367_switch_enable:%d\n", i, ipq5332_edma_dev[i]->rtl8367_switch_enable);
#endif
#ifdef CONFIG_TP_EXT_SWITCH_RTL8372
		ipq5332_edma_dev[i]->rtl8372_switch_enable = rtl8372_switch_enable;
		printf("ipq5332_edma_dev[%d]->rtl8372_switch_enable:%d\n", i, ipq5332_edma_dev[i]->rtl8372_switch_enable);
#endif

#endif
#ifdef CONFIG_TP_EXT_PHY_RTL8221B
		ipq5332_edma_dev[i]->rtl8221_enable = rtl8221_enable;
		printf("ipq5332_edma_dev[%d]->rtl8221_enable:%d\n", i, ipq5332_edma_dev[i]->rtl8221_enable);
#endif
...
/* For GE550V2, because the RTL8251 MDIO bus is connected to the RTL8372,
RTL8372 need to be initialized before RTL8251, so the order is reversed. */
#ifdef GE550V2_RTL8372_RTL8251B
		for (phy_id =  IPQ5332_PHY_MAX - 1 ; phy_id >= 0; phy_id--) {
#else
...
#ifdef CONFIG_TP_EXT_PHY_RTL8221B
			printf("phy_id:%d\n", phy_id);
			if (0 == phy_id && rtl8221_enable){
				phy_info->phy_type = RTL8221B_PHY_TYPE;
				phy_chip_id = RTL8221;
...
#ifdef CONFIG_TP_EXT_SWITCH_RTL8367_ALL
			printf("phy_id:%d\n", phy_id);
			if (1 == phy_id && rtl8367_switch_enable){
				phy_info->phy_type = RTL8367S_SWITCH_TYPE;
				phy_chip_id = RTL8367;
				...
				*/
				...
				rtl8367_sw_init();
				printf("..............probe rtk switch %d\n", g_switch_chip);
				rtk_port_phyEnableAll_set(ENABLED);
			}
#endif
#ifdef CONFIG_TP_EXT_SWITCH_RTL8372
			printf("phy_id:%d\n", phy_id);
			if (1 == phy_id && rtl8372_switch_enable){
				phy_info->phy_type = RTL8372_SWITCH_TYPE;
				phy_chip_id = RTL8372;

				ipq5332_port_mac_clock_reset(phy_id);
				/* Force Link-speed: 10000M
				* Force Link-status: enable
				*/
				ipq5332_10g_r_speed_set(phy_id, 0);
				clk[1] = 0x0;
				clk[3] = 0x0;
				clk[0] = 0x301;
				clk[2] = 0x401;
...
#endif
#ifdef CONFIG_TP_EXT_PHY_RTL8251B
			if (phy_info->phy_type == RTL8251B_PHY_TYPE) {
				...
				phy_chip_id = (phy_chip_id1 << 16) |
						phy_chip_id2;
				printf("phy_id is: 0x%x, phy_addr = 0x%x,"
							"phy_chip_id1 = 0x%x,"
							"phy_chip_id2 = 0x%x,"
							"phy_chip_id = 0x%x,"
							"RTL8251 = 0x%x \n",
							phy_id, phy_addr, phy_chip_id1,
							phy_chip_id2, phy_chip_id, RTL8251B_PHY);
			}
#endif
#else
			if (phy_info->phy_type == AQ_PHY_TYPE) {
...
#ifdef CONFIG_TP_EXT_PHY_RTL8251B
			case RTL8251B_PHY:
				rtl8251b_init(&ipq5332_edma_dev[i]->ops[phy_id], phy_addr);
				break;
#endif
			default:
				if (phy_info->phy_type != SFP_PHY_TYPE)
