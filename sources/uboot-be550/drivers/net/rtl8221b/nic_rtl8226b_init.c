    ...
    )
{
	...
	UINT16 org_id = 0, rev_id = 0;

	MmdPhyRead(hDevice, MMD_PMAPMD, 0x02, &org_id);
	MmdPhyRead(hDevice, MMD_PMAPMD, 0x03, &rev_id);
	phydata = (org_id<<16) | rev_id;
	printf("[rtl8221b] Rtl8226b_phy_init: phy_adress=%d, phy_id=0x%x\r\n", MMD_PMAPMD, phydata);
    ...
    if ((rev_num == 8) &&(mod_num == 4))    {
        //printf("rtl8221b and go init flow...\n");
    }
    else{
        //printf("Not rtl8221b and skip init flow...id = %x \n",phydata);
        goto exit;
    }
