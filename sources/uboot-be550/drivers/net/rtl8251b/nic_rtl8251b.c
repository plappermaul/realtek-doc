    ...
    )
{
    ...
    pPhyAbility->adv_5G = (phydata & BIT_8) ? (1) : (0);
    ...
    )
{
    ...
    phydata |= (pPhyAbility->adv_5G ? (BIT_8) : (0));
    status = MmdPhyWrite(hDevice, MMD_AN, 32, phydata);
    if (status != SUCCESS)
        goto exit;   
...
BOOLEAN
Rtl8251b_duplex_set(                // christy add 0430
    IN HANDLE hDevice,
    IN BOOL Enable
    )
{
...
//    int i = 0;
    ...
    status = Rtl8251b_is_link(hDevice, &linkOK);
    if (status != SUCCESS)
        goto exit;
    ...
    )
{
    ...
    if (support)
    {
        ...
        status = Rtl8251b_autoNegoEnable_set(hDevice, FALSE);
        if (status != SUCCESS)
            goto exit;
    }
    else
    ...
    )
{
    ...
    pEeeEnable->EEE_5G = (phydata & BIT_1) ? (TRUE) : (FALSE);
    ...
    )
{
    ...
    status = Rtl8251b_autoNegoEnable_get(hDevice, &AnEnable);
    if (status != SUCCESS)
        goto exit;
	...
	)
{
	...
	status = MmdPhyRead(hDevice, MMD_PMAPMD, 0x0, &phydata);
	if (status != SUCCESS)
		goto exit;
	...
	)
{
	...
	status = MmdPhyWrite(hDevice, MMD_PMAPMD, 0x0, phydata);
	if (status != SUCCESS)
		goto exit;
    ...
    )
{
    ...
    if (linkOK)
    {
        status = Rtl8251b_speed_get(hDevice, &Speed);
        if (status != SUCCESS)
            goto exit;
        ...
        if (Speed == 10)
        {
            printf("RTCT is not supported when port link at 10M.\n");
            status = FAILURE;
            goto exit;
        }
    }
    ...
    )
{
    ...
    else
    {
        ...
        status = Rtl8251b_rtctResult_convert(phydata, &pRtctResult->channelAStatus);
        if (status != SUCCESS)
            goto exit;
        ...
        status = Rtl8251b_rtctResult_convert(phydata, &pRtctResult->channelBStatus);
        if (status != SUCCESS)
            goto exit;
        ...
        status = Rtl8251b_rtctResult_convert(phydata, &pRtctResult->channelCStatus);
        if (status != SUCCESS)
            goto exit;
        ...
        status = Rtl8251b_rtctResult_convert(phydata, &pRtctResult->channelDStatus);
        if (status != SUCCESS)
            goto exit;
    }
    ...
    )
{
    ...
    case LINK_SPEED_5G:
    {
        ...
        // 5G Test Mode
        // MMD 1.0.15:0 = 0x205C
        status = MmdPhyWrite(hDevice, MMD_PMAPMD, 0, 0x205C);
        if (status != SUCCESS)
            goto exit;
    ...
    )
{
    ...
    *perdesLink = (phydata & BIT_1) ? (TRUE) : (FALSE);
	...
	if (phydata == 1){
        ...
        phydata1 &= (BIT_14 | BIT_13 | BIT_12 | BIT_11 | BIT_10);

        if(phydata == 2)
            *SerdesMode = PHY_SERDES_MODE_SGMII;
        else if(phydata == 21)
            *SerdesMode = PHY_SERDES_MODE_5000BASEX;
        else if(phydata == 13)
        ...
        else if((phydata == 26)&&(phydata1 == 0))
            *SerdesMode = PHY_SERDES_MODE_SFI_10GBASER;
        else if((phydata == 26)&&(phydata1 == 2))
            *SerdesMode = PHY_SERDES_MODE_5000BASER;
        else if(phydata == 31)
    ...
    printf("Rtl8251b_serdes_link_get %d",*SerdesMode);
...
{
    /* The RTL8251B supports SERDES setting options related to Link_down_option, Rate_adaptor_en, 
    and SERDES_speed at the different Ethernet speeds which is different from RTL8221. 
    According to the Ethernet speed, use a specific address to set SERDES Setting Options*/
    BOOL status = FAILURE;
    UINT16 phydata = 0;
    ...
    if ((functioninput >= 0) && (functioninput <= 7))
    {
        ...
        status = MmdPhyRead(hDevice, MMD_VEND1, 0x6973, &phydata);
        if (status != SUCCESS)
            goto exit;

        phydata &= (~(BIT_11 | BIT_10 | BIT_9 | BIT_8 ));
        phydata |= functioninput;

        status = MmdPhyWrite(hDevice, MMD_VEND1, 0x6973, phydata);
        if (status != SUCCESS)
            goto exit;
        ...
        status = MmdPhyRead(hDevice, MMD_VEND1, 0x6974, &phydata);
        if (status != SUCCESS)
            goto exit;
        ...
        status = MmdPhyWrite(hDevice, MMD_VEND1, 0x6974, phydata);
        if (status != SUCCESS)
            goto exit;
        ...
        status = MmdPhyRead(hDevice, MMD_VEND1, 0x6975, &phydata);
        if (status != SUCCESS)
            goto exit;
        ...
        status = MmdPhyWrite(hDevice, MMD_VEND1, 0x6975, phydata);
        if (status != SUCCESS)
            goto exit;
        ...
        /* 100M: 30.0x6976.7:0 */
        status = MmdPhyRead(hDevice, MMD_VEND1, 0x6976, &phydata);
        if (status != SUCCESS)
            goto exit;
        ...
        status = MmdPhyWrite(hDevice, MMD_VEND1, 0x6976, phydata);
        if (status != SUCCESS)
            goto exit;


        /* 10M: 30.0x6977.7:0 */
        status = MmdPhyRead(hDevice, MMD_VEND1, 0x6977, &phydata);
        if (status != SUCCESS)
            goto exit;
        ...
        status = MmdPhyWrite(hDevice, MMD_VEND1, 0x6977, phydata);
        if (status != SUCCESS)
            goto exit;
    }
    ...
    )
{
    ...
    status = MmdPhyWrite(hDevice, MMD_VEND1, 0x7588, 0x00f1);
    if (status != SUCCESS)
        goto exit;

    /* Enable/Disable USXGMII Auto-Negotiation */
    /* If you need to disable HiSGMII Auto-Negotiation, see 《SERDES Mode Setting Flow Application Note》 */
    if (Enable)
        phydata = 0x854F;
    else
        phydata = 0x854E;
