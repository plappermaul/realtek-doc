...
static inline unsigned int rtk_mdio_write(int mii_id, int regnum, unsigned int value)
{
	return ipq_mdio_write(mii_id, regnum, (unsigned short)value);
}

static inline int rtk_mdio_read(int mii_id, int regnum, unsigned int *data)
{
	unsigned short temp_data = 0;

	ipq_mdio_read(mii_id, regnum, &temp_data);
...
static inline int rtk_mdio_read_h(int mii_id, int regnum, rtk_uint16 *data)
{
	unsigned short temp_data = 0;
...
#define MDC_MDIO_READ_H(preamableLength, phyID, regID, pData) rtk_mdio_read_h(phyID, regID, pData)
...
rtk_int32 rtl8373_smi_read(rtk_uint32 mAddrs, rtk_uint32 *rData)
{
    ...
    MDC_MDIO_READ_H(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID, MDC_MDIO_DATA_LOW, &datal);

    /* Read data from register 24 */
    MDC_MDIO_READ_H(MDC_MDIO_PREAMBLE_LEN, MDC_MDIO_PHY_ID, MDC_MDIO_DATA_HIGH, &datah);
