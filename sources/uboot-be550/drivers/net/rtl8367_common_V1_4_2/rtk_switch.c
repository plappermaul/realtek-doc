...
static rtk_api_ret_t _rtk_switch_init(void)
{
    ...
    halCtrl = hal_find_device();

    /* Find device */
    if(halCtrl== NULL)
    {
    	printf("hal_find_device  failed!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n");
    	switchChip = 4;
	...
	g_switch_chip = switchChip;
	printf("switch debug=========switchChip:%d\n",switchChip);
    if((retVal = dal_mgmt_attachDevice(switchChip)) != RT_ERR_OK)
    {
        printf("dal_mgmt_attachDevice fail! ret = %d\n", retVal);
        return retVal;
    }
    ...
    {
        printf("dal_mgmt_attachDertk_switch_initialState_setvice fail! ret = %d\n", retVal);
        return retVal;
    }
    ...
    {
        printf("rtk_switch_initialState_set fail! ret = %d\n", retVal);
        return retVal;
    }
