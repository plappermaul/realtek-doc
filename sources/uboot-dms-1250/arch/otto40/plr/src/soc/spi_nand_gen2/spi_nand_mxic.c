...
mxic_ecc_encode(u32_t ecc_ability, void *dma_addr, void *fake_ptr_cs) { return; }


__SECTION_RUNTIME s32_t
mxic_ecc_decode(u32_t ecc_ability, void *dma_addr, void *fake_ptr_cs)
{
...
#if (SNAF_PLANE_PATCH==1)
    {  // MX35LF2G14AC is 2-plane select
        .man_id              = MID_MXIC,
...
probe_mxic_spi_nand_chip(void)
{
    ...
    for(i=0 ; i<ELEMENT_OF_SNAF_INFO(mxic_chip_info) ; i++){
        if((mxic_chip_info[i].dev_id == did8) || (mxic_chip_info[i].dev_id == DEFAULT_DATA_BASE))
        {
            ...
            #elif __DEVICE_USING_DIO
            mxic_chip_info[i]._cmd_info = _nsu_dio_cmd_info_ptr;
            #else
            ...
            if(ECC_USE_ODE==mxic_chip_info[i]._ecc_ability) {
                nsu_model_info._page_read_ecc  = _nsu_page_read_with_ode_ptr;
                nsu_model_info._page_write_ecc = _nsu_page_write_with_ode_ptr;
                mxic_chip_info[i]._ecc_encode = mxic_ecc_encode;
                mxic_chip_info[i]._ecc_decode = mxic_ecc_decode;
            } else {
