...
_init_tlb(void) {
    ...
    u32_t end_of_mapping = PADDR(OTTO_SRAM_START + OTTO_SRAM_SIZE);
    if (mmu_drv_init(PADDR((u32_t)&mapped_physical_sram_start), end_of_mapping)==0) {
