...
unsigned int disable_DRAM_prefech(unsigned int side_id)
{
	...
	old_mcr = MEM32_READ(MCR);
	if( side_id & MCR_PREFETCH_INS_SIDE )
		MEM32_WRITE(MCR, MEM32_READ(MCR) & ((unsigned int)MCR_PREFETCH_DIS_IMASK));
	if( side_id & MCR_PREFETCH_DATA_SIDE)
		MEM32_WRITE(MCR, MEM32_READ(MCR) & ((unsigned int)MCR_PREFETCH_DIS_DMASK));
...
unsigned int enable_DRAM_prefech(unsigned int mode, unsigned int side_id)
{
	...
	if( side_id & MCR_PREFETCH_INS_SIDE ){
		...
		if( mode == MCR_PREFETCH_OLD_MECH)
			MEM32_WRITE(MCR, MEM32_READ(MCR) | MCR_PREFETCH_MODE_IOLD | MCR_PREFETCH_ENABLE_INS);
		else
			MEM32_WRITE(MCR, MEM32_READ(MCR) | MCR_PREFETCH_MODE_INEW | MCR_PREFETCH_ENABLE_INS);
	...
	if( side_id & MCR_PREFETCH_DATA_SIDE ){
		...
		if( mode == MCR_PREFETCH_OLD_MECH)
			MEM32_WRITE(MCR, MEM32_READ(MCR) | MCR_PREFETCH_MODE_DOLD | MCR_PREFETCH_ENABLE_DATA);
		else
			MEM32_WRITE(MCR, MEM32_READ(MCR) | MCR_PREFETCH_MODE_DNEW | MCR_PREFETCH_ENABLE_DATA);
	}
