...
static int env_print(char *name)
{
	...
	if (name) {		/* print a single name */
		...
		e.key = name;
		e.data = NULL;
		hsearch_r(e, FIND, &ep, &env_htab[g_env_id]);
		if (ep == NULL)
			return 0;
		len = printf("%s=%s\n", ep->key, ep->data);
		return len;
	...
	/* print whole list */
	len = hexport_r(&env_htab[g_env_id], '\n', &res, 0, 0, NULL);
...
int do_env_print2 (cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	int i;
	int rcode = 0;
...
{
	ENTRY *match;
	unsigned char matched[env_htab[g_env_id].size / 8];
	int rcode = 1, arg = 1, idx;

	if (argc < 2)
		return cmd_usage(cmdtp);

	memset(matched, 0, env_htab[g_env_id].size / 8);

	while (arg <= argc) {
		idx = 0;
		while ((idx = hstrstr_r(argv[arg], idx, &match, &env_htab[g_env_id]))) {
...
int _do_env_set(int flag, int argc, char * const argv[])
{
	...
	if (argc < 3 || argv[2] == NULL) {
		int rc = hdelete_r(name, &env_htab[g_env_id]);
		return !rc;
	}
	...
	e.key	= name;
	e.data	= value;
	hsearch_r(e, ENTER, &ep, &env_htab[g_env_id]);
	free(value);
	if (!ep) {
...
int setenv2(const char *varname, const char *varvalue)
{
	const char * const argv[4] = { "setenv", varname, varvalue, NULL };
...
int do_env_set2(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	if (argc < 2)
		return cmd_usage(cmdtp);
...
int do_env_save2(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	printf("Saving Environment2 to %s...\n", env_name_spec);

	return saveenv2() ? 1 : 0;
}

U_BOOT_CMD(
	saveenv2, 1, 0,	do_env_save2,
	"save environment variables to persistent storage",
	""
);
#endif
...
NXTARG:		;
	...
	if (sep) {		/* export as text file */
		len = hexport_r(&env_htab[g_env_id], sep, &addr, size, argc, argv);
		if (len < 0) {
	...
	len = hexport_r(&env_htab[g_env_id], '\0', &res, ENV_SIZE, argc, argv);
	if (len < 0) {
		error("Cannot export environment: errno = %d\n", errno);
		return 1;
	}
			 ...
			 int argc, char * const argv[])
{
	...
	if (himport_r(&env_htab[g_env_id], addr, size, sep, del ? 0 : H_NOCLEAR) == 0) {
...
static cmd_tbl_t cmd_env_sub[] = {
	...
	U_BOOT_CMD_MKENT(print2, CONFIG_SYS_MAXARGS, 1, do_env_print2, "", ""),
#if defined(CONFIG_CMD_RUN)
	U_BOOT_CMD_MKENT(run, CONFIG_SYS_MAXARGS, 1, do_run, "", ""),
#endif
#if defined(CONFIG_CMD_SAVEENV) && !defined(CONFIG_ENV_IS_NOWHERE)
	U_BOOT_CMD_MKENT(save, 1, 0, do_env_save, "", ""),
	U_BOOT_CMD_MKENT(save2, 1, 0, do_env_save2, "", ""),
#endif
	U_BOOT_CMD_MKENT(set, CONFIG_SYS_MAXARGS, 0, do_env_set, "", ""),
	U_BOOT_CMD_MKENT(set2, CONFIG_SYS_MAXARGS, 0, do_env_set2, "", ""),
};
...
U_BOOT_CMD_COMPLETE(
	printenv2, CONFIG_SYS_MAXARGS, 1,	do_env_print2,
	"print environment variables",
	"\n    - print values of all environment variables\n"
	"printenv name ...\n"
	"    - print value of environment variable 'name'",
...
U_BOOT_CMD_COMPLETE(
	setenv2, CONFIG_SYS_MAXARGS, 0,	do_env_set2,
	"set environment variables",
	"name value ...\n"
	"    - set environment variable 'name' to 'value ...'\n"
	"setenv name\n"
