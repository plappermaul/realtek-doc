...
#define PARTITION_BDINFO     (CONFIG_SYS_FLASH_BASE + PARTITION_ADDR(FLASH_INDEX_BDINFO))
#define PARTITION_BDINFO_END PARTITION_BDINFO + UNIT_SIZE - 1
#endif
...
void sysinfo_mem_init(void)
{
    ...
    osal_memcpy(glb_sysinfo_addr, (const void *)PARTITION_BDINFO, SYSINFO_ENV_SIZE);
#endif
}
...
int savesys(void)
{
    ...
    ret = spi_flash_erase(env_flash, BDINFO_START, SYSINFO_ENV_SIZE);
    if (ret)
    {
        puts("fail\n");
        return ret;
    ...
    ret = spi_flash_write(env_flash, BDINFO_START, SYSINFO_ENV_SIZE, glb_sysinfo_addr);
    if (ret)
    {
        puts("fail\n");
        return ret;
