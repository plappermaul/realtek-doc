 ...
 */
int32
uart_getc(uint32 unit, uint8 u8len, uint8 *pData)
{
    uint32 index;
    ...
    if (serial_in(unit, UART_REG(unit).uart1_line_sts_addr.reg_addr) & UART_LSR_DR)
    {
        ...
        {
            pData[index] = serial_in(unit, UART_REG(unit).uart1_rx_tx_div_lsb_addr.reg_addr);
        }
        return RT_ERR_OK;
    }
    else
 ...
 */
int32
uart_putc(uint32 unit, uint8 u8len, uint8 *data)
{
    int index;
    ...
    for (index = 0; index < u8len; index++)
    {
        serial_out(unit, UART_REG(unit).uart1_rx_tx_div_lsb_addr.reg_addr, data[index]);
    }
    return RT_ERR_OK;
} /* end of uart_putc */
