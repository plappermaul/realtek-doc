...
#define ROM_SIZE (PARTITION_SIZE(FLASH_INDEX_LOADER) \
                    + PARTITION_SIZE(FLASH_INDEX_LOADER_ENV) \
                    + PARTITION_SIZE(FLASH_INDEX_LOADER_ENV2) \
                    + PARTITION_SIZE(FLASH_INDEX_BDINFO) \
                    + PARTITION_SIZE(FLASH_INDEX_JFFS2) \
                    + PARTITION_SIZE(FLASH_INDEX_KERNEL) \
                    + PARTITION_SIZE(FLASH_INDEX_ROOTFS) \
                    + PARTITION_SIZE(FLASH_INDEX_KERNEL2) \
                    + PARTITION_SIZE(FLASH_INDEX_ROOTFS2))
#else
#define ROM_SIZE (PARTITION_SIZE(FLASH_INDEX_LOADER) \
                    + PARTITION_SIZE(FLASH_INDEX_LOADER_BDINFO) \
                    + PARTITION_SIZE(FLASH_INDEX_SYSINFO) \
                    + PARTITION_SIZE(FLASH_INDEX_JFFS2_CFG) \
                    + PARTITION_SIZE(FLASH_INDEX_JFFS2_LOG) \
                    + PARTITION_SIZE(FLASH_INDEX_KERNEL))
#endif
...
#endif
struct rtk_partition_info_t rtk_flash_partition_size[] = {
     [0] = {
         .flash_total_size = FLASH_SIZE_32MB,
#if defined(CONFIG_DUAL_IMAGE)
         .loader_offset = (0x00000000U),
         .loader_size = (UNIT_SIZE*13),
         .env_offset = (0x000D0000U),
         .env_size = UNIT_SIZE,
         .env2_offset = (0x000e0000U),
         .env2_size = UNIT_SIZE,
         .bdinfo_offset = (0x000f0000U),
         .bdinfo_size = UNIT_SIZE,
         .jffs2_offset = (0x00100000U),
         .jffs2_size = (0x200000U),
         .kernel_offset = (0x00300000U),
         .kernel_size = (0x180000U),
         .rootfs_offset = (0x480000U),
         .rootfs_size = (0xD00000U),
         .kernel2_offset = (0x1180000U),
         .kernel2_size = (0x180000U),
         .rootfs2_offset = (0x1300000U),
         .rootfs2_size = (0xD00000U)
#else
         .loader_offset = (0x00000000U),
    ...
    switch(partition_idx){
        case FLASH_INDEX_LOADER:            /* LOADER */
            ...
            *size = rtk_flash_partition_size[size_idx].loader_size;
            break;
        case FLASH_INDEX_LOADER_ENV:        /* LOADER_ENV */ 
            *offset = rtk_flash_partition_size[size_idx].env_offset;
            *size = rtk_flash_partition_size[size_idx].env_size;				
            break;
        case FLASH_INDEX_LOADER_ENV2:       /* LOADER_ENV2 */ 
            *offset = rtk_flash_partition_size[size_idx].env2_offset;
            *size = rtk_flash_partition_size[size_idx].env2_size;				
            break;
        case FLASH_INDEX_BDINFO:            /* BDINFO */
            *offset = rtk_flash_partition_size[size_idx].bdinfo_offset;
            *size = rtk_flash_partition_size[size_idx].bdinfo_size;					
            break;
        case FLASH_INDEX_JFFS2:             /* JFFS2_CFG */
            *offset = rtk_flash_partition_size[size_idx].jffs2_offset;
            *size = rtk_flash_partition_size[size_idx].jffs2_size;					
            break;
        case FLASH_INDEX_KERNEL:            /* KERNEL */
            ...
            *size = rtk_flash_partition_size[size_idx].kernel_size;
            break;
        case FLASH_INDEX_ROOTFS:            /* ROOTFS */
            *offset = rtk_flash_partition_size[size_idx].rootfs_offset;
            *size = rtk_flash_partition_size[size_idx].rootfs_size;	
            break;
#if defined(CONFIG_DUAL_IMAGE)
        case FLASH_INDEX_KERNEL2:           /* KERNEL2 */
            ...
            *size = rtk_flash_partition_size[size_idx].kernel2_size;
            break;
        case FLASH_INDEX_ROOTFS2:            /* ROOTFS2 */
            *offset = rtk_flash_partition_size[size_idx].rootfs2_offset;
            *size = rtk_flash_partition_size[size_idx].rootfs2_size;	
            break;
#endif
        default:
            break;
...
int upgrade_rom (char *filename, int force)
{
    ...
    if (!force)
    {
        ...
        offset = (PARTITION_SIZE(FLASH_INDEX_LOADER) + PARTITION_SIZE(FLASH_INDEX_LOADER_ENV) + PARTITION_SIZE(FLASH_INDEX_LOADER_ENV2) + PARTITION_SIZE(FLASH_INDEX_BDINFO));
        if (_write2flash(PARTITION_ADDR(FLASH_INDEX_JFFS2), (ROM_SIZE - offset), offset))
        {
