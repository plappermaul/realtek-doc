 ...
 */
int32
rtk_trap_portOamPduAction_get ( rtk_port_t port, rtk_action_t* pAction )
{
    int32   ret;

    if (NULL == RT_MAPPER->trap_portOamPduAction_get)
        return (RT_ERR_DRIVER_NOT_FOUND);

    RTK_API_LOCK();
    ret = RT_MAPPER->trap_portOamPduAction_get(port, pAction);
    RTK_API_UNLOCK();
    return (ret);
 ...
 */
int32
rtk_trap_portOamPduAction_set ( rtk_port_t port, rtk_action_t action )
{
    int32   ret;

    if (NULL == RT_MAPPER->trap_portOamPduAction_set)
        return (RT_ERR_DRIVER_NOT_FOUND);

    RTK_API_LOCK();
    ret = RT_MAPPER->trap_portOamPduAction_set(port, action);
    RTK_API_UNLOCK();
    return (ret);
 ...
 * Description:
 *      Get the configuration about uni trap priority for all kinds of trap.
 * Input:
 *      None.
 * Output:
 *      pEnable - status of uni trap priority.
 * Return:
 *      RT_ERR_OK
 *      RT_ERR_FAILED
 *      RT_ERR_NULL_POINTER - NULL pointer
 * Note:
 *      The status of uni trap priority:
 *      - DISABLED
 *      - ENABLED
 */
int32
...
{
    int32   ret;
    if (NULL == RT_MAPPER->trap_uniTrapPriorityEnable_get)
        return RT_ERR_DRIVER_NOT_FOUND;
    RTK_API_LOCK();
    ret = RT_MAPPER->trap_uniTrapPriorityEnable_get( pEnable);
    RTK_API_UNLOCK();
    return ret;
 ...
 * Description:
 *      Set the configuration about uni trap priority for all kinds of trap.
 * Input:
 *      enable - status of uni trap priority for all kinds of trap.
 * Output:
 *      None.
 * Return:
 *      RT_ERR_OK
 ...
 *      RT_ERR_INPUT   - Invalid input parameter
 * Note:
 *      The status of uni trap priority for all kinds of trap.:
 *      - DISABLED
 *      - ENABLED
 */
int32
...
{
    int32   ret;
    if (NULL == RT_MAPPER->trap_uniTrapPriorityEnable_set)
        return RT_ERR_DRIVER_NOT_FOUND;
    RTK_API_LOCK();
    ret = RT_MAPPER->trap_uniTrapPriorityEnable_set( enable);
    RTK_API_UNLOCK();
    return ret;
...
{
    int32   ret;
    if (NULL == RT_MAPPER->trap_uniTrapPriorityPriority_get)
        return RT_ERR_DRIVER_NOT_FOUND;
    RTK_API_LOCK();
    ret = RT_MAPPER->trap_uniTrapPriorityPriority_get( pPriority);
    RTK_API_UNLOCK();
    return ret;
...
{
    int32   ret;
    if (NULL == RT_MAPPER->trap_uniTrapPriorityPriority_set)
        return RT_ERR_DRIVER_NOT_FOUND;
    RTK_API_LOCK();
    ret = RT_MAPPER->trap_uniTrapPriorityPriority_set( priority);
    RTK_API_UNLOCK();
    return ret;
...
rtk_trap_cpuTrapHashMask_set(rtk_trap_hash_t type, rtk_enable_t state)
{
    ...
    if (NULL == RT_MAPPER->trap_cpuTrapHashMask_set)
        return RT_ERR_DRIVER_NOT_FOUND;

    RTK_API_LOCK();
    ret = RT_MAPPER->trap_cpuTrapHashMask_set(type, state);
    RTK_API_UNLOCK();
...
rtk_trap_cpuTrapHashMask_get(rtk_trap_hash_t type, rtk_enable_t *pState)
{
    ...
    if (NULL == RT_MAPPER->trap_cpuTrapHashMask_get)
        return RT_ERR_DRIVER_NOT_FOUND;

    RTK_API_LOCK();
    ret = RT_MAPPER->trap_cpuTrapHashMask_get(type, pState);
    RTK_API_UNLOCK();
...
rtk_trap_cpuTrapHashPort_set(uint32 value, rtk_port_t port)
{
    ...
    if (NULL == RT_MAPPER->trap_cpuTrapHashPort_set)
        return RT_ERR_DRIVER_NOT_FOUND;

    RTK_API_LOCK();
    ret = RT_MAPPER->trap_cpuTrapHashPort_set(value, port);
    RTK_API_UNLOCK();
...
rtk_trap_cpuTrapHashPort_get(uint32 value, rtk_port_t *pPort)
{
    ...
    if (NULL == RT_MAPPER->trap_cpuTrapHashPort_get)
        return RT_ERR_DRIVER_NOT_FOUND;

    RTK_API_LOCK();
    ret = RT_MAPPER->trap_cpuTrapHashPort_get(value, pPort);
    RTK_API_UNLOCK();
 ...
 */
int32
rtk_trap_cpuTrapHashState_set(rtk_enable_t state)
{
    int32   ret;

    /* function body */
    if (NULL == RT_MAPPER->trap_cpuTrapHashState_set)
        return RT_ERR_DRIVER_NOT_FOUND;
    RTK_API_LOCK();
    ret = RT_MAPPER->trap_cpuTrapHashState_set(state);
    RTK_API_UNLOCK();
    return ret;
}   /* end of rtk_trap_cpuTrapHashState_set */
 ...
 */
int32
rtk_trap_cpuTrapHashState_get(rtk_enable_t *pState)
{
    int32   ret;

    /* function body */
    if (NULL == RT_MAPPER->trap_cpuTrapHashState_get)
        return RT_ERR_DRIVER_NOT_FOUND;
    RTK_API_LOCK();
    ret = RT_MAPPER->trap_cpuTrapHashState_get(pState);
    RTK_API_UNLOCK();
    return ret;
}   /* end of rtk_trap_cpuTrapHashState_get */

/* Function Name:
 *      rtk_trap_cpuTrapInicHashState_set
 * Description:
 ...
 */
int32
rtk_trap_cpuTrapInicHashState_set(rtk_enable_t state)
{
    int32   ret;

    /* function body */
    if (NULL == RT_MAPPER->trap_cpuTrapInicHashState_set)
        return RT_ERR_DRIVER_NOT_FOUND;
    RTK_API_LOCK();
    ret = RT_MAPPER->trap_cpuTrapInicHashState_set(state);
    RTK_API_UNLOCK();
    return ret;
}   /* end of rtk_trap_cpuTrapInicHashState_set */

/* Function Name:
 *      rtk_trap_cpuTrapInicHashPort_get
 * Description:
 ...
 */
int32
rtk_trap_cpuTrapInicHashState_get(rtk_enable_t *pState)
{
    int32   ret;

    /* function body */
    if (NULL == RT_MAPPER->trap_cpuTrapInicHashState_get)
        return RT_ERR_DRIVER_NOT_FOUND;
    RTK_API_LOCK();
    ret = RT_MAPPER->trap_cpuTrapInicHashState_get(pState);
    RTK_API_UNLOCK();
    return ret;
}   /* end of rtk_trap_cpuTrapHashState_get */

/* Function Name:
 *      rtk_trap_cpuTrapInicHashSelect_set
 * Description:
 ...
 */
int32
rtk_trap_cpuTrapInicHashSelect_set(uint32 select)
{
    int32   ret;

    /* function body */
    if (NULL == RT_MAPPER->trap_cpuTrapInicHashSelect_set)
        return RT_ERR_DRIVER_NOT_FOUND;
    RTK_API_LOCK();
    ret = RT_MAPPER->trap_cpuTrapInicHashSelect_set(select);
    RTK_API_UNLOCK();
    return ret;
}   /* end of rtk_trap_cpuTrapInicHashSelect_set */

/* Function Name:
 *      rtk_trap_cpuTrapInicHashSelect_get
 * Description:
 ...
 */
int32
rtk_trap_cpuTrapInicHashSelect_get(uint32 *pSelect)
{
    int32   ret;

    /* function body */
    if (NULL == RT_MAPPER->trap_cpuTrapInicHashSelect_get)
        return RT_ERR_DRIVER_NOT_FOUND;
    RTK_API_LOCK();
    ret = RT_MAPPER->trap_cpuTrapInicHashSelect_get(pSelect);
    RTK_API_UNLOCK();
    return ret;
}   /* end of rtk_trap_cpuTrapInicHashSelect_get */
