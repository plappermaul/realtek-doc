...
ca_uint32_t ca_rtk_onu_state_change_intr(ca_device_id_t device_id,ca_uint32_t event_id,ca_void_t *event,ca_uint32_t len,ca_void_t *cookie)
{
    ...
    else if(CA_PON_MODE_IS_2D5GPON(CA_PON_MODE))
    {
        if(gpon_state_ptr->status == 0)
        {
            intrSubType = 1; //O1
...
dal_rtl9607f_intr_init(void)
{
    ...
    if((ret = dal_rtl9607f_intr_imr_set(INTR_TYPE_LOS,ENABLED)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_INTR | MOD_DAL), "");
        intr_init = INIT_NOT_COMPLETED;
        return ret;
    ...
    if((ret = dal_rtl9607f_intr_imr_set(INTR_TYPE_LOF,ENABLED)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_INTR | MOD_DAL), "");
        intr_init = INIT_NOT_COMPLETED;
        return ret;
