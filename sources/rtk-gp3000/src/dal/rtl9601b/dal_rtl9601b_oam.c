 ...
 * Data Declaration
 */
static uint32   rtl9601b_oam_init = {INIT_NOT_COMPLETED}; 
...
dal_rtl9601b_oam_init(void)
{
    ...
    rtl9601b_oam_init = INIT_COMPLETED;
    
    HAL_SCAN_ALL_PORT(port)
    {
        if ((ret = dal_rtl9601b_oam_parserAction_set(port, OAM_PARSER_ACTION_FORWARD)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
            rtl9601b_oam_init = INIT_NOT_COMPLETED;
            return ret;
        }
        
        if ((ret = dal_rtl9601b_oam_multiplexerAction_set(port, OAM_MULTIPLEXER_ACTION_FORWARD)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_ACL|MOD_DAL), "");
            rtl9601b_oam_init = INIT_NOT_COMPLETED;
            return ret;
...
dal_rtl9601b_oam_parserAction_set(rtk_port_t port, rtk_oam_parser_act_t action)
{
    ...
    val = (uint32)action;
    if ((ret = reg_array_field_write(RTL9601B_OAM_P_CTRL_0r, port, REG_ARRAY_INDEX_NONE, RTL9601B_OAM_PARSERf, &val)) != RT_ERR_OK)
    {
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9601b_oam_parserAction_set */
...
dal_rtl9601b_oam_parserAction_get(rtk_port_t port, rtk_oam_parser_act_t *pAction)
{
    ...
    if ((ret = reg_array_field_read(RTL9601B_OAM_P_CTRL_0r, port, REG_ARRAY_INDEX_NONE, RTL9601B_OAM_PARSERf, &val)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_OAM), "");
        return ret;
    }
    ...
    *pAction = (rtk_oam_parser_act_t)val;    
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9601b_oam_parserAction_get */
...
dal_rtl9601b_oam_multiplexerAction_set(rtk_port_t port, rtk_oam_multiplexer_act_t action)
{
    ...
    val = (uint32)action;
    if ((ret = reg_array_field_write(RTL9601B_OAM_P_CTRL_1r, port, REG_ARRAY_INDEX_NONE, RTL9601B_OAM_MULTIPLEXERf, &val)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_OAM), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9601b_oam_multiplexerAction_set */
...
dal_rtl9601b_oam_multiplexerAction_get(rtk_port_t port, rtk_oam_multiplexer_act_t *pAction)
{
    ...
    if ((ret = reg_array_field_read(RTL9601B_OAM_P_CTRL_1r, port, REG_ARRAY_INDEX_NONE, RTL9601B_OAM_MULTIPLEXERf, &val)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_OAM), "");
        return ret;
    }
