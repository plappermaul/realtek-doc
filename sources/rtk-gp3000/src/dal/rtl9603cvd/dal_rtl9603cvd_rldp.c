 ...
 * Data Declaration
 */
static uint32    rtl9603cvd_rldp_init = {INIT_NOT_COMPLETED}; 
...
dal_rtl9603cvd_rldp_init(void)
{
    ...
    rtl9603cvd_rldp_init = INIT_COMPLETED;
    ...
    config.num_loop = 4;
    if ((ret = dal_rtl9603cvd_rldp_config_set(&config)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_RLDP|MOD_DAL), "");
        rtl9603cvd_rldp_init = INIT_NOT_COMPLETED;
        return ret;
    }
    ...
    HAL_SCAN_ALL_PORT(port)
    {
        ...
        {
            if ((ret = dal_rtl9603cvd_rldp_portConfig_set(port, &portConfig)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_RLDP|MOD_DAL), "");
                rtl9603cvd_rldp_init = INIT_NOT_COMPLETED;
                return ret;
    ...
    HAL_SCAN_ALL_PORT(port)
    {
        if(HAL_IS_RLDP_PORT(port))
        {
            if ((ret = dal_rtl9603cvd_rldp_portStatus_clear(port, &portStatus)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_RLDP|MOD_DAL), "");
                rtl9603cvd_rldp_init = INIT_NOT_COMPLETED;
                return ret;
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603cvd_rldp_init */


/* Function Name:
 *      dal_rtl9603cvd_rldp_config_set
 * Description:
 ...
 */
int32
dal_rtl9603cvd_rldp_config_set(rtk_rldp_config_t *pConfig)
{
    int32   ret;
    uint32 tmp[2];
    uint32 val;
    ...
    /* check Init status */
    RT_INIT_CHK(rtl9603cvd_rldp_init);
    ...
    RT_PARAM_CHK((RTL9603CVD_RLDP_INTERVAL_MAX < pConfig->interval_check), RT_ERR_INPUT);
    RT_PARAM_CHK((RTL9603CVD_RLDP_NUM_MAX < pConfig->num_check), RT_ERR_INPUT);	
    RT_PARAM_CHK((RTL9603CVD_RLDP_INTERVAL_MAX < pConfig->interval_loop), RT_ERR_INPUT);
    RT_PARAM_CHK((RTL9603CVD_RLDP_NUM_MAX < pConfig->num_loop), RT_ERR_INPUT);

    val = pConfig->rldp_enable;
    if ((ret = reg_field_write(RTL9603CVD_RLDP_CTRL_1r, RTL9603CVD_ENf, &val)) != RT_ERR_OK)
    {
    ...
    val = pConfig->fc_mode;
    if ((ret = reg_field_write(RTL9603CVD_RLDP_CTRL_0r, RTL9603CVD_ACT_RUNOUTDSCf, &val)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_RLDP), "");
        return ret;
    }
    ...
    tmp[0] =
        (pConfig->magic.octet[2] << 24) |
            (pConfig->magic.octet[3] << 16) |
            (pConfig->magic.octet[4] << 8) |
            (pConfig->magic.octet[5]);
    tmp[1] = (pConfig->magic.octet[0] << 8) | (pConfig->magic.octet[1]);

    if ((ret = reg_field_write(
                    RTL9603CVD_RLDP_MAGIC_NUMr,
                    RTL9603CVD_NUM_31_0f,
                    &tmp[0])) != RT_ERR_OK) {
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603cvd_rldp_config_set */


/* Function Name:
 *      dal_rtl9603cvd_rldp_config_get
 * Description:
    ...
    rtk_rldp_config_t *pConfig)
{
    ...
    if ((ret = reg_field_read(
                    RTL9603CVD_RLDP_CTRL_1r,
                    RTL9603CVD_ENf, &val)) != RT_ERR_OK) {
    ...
    if ((ret = reg_field_read(
                    RTL9603CVD_RLDP_CTRL_0r,
                    RTL9603CVD_ACT_RUNOUTDSCf, &val)) != RT_ERR_OK) {
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603cvd_rldp_config_get */


/* Function Name:
 *      dal_rtl9603cvd_rldp_portConfig_set
 * Description:
    ...
    rtk_rldp_portConfig_t *pPortConfig)
{
    ...
    RT_PARAM_CHK(!HAL_IS_RLDP_PORT(port), RT_ERR_PORT_ID);
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603cvd_rldp_portConfig_set */

/* Function Name:
 *      dal_rtl9603cvd_rldp_portConfig_get
 * Description:
 ...
 */
int32
dal_rtl9603cvd_rldp_portConfig_get(rtk_port_t port, rtk_rldp_portConfig_t *pPortConfig)
{
    int32 ret;
    uint32 val;
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603cvd_rldp_portConfig_get */


/* Function Name:
 *      dal_rtl9603cvd_rldp_status_get
 * Description:
 ...
 */
int32
dal_rtl9603cvd_rldp_status_get(rtk_rldp_status_t *pStatus)
{
    int32   ret;
    uint32 tmp[2];
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603cvd_rldp_status_get */


/* Function Name:
 *      dal_rtl9603cvd_rldp_portStatus_get
 * Description:
 *      Get RLDP module status
 * Input:
 *      port    - port number to be get
    ...
    rtk_rldp_portStatus_t *pPortStatus)
{
    ...
    if ((ret = reg_array_field_read(
                    RTL9603CVD_RLDP_PORT_LP_ENTER_STSr,
                    port,
                    REG_ARRAY_INDEX_NONE,
                    RTL9603CVD_STSf,
                    &val)) != RT_ERR_OK) {
    ...
    if ((ret = reg_array_field_read(
                    RTL9603CVD_RLDP_PORT_LP_LEAVE_STSr,
                    port,
                    REG_ARRAY_INDEX_NONE,
                    RTL9603CVD_STSf,
                    &val)) != RT_ERR_OK) {
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603cvd_rldp_portStatus_get */
 ...
 *          Name:
 *
 *                  dal_rtl9603cvd_rldp_portStatus_clear
 * Description
 *          :
 *      Clear
 *                  RLDP module status * Input:
 *
    ...
    rtk_rldp_portStatus_t *pPortStatus)
{
    ...
    RT_PARAM_CHK((NULL == pPortStatus), RT_ERR_NULL_POINTER);
    RT_PARAM_CHK(
        (RTK_RLDP_LOOPSTS_END <= pPortStatus->loop_enter),
        RT_ERR_INPUT);
    RT_PARAM_CHK(
        (RTK_RLDP_LOOPSTS_END <= pPortStatus->loop_leave),
        RT_ERR_INPUT);
    ...
    return RT_ERR_OK;
} /* end of dal_rtl9603cvd_rldp_portStatus_clear */
