...
ca_uint32_t ca_rtk_onu_dying_gasp_intr(ca_device_id_t device_id,ca_uint32_t event_id,ca_void_t *event,ca_uint32_t len,ca_void_t *cookie)
{
    ...
    if (RT_ERR_OK != dal_ca8277b_switch_chip_reset())
    {
...
dal_ca8277b_intr_init(void)
{
    ...
    if((ret = dal_ca8277b_intr_imr_set(INTR_TYPE_LINK_CHANGE,ENABLED)) != RT_ERR_OK)
    {
    ...
    if((ret = dal_ca8277b_intr_imr_set(INTR_TYPE_GPON,ENABLED)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_INTR | MOD_DAL), "");
        intr_init = INIT_NOT_COMPLETED;
        return ret;
    ...
    if((ret = dal_ca8277b_intr_imr_set(INTR_TYPE_DYING_GASP,ENABLED)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_INTR | MOD_DAL), "");
        intr_init = INIT_NOT_COMPLETED;
        return ret;
...
dal_ca8277b_intr_imr_set(rtk_intr_type_t intr, rtk_enable_t enable)
{
    ...
    return RT_ERR_OK;
} /* end of dal_ca8277b_intr_imr_set */
...
dal_rt_ca8277b_intr_isr_rx_callback_register(rt_intr_type_t intr,rt_intr_isr_rx_callback isrRx)
{
    ...
    return RT_ERR_OK;
}   /* end of dal_rt_ca8277b_intr_isr_rx_callback_register */
