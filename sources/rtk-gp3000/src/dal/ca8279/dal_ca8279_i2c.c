...
static uint8 gbl_clock = 100; //100kHZ
    ...
    uint32* pClock )
{
    ...
    *pClock = gbl_clock;
    ...
    return RT_ERR_OK;
}   /* end of dal_ca8279_i2c_clock_get */
    ...
    uint32 clock )
{
    ...
    /* function body */
    if((ret = cap_i2c_enable_set(0,1,clock)) != CA_E_OK)
    {
        RT_ERR(ret, (MOD_HWMISC | MOD_DAL), "");
        return RT_ERR_FAILED;
    }
    ...
    return RT_ERR_OK;
}   /* end of dal_ca8279_i2c_clock_set */
    ...
    uint32 count)
{
    ...
    for(i = 0 ; i < count ; i++)
    {
        if((ret = dal_ca8279_i2c_read(i2cPort, devID, accessAddr, &pData[i])) != RT_ERR_OK)
        {
            return ret;
        }
    ...
    uint32 count)
{
    ...
    for(i = 0 ; i < count ; i++)
    {
        if((ret = dal_ca8279_i2c_write(i2cPort, devID, accessAddr, pData[i])) != RT_ERR_OK)
        {
            return ret;
        }
        if(accessAddr == ((((uint64)1)<<addrBits) - 1))
