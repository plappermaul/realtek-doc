 ...
 */
int32 dal_ca8279_init(void)
{
    return RT_ERR_OK;
} /* end of dal_ca8279_init */



/* Function Name:
 *      dal_ca8279_mapper_get
 * Description:
 ...
 * Note:
 */
dal_mapper_t *dal_ca8279_mapper_get(void)
{
    return &dal_ca8279_mapper;
} /* end of dal_ca8279_mapper_get */



void dal_ca8279_mapper_register(dal_mapper_t *pMapper)
{
    int32 ret;

    memcpy(&dal_ca8279_mapper,pMapper,sizeof(dal_mapper_t));

    /*initial function*/
    if((ret = dal_ca8279_mapper.switch_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.l2_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.vlan_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.stat_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.rate_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.qos_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.intr_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.port_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.sec_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.epon_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.trap_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.i2c_init(0)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.led_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.gpon_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.mirror_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.ponmac_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

    if((ret = dal_ca8279_mapper.gpio_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }

#if defined(CONFIG_COMMON_RT_API)
    if((ret = dal_ca8279_mapper.rt_ponmisc_init()) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_SEC), "");
    }
#endif
...
#if defined(CONFIG_SDK_KERNEL_LINUX)
EXPORT_SYMBOL(dal_ca8279_mapper_register);
#endif
