...
static int _ponmisc_mode_set(ca_uint8_t pon_mode)
{
    ...
    {
        if((pon_mode == ONU_PON_MAC_GPON) || (pon_mode == ONU_PON_MAC_EPON_1G))
        {
            //assume disable state is for GPON/EPON
            if((ret = dal_rtl8277c_ponmac_trxSwitchGpio_set(DISABLED)) != CA_E_OK)
            {
                ca_printf("%s %d dal_rtl8277c_ponmac_trxSwitchGpio_set fail \n",__FUNCTION__,__LINE__);
                return RT_ERR_FAILED;
            }
        }
        else
        {
            ...
            if((ret = dal_rtl8277c_ponmac_trxSwitchGpio_set(ENABLED)) != CA_E_OK)
            {
                ca_printf("%s %d dal_rtl8277c_ponmac_trxSwitchGpio_set fail \n",__FUNCTION__,__LINE__);
                return RT_ERR_FAILED;
            }
...
_dal_rt_rtl8277c_ponmisc_tcont_llid_logic2phy_init(ca_uint32_t pon_mode)
{
    ...
    for(portId=portStartIdx;portId< RTL8277C_TCONT_LLID_DQ_CHECK_START_IDX; portId++)
    {
...
dal_rt_rtl8277c_ponmisc_mode_get(rt_ponmisc_ponMode_t *pPonMode,rt_ponmisc_ponSpeed_t *pPonSpeed)
{
    ...
    switch(CA_PON_MODE)
    {
        ...
        case ONU_PON_MAC_ETHERNET_USXGMII_500M:
    ...
    else if(ponMode == RT_USXGMII_MODE)
    {
        switch(ponSpeed)
        {
            ...
            case RT_1G_SPEED:
                ...
                break;
            case RT_500M_SPEED:
                ponmode = ONU_PON_MAC_ETHERNET_USXGMII_500M;
                break;
            case RT_100M_SPEED:
