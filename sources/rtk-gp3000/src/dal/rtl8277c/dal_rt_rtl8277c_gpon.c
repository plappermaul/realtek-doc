...
static rt_gpon_omci_rx_callback gFttrOmciRxCallback = NULL;
...
#define RTL8277C_MAX_US_SID (512) 
...
/*The XGEM/GEM port table for data use from 8~255*/
#define RTL8277C_GPON_GEM_IDX_MIN    (8)
#define RTL8277C_GPON_GEM_IDX_MAX    (255)
...
_dal_rtl8277c_gpon_db_init(void)
{
    ...
    {
        g8277cGemUsFlowMap = kmalloc(sizeof(dal_rtl8277c_usFlowMap_t) * RTL8277C_MAX_US_SID, GFP_KERNEL);
    ...
    for(gemFlow = 0;gemFlow<RTL8277C_MAX_US_SID;gemFlow++)
    {
    ...
    for(tcontId=0;tcontId<RTL8277C_MAX_TCONT;tcontId++)
    {
        ...
        if((ret = dal_rt_rtl8277c_ponmisc_tcont_llid_logic2phy_get(tcontId, &g8277cTcontTbl[tcontId].phyTcontId)) != RT_ERR_OK)
        {
...
__tcont_tbl_is_empty (unsigned short ignoreGemFlowId)
{
    ...
    for (gemFlowId = 0; gemFlowId < RTL8277C_MAX_US_SID; gemFlowId++) 
    {
...
{    
    ...
    {      
        /* parameter check */
        RT_PARAM_CHK((gemFlowId >= RTL8277C_MAX_US_SID), RT_ERR_INPUT);
...
_rtl8277c_gpon_aal_tcont_set(unsigned int tcontId, unsigned int allocId)
{
    ...
    //("Add TCONT %u Alloc-ID %u",tcontId,  allocId);
...
_rtl8277c_gpon_aal_tcont_del(unsigned int tcontId)
{
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        RTL8277C_LOG_DBG("Del TCONT %u Alloc-ID %u",tcontId,  allocId);
    }
    else
    {
#if 0
...
_rtl8277c_gpon_aal_usGem_set(unsigned int usFlowId, const rt_gpon_usFlow_t *pUsFlow, unsigned short *pGemIdx)
{
    ...
    unsigned int internal_gem_idx, us_sid;
    int ret;
    unsigned char isFound = FALSE;

    /* parameter check */
    RT_PARAM_CHK((NULL == pUsFlow), RT_ERR_NULL_POINTER);
    RT_PARAM_CHK((usFlowId >= RTL8277C_MAX_US_SID), RT_ERR_INPUT);
#if 0
    ...
    for(us_sid = 0; us_sid < RTL8277C_MAX_US_SID; us_sid++)
    {
        if(TRUE == g8277cGemUsFlowMap[us_sid].isValid &&
            g8277cGemUsFlowMap[us_sid].gemPortId == pUsFlow->gemPortId)
        {
            internal_gem_idx = g8277cGemUsFlowMap[us_sid].gemIdx;
            isFound = TRUE;
            RTL8277C_LOG_ERR("%s:%d internal_gem_idx=%d, gemPortId=%d exist", 
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        ...
        if(!isFound)
        {
            for(internal_gem_idx = RTL8277C_GPON_GEM_IDX_MIN; internal_gem_idx <= RTL8277C_GPON_GEM_IDX_MAX; internal_gem_idx++)
            {
                if (CA_E_OK != aal_gpon_us_gem_port_get(SATURN_DEV_ID, internal_gem_idx, &gem_id))
                {
                    RTL8277C_LOG_ERR("%s:%d::ERROR get us gem fail",__FUNCTION__,__LINE__);
                    return RT_ERR_FAILED;
                }
                ...
                if(gem_id == 0x0)
                {
                    ...
                    RTL8277C_LOG_DBG("%s:%d internal_gem_idx=%d, gemPortId=%d new", 
                        __FUNCTION__, __LINE__, internal_gem_idx, pUsFlow->gemPortId);
                    break;
                }  
            }
...
_rtl8277c_gpon_aal_usGem_del(unsigned int usFlowId)
{
    ...
    /*Check the gem port is used by other usFlowId*/
    gemPortId = g8277cGemUsFlowMap[usFlowId].gemPortId;
    for (us_flow_id = 0; us_flow_id < RTL8277C_MAX_US_SID; us_flow_id++) 
    {
        if(us_flow_id == usFlowId)
            continue;
        
        if (g8277cGemUsFlowMap[us_flow_id].isValid) 
        {
            if(g8277cGemUsFlowMap[us_flow_id].gemPortId == gemPortId)
            {
                RTL8277C_LOG_ERR("%s:%d::gemPortId:%u exist, ignore", __FUNCTION__, __LINE__, gemPortId);                    
                return RT_ERR_OK;
            }
        }
    }
...
//            }
    ...
    else
    {
        ...
        us_xgem_cfg.xgem_id = AAL_XGPON_IDLE_XGEM_PORT_ID;
...
int _rtl8277c_receive_from_nic(struct napi_struct *napi,struct net_device *dev, struct sk_buff *skb, nic_hook_private_t *nh_priv)
{
    ...
    uint8_t * omci_ptr = NULL;
    uint32_t omci_len;
    uint8 isOmciFlag = FALSE;
    uint8 isExcuteOmciRxCbFlag = FALSE;
    uint8 isExcuteFttrOmciRxCbFlag = FALSE;
    rt_gpon_omci_pkt_hdr_t *pHdr = NULL;
    ...
    if (isOmciFlag == TRUE)
    {
        ...
        pHdr = (rt_gpon_omci_pkt_hdr_t *)omci_ptr;

        if ((0x0a == pHdr->devId || 0x0b == pHdr->devId ) && gOmciRxCallback)
        {
            isExcuteOmciRxCbFlag = TRUE;
        } 
                  
#if defined(CONFIG_CMCC_FTTR_PLUS)
        else if ((0x1a == pHdr->devId || 0x1b == pHdr->devId ) && gFttrOmciRxCallback)
        {
            isExcuteFttrOmciRxCbFlag= TRUE;

        }            
        else if ((0x1a == pHdr->devId || 0x1b == pHdr->devId ) && gOmciRxCallback)
        {
            isExcuteOmciRxCbFlag= TRUE;

        }            
#endif
        if(isExcuteOmciRxCbFlag || isExcuteFttrOmciRxCbFlag)
        {
            ...
            if(isExcuteOmciRxCbFlag) 
                gOmciRxCallback(omci_len, omci_ptr);
            else if(isExcuteFttrOmciRxCbFlag)
                gFttrOmciRxCallback(omci_len, omci_ptr);
...
static int _rtl8277c_gpon_omcc_get(rt_gpon_omcc_t *pOmcc)
{
    ...
    else
    {
        ...
        if (CA_E_OK != aal_xgpon_usbg_xgem_cfg_get(SATURN_DEV_ID, 6, &us_xgem_cfg))
        {
            RTL8277C_LOG_ERR("%s:%d::ERROR get usbg xgem fail",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
 ...
 */
int32
dal_rt_rtl8277c_gpon_serialNumber_get(rt_gpon_serialNumber_t *pSN)
{
    ca_status_t ret = CA_E_OK;
    aal_xgpn_plmp_rand_cfg_mask_t  rand_mask;
    aal_xgpn_plmp_rand_cfg_t       rand;
    ...
    else
    {
        ...
        if(CA_E_OK != ret)
        {
            RTL8277C_LOG_ERR("%s %d get gpon sn  fail\n",__FUNCTION__,__LINE__);
            return RT_ERR_FAILED;
        }
    ...
    return RT_ERR_OK;
}   /* end of dal_rtl8277c_gpon_serialNumber_get */
 ...
 */
int32
dal_rt_rtl8277c_gpon_registrationId_get(rt_gpon_registrationId_t *pRegId)
{
    ca_status_t   ret;
    aal_gpon_passwd_t passwd = {0};
    ...
    return RT_ERR_OK;
}   /* end of dal_rt_rtl8277c_registrationId_get */
...
dal_rt_rtl8277c_gpon_deactivate(void)
{
    ...
    if(CA_PON_MODE == ONU_PON_MAC_GPON)
    {
        /* do datapath reset when disable gponmac */
        aal_gpon_datapath_reset(SATURN_DEV_ID);
        ca_printf("%s(): datapath reset!\r\n", __func__);
    }
...
dal_rt_rtl8277c_gpon_omci_rx_callback_register(rt_gpon_omci_rx_callback omciRx)
{
    ...
    {
        if((ret = nic_register_rxhook(0xffffffff,RENIC_RXPRI_PATCH,_rtl8277c_receive_from_nic))!=RT_ERR_OK)
        {
...
dal_rt_rtl8277c_gpon_tcont_set(uint32 *pTcontId, uint32 allocId)
{
    ...
    RTL8277C_LOG_DBG("Tcont_add alloc:%u tcont:%d phytcont=%d\n",allocId,*pTcontId,LOGIC_TCONID_TO_RTL8277C_TCONID(tcontId));  
...
dal_rt_rtl8277c_gpon_tcont_del(uint32 tcontId)
{
    ...
    RTL8277C_LOG_DBG("Tcont_del tcont:%d phytcont=%d\n",tcontId,LOGIC_TCONID_TO_RTL8277C_TCONID(tcontId));
...
int32 dal_rt_rtl8277c_gpon_usFlow_set(uint32 usFlowId, const rt_gpon_usFlow_t *pUsFlow)
{
    ...
    RTL8277C_LOG_DBG(
    "uFlow_set flowId:%d gemId:%d tcont:%d tcontQ:%d\n",
    usFlowId,
    pUsFlow->gemPortId,
    pUsFlow->tcontId,
    pUsFlow->tcQueueId);
...
int32 dal_rt_rtl8277c_gpon_usFlow_del(uint32 usFlowId)
{
    ...
    RTL8277C_LOG_DBG("uFlow_del flowId:%d gemIdx:%d\n",
        usFlowId,
...
int32 dal_rt_rtl8277c_gpon_usFlow_delAll(void)
{
    ...
    for(i=0;i<RTL8277C_MAX_US_SID;i++)
    {
        /*Delete DS gem flow*/
    ...
    switch(attributeType)
    {
        ...
        case RT_GPON_ATTRIBUTE_TYPE_GTC_INFO:
            if(CA_PON_MODE == ONU_PON_MAC_GPON)
            {
                ...
                {
                    pAttributeValue->gtc_info.responTime = 35259 - (0x200-(frm_cfg.frame_var&0x1ff))*643/100;
                }
                else
                {
                    pAttributeValue->gtc_info.responTime = 35259 + frm_cfg.frame_var*643/100;
                }
            }
            else
            {
...
dal_rt_rtl8277c_gpon_fttr_omci_rx_callback_register(rt_gpon_omci_rx_callback fttrOmciRx)
{
   ...
   if(DISABLED == gOmciTrapEnable)
   {
    ...
    gFttrOmciRxCallback = fttrOmciRx;
    
    return RT_ERR_OK;
}   /* end of dal_rt_rtl8277c_gpon_fttr_omci_rx_callback_register */
