...
#define RTL8277C_CLS_RULE_DEFAULT_RESERVED_NUM 48
...
#define RTL8277C_CLS_XLATEVID_NOT_USED 0xFFFFFFFE
...
extern int32 dal_rtl8277c_ca_specpkt_ctrl_ldpid_update (rtk_port_t port);
#endif
    ...
    if(RT_CLS_FILTER_OUTER_TAGIf_BIT & pClsRule->filterRule.filterMask && pClsRule->filterRule.outerTagIf == 1)
    {
        ...
        else
        {
            ...
            if(pClsRule->filterRule.outerTagTpid != RTL8277C_CLS_DEFAULT_STAG_TPID && pClsRule->filterRule.outerTagTpid == RTL8277C_CLS_DEFAULT_CTAG_TPID)
            {
                ...
                memset(&ilpbCfg,0,sizeof(aal_ilpb_cfg_t));
                if(CA_E_OK == aal_port_ilpb_cfg_get(DEFAULT_DEVICE_ID,RTL8277C_CLS_EXT_WAN_PORT,&ilpbCfg))
                {
                    ilpbCfg.s_tpid_match |= 1;
                    aal_port_ilpb_cfg_set(DEFAULT_DEVICE_ID,RTL8277C_CLS_EXT_WAN_PORT,ilpbCfgMask,&ilpbCfg);
                }
...
static void _8277c_cls_vlan_aggregate_cf_delAll(void)
{
    ...
    struct list_head *pEntry    = NULL;
    struct list_head *pTmpEntry = NULL;
    dal_8277c_cls_rule_mcgid_t *pMcgidEntryData= NULL;

    list_for_each_safe(cur_next, cur_tmp, &vlanRuleIndexHead)
    {
        cur = list_entry(cur_next, dal_8277c_cls_vlan_rule_index_t, list);

        while(_8277c_cls_rule_index_find(cur->index+RTL8277C_CLS_XLATEVID_INDEX_SHIFT_BASE, RT_CLS_DIR_DS, &index,RTL8277C_CLS_RULE_PORT_IGNORE, 1) == RT_ERR_OK)
        {
        ...
        {
            pMcgidEntryData = list_entry(pEntry, dal_8277c_cls_rule_mcgid_t, list);
            if (pMcgidEntryData->ruleIndex == cur->index+RTL8277C_CLS_XLATEVID_INDEX_SHIFT_BASE)
            {
                                              ...
                                              unsigned int uniMask)
{
    ...
    if((pDsVlanRule->filterRule.filterMask & RT_CLS_FILTER_OUTER_TAGIf_BIT) && pDsVlanRule->filterRule.outerTagIf == 1)
    {
        ...
        {
            xlate_cvid = RTL8277C_CLS_XLATEVID_TRANSPATENT;
        }
        ...
        {
            if(outerVID == RTL8277C_CLS_XLATEVID_NOT_USED)
                xlate_cvid = RTL8277C_CLS_XLATEVID_TRANSPATENT;
            else
                xlate_cvid = outerVID;
        }
...
static int32 _8277c_cls_vlan_aggregate_cf_set(unsigned int mode, rt_cls_rule_t *pClsRule,int portIndex, dal_8277c_cls_vlan_xlate_info_t *pXlateVID)
{
    aal_l2_cf_rule_t entry;
    int32 index=0, i;
    int32 ret=RT_ERR_FAILED;
    uint32 targetUniMask = 0x0;
    dal_8277c_cls_rule_mcgid_t *pEntryData = NULL;
    ...
    if( mode == RTL8277C_CLS_XLATEVID_TWO_MORE)  /* LUT */
    {
        ret = _8277c_cls_rule_index_get(pClsRule->index+RTL8277C_CLS_XLATEVID_INDEX_SHIFT_BASE, pClsRule->direction, &index, portIndex, 1, 1);
        if(ret != RT_ERR_OK)
        {
    ...
    {
        ret = _8277c_cls_rule_index_get(pClsRule->index+RTL8277C_CLS_XLATEVID_INDEX_SHIFT_BASE, pClsRule->direction, &index, HAL_GET_PON_PORT(), 0, 1);
        if(ret != RT_ERR_OK)
        {
            printk("[%s] %d Get cls rule index fail ret:%d ruleIndex:%d dir:%d"
                ,__FUNCTION__, __LINE__, ret, pClsRule->index, pClsRule->direction);
...
#if defined(CONFIG_SDK_KERNEL_LINUX)
        pEntryData = kmalloc(sizeof(dal_8277c_cls_rule_mcgid_t), GFP_KERNEL);
        if(!pEntryData)
        {
        ...
        memset(pEntryData, 0, sizeof(dal_8277c_cls_rule_mcgid_t));
        pEntryData->ruleIndex = pClsRule->index+RTL8277C_CLS_XLATEVID_INDEX_SHIFT_BASE;
        ...
        for(i = 0; i < RTK_MAX_NUM_OF_PORTS; i++)
        {
            if(pXlateVID[i].xlateMode != RTL8277C_CLS_XLATEVID_NOT_USED)
            {
                ...
                portAction.ldpid = i;
                if(pXlateVID[i].vid == RTL8277C_CLS_XLATEVID_UNTAG)
                {
...
static int _8277c_cls_l2_vlan_aggregate(void)
{
    ...
    list_for_each_safe(cur_next, cur_tmp, &dsRuleHead)
    {
        ...
        _8277c_cls_vlan_aggregate_fill_xlateVID(pXlateVID, &cur->rule, outerVID, curUniMask);
            ...
            if ((cur->rule.filterRule.flowId == tar->rule.filterRule.flowId) && (cur->rule.index != tar->rule.index))
            {
                if (_8277c_cls_is_same_rule_by_ds_vlan_filter( &cur->rule, &tar->rule) == TRUE)
                {
                    ...
                    if (cur->rule.index < tar->rule.index || (tar->rule.filterRule.filterMask & fwdAllMask) == 0)
                    {
                        ...
                        _8277c_cls_vlan_aggregate_fill_xlateVID(pXlateVID, &(tar->rule), outerVID, tarUniMask);
                    }
                    else
                    {
        ...
        if (setupAggVlanCf == TRUE)
        {
            for(i = 0; i < RTK_MAX_NUM_OF_PORTS; i++)
            {
                if(HAL_IS_CPU_PORT(i))
                    continue;
                if (pXlateVID[i].xlateMode == RTL8277C_CLS_XLATEVID_TWO_MORE)
                {
                    if ((ret = _8277c_cls_vlan_aggregate_cf_set(RTL8277C_CLS_XLATEVID_TWO_MORE,  &(cur->rule), i, pXlateVID)) != RT_ERR_OK)
                        printk("omci_set_vlan_aggregate_cf Fail, ret = 0x%X", ret);
                }
            }

            if ((ret = _8277c_cls_vlan_aggregate_cf_set(RTL8277C_CLS_XLATEVID_ONLY_ONE,  &(cur->rule), i, pXlateVID)) != RT_ERR_OK)
            ...
            if (!pVlanRuleIndexEntry)
                return RT_ERR_FAILED;
            memset(pVlanRuleIndexEntry, 0, sizeof(dal_8277c_cls_vlan_rule_index_t));
...
static int32 _8277c_cls_rule_set(rt_cls_rule_t *pClsRule)
{
    ...
    if(pClsRule->direction == RT_CLS_DIR_DS)
    {
        ...
        list_for_each_safe(pEntry, pTmpEntry, &mcgidHead)
        {
            pEntryData = list_entry(pEntry, dal_8277c_cls_rule_mcgid_t, list);
...
int32 dal_rt_rtl8277c_cls_init(void)
{
    ...
    uint32 index = RTL8277C_QOS_INGRESS_QOS_RESERVED_NUM, i;
    rtk_port_t port;
    int ruleMax = HAL_CLASSIFY_ENTRY_MAX();
    aal_ilpb_cfg_t ilpbCfg;
    aal_ilpb_cfg_msk_t ilpbCfgMask;
    ...
    aal_ple_dft_fwd_set(0,RTL8277C_CLS_EXT_WAN_PORT,AAL_PLE_NONKWN_TYPE_UUC,dftMask,&dftCfg);
    ...
    dftMask.bf.dft_fwd_mc_group_id_valid = 1;
    dftMask.bf.dft_fwd_mc_group_id = 1;
    aal_ple_dft_fwd_set(0,RTL8277C_CLS_EXT_WAN_PORT,AAL_PLE_NONKWN_TYPE_BC,dftMask,&dftCfg);
    aal_ple_dft_fwd_set(0,RTL8277C_CLS_EXT_WAN_PORT,AAL_PLE_NONKWN_TYPE_UL2MC,dftMask,&dftCfg);
    aal_ple_dft_fwd_set(0,RTL8277C_CLS_EXT_WAN_PORT,AAL_PLE_NONKWN_TYPE_UL3MC,dftMask,&dftCfg);
    ...
    ethertypeTable[RTL8277C_TRAP_LOOP_DETECT_EV2PT_ID] = RTL8277C_TRAP_LOOP_DETECT_ETHERTYPE;
...
int32 dal_rt_rtl8277c_cls_rule_add(rt_cls_rule_t *pClsRule)
{
    ...
    /* check Init status */
    RT_INIT_CHK(cls_init);
    RT_PARAM_CHK(pClsRule->flowId >= RTL8277C_CLS_SID_MAX, RT_ERR_INPUT);
 ...
 *      None.
 */
int32 dal_rt_rtl8277c_cls_rule_get(rt_cls_get_op_t op, uint32 index, rt_cls_dir_t dir, rt_cls_rule_info_t *pClsRuleInfo)
{
    int32   ret = RT_ERR_OK;
...
int32 dal_rt_rtl8277c_cls_extend_rule_add(rt_cls_extend_rule_t *pClsRule)
{
    aal_l2_cf_rule_t entry;
    int32 ret = RT_ERR_OK;
    uint8 ethertypeCode;
    ...
    if(pClsRule->filterRule.mask.ethertype != 0)
    {
        ...
        _8277c_cls_ethertype_encode_get(pClsRule->filterRule.data.ethertype, &ethertypeCode);
...
int32 dal_rt_rtl8277c_cls_extend_rule_delete(uint32 ruleIndex)
{
    int32 ret = RT_ERR_OK;
 ...
 *      None.
 */
int32 dal_rt_rtl8277c_cls_extend_rule_get(uint32 index, rt_cls_extend_rule_t *pClsRule)
{
    aal_l2_cf_key_t key;
...
#if defined(CONFIG_SDK_RTL8198X)
    dal_rtl8277c_ca_specpkt_ctrl_ldpid_update(port);
#endif
...
int32 dal_rt_rtl8277c_cls_vlan_paser_set(rt_port_t port, rt_cls_vlan_attribute_t type, uint32 data)
{
    aal_ilpb_cfg_t ilpbCfg;
    aal_ilpb_cfg_msk_t ilpbCfgMask;
...
int32 dal_rt_rtl8277c_cls_vlan_paser_get(rt_port_t port, rt_cls_vlan_attribute_t type, uint32 *pData)
{
    aal_ilpb_cfg_t ilpbCfg;
    aal_l2_vlan_default_cfg_t vlan;
    ca_status_t ret;
