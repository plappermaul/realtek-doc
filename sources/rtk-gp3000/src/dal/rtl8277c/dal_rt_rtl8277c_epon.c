...
static uint8 eponChurningStatus = FALSE;
...
#define RTL8277C_EPON_MPCP_INFO_LOG_TRACE 1

#define __RTL8277C_EPON_MPCP_DBG_EVENT_LOG_ENTRY  512



#ifdef RTL8277C_EPON_MPCP_INFO_LOG_TRACE
static rt_epon_mpcp_eventlog_time_entry_t  mpcp_even_log_entry[__RTL8277C_EPON_MPCP_DBG_EVENT_LOG_ENTRY];


static uint16 rtl8277c_mpcp_event_log_idx = 0;

static rt_enable_t rtl8277c_mpcp_log_overwrite = FALSE;
static rt_enable_t rtl8277c_mpcp_log_isDBOverwrite = FALSE;

static uint16 rtl8277c_dbg_mpcp_rx_reg_flag3_cnt = 0;
static uint16 rtl8277c_dbg_mpcp_rx_reg_flag2_cnt = 0;
#endif /*RTL8277C_EPON_MPCP_INFO_LOG_TRACE*/
...
void _dal_rtl8277c_epon_infoChange_notify(rt_epon_info_notify_t info)
{
...
#ifdef RTL8277C_EPON_MPCP_INFO_LOG_TRACE
	if((rtl8277c_mpcp_event_log_idx+1)>=__RTL8277C_EPON_MPCP_DBG_EVENT_LOG_ENTRY)
	{
		if(rtl8277c_mpcp_log_overwrite==TRUE)
		{	
			rtl8277c_mpcp_event_log_idx=0;
		}
		else
		{
			return;
	...
	mpcp_even_log_entry[rtl8277c_mpcp_event_log_idx].type=info.event;
#if defined(CONFIG_SDK_KERNEL_LINUX)
	mpcp_even_log_entry[rtl8277c_mpcp_event_log_idx].dbgLogTime=ktime_get();
#endif
	if(info.event==RT_EPON_INFO_MPCP_RX_REG_PKT)
		mpcp_even_log_entry[rtl8277c_mpcp_event_log_idx].subType=info.info;
	else
		mpcp_even_log_entry[rtl8277c_mpcp_event_log_idx].subType=0;

	rtl8277c_mpcp_event_log_idx++;
#endif /*RTL8277C_EPON_MPCP_INFO_LOG_TRACE*/
}
...
#ifdef RTL8277C_EPON_MPCP_INFO_LOG_TRACE
	if(evt_ptr->flag==3)
		rtl8277c_dbg_mpcp_rx_reg_flag3_cnt++;
	if(evt_ptr->flag==2)
		rtl8277c_dbg_mpcp_rx_reg_flag2_cnt++;
#endif
...
#ifdef RTL8277C_EPON_MPCP_INFO_LOG_TRACE
	rtl8277c_mpcp_event_log_idx=0;
	rtl8277c_dbg_mpcp_rx_reg_flag3_cnt = 0;
	rtl8277c_dbg_mpcp_rx_reg_flag2_cnt = 0;
	rtl8277c_mpcp_log_isDBOverwrite = FALSE;
	memset(mpcp_even_log_entry,0x0,sizeof(rt_epon_mpcp_eventlog_time_entry_t)*__RTL8277C_EPON_MPCP_DBG_EVENT_LOG_ENTRY);
#endif /*RTL8277C_EPON_MPCP_INFO_LOG_TRACE*/
...
dal_rt_rtl8277c_epon_oam_rx_callback_register(rt_epon_oam_rx_callback oamRx)
{
    ...
    {
        if((ret = nic_register_rxhook(0xffffffff,RENIC_RXPRI_PATCH,_oam_receive_from_nic))!=RT_ERR_OK)
        {
...
)
{
	...
	if(llid==0&&enable==0)
	{
		...
		{
			/*check if Churnibg key enable*/
			if(eponChurningStatus==TRUE)
			{/*reset coresec10*/
				aal_epon_coresec_glb_cfg_msk_t  corsecMsk;
				aal_epon_coresec_glb_cfg_t      cosecCfg;

				memset(&corsecMsk,0x0,sizeof(aal_epon_coresec_glb_cfg_msk_t));
				memset(&cosecCfg,0x0,sizeof(aal_epon_coresec_glb_cfg_t));
											
				corsecMsk.s.soft_reset=1;
				cosecCfg.soft_reset=1;
				ret = aal_epon_crypto_coresec_glb_cfg_set(0,AAL_EPON_CRYPTO_DIRECTION_RX,corsecMsk,&cosecCfg);
				
				printk(KERN_INFO "Churning enable reset coresec\n");
					
				memset(&corsecMsk,0x0,sizeof(aal_epon_coresec_glb_cfg_msk_t));
				memset(&cosecCfg,0x0,sizeof(aal_epon_coresec_glb_cfg_t));
										
				corsecMsk.s.soft_reset=1;
				cosecCfg.soft_reset=0;
				ret = aal_epon_crypto_coresec_glb_cfg_set(0,AAL_EPON_CRYPTO_DIRECTION_RX,corsecMsk,&cosecCfg);
			}
		}
		/*set gate timepout to 500ms  and enable gate expired deregister */
		dal_rt_rtl8277c_epon_mpcp_gate_timer_set(500,1);
		eponChurningStatus=FALSE;
	}
    ...
    else
    {
		...
		if((rtl8277c_mpcp_event_log_idx+1)>=__RTL8277C_EPON_MPCP_DBG_EVENT_LOG_ENTRY)
		{
			...
			mpcp_even_log_entry[rtl8277c_mpcp_event_log_idx].dbgLogTime=ktime_get();
#endif
			mpcp_even_log_entry[rtl8277c_mpcp_event_log_idx].type=RT_EPON_INFO_MPCP_LLID_ENTRY_DISABLE_BY_SW;
			rtl8277c_mpcp_event_log_idx++;
		}		
#endif /*RTL8277C_EPON_MPCP_INFO_LOG_TRACE*/
    return ret;
...
int32 dal_rt_rtl8277c_epon_churningKey_set(rt_epon_churningKeyEntry_t *pEntry)
{
    ...
    else if (pon_mode == ONU_PON_MAC_EPON_D10G || pon_mode == ONU_PON_MAC_EPON_BI10G)
    {
	    ...
	    //CA_RET_VALUE_CHECK(ret);
		/*set gate timepout to 100ms */
		if(eponChurningStatus!=TRUE)
			dal_rt_rtl8277c_epon_mpcp_gate_timer_set(100,0);
	}
#else 
    ca_status_t ret=CA_E_OK;
    rtk_port_t pon_port;
...
/* Function Name:
 *      dal_rt_rtl8277c_epon_mibCounter_clear
 * Description:
 ...
 * Note:
 */
int32 dal_rt_rtl8277c_epon_mibCounter_clear(void)
{
    ca_status_t ret=CA_E_OK;
    rt_port_t pon_port;
    ca_port_id_t port_id;
    ...
    return RT_ERR_OK;
} /* end of dal_rt_rtl8277c_epon_mibCounter_clear */
...
/* Function Name:
 *      dal_rt_rt8277c_epon_mpcp_dbg_log_dump
 * Description:
 *      dump mpcp debug log
 * Input:
 ...
 *      None
 */
int32 dal_rt_rt8277c_epon_mpcp_dbg_log_dump(rt_enable_t overWrite,rt_enable_t clearLog)
{
#ifdef RTL8277C_EPON_MPCP_INFO_LOG_TRACE	
	int i,j;
	if(rtl8277c_mpcp_log_overwrite==TRUE && rtl8277c_mpcp_log_isDBOverwrite==TRUE)
	{
		for(j=0;j<__RTL8277C_EPON_MPCP_DBG_EVENT_LOG_ENTRY;j++)
		{
			i = rtl8277c_mpcp_event_log_idx+j;
			i= i%__RTL8277C_EPON_MPCP_DBG_EVENT_LOG_ENTRY;
			/*dump entry*/
			/*dump entry*/
			printk("\n [%d] %lld ns   (%d)  (%d)",i
										,mpcp_even_log_entry[i].dbgLogTime
										,mpcp_even_log_entry[i].type
										,mpcp_even_log_entry[i].subType);
	    }
	}	
	else
	{
		for(i=0;i<rtl8277c_mpcp_event_log_idx;i++)
		{
			/*dump entry*/
			printk("\n [%d] %lld ns   (%d)  (%d)",i
										,mpcp_even_log_entry[i].dbgLogTime
	...
	printk("\n rtl8277c_dbg_mpcp_rx_reg_flag3_cnt:%d",rtl8277c_dbg_mpcp_rx_reg_flag3_cnt);
	printk("\n rtl8277c_dbg_mpcp_rx_reg_flag2_cnt:%d",rtl8277c_dbg_mpcp_rx_reg_flag2_cnt);
	...
	rtl8277c_mpcp_log_overwrite=overWrite;
