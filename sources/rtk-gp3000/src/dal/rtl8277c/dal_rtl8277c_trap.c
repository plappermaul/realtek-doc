...
#define DAL_RTL8277C_TRAP_ETH_PORT_MAX 8
#define DAL_RTL8277C_TRAP_CLS_RULE_PER_PORT 4


typedef struct dal_8277c_trap_config_s
{
    rt_action_t action;
    rt_enable_t stormEn;
    uint8 policerId;
...
uint8 rtl8277c_trap_port_spb_idx_map[8] = {1,2,3,4,5,7,6,7};
#else
uint8 rtl8277c_trap_port_spb_idx_map[8] = {1,2,3,4,5,5,6,7};
#endif

dal_8277c_trap_config_t rtl8277c_trap_loop_detect_config[DAL_RTL8277C_TRAP_ETH_PORT_MAX];
...
int32 _dal_rtl8277c_trap_action_get(aal_l2_specpkt_type_t type, rt_port_t port, rt_action_t *pAction)
{
    ...
    if(ctrl.spcl_fwd == 1 && ctrl.ldpid == DAL_RTL8277C_TRAP_CPU_LDPID)
...
int32 _dal_rtl8277c_trap_loopDetectClsIdx_get(rt_port_t port, uint8 *index, uint8 *subIndex)
{
    ...
    if(port >= DAL_RTL8277C_TRAP_ETH_PORT_MAX)
        return RT_ERR_FAILED;
    if(rtl8277c_trap_loop_detect_config[port].clsIndex != 0xff)
    {
        *index = rtl8277c_trap_loop_detect_config[port].clsIndex;
        *subIndex = rtl8277c_trap_loop_detect_config[port].clsSubIndex;
        return RT_ERR_OK;
    }
...
int32 _dal_rtl8277c_trap_loopDetectCls_update(rt_port_t port)
{
    aal_l2_cls_entry_status_t entryStatus;
    ...
    if(rtl8277c_trap_loop_detect_config[port].action == ACTION_FORWARD && rtl8277c_trap_loop_detect_config[port].stormEn == DISABLED)
    {
        /* disable CLS rule */
        if(rtl8277c_trap_loop_detect_config[port].clsIndex != 0xff)
        {
            ...
            if(rtl8277c_trap_loop_detect_config[port].clsSubIndex == 0)
                aal_l2_cls_l2r_1st_key_set(0, rtl8277c_trap_loop_detect_config[port].clsIndex, keyMask, &key);
            else
                aal_l2_cls_l2r_2nd_key_set(0, rtl8277c_trap_loop_detect_config[port].clsIndex, keyMask, &key);
            ...
            aal_l2_cls_fib_set(0, rtl8277c_trap_loop_detect_config[port].clsIndex, fibMask, &fib);

            ret = aal_l2_cls_entry_status_get(0, rtl8277c_trap_loop_detect_config[port].clsIndex, &entryStatus);
            if(ret != CA_E_OK)
            {
                return RT_ERR_FAILED;
            }
            ...
            ret = aal_l2_cls_entry_status_set(0, rtl8277c_trap_loop_detect_config[port].clsIndex, entryStatusMask, &entryStatus);
            if(ret != CA_E_OK)
            {
                return RT_ERR_FAILED;
            }

            rtl8277c_trap_loop_detect_config[port].clsIndex = 0xff;
        }
    }
    else
    {
        if(RT_ERR_OK != _dal_rtl8277c_trap_loopDetectClsIdx_get(port, &clsIndex, &clsSubIndex))
            return RT_ERR_FAILED;

        rtl8277c_trap_loop_detect_config[port].clsIndex = clsIndex;
        rtl8277c_trap_loop_detect_config[port].clsSubIndex = clsSubIndex;
        ...
        key.ethernet_type = RTL8277C_TRAP_LOOP_DETECT_EV2PT_ID;
        if(clsSubIndex == 0)
            aal_l2_cls_l2r_1st_key_set(0, rtl8277c_trap_loop_detect_config[port].clsIndex, keyMask, &key);
        else
            aal_l2_cls_l2r_2nd_key_set(0, rtl8277c_trap_loop_detect_config[port].clsIndex, keyMask, &key);
        ...
        if(clsSubIndex == 0)
        {
            if(rtl8277c_trap_loop_detect_config[port].action == ACTION_TRAP2CPU)
            {
                fib.dp_valid_0 = 1;
                fib.dp_value_0 = AAL_LPORT_SPCL;
            }
            if(rtl8277c_trap_loop_detect_config[port].stormEn == ENABLED)
            {
                fib.groupid_valid_0 = 1;
                fib.groupid_0 = rtl8277c_trap_loop_detect_config[port].policerId;
            }
            if(rtl8277c_trap_loop_detect_config[port].priority != 0xff)
            {
                fib.cos_0 = rtl8277c_trap_loop_detect_config[port].priority;
                fib.cos_valid_0 = 1;
            }
        ...
        else
        {
            ...
            if(rtl8277c_trap_loop_detect_config[port].stormEn == ENABLED)
            {
                ...
                fib.groupid_1 = rtl8277c_trap_loop_detect_config[port].policerId;
            }
            if(rtl8277c_trap_loop_detect_config[port].priority != 0xff)
            {
                fib.cos_1 = rtl8277c_trap_loop_detect_config[port].priority;
                fib.cos_valid_1 = 1;
            }
...
dal_rtl8277c_trap_init(void)
{
    ...
    for(i=0;i<sizeof(rtl8277c_trap_port_spb_idx_map);i++)
    {
        ...
        ilpbCfg.cls_length = DAL_RTL8277C_TRAP_CLS_RULE_PER_PORT;
        start += DAL_RTL8277C_TRAP_CLS_RULE_PER_PORT;
    ...
    for(i = 0; i < DAL_RTL8277C_TRAP_ETH_PORT_MAX; i++)
    {
        memset(&rtl8277c_trap_loop_detect_config[i], 0 ,sizeof(dal_8277c_trap_config_t));
        rtl8277c_trap_loop_detect_config[i].clsIndex = 0xff;
        rtl8277c_trap_loop_detect_config[i].priority = 0xff;
    }

    mask.s.ethertype = 1;
    mask.s.encoded_value = 1;
    config.ethertype = RTL8277C_TRAP_LOOP_DETECT_ETHERTYPE;
    config.encoded_value = RTL8277C_TRAP_LOOP_DETECT_EV2PT_ID;
    if(CA_E_OK != aal_l2_specpkt_ethtype_encode_set(0, RTL8277C_TRAP_LOOP_DETECT_EV2PT_ID,mask,&config) )
    {
        printk(KERN_ERR"Set ethertype fail, ethertype 0x%04x, entry id %d\n",RTL8277C_TRAP_LOOP_DETECT_ETHERTYPE, RTL8277C_TRAP_LOOP_DETECT_EV2PT_ID);
        return RT_ERR_FAILED;
    }
...
{
    return _dal_rtl8277c_trap_action_get(AAL_L2_SPECPKT_TYPE_DHCP, port, pAction);
...
{
    return _dal_rtl8277c_trap_action_set(AAL_L2_SPECPKT_TYPE_DHCP, port, action);
...
{
    return _dal_rtl8277c_trap_action_get(AAL_L2_SPECPKT_TYPE_PPPOE_DISC, port, pAction);
...
{
    return _dal_rtl8277c_trap_action_set(AAL_L2_SPECPKT_TYPE_PPPOE_DISC, port, action);
...
{
    return _dal_rtl8277c_trap_action_get(AAL_L2_SPECPKT_TYPE_BPDU, port, pAction);
...
{
    return _dal_rtl8277c_trap_action_set(AAL_L2_SPECPKT_TYPE_BPDU, port, action);
...
{
    return _dal_rtl8277c_trap_action_get(AAL_L2_SPECPKT_TYPE_MYMAC, port, pAction);
...
{
    return _dal_rtl8277c_trap_action_set(AAL_L2_SPECPKT_TYPE_MYMAC, port, action);
...
{
    return _dal_rtl8277c_trap_action_get(AAL_L2_SPECPKT_TYPE_UDF_0, port, pAction);
...
{
    return _dal_rtl8277c_trap_action_set(AAL_L2_SPECPKT_TYPE_UDF_0, port, action);
...
{
    return _dal_rtl8277c_trap_action_get(AAL_L2_SPECPKT_TYPE_ARP, port, pAction);
...
{
    return _dal_rtl8277c_trap_action_set(AAL_L2_SPECPKT_TYPE_ARP, port, action);
}/* end of dal_rtl8277c_trap_portArpPktAction_set */

/* Function Name:
 *      dal_rtl8277c_trap_portLoopDetectPktAction_get
 * Description:
 ...
 */
extern int32
dal_rtl8277c_trap_portLoopDetectPktAction_get(rt_port_t port, rt_action_t *pAction)
{
    aal_l2_cls_fib_t fib;
    ca_status_t ret=CA_E_OK;
    ...
    *pAction = rtl8277c_trap_loop_detect_config[port].action;


    return RT_ERR_OK;
}/* end of dal_rtl8277c_trap_portLoopDetectPktAction_get */

/* Function Name:
 *      dal_rtl8277c_trap_portLoopDetectPktAction_set
 * Description:
 ...
 */
extern int32
dal_rtl8277c_trap_portLoopDetectPktAction_set(rt_port_t port, rt_action_t action)
{
    int32 ret;
    ...
    rtl8277c_trap_loop_detect_config[port].action = action;

    ret = _dal_rtl8277c_trap_loopDetectCls_update(port);

    return ret;
}/* end of dal_rtl8277c_trap_portLoopDetectPktAction_set */

/* Function Name:
 *      dal_rtl8277c_trap_portLoopDetectPktPolicer_set
 * Description:
 ...
 */
extern int32
dal_rtl8277c_trap_portLoopDetectPktPolicer_set(rt_port_t port, rtk_enable_t enable, uint8 policerId)
{
    int32 ret;
    ...
    rtl8277c_trap_loop_detect_config[port].stormEn = enable;
    rtl8277c_trap_loop_detect_config[port].policerId = policerId;

    ret = _dal_rtl8277c_trap_loopDetectCls_update(port);

    return ret;
}/* end of dal_rtl8277c_trap_portLoopDetectPktPolicer_set */



/* Function Name:
 *      dal_rtl8277c_trap_portDot1xPktAction_get
 * Description:
 ...
 */
extern int32
dal_rtl8277c_trap_portDot1xPktAction_get(rt_port_t port, rt_action_t *pAction)
{
    return _dal_rtl8277c_trap_action_get(AAL_L2_SPECPKT_TYPE_802_DOT1X, port, pAction);
}/* end of dal_rtl8277c_trap_portDot1xPktAction_get */


/* Function Name:
 *      dal_rtl8277c_trap_portDot1xPktAction_set
 * Description:
 ...
 */
extern int32
dal_rtl8277c_trap_portDot1xPktAction_set(rt_port_t port, rt_action_t action)
{
    return _dal_rtl8277c_trap_action_set(AAL_L2_SPECPKT_TYPE_802_DOT1X, port, action);
}/* end of dal_rtl8277c_trap_portDot1xPktAction_set */
...
/* Function Name:
 *      dal_rtl8277c_trap_priority_get
 * Description:
 ...
 */
extern int32
dal_rtl8277c_trap_priority_get(rt_port_t port, rt_trap_protocol_t type, uint8 *pPri)
{
    ...
    if(type == RT_TRAP_PROTOCOL_LOOP_DETECT)
    {
        if(rtl8277c_trap_loop_detect_config[port].priority == 0xff)
            ...
            *pPri = rtl8277c_trap_loop_detect_config[port].priority;
        return RT_ERR_OK;
    }
    ...
    memset(&ctrl,0,sizeof(aal_l2_specpkt_ctrl_t));
    ret = aal_l2_specpkt_ctrl_get(0, rtl8277c_trap_port_spb_idx_map[port], specialType, &ctrl);
    if(ret != CA_E_OK)
        return RT_ERR_FAILED;
    ...
    *pPri = ctrl.cos;
    ...
    return ret;
}/* end of dal_rtl8277c_trap_priority_get */

/* Function Name:
 *      dal_rtl8277c_trap_priority_set
 * Description:
 ...
 */
extern int32
dal_rtl8277c_trap_priority_set(rt_port_t port, rt_trap_protocol_t type, uint8 pri)
{
    aal_l2_specpkt_type_t specialType;
    aal_l2_specpkt_ctrl_t ctrl;
    aal_l2_specpkt_ctrl_mask_t ctrlMask;
    ...
    if(type == RT_TRAP_PROTOCOL_LOOP_DETECT)
    {
        rtl8277c_trap_loop_detect_config[port].priority = pri;
        return _dal_rtl8277c_trap_loopDetectCls_update(port);
    }
    ...
    ret = aal_l2_specpkt_ctrl_set(0, rtl8277c_trap_port_spb_idx_map[port], specialType, ctrlMask, &ctrl);
    if(ret != CA_E_OK)
        return RT_ERR_FAILED;

    return ret;
}/* end of dal_rtl8277c_trap_priority_set */
