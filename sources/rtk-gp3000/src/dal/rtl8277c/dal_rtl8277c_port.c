...
static int32 _dal_rtl8277c_aal2DalSdsSpeed(aal_sds_mode_t aal_speed, rtk_port_speed_t *dal_speed)
{
    ...
    case AAL_SDS_SPEED_10M:
        *dal_speed = PORT_SPEED_10M;
        break;
    case AAL_SDS_SPEED_500M:
        *dal_speed = PORT_SPEED_500M;
        break;
...
static int32 _dal_rtl8277c_aal2DalSdsMode(aal_sds_speed_t aal_mode, rtk_port_sds_mode_t *dal_mode)
{
    ...
    case AAL_SDS_MODE_QXGMII:
        *dal_mode = LAN_SDS_MODE_QXGMII;
        break;
    case AAL_SDS_MODE_USXGMII_5G:
        *dal_mode = LAN_SDS_MODE_USXGMII_5G;
        break;
...
#endif
    NI_HV_GLB_STATIC_CFG_t glb_static_cfg;
    ...
    if(ps == 1)
    {
        ...
        case CA_PORT_ID_NI4:
            ...
            *pDuplex = ((if_config.gmii_like_half_duplex == 0) ? PORT_FULL_DUPLEX : PORT_HALF_DUPLEX);
            ...
            break;
        case CA_PORT_ID_NI5:
            glb_static_cfg.wrd = CA_NE_REG_READ(NI_HV_GLB_STATIC_CFG);
            if(glb_static_cfg.bf.p5_ge_sel == 0)
            {
                /* RXAUI mode, return full duplex & 10G */
...
#else
#ifdef CONFIG_COMMON_RT_PONMISC
                if((ret = dal_rt_rtl8277c_ponmisc_mode_get(&ponMode, &ponSpeed)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_PORT | MOD_DAL), "");
                    return RT_ERR_FAILED;
                }
...
/* Function Name:
 *      dal_rtl8277c_port_phyTestMode_get
 * Description:
 ...
 */
int32
dal_rtl8277c_port_phyTestMode_get(rtk_port_t port, rtk_port_phy_test_mode_t *pTestMode)
{
    int32 ret;
    uint32 data;
    ...
    /* function body */
    if((ret = dal_rtl8277c_port_phyReg_get(port, 0xA41, 9, &data)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_PORT), "");
        return ret;
    }
    ...
    *pTestMode = (data & 0xE000) >> 13;
    ...
    return RT_ERR_OK;
}   /* end of dal_rtl8277c_port_phyTestMode_get */

/* Function Name:
 *      dal_rtl8277c_port_phyTestMode_set
 * Description:
 ...
 */
int32
dal_rtl8277c_port_phyTestMode_set(rtk_port_t port, rtk_port_phy_test_mode_t testMode)
{
    int32 ret;
    uint32 data;
    ...
    if((ret = dal_rtl8277c_port_phyReg_set(port, 0xA41, 9, data)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_PORT), "");
        return ret;
    }
    ...
    return RT_ERR_OK;
}   /* end of dal_rtl8277c_port_phyTestMode_set */
...
static int _rtl8277c_lan_sds2_mode_set(uint32 mode)
{
    ...
    case LAN_SDS_MODE_QXGMII:
        ...
        break;
    case LAN_SDS_MODE_USXGMII_5G:
        ret = aal_sds_mode_set(0, 6, AAL_SDS_MODE_USXGMII_5G, AAL_SDS_SPEED_DEFAULT);
        break;
    default:
        return RT_ERR_CHIP_NOT_SUPPORTED;
    }
...
dal_rtl8277c_port_serdesMode_get(uint8 num, uint8 *cfg)
{
    ...
    if((ret = _dal_rtl8277c_aal2DalSdsMode(aal_mode, &dal_mode)) != RT_ERR_OK)
    {
        return ret;
    }
...
dal_rtl8277c_port_serdesModeSpeed_set(uint8 num, rtk_port_sds_mode_t mode, rtk_port_speed_t speed)
{
    ...
    case LAN_SDS_MODE_USXGMII:
        ...
        break;
    case LAN_SDS_MODE_USXGMII_5G:
        aal_mode = AAL_SDS_MODE_USXGMII_5G;
        break;
    case LAN_SDS_MODE_RGMII:
...
dal_rtl8277c_port_serdesModeSpeed_get(uint8 num, rtk_port_sds_mode_t *pMode, rtk_port_speed_t *pSpeed)
{
    ...
    if((ret = _dal_rtl8277c_aal2DalSdsMode(aal_mode, pMode)) != RT_ERR_OK)
    {
        return ret;
    }
    if(aal_mode == AAL_SDS_MODE_SGMII && aal_speed == AAL_SDS_SPEED_2G5)
    ...
    if((ret = _dal_rtl8277c_aal2DalSdsSpeed(aal_speed, pSpeed)) != RT_ERR_OK)
    {
        return ret;
    }
...
/* Function Name:
 *      dal_rtl8277c_port_serdesTxRxPolarity_set
 * Description:
 ...
 */
int32
dal_rtl8277c_port_serdesTxRxPolarity_set(uint8 num, rtk_port_sds_pn_t tx_polarity, rtk_port_sds_pn_t rx_polarity)
{
    int ret;
    uint32 aal_port, aal_txPolarity, aal_rxPolarity;
    aal_sds_mode_t mode;
    ...
    if(AAL_SDS_MODE_NONE == mode)
    {
        return RT_ERR_NOT_ALLOWED;
    }

    aal_txPolarity = (LAN_SDS_PN_INVERT == tx_polarity) ? 1 : 0;
    aal_rxPolarity = (LAN_SDS_PN_INVERT == rx_polarity) ? 1 : 0;
    if((ret = aal_sds_txrx_polarity_set(0, aal_port, aal_txPolarity, aal_rxPolarity) != CA_E_OK))
        {
            RT_ERR(ret, (MOD_PORT | MOD_DAL), "");
            return RT_ERR_FAILED;
        }
...
/* Function Name:
 *      dal_rtl8277c_port_serdesTxRxPolarity_get
 * Description:
 *      Get Serdes Tx/Rx P/N polarity
 * Input:
 *      num   - serdes number
 ...
 */
int32
dal_rtl8277c_port_serdesTxRxPolarity_get(uint8 num, rtk_port_sds_pn_t *tx_polarity, rtk_port_sds_pn_t *rx_polarity)
{
    int ret;
    uint32 aal_port, aal_txPolarity, aal_rxPolarity;
    aal_sds_mode_t mode;
    ...
    if((ret = aal_sds_txrx_polarity_get(0, aal_port, &aal_txPolarity, &aal_rxPolarity) != CA_E_OK))
        {
            RT_ERR(ret, (MOD_PORT | MOD_DAL), "");
            return RT_ERR_FAILED;
        }
    *tx_polarity = (aal_txPolarity != 0) ? LAN_SDS_PN_INVERT : LAN_SDS_PN_NORMAL;
    *rx_polarity = (aal_rxPolarity != 0) ? LAN_SDS_PN_INVERT : LAN_SDS_PN_NORMAL;

    return RT_ERR_OK;
}

/* Function Name:
 *      dal_rtl8277c_port_serdesStatus_get
 * Description:
 *      Get Serdes status
 * Input:
 *      num   - serdes number
 ...
 */
int32
dal_rtl8277c_port_serdesStatus_get(uint8 num, rtk_port_sds_status_t *pStatus)
{
    int ret;
    uint32 aal_port;
 ...
 *      Speed option only use for P0~P3 when P6 SDS configure as QXGMII
 */
int32
dal_rtl8277c_port_macForceAbility_set(rtk_port_t port,rtk_port_macAbility_t macAbility)
{
    ...
    aal_ni_eth_mac_config_t mac_config;
    aal_ni_xge_flowctrl_mask_t xge_mask;
    aal_ni_xge_flowctrl_t xge_config;
    aal_ni_xge_cfg_rx_mask_t xge_rx_mask;
    aal_ni_xge_cfg_rx_t xge_rx_config;
    aal_pon_act_eth_glb_flow_ctrl_mask_t act_flow_ctrl_mask;
    aal_pon_act_eth_glb_flow_ctrl_config_t act_flow_ctrl_cfg;
    aal_pon_act_eth_glb_flow_misc_mask_t act_flow_misc_mask;
    aal_pon_act_eth_glb_flow_misc_config_t act_flow_misc_cfg;
    aal_pon_act_eth_xge_mac_cfg_rx_mask_t act_xge_mac_cfg_rx_mask;
    aal_pon_act_eth_xge_mac_cfg_rx_config_t act_xge_mac_cfg_rx_cfg;
    aal_l3_te_flow_ctrl_mask_t  l3te_fc_mask;
    aal_l3_te_flow_ctrl_t l3te_fc_cfg;
    ...
    /* Speed option only support p0-3 when P6 SDS configured as QXGMII */
    if(sds_mode == AAL_SDS_MODE_QXGMII)
    {
        speed01 = macAbility.speed & 0x3;
        speed23 = macAbility.speed >> 2;
        mac_reg = NI_HV_PT_PORT_GLB_CFG + port * APB0_NI_HV_PT_STRIDE;
        switch(port)
        {
        case CA_PORT_ID_NI0:
        case CA_PORT_ID_NI1:
    ...
    case CA_PORT_ID_NI5:
        ...
        mac_mask.s.rx_pause_en = 1;
        mac_mask.s.tx_pause_en = 1;
        ...
        mac_config.rx_pause_en = 1; /* Keep this on, using rx_flow_disable to control */
        mac_config.tx_pause_en = 1; /* Keep this on, using tx_flow_disable to control */
        ...
        if(CA_PORT_ID_NI5 == port)
        {
            aal_ni_xaui_mac_tx_ctrl_mask_t xaui_mask;
            aal_ni_xaui_mac_tx_ctrl_t xaui_tx_cfg;

            xaui_mask.u32 = 0;
            memset(&xaui_tx_cfg, 0, sizeof(aal_ni_xaui_mac_tx_ctrl_t));

            xaui_mask.s.pau_flow_en = 1;
            /* Turn flow control if Tx or Rx is on */
            xaui_tx_cfg.pau_flow_en = (macAbility.txFc == ENABLED) ? 1 : (macAbility.rxFc == ENABLED) ? 1 : 0;
            if((ret = aal_ni_xaui_mac_tx_ctrl_set(0, xaui_mask, &xaui_tx_cfg)) != CA_E_OK)
            {
                return RT_ERR_FAILED;
            }
        }
    ...
    case CA_PORT_ID_NI6:
        /* Tx/Rx FC */
        xge_mask.u32 = 0;
        memset(&xge_config, 0, sizeof(aal_ni_xge_flowctrl_t));

        xge_mask.s.pau_rx_en = 1;
        xge_mask.s.pau_tx_en = 1;
        xge_mask.s.qbb_rx_en = 1;
        xge_mask.s.qbb_tx_en = 1;
        xge_mask.s.internal_txstop_sel = 1;
        xge_mask.s.rx_drp_pau_en = 1;
        xge_mask.s.te_pausereq_sel = 1;
        xge_mask.s.rx_pausereq_sel = 1;

        xge_config.pau_rx_en = macAbility.rxFc == ENABLED ? 1 : 0;
        xge_config.pau_tx_en = macAbility.txFc == ENABLED ? 1 : 0;
        xge_config.qbb_rx_en = 0;
        xge_config.qbb_tx_en = 0;
        xge_config.internal_txstop_sel = 1;
        xge_config.rx_drp_pau_en = 1;
        xge_config.te_pausereq_sel = 1;
        xge_config.rx_pausereq_sel = 0;

        if((ret = aal_ni_xge_flowctrl_set(0, port - CA_PORT_ID_NI6, xge_mask, &xge_config)) != CA_E_OK)
        {
            return RT_ERR_FAILED;
        }

        xge_rx_mask.u32 = 0;
        memset(&xge_rx_config, 0, sizeof(aal_ni_xge_cfg_rx_t));

        xge_rx_mask.s.pause_term = 1;

        xge_rx_config.pause_term = 0;

        if((ret = aal_ni_xge_rx_config_set(0, port - CA_PORT_ID_NI6, xge_rx_mask, &xge_rx_config)) != CA_E_OK)
        {
            return RT_ERR_FAILED;
        }
        ...
        mac_config.rx_pause_en = (macAbility.rxFc == ENABLED ? 1 : 0);
        mac_config.tx_pause_en = (macAbility.txFc == ENABLED ? 1 : 0);
        mac_config.tx_flow_disable = (macAbility.txFc == DISABLED ? 1 : 0);
        ...
        break;
    case CA_PORT_ID_NI7:
        act_flow_ctrl_mask.u32 = 0;
        memset(&act_flow_ctrl_cfg, 0, sizeof(aal_pon_act_eth_glb_flow_ctrl_config_t));

        act_flow_ctrl_mask.s.pau_rx_en = 1;
        act_flow_ctrl_mask.s.pau_tx_en = 1;
        act_flow_ctrl_mask.s.qbb_rx_en = 1;
        act_flow_ctrl_mask.s.qbb_tx_en = 1;
        act_flow_ctrl_mask.s.txstop_sel = 1;
        act_flow_ctrl_mask.s.rx_drp_pau_en = 1;

        act_flow_ctrl_cfg.pau_rx_en = (macAbility.rxFc == ENABLED ? 1 : 0);
        act_flow_ctrl_cfg.pau_tx_en = (macAbility.txFc == ENABLED ? 1 : 0);
        act_flow_ctrl_cfg.qbb_rx_en = 0;
        act_flow_ctrl_cfg.qbb_tx_en = 0;
        act_flow_ctrl_cfg.txstop_sel = 1;
        act_flow_ctrl_cfg.rx_drp_pau_en = 1;

        if((ret = aal_pon_act_eth_glb_flow_control_set(0, act_flow_ctrl_mask, &act_flow_ctrl_cfg)) != CA_E_OK)
        {
            return RT_ERR_FAILED;
        }

        act_flow_misc_mask.u32 = 0;
        memset(&act_flow_misc_cfg, 0, sizeof(aal_pon_act_eth_glb_flow_misc_config_t));

        act_flow_misc_mask.s.en_l2tm_txpfc = 1;
        act_flow_misc_mask.s.en_tqm_txpfc = 1;
        act_flow_misc_mask.s.en_ni_txfc = 1;

        act_flow_misc_cfg.en_l2tm_txpfc = 1;
        act_flow_misc_cfg.en_tqm_txpfc = 1;
        act_flow_misc_cfg.en_ni_txfc = 1;

        if((ret = aal_pon_act_eth_glb_flow_misc_set(0, act_flow_misc_mask, &act_flow_misc_cfg)) != CA_E_OK)
        {
            return RT_ERR_FAILED;
        }

        act_xge_mac_cfg_rx_mask.u32 = 0;
        memset(&act_xge_mac_cfg_rx_cfg, 0, sizeof(aal_pon_act_eth_xge_mac_cfg_rx_config_t));

        act_xge_mac_cfg_rx_mask.s.pause_term = 1;
        act_xge_mac_cfg_rx_cfg.pause_term = 0;

        if((ret = aal_pon_act_eth_xge_mac_cfg_rx_set(0, act_xge_mac_cfg_rx_mask, &act_xge_mac_cfg_rx_cfg)) != CA_E_OK)
        {
            return RT_ERR_FAILED;
        }
...
#ifdef CONFIG_L3FE_FLOW_CONTROL_CONFIG_FOR_SINGLE_WAN
        l3te_fc_mask.u32 = 0;
        memset(&l3te_fc_cfg, 0, sizeof(aal_l3_te_flow_ctrl_t));
        l3te_fc_mask.s.tx_wan_ena = 1;
        l3te_fc_cfg.tx_wan_ena = (macAbility.txFc == ENABLED ? 1 : 0);

        if((ret = aal_l3_te_flow_control_set(0, l3te_fc_mask, &l3te_fc_cfg)) != CA_E_OK)
        {
            return RT_ERR_FAILED;
        }
#endif
...
dal_rtl8277c_port_macForceAbility_get(rtk_port_t port,rtk_port_macAbility_t *pMacAbility)
{
    ...
    case CA_PORT_ID_NI5:
        ...
        break;
    case CA_PORT_ID_NI6:
        if((ret = aal_ni_xge_flowctrl_get(0, port - CA_PORT_ID_NI6, &xge_config)) != CA_E_OK)
        {
            return RT_ERR_FAILED;
        }
        pMacAbility->rxFc = (1 == xge_config.pau_rx_en ? ENABLED : DISABLED);
        pMacAbility->txFc = (1 == xge_config.pau_tx_en ? ENABLED : DISABLED);
        break;
    case CA_PORT_ID_NI7:
        if((ret = aal_pon_act_eth_glb_flow_control_get(0, &act_flow_ctrl_cfg)) != CA_E_OK)
        {
            return RT_ERR_FAILED;
        }
        pMacAbility->rxFc = (1 == act_flow_ctrl_cfg.pau_rx_en? ENABLED : DISABLED);
        pMacAbility->txFc = (1 == act_flow_ctrl_cfg.pau_tx_en ? ENABLED : DISABLED);
        break;
    }
...
/* Function Name:
 *      dal_rtl8277c_port_rtctResult_get
 * Description:
 ...
 */
int32
dal_rtl8277c_port_rtctResult_get(rtk_port_t port, rtk_rtctResult_t *pRtctResult)
{
    int ret;
    uint32 channel;
    uint32 value, data;
    ...
    if((ret = dal_rtl8277c_port_phyReg_get(port, 0xa42, 17, &value)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_PORT|MOD_DAL), "");
        return ret;
    }
    ...
    for(channel = 0;channel < 4 ; channel ++)
    {
        ...
        if((ret = dal_rtl8277c_port_phyReg_set(port, 0xa42, 27, value)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_PORT|MOD_DAL), "");
            return ret;
        }
        if((ret = dal_rtl8277c_port_phyReg_get(port, 0xa42, 28, &data)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_PORT|MOD_DAL), "");
            return ret;
        }
    ...
    return RT_ERR_OK;
} /*end of dal_rtl8277c_port_rtctResult_get*/

/* Function Name:
 *      dal_rtl8277c_port_rtct_start
 * Description:
 ...
 */
int32
dal_rtl8277c_port_rtct_start(rtk_portmask_t *pPortmask)
{
    int ret;
    uint32 port, value;
    ...
    HAL_SCAN_ALL_ETH_PORT(port)
    {
        ...
        {
            if((ret = dal_rtl8277c_port_link_get(port, &link)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_PORT|MOD_DAL), "");
                return ret;
            }
            ...
            value &= ~(1 << 15);
            if((ret = dal_rtl8277c_port_phyReg_set(port, 0xa42, 17, value)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_PORT|MOD_DAL), "");
                return ret;
            }
    ...
    return RT_ERR_OK;
} /* end of dal_rtl8277c_port_rtct_start */


/* Function Name:
 *      dal_rtl8277c_port_macLoopbackEnable_get
 * Description:
 ...
 */
extern int32
dal_rtl8277c_port_macLoopbackEnable_get(rt_port_t port, rt_enable_t *pEnable, rt_enable_t *pSwapEnable)
{
    ca_status_t ret = CA_E_OK;
    aal_ilpb_cfg_t ilpb;
...
}/* end of dal_rtl8277c_port_macLoopbackEnable_get */

/* Function Name:
 *      dal_rtl8277c_port_macLoopbackEnable_set
 * Description:
 ...
 */
extern int32
dal_rtl8277c_port_macLoopbackEnable_set(rt_port_t port, rt_enable_t enable, rt_enable_t swapEnable)
{
    ca_status_t ret = CA_E_OK;
    aal_ilpb_cfg_t ilpb;
...
}/* end of dal_rtl8277c_port_macLoopbackEnable_set */
