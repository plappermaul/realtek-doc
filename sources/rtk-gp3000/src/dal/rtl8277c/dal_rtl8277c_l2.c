...
dal_rtl8277c_l2_init(void)
{
    ...
    if((ret = dal_rtl8277c_l2_aging_set(RTK_L2_DEFAULT_AGING_TIME / 10)) != RT_ERR_OK)
    {
...
/* Function Name:
 *      dal_rt_rtl8277c_l2_mcastAddr_add
 * Description:
 ...
 */
extern int32
dal_rt_rtl8277c_l2_mcastAddr_add(rtk_l2_mcastAddr_t *pMcastAddr)
{
    ca_status_t ret=CA_E_OK;
    ca_uint32_t time=0;
    aal_fdb_entry_cfg_t  fdbEntry;
    ...
    memset(&fdbEntry, 0, sizeof(aal_fdb_entry_cfg_t));
    memcpy(fdbEntry.mac,pMcastAddr->mac.octet,sizeof(ca_mac_addr_t));

    fdbEntry.key_vid        = pMcastAddr->vid;
    fdbEntry.lrnt_vid       = pMcastAddr->vid;
    fdbEntry.static_flag    = 1;
    fdbEntry.sa_permit      = 1;
    fdbEntry.da_permit      = 1;
    fdbEntry.mc_flag        = 1;
    /* portmask.bits[0] reuse as mcgid field */
    fdbEntry.port_id        = pMcastAddr->portmask.bits[0];
...
}/* end of dal_rt_rtl8277c_l2_mcastAddr_add */

/* Function Name:
 *      dal_rt_rtl8277c_l2_mcastAddr_del
 * Description:
 ...
 */
extern int32
dal_rt_rtl8277c_l2_mcastAddr_del(rtk_l2_mcastAddr_t *pMcastAddr)
{
    ca_status_t ret=CA_E_OK;
    ca_mac_addr_t mac;
    ca_vlan_id_t key_vid;
    ...
    memcpy(mac,pMcastAddr->mac.octet,sizeof(ca_mac_addr_t));
    key_vid=pMcastAddr->vid;
    memset(&fdb_entry,0,sizeof(aal_fdb_entry_cfg_t));
    ...
    return RT_ERR_OK;
}/* end of dal_rt_rtl8277c_l2_mcastAddr_del */

/* Function Name:
 *      dal_rt_rtl8277c_l2_mcastAddr_get
 * Description:
 ...
 */
extern int32
dal_rt_rtl8277c_l2_mcastAddr_get(rtk_l2_mcastAddr_t *pMcastAddr)
{
    ca_status_t ret=CA_E_OK;
    ca_mac_addr_t mac;
    ca_vlan_id_t key_vid;
    ...
    /* portmask.bits[0] reuse as mcgid field */
    pMcastAddr->portmask.bits[0] = fdb_entry.port_id;

    return RT_ERR_OK;
}/* end of dal_rt_rtl8277c_l2_mcastAddr_get */
