...
//  REG32(REG_REG42) |= (REG_PCM_CMU_EN);
	...
	regValue |= (1<<16);
#elif defined(CONFIG_RTL9603CVD) || defined(CONFIG_RTL9603CVD_SERIES)
	regValue |= (1<<14);
#endif	
	RTL_W32(IO_MODE_EN , regValue);
#endif
...
static int prbs_gpon(unsigned int value)
{
    ...
    data = 0x18000;
    if((ret = reg_write(RTL9603CVD_WSDS_DIG_1Dr, &data)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
    ...
    data = 0x4000 | pattern;
    if((ret = reg_write(RTL9603CVD_WSDS_DIG_19r, &data)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
    data = 0x1;
    if((ret = reg_write(RTL9603CVD_WSDS_DIG_1Ar, &data)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
    ...
    mdelay(1000);
    if((ret = reg_read(RTL9603CVD_WSDS_DIG_1Br, &err)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
    if((ret = reg_read(RTL9603CVD_WSDS_DIG_1Cr, &status)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
    ...
    data = 0;
    if((ret = reg_write(RTL9603CVD_SDS_ANA_MISC_REG01r, &data)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
...
static int prbs_epon(unsigned int value)
{
    ...
    /* Configure EPON mode */
    data = 0x1f;
    if((ret = reg_write(RTL9603CVD_SDS_CFGr, &data)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
    data = 0xf30;
    if((ret = reg_write(RTL9603CVD_WSDS_DIG_00r, &data)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
    ...
    data = 0x71e0;
    if((ret = reg_write(RTL9603CVD_SDS_REG2r, &data)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
    ...
    data = 0x1401;
    if((ret = reg_write(RTL9603CVD_SDS_REG0r, &data)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
    ...
    data = pattern << 8;
    if((ret = reg_write(RTL9603CVD_SDS_EXT_REG11r, &data)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
    data = 0x804;
    if((ret = reg_write(RTL9603CVD_SDS_EXT_REG12r, &data)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
    data = 0;
    if((ret = reg_write(RTL9603CVD_SDS_EXT_REG4r, &data)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
    data = 0;
    if((ret = reg_write(RTL9603CVD_SDS_EXT_REG5r, &data)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
    ...
    if((ret = reg_read(RTL9603CVD_SDS_EXT_REG4r, &err[0])) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
    if((ret = reg_read(RTL9603CVD_SDS_EXT_REG5r, &err[1])) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
...
static int prbs_fiber(unsigned int value)
{
    ...
    if((ret = reg_read(RTL9603CVD_SDS_EXT_REG29r, &data)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        return -1;
    }
...
{
    char flag[64];
    int ret, gponPrbs, eponPrbs, fiberPrbs;

    if (buffer && !copy_from_user(&flag, buffer, sizeof(flag)))
    {
        sscanf(flag, "%d %d %d", &gponPrbs, &eponPrbs, &fiberPrbs);
...
uint32 gpio_array0[2][14] = {
    { 0, 62,  4, 38, 33, 20, 21, 28, 16 },
    { 5, 60, 66,  9, 37, 36, 39, 29, 17}
};
...
static int gpio_test_read_proc(struct seq_file *seq, void *v)
{
    ...
    seq_printf(seq, "0 -> RTL9603CVD QC\n");
    seq_printf(seq, "1 -> RTL9603CVD OLT burn in/Check\n");
...
/*****************************************************
*
* PCIE Test Related fuction
*
*****************************************************/
static int pcie_test_read_proc(struct seq_file *seq, void *v)
{
    return 0;
}

static int pcie_test_write_proc(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    int ret;
    char flag[64];
    uint32 value;
    ...
    {
        value = 0x49411401;
        if ((ret = ioal_socMem32_write(0xb8b01000, value)) != RT_ERR_OK)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            return count;
        }
...
static int pcie_test_open_proc(struct inode *inode, struct file *file)
{
	return single_open(file, pcie_test_read_proc, NULL);
}
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,6,0)
struct proc_ops pcie_fop = {
	.proc_open = pcie_test_open_proc,
	.proc_write = pcie_test_write_proc,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
#else
struct file_operations pcie_fop = {
	.open = pcie_test_open_proc,
	.write = pcie_test_write_proc,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
static int eth_test_write_proc(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    ...
    if (buffer && !copy_from_user(&flag, buffer, sizeof(flag)))
    {
        /* DMP */
        value = 0x2f;
        if((ret = reg_write(RTL9603CVD_EN_FORCE_P_DMPr, &value)) != RT_ERR_OK)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            return -1;
        }
        value = 1<<2;
        if((ret = reg_array_field_write(RTL9603CVD_FORCE_P_DMPr, 0, REG_ARRAY_INDEX_NONE, RTL9603CVD_FORCE_PROT_MASKf, &value)) != RT_ERR_OK)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            return -1;
        }
        value = 1<<0;
        if((ret = reg_array_field_write(RTL9603CVD_FORCE_P_DMPr, 1, REG_ARRAY_INDEX_NONE, RTL9603CVD_FORCE_PROT_MASKf, &value)) != RT_ERR_OK)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            return -1;
        }
        value = 1<<1;
        if((ret = reg_array_field_write(RTL9603CVD_FORCE_P_DMPr, 2, REG_ARRAY_INDEX_NONE, RTL9603CVD_FORCE_PROT_MASKf, &value)) != RT_ERR_OK)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            return -1;
        }
        value = 1<<3;
        if((ret = reg_array_field_write(RTL9603CVD_FORCE_P_DMPr, 3, REG_ARRAY_INDEX_NONE, RTL9603CVD_FORCE_PROT_MASKf, &value)) != RT_ERR_OK)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            return -1;
        }
        value = 1<<2;
        if((ret = reg_array_field_write(RTL9603CVD_FORCE_P_DMPr, 5, REG_ARRAY_INDEX_NONE, RTL9603CVD_FORCE_PROT_MASKf, &value)) != RT_ERR_OK)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            return -1;
        }

        /* Generate packet for GE port */
        if((ret = rtk_port_phyReg_set(3, 0xc80, 17, 0x0070)) != RT_ERR_OK)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            return -1;
        }
        if((ret = rtk_port_phyReg_set(3, 0xc80, 17, 0x0003)) != RT_ERR_OK)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            return -1;
        }
        if((ret = rtk_port_phyReg_set(3, 0xc80, 19, 0x05EE)) != RT_ERR_OK)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            return -1;
        }
        if((ret = rtk_port_phyReg_set(3, 0xc80, 20, 0x0000)) != RT_ERR_OK)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            return -1;
        }
        if((ret = rtk_port_phyReg_set(3, 0xc80, 21, 0x0000)) != RT_ERR_OK)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            return -1;
        }
        ...
        /* Generate packet for FE port */
        pPktPayload = (uint8 *)kmalloc(sizeof(uint8) * 1500, GFP_KERNEL);
        if(NULL == pPktPayload)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            return -1;
        ...
        pPktPayload[12] = 0xff;
        pPktPayload[13] = 0xff;
        /* The rest of packet remain random */
        memset(&txInfo, 0, sizeof(txInfo));
        if((ret = mac_debug_txPktContCnt_set(100)) != RT_ERR_OK)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            kfree(pPktPayload);
            return -1;
        ...
        /* Link down CPU port to allow packet send from CPU */
        if((ret = rtk_port_macForceAbilityState_set(5, ENABLED)) != RT_ERR_OK)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
        }

        if((ret = mac_debug_txPktCont_send(pPktPayload, 1500, &txInfo, 1)) != RT_ERR_OK)
        {
            QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            kfree(pPktPayload);
            return -1;
...
static int olt_led_poll(void *arg)
{
    ...
    /* Init GPIO for LED */
    if((ret = rtk_gpio_state_set(56, ENABLED)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
    }
    if((ret = rtk_gpio_mode_set(56, GPIO_OUTPUT)) != RT_ERR_OK)
    {
        QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
    }
    ...
    for(;;) {
        ...
        do {
            ...
            if((ret = rtk_gpio_databit_set(56, cpu_toogle == 0 ? 0 : 1 )) != RT_ERR_OK)
            {
                QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            }
...
static int olt_init_write_proc(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    ...
    if (buffer && !copy_from_user(&flag, buffer, sizeof(flag)))
    {
        ...
        case 1:
            ...
            /* Link down CPU port to prevent packet sent from CPU */
            if((ret = rtk_port_macForceAbilityState_set(5, DISABLED)) != RT_ERR_OK)
            {
                QC_PRINTK("%s:%d %d\n", __FILE__, __LINE__, ret);
            }
            ...
            rtk_led_config_set(5,LED_TYPE_UTP1,&ledConfig);
            rtk_led_config_set(15,LED_TYPE_UTP2,&ledConfig);
            rtk_led_config_set(17,LED_TYPE_UTP3,&ledConfig);
            rtk_led_config_set(2,LED_TYPE_UTP4,&ledConfig);
...
/*****************************************************
*
* GMAC Related fuction
*
*****************************************************/

int gmac_test_pkt_rx(
    ...
    struct rx_info *pRxInfo)
{
    ...
    /* Check RX packets */
    /* DA */
    if((skb->data[0] != 0x00) || (skb->data[1] != 0x11) || (skb->data[2] != 0x22) ||
       (skb->data[3] != 0x33) ||(skb->data[4] != 0x44) || (skb->data[5] != 0x55)) 
    {
        QC_PRINTK("%s:%d diff in DA %02x:%02x:%02x:%02x:%02x:%02x\n", __FILE__, __LINE__,
            skb->data[0], skb->data[1], skb->data[2], skb->data[3], skb->data[4], skb->data[5]);
    }
    /* SA */
    if((skb->data[6] != 0x00) || (skb->data[7] != 0xaa) || (skb->data[8] != 0xbb) ||
       (skb->data[9] != 0xcc) ||(skb->data[10] != 0xdd) || (skb->data[11] != 0xee)) 
    {
        QC_PRINTK("%s:%d diff in SA %02x:%02x:%02x:%02x:%02x:%02x\n", __FILE__, __LINE__,
            skb->data[6], skb->data[7], skb->data[8], skb->data[9], skb->data[10], skb->data[11]);
    }
    /* Ethertype */
    if((skb->data[12] != 0xff) || (skb->data[13] != 0xff)) 
    {
        QC_PRINTK("%s:%d diff in ethertype %02x%02x\n", __FILE__, __LINE__,
            skb->data[12], skb->data[13]);
    }
    for( i = 14 ; i < 1500 ; i ++)
    {
        if(skb->data[i] != i % 0x100)
        {
            QC_PRINTK("%s:%d diff in data[%d] = %02x \n", __FILE__, __LINE__, i, skb->data[i]);
            break;
        }
    }
...
static int gmac_read_proc(struct seq_file *seq, void *v)
{
    return 0;
}

static int gmac_write_proc(struct file *file, const char *buffer, size_t count, loff_t *data)
{
    int ret, i;
    char flag[64];
    uint32 value;
    ...
    if (buffer && !copy_from_user(&flag, buffer, sizeof(flag)))
    {
        ...
        pPktPayload[12] = 0xff;
        pPktPayload[13] = 0xff;
        /* The rest of packet set to fix pattern 0x01 ~ 0xff */
        for(i = 14; i < 1500 ; i ++)
        {
            pPktPayload[i] = i % 0x100;
        }
        memset(&txInfo, 0, sizeof(txInfo));
        if((ret = mac_debug_txPktContCnt_set(100)) != RT_ERR_OK)
        {
        ...
        drv_nic_register_rxhook(0xFF, RE8686_RXPRI_DUMP, gmac_test_pkt_rx);
...
static int gmac_open_proc(struct inode *inode, struct file *file)
{
	return single_open(file, gmac_read_proc, NULL);
}
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5,6,0)
struct proc_ops gmac_fop = {
	.proc_open = gmac_open_proc,
	.proc_write = gmac_write_proc,
	.proc_read = seq_read,
	.proc_lseek = seq_lseek,
	.proc_release = single_release,
};
#else
struct file_operations gmac_fop = {
	.open = gmac_open_proc,
	.write = gmac_write_proc,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,
};
...
static int __init qc_test_module_init(void)
{
	...
	pcie = proc_create("pcie", 0, dir, &pcie_fop);
	if (!pcie) {
		QC_PRINTK("proc usb_test create fail\n");
	}
	...
	debug = proc_create("gmac", 0, dir, &gmac_fop);
	if (!debug) {
		QC_PRINTK("proc gmac create fail\n");
	}
