...
nand_spi_model_info_t snaf_rom_general_model = {
    ...
    ._wait_nand_spi_ready = nsc_wait_nand_spi_oip_ready,
};

SECTION_NAND_SPI u32_t 
_pio_raw_cmd(u32_t opcode, u32_t data, u32_t w_io_len, u32_t r_io_len)
{
    ACTIVATE_NAND_SPI_CS();
    ...
    DEACTIVE_NAND_SPI_CS();
    return rd_val;
}


SECTION_NAND_SPI static void
_pio_write_data(nand_spi_flash_info_t *info, u32_t col_addr, u32_t wr_bytes, void *wr_buf)
{
    u32_t w_data;
    u32_t w_io_len;    
    ...
    while(wr_bytes > 0){
        ...
        if(wr_bytes < 4){
            TRIG_CTRLR_PIO_WRITE((*((u32_t *)tmp)),(w_io_len | CMR_LEN(wr_bytes)));
            wr_bytes = 0;
        }else{
            TRIG_CTRLR_PIO_WRITE((*((u32_t *)tmp)),(w_io_len | CMR_LEN(4)));
            wr_bytes -= 4;
        }
    }
...
SECTION_NAND_SPI static void 
_pio_read_data(nand_spi_flash_info_t *info, u32_t col_addr, u32_t wr_bytes, void *wr_buf) 
{
    u32_t w_data;
    u32_t w_io_len;
    u32_t r_io_len;
    ...
    if (wr_bytes >= 4) {
        while ((u32_t)wr_buf < (wr_bound & 0xFFFFFFFC)) {
            *((u32_t *)tmp) = TRIG_CTRLR_PIO_READ((r_io_len|CMR_LEN(4)));
            *((u8_t *)wr_buf++) = tmp[0];
            *((u8_t *)wr_buf++) = tmp[1];
            *((u8_t *)wr_buf++) = tmp[2];
            *((u8_t *)wr_buf++) = tmp[3];
...
snaf_pio_read_data(nand_spi_flash_info_t *info, void *ram_addr, u32_t wr_bytes, u32_t blk_pge_addr, u32_t col_addr)
{
...
snaf_pio_write_data(nand_spi_flash_info_t *info, void *ram_addr, u32_t wr_bytes, u32_t blk_pge_addr, u32_t col_addr)
{
    nsc_write_enable();
...
SECTION_NAND_SPI void 
snaf_page_read(nand_spi_flash_info_t *info, void *dma_addr, u32_t blk_pge_addr)
{
...
SECTION_NAND_SPI s32_t
snaf_page_write(nand_spi_flash_info_t *info, void *dma_addr, u32_t blk_pge_addr)
...
SECTION_NAND_SPI s32_t 
snaf_page_write_with_ecc_encode(nand_spi_flash_info_t *info, void *dma_addr, u32_t blk_pge_addr, void *p_eccbuf)
{
    ecc_encode_bch(info, dma_addr, p_eccbuf);
    return snaf_page_write(info, dma_addr, blk_pge_addr);
}

SECTION_NAND_SPI s32_t
snaf_page_read_with_ecc_decode(nand_spi_flash_info_t *info, void *dma_addr, u32_t blk_pge_addr, void *p_eccbuf)
{
    snaf_page_read(info, dma_addr, blk_pge_addr);
    return ecc_decode_bch(info, dma_addr, p_eccbuf);
}
