 ...
 *
 * $Revision: 96894 $
 * $Date: 2019-05-15 15:00:32 +0800 (Wed, 15 May 2019) $
 *
...
/* model */
#define RTMODEL_PORT_NONE       (0xffffffff)
#define RTMODEL_MAX_UNIT        3
...
/* model */
static uint32 mdev_reg_base;
static uint32 mdev_reg_offset;
static rtmodel_topology_t *mdev_tp;
...
_testio_set_eFuse_check(uint32 unit)
{
    ...
    _testio_set_ocp(unit, 0xdd02, 0xB0FF);
    do
    {
        _testio_get_ocp(unit, 0xdd02, &data);
        //osal_printf("0xdd02:%#x\n", data);
    } while (data >> 15);


    _testio_get_ocp(unit, 0xdd00, &data);
    //osal_printf("0xdd00:%#x\n", data);
    data = data & 0x3;
    if (data == 0)
    ...
    else if (data == 1)
    {
        ...
        for (i = 0; i < 128; i++)
        {
            _testio_set_ocp(unit, 0xdd02, 0x9000 | i);
            do
            {
                _testio_get_ocp(unit, 0xdd02, &data);
            } while (data >> 15);
...
int32 model_mem_base_set(uint32 base, uint32 offset, rtmodel_topology_t *dev_tp)
{
    mdev_reg_base   = base;
    mdev_reg_offset = offset;
    mdev_tp         = dev_tp;
    //osal_printf("%s():%d  base:%#x  offset:%#x\n", __FUNCTION__, __LINE__, base, offset);

#if 0
    mdev_tp[0].platform_port[0] = 0;
    mdev_tp[0].platform_port[1] = 2;
    mdev_tp[0].peer_devId[3] = 1;
    mdev_tp[0].peer_port[3] = 5;

    mdev_tp[1].platform_port[0] = 4;
    mdev_tp[1].platform_port[1] = 6;
    mdev_tp[1].peer_devId[5] = 0;
    mdev_tp[1].peer_port[5] = 3;
#endif
    return RT_ERR_OK;
}

int32 model_portMap_set(rtk_portmask_t *ports, uint32  devId, rtk_portmask_t *mPorts)
{
    rtk_port_t  mport, port;

    if (mdev_tp == NULL)
    {
        osal_printf("%s():%d  Failed! mdev_tp is not initialized\n", __FUNCTION__, __LINE__);
        return RT_ERR_OK;
    }

    RTK_PORTMASK_SCAN(*mPorts, mport)
    {
        port = RTK_PORTMASK_GET_FIRST_PORT(*ports);
        RTK_PORTMASK_PORT_CLEAR(*ports, port);
        mdev_tp[devId].platform_port[mport] = port;
    }

    return RT_ERR_OK;
}

int32 model_devConnect_set(uint32 srcDevId, rtk_portmask_t *srcPorts, uint32 dstDevId, rtk_portmask_t *dstPorts)
{
    rtk_port_t  sPort, dPort;
    ...
    RTK_PORTMASK_SCAN(*srcPorts, sPort)
    {
        dPort = RTK_PORTMASK_GET_FIRST_PORT(*dstPorts);
        RTK_PORTMASK_PORT_CLEAR(*dstPorts, dPort);
        mdev_tp[srcDevId].peer_devId[sPort] = dstDevId;
        mdev_tp[srcDevId].peer_port[sPort] = dPort;
        mdev_tp[dstDevId].peer_devId[dPort] = srcDevId;
        mdev_tp[dstDevId].peer_port[dPort] = sPort;
    }

    return RT_ERR_OK;
}

int32 model_topology_dump(void)
{
    uint32 i, j;
    ...
    for (i = 0; i < 3; i++)
    {
        ...
        {
            if (mdev_tp[i].platform_port[j] != RTMODEL_PORT_NONE)
                osal_printf("dev[%d].port %d  => platform.port %d \n", i, j, mdev_tp[i].platform_port[j]);
            if (mdev_tp[i].peer_devId[j] != RTMODEL_PORT_NONE)
                osal_printf("dev[%d].port %d  => dev[%d].port %d \n", i, j, mdev_tp[i].peer_devId[j], mdev_tp[i].peer_port[j]);
        }
    }
    ...
    /** MODEL TEST **/
        ...
        case RTDRV_EXT_REG_DUMP_SET:    /* dump all reg */
            copy_from_user(&buf.sdk_cfg, user, sizeof(rtdrv_ext_sdkCfg_t));
            {
                ...
                for(reg_idx = 0; reg_idx < rtk_mango_reg_num; reg_idx++)
                {
                    p_fields = rtk_mango_reg_list[reg_idx].pFields;
                    osal_printf("%s\n", rtk_mango_regName_list[reg_idx].name);

                    for(field_idx = 0; field_idx < rtk_mango_reg_list[reg_idx].field_num; field_idx++)
                    {
                        ret = reg_field_read(unit, reg_idx, p_fields[field_idx].name, &val);
                        if(RT_ERR_OK == ret)
                            osal_printf("  %s - %s = 0x%08x\n", rtk_mango_regName_list[reg_idx].name, \
                                    rtk_mango_regField_name_list[p_fields[field_idx].name], val);
                        else
                            osal_printf("  %s - %s = fail\n", rtk_mango_regName_list[reg_idx].name, \
                                    rtk_mango_regField_name_list[p_fields[field_idx].name]);
                    }

                    if(rtk_mango_reg_list[reg_idx].hport == cpu_port)
                    {
                        for(field_idx = 0; field_idx < rtk_mango_reg_list[reg_idx].field_num; field_idx++)
                        {
        ...
        case RTDRV_EXT_MODEL_DEV_SET:
            copy_from_user(&buf.model_cfg, user, sizeof(rtdrv_ext_modelCfg_t));
            osal_printf("%s():%d  unit:%d  devId:%d  base:%#x  offset:%#x  devBase:%#x\n", __FUNCTION__, __LINE__, buf.model_cfg.unit, buf.model_cfg.devId, mdev_reg_base, mdev_reg_offset, mdev_reg_base + buf.model_cfg.devId * mdev_reg_offset);
            ret = ioal_init_memRegion_set(buf.model_cfg.unit, IOAL_MEM_SWCORE, mdev_reg_base + buf.model_cfg.devId * mdev_reg_offset);
            break;

        case RTDRV_EXT_MODEL_PORTMAP_SET:
            copy_from_user(&buf.model_cfg, user, sizeof(rtdrv_ext_modelCfg_t));
            ret = model_portMap_set(&buf.model_cfg.ports, buf.model_cfg.devId, &buf.model_cfg.mPorts);
            break;

        case RTDRV_EXT_MODEL_DEV_CONNECT_SET:
            copy_from_user(&buf.model_cfg, user, sizeof(rtdrv_ext_modelCfg_t));
            ret = model_devConnect_set(buf.model_cfg.devId, &buf.model_cfg.ports, buf.model_cfg.dstDevId, &buf.model_cfg.mPorts);
            break;
...
int32 do_rtdrv_ext_get_ctl(struct sock *sk, int cmd, void *user_in, int *len)
{
    ...
    /** IOL **/
        ...
        case RTDRV_EXT_IOL_MAX_LENGTH_GET:
            ...
            copy_to_user(user, &buf.iol_cfg, sizeof(rtdrv_ext_iolCfg_t));
            break;
        case RTDRV_EXT_MODEL_TOPO_DUMP:
            copy_from_user(&buf.diag_cfg, user, sizeof(rtdrv_ext_diagCfg_t));
            ret = model_topology_dump();
            break;
        ...
        case RTDRV_EXT_TESTIO_GET_EFUSE:
            ...
            copy_to_user(user, &buf.diag_cfg, sizeof(rtdrv_ext_diagCfg_t));
            break;
        case RTDRV_EXT_MODEL_DEV_GET:
            {
            uint32 curBase;
            copy_from_user(&buf.model_cfg, user, sizeof(rtdrv_ext_modelCfg_t));
            ret = ioal_init_memRegion_get(buf.model_cfg.unit, IOAL_MEM_SWCORE, &curBase);
            buf.model_cfg.devId = (curBase - mdev_reg_base) / mdev_reg_offset;
            osal_printf("%s():%d  unit:%d  devId:%d  base:%#x  offset:%#x  devBase:%#x\n", __FUNCTION__, __LINE__, buf.model_cfg.unit, buf.model_cfg.devId, mdev_reg_base, mdev_reg_offset, mdev_reg_base + buf.model_cfg.devId * mdev_reg_offset);
...
EXPORT_SYMBOL(model_mem_base_set);
