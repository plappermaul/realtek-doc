...
#define PKTGEN_MONITOR_POLLING_STACK_SIZE 4096
#define PKTGEN_MONITOR_POLLING_THREAD_PRI 3
...
typedef struct pktgen_monitor_ctrl_s {
    osal_thread_t       polling_thread_id;
    uint32              alive;
    uint32              fault_cnt;
} pktgen_monitor_ctrl_t;

static pktgen_monitor_ctrl_t     *p_pktgen_monitor_ctrl;
...
static void _pktgen_monitor_polling_thread(void *pInput)
{
    int32   ret;
    uint32  unit;
    uint32  val;
    ...
    /* forever loop */
    while (p_pktgen_monitor_ctrl->alive != 0)
    {
        /* scan each port linkstatus */
        HWP_UNIT_TRAVS_LOCAL(unit)
        {
            /* get value from CHIP*/
            /* read twice for latency value */
            reg_field_read(unit, LONGAN_MAC_LINK_STSr, LONGAN_LINK_STS_28_0f, &mac_link);
            if ((ret = reg_field_read(unit, LONGAN_MAC_LINK_STSr, LONGAN_LINK_STS_28_0f, &mac_link)) != RT_ERR_OK)
            {
                osal_printf("Pktgen Monitor: get mac link status fail\n");
                goto EXIT;
            }

            /* read real time phy link status */
            if ((ret = reg_field_read(unit, LONGAN_PHY_LINK_STSr, LONGAN_LINK_STS_27_0f, &phy_link)) != RT_ERR_OK)
            {
                osal_printf("Pktgen Monitor: get phy link status fail\n");
                goto EXIT;
            }

            fault_port = (mac_link & ((1 << HWP_CPU_MACID(unit)) - 1)) ^ phy_link;
            if (0 != fault_port)
            {
                p_pktgen_monitor_ctrl->fault_cnt++;
                if(p_pktgen_monitor_ctrl->fault_cnt > 5)
                {
                    osal_printf("Pktgen Monitor: mac link 0x%x, phy link 0x%x\n", mac_link, phy_link);
                    /* q reset */
                    val = 1;
                    ret = reg_field_write(unit, LONGAN_RST_GLB_CTRL_0r, LONGAN_SW_Q_RSTf, &val);
                    if(ret == RT_ERR_OK)
                    {
                        p_pktgen_monitor_ctrl->fault_cnt = 0;
                        osal_printf("Pktgen Monitor: force q reset\n");
                    }
                    else
                    {
                        osal_printf("Pktgen Monitor: force q reset fail\n");
                        goto EXIT;
                    }
                }
            }
...
EXIT:
    /* deinit p_pktgen_monitor_ctrl */
    osal_free(p_pktgen_monitor_ctrl);
    p_pktgen_monitor_ctrl = NULL;

    osal_thread_exit(0);

    return;
} /* end of _pktgen_monitor_polling_thread */

int32 pktgen_monitor_enable(void)
{
    /* parameter check */

    if (NULL == p_pktgen_monitor_ctrl)
    {
        /* init p_pktgen_monitor_ctrl */
        p_pktgen_monitor_ctrl = osal_alloc(sizeof(pktgen_monitor_ctrl_t));
    }
    else
    {
        if ((p_pktgen_monitor_ctrl->polling_thread_id) != 0)
        {
            osal_printf("pktgen monitor thread exist: id = %d\n", p_pktgen_monitor_ctrl->polling_thread_id);
            return RT_ERR_THREAD_EXIST;
        }
    }
    p_pktgen_monitor_ctrl->alive = 1;

    /* create polling thread */
    p_pktgen_monitor_ctrl->polling_thread_id = osal_thread_create("PktgenMonitorThread", PKTGEN_MONITOR_POLLING_STACK_SIZE, PKTGEN_MONITOR_POLLING_THREAD_PRI
                            , (void *)_pktgen_monitor_polling_thread, NULL);

    if (0 == (p_pktgen_monitor_ctrl->polling_thread_id))
    {
        osal_printf("pktgen monitor thread create failed\n");
...
int32 pktgen_monitor_disable(void)
{
    /* parameter check */

    /* reset alive to 0, thread will suicide after finish all waiting job */
    p_pktgen_monitor_ctrl->alive = 0;

    /* reset p_pktgen_monitor_ctrl->polling_thread_id to 0 */
    p_pktgen_monitor_ctrl->polling_thread_id = 0;
...
//    rtk_port_t basePortId;
    ...
    /**testing cases**/
        ...
        case RTDRV_EXT_PKTGEN_MONITOR_STATE_SET:
            copy_from_user(&buf.pktgen_cfg, user, sizeof(rtdrv_ext_pktGenCfg_t));
            {
                switch(buf.pktgen_cfg.enable)
                {
                    case ENABLED: // start
                        ret = pktgen_monitor_enable();
                        break;
                    case DISABLED: // stop
                        ret = pktgen_monitor_disable();
                        break;
                    default:
                        ret = RT_ERR_FAILED;
                        break;
        ...
        case RTDRV_EXT_REG_DUMP_SET:    /* dump all reg */
            copy_from_user(&buf.sdk_cfg, user, sizeof(rtdrv_ext_sdkCfg_t));
            {
                ...
                for(reg_idx = 0; reg_idx < rtk_longan_reg_num; reg_idx++)
                {
                    p_fields = rtk_longan_reg_list[reg_idx].pFields;
                    osal_printf("%s\n", rtk_longan_regName_list[reg_idx].name);

                    for(field_idx = 0; field_idx < rtk_longan_reg_list[reg_idx].field_num; field_idx++)
                    {
                        ret = reg_field_read(unit, reg_idx, p_fields[field_idx].name, &val);
                        if(RT_ERR_OK == ret)
                            osal_printf("  %s - %s = 0x%08x\n", rtk_longan_regName_list[reg_idx].name, \
                                    rtk_longan_regField_name_list[p_fields[field_idx].name], val);
                        else
                            osal_printf("  %s - %s = fail\n", rtk_longan_regName_list[reg_idx].name, \
                                    rtk_longan_regField_name_list[p_fields[field_idx].name]);
                    }

                    if(rtk_longan_reg_list[reg_idx].hport == cpu_port)
                    {
                        for(field_idx = 0; field_idx < rtk_longan_reg_list[reg_idx].field_num; field_idx++)
                        {
...
int32 do_rtdrv_ext_get_ctl(struct sock *sk, int cmd, void *user_in, int *len)
{
    ...
    /** MODEL TEST **/
        ...
        case RTDRV_EXT_PKTGEN_MONITOR_STATE_GET:
            copy_from_user(&buf.pktgen_cfg, user, sizeof(rtdrv_ext_pktGenCfg_t));
            {
                if (NULL == p_pktgen_monitor_ctrl)
                    buf.pktgen_cfg.enable = 0;
                else
                {
                    if (0 == p_pktgen_monitor_ctrl->alive)
                        buf.pktgen_cfg.enable = 0;
                    else
                        buf.pktgen_cfg.enable = 1;
                }
                ret = RT_ERR_OK;
            }
            copy_to_user(user, &buf.pktgen_cfg, sizeof(rtdrv_ext_pktGenCfg_t));
