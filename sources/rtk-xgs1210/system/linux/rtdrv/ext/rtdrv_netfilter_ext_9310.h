...
enum rtdrv_ext_model_set_e
{
    ...
    RTDRV_EXT_MODEL_DEV_SET,
    RTDRV_EXT_MODEL_PORTMAP_SET,
    RTDRV_EXT_MODEL_DEV_CONNECT_SET,
};
...
{
    RTDRV_EXT_MODEL_DEV_GET = (RTDRV_EXT_BASE_CTL + RTDRV_EXT_MODEL_OFFSET),
    RTDRV_EXT_MODEL_TOPO_DUMP,
};
...
typedef struct rtdrv_ext_modelCfg_s
{
    ...
    uint32  devId;
    uint32  dstDevId;
    rtk_portmask_t ports;
    rtk_portmask_t mPorts;
...
typedef struct rtmodel_topology_s
{
    uint32  platform_port[RTK_MAX_PORT_PER_UNIT];
    uint32  peer_devId[RTK_MAX_PORT_PER_UNIT];
    uint32  peer_port[RTK_MAX_PORT_PER_UNIT];
} rtmodel_topology_t;
