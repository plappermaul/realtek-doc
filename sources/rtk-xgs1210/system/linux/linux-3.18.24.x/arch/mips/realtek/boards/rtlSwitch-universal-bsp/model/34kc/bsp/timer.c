...
static void __init rtl9300_bsp_ext_time_init_all(void){
    ...
    for(i=0; i<NR_CPUS && i<TC_MAX; i++){
        offset = i*0x10;
        if (REG32(RTL93XXMP_TC0INT + offset) & RTL93XXMP_TCIP){
            REG32(RTL93XXMP_TC0INT + offset) |= RTL93XXMP_TCIP;
        }
            /* disable timer before setting CDBR */
            REG32(RTL93XXMP_TC0CTL + offset) = 0; /* disable timer before setting CDBR */
        REG32(RTL93XXMP_TC0DATA + offset ) =  TCDATA;
    }
    ...
    TCCTL = RTL93XXMP_TCEN | RTL93XXMP_TCMODE_TIMER | DIVISOR_RTL9300 ;
        for(i=0; i<NR_CPUS && i<TC_MAX; i++){
       offset = i*0x10;
       REG32(RTL93XXMP_TC0CTL+ offset) = TCCTL;
    }
}
