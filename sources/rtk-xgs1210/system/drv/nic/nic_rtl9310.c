 ...
 *
 * $Revision: 93737 $
 * $Date: 2018-11-21 16:53:51 +0800 (Wed, 21 Nov 2018) $
 *
...
int32 r9310_ntfyBaseAddr_get(uint32 unit, uint32 *pVal)
{
    ioal_mem32_read(unit, RTL9310_L2_NTFY_RING_BASE_ADDR_ADDR, pVal);
    return RT_ERR_OK;
}

int32 r9310_ntfyBaseAddr_set(uint32 unit, uint32 val)
{
    ioal_mem32_write(unit, RTL9310_L2_NTFY_RING_BASE_ADDR_ADDR, val);
    return RT_ERR_OK;
}

int32 r9310_ringBaseAddr_get(uint32 unit, nic_dir_t dir, uint32 ring, uint32 *pVal)
{
    if (dir == NIC_DIR_RX)
        ioal_mem32_read(unit, RTL9310_DMA_IF_RX_BASE_DESC_ADDR_CTRL_ADDR(ring), pVal);
    else
        ioal_mem32_read(unit, RTL9310_DMA_IF_TX_BASE_DESC_ADDR_CTRL_ADDR(ring), pVal);
    return RT_ERR_OK;
}

int32 r9310_ringBaseAddr_set(uint32 unit, nic_dir_t dir, uint32 ring, uint32 val)
{
    if (dir == NIC_DIR_RX)
        ioal_mem32_write(unit, RTL9310_DMA_IF_RX_BASE_DESC_ADDR_CTRL_ADDR(ring), val);
    else
        ioal_mem32_write(unit, RTL9310_DMA_IF_TX_BASE_DESC_ADDR_CTRL_ADDR(ring), val);
    return RT_ERR_OK;
}

int32 r9310_ringCurAddr_get(uint32 unit, nic_dir_t dir, uint32 ring, uint32 *pVal)
{
    if (dir == NIC_DIR_RX)
        ioal_mem32_read(unit, RTL9310_DMA_IF_RX_CUR_DESC_ADDR_CTRL_ADDR(ring), pVal);
    else
        ioal_mem32_read(unit, RTL9310_DMA_IF_TX_CUR_DESC_ADDR_CTRL_ADDR(ring), pVal);
    return RT_ERR_OK;
}
