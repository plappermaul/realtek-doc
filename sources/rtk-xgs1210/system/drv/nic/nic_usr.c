 ...
 *
 * $Revision: 96489 $
 * $Date: 2019-04-22 11:06:18 +0800 (Mon, 22 Apr 2019) $
 *
...
static osal_mutex_t     nic_nicreset_sem[RTK_MAX_NUM_OF_UNIT];
...
#define NIC_SEM_NICRESET_LOCK(unit)    \
do {\
    if (osal_sem_mutex_take(nic_nicreset_sem[unit], OSAL_SEM_WAIT_FOREVER) != RT_ERR_OK)\
    {\
        RT_ERR(RT_ERR_SEM_LOCK_FAILED, (MOD_NIC), "nicreset semaphore lock failed");\
        return RT_ERR_SEM_LOCK_FAILED;\
    }\
} while(0)
#define NIC_SEM_NICRESET_UNLOCK(unit)   \
do {\
    if (osal_sem_mutex_give(nic_nicreset_sem[unit]) != RT_ERR_OK)\
    {\
        RT_ERR(RT_ERR_SEM_UNLOCK_FAILED, (MOD_NIC), "nicreset semaphore unlock failed");\
        return RT_ERR_SEM_UNLOCK_FAILED;\
    }\
} while(0)
...
_nic_isr_rxRoutine(uint32 unit, uint32 ringId)
{
    ...
    do
    {
        ...
        {
            jumboFlag = TRUE;
            if (RT_ERR_OK == _nic_init_conf.pkt_alloc(unit, JUMBO_BUFF_SIZE, 0, &pPacket))
            {
                ...
                pPacket->buf_id = (void *)((uint32)pPacket->buf_id | FLAG_JUMBO_PKT);

                osal_memcpy(pPacket->data, pktHdr->packet->data, pktHdr->buf_len);
        ...
        else if (jumboFlag == TRUE)     /* Jumbo other */
        {
            if (jumboBuffAlloc)
            {
                osal_memcpy(pPacket->data + pPacket->length, pktHdr->packet->data, pktHdr->buf_len);
                pPacket->length += pktHdr->buf_len;
        ...
        if (pktHdr->more == 0 && ((nic_debug_flag & DEBUG_CPU_LOOPBACK_BIT) || (nic_debug_flag & DEBUG_CPU_CALCRC_BIT)))
        {
            crcVal = chksum_crc32(pPacket->data, pPacket->length - 4);

            if (pPacket->data[pPacket->length - 1] != (crcVal >> 24) || pPacket->data[pPacket->length - 2] != ((crcVal >> 16) & 0xff) ||
                pPacket->data[pPacket->length - 3] != ((crcVal >> 8) & 0xff) || pPacket->data[pPacket->length - 4] != (crcVal & 0xff))
            {
                int i;
                ...
                for (i = 0; i < dump_len; i++)
                {
                    ...
                    if (0 == (i % 16))
                        osal_printf("[%04X] ", i);
                    osal_printf("%02X ", *(pPacket->data + i));
                    if (15 == (i % 16))
                        osal_printf("\n");
                }
                osal_printf("\n");
        ...
        if (jumboFlag == 0 || (jumboFlag && pktHdr->more == 0 && jumboBuffAlloc))
        {
        ...
        if (NULL == pktHdr->packet)
        {
            ...
            if (RT_ERR_OK == _nic_init_conf.pkt_alloc(unit, _nic_init_conf.pkt_size, PKTBUF_RX, &pPacket))
            {
                ...
                pktHdr->buf_addr = (uint8 *)(DMA_ADDR_USR2KRN(unit, pPacket->data));
        ...
        if (jumboFlag == 0)
            releaseCnt++;
        else if (jumboFlag && pktHdr->more == 0)
        {
            releaseCnt++;
            jumboFlag = FALSE;
...
_nic_init(uint32 unit, drv_nic_initCfg_t *pInitCfg)
{
    ...
    for (i = 0; i < rxRingNum; i++)
    {
        ...
        RT_ERR_CHK(osal_cache_memory_flush((uint32)pNic_rxFDPBase[i], sizeof(uint32) * rxRingIdSize[i]), ret);
        pNic_rxFDPBase[i] = (uint32*)UNCACHE(pNic_rxFDPBase[i]);
    ...
    for (i = 0; i < txRingNum; i++)
    {
        ...
        RT_ERR_CHK(osal_cache_memory_flush((uint32)pNic_txFDPBase[i], sizeof(uint32) * txRingIdSize[i]), ret);
        pNic_txFDPBase[i] = (uint32*)UNCACHE(pNic_txFDPBase[i]);
    ...
    RT_ERR_CHK(osal_cache_memory_flush(base, (uint32)ptr - base), ret);
    pRxPktHdr = (nic_pkthdr_t *)UNCACHE(((uint32 *)pRxPktHdr));
    pTxPktHdr = (nic_pkthdr_t *)UNCACHE(((uint32 *)pTxPktHdr));
...
drv_nic_init(uint32 unit, drv_nic_initCfg_t *pInitCfg)
{
    ...
    nic_nicreset_sem[unit] = osal_sem_mutex_create();
    if (0 == nic_nicreset_sem[unit])
    {
        RT_ERR(RT_ERR_FAILED, (MOD_NIC), "nicreset semaphore create failed");
        return RT_ERR_FAILED;
    }
...
drv_nic_reset(uint32 unit)
{
    ...
    {
        _nic_holRingSize_set(unit, i, 0);
        _nic_holRingCnt_set(unit, i, rxRingIdSize[i]);
    }
