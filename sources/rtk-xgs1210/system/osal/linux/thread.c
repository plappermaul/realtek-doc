 ...
 */
static void
osal_kthread_entry_point(kthread_ctrl_t *pKt_ctrl)
{
...
#endif
#if ((LINUX_VERSION_CODE >= KERNEL_VERSION(2,6,27)) && (LINUX_VERSION_CODE < KERNEL_VERSION(3,6,0)))
    kernel_thread((int (*)(void *))thread_entry_point, pKt_ctrl, 0);
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3,6,0))
    pKt_ctrl->thread = kthread_create((int (*)(void *))osal_kthread_entry_point, pKt_ctrl, "%s", pKt_ctrl->name);
    if(pKt_ctrl->thread == NULL)
    {
        printk("\ntask create FAILED !!!\n");
        osal_sem_destroy(pKt_ctrl->wait_sem);
        return (osal_thread_t)NULL;
    }
