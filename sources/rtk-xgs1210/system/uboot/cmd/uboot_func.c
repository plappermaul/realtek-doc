...
#define FLASH_HTP_LOG_ADDR          0xb4ff0000     /* b4ff0000 ~ b4ffffff */
#define FLASH_HTP_FLASH_TST_START   0xb4fe0000     /* b4fe0000 ~ b4ff0000 */
#define FLASH_HTP_FLASH_TST_END     FLASH_HTP_LOG_ADDR

#if defined(CONFIG_PRODUCT_XGS1250)
/* FAN Speed threshold */
#define FAN_GPIO_ACTIVE         0

#define FAN_INT_GPIO_PIN        1
#define SYS_FAN_MIN_THRESHOLD   5500
#define SYS_FAN_MAX_THRESHOLD   9000

#define FAN_READ_ROUND          2862800 /* About 1 seconds */
#endif
 ...
 *      rtk_network_port_on
 * Description:
 *      Enable specific port networking functionality and configure hardware to be able to send and receive packets.
 * Input:
 *      None
 * Output:
 *      None
 ...
 *      None
 */
void rtk_network_port_on(uint32 unit, uint32 port)
{
...
/* Function Name:
 *      rtk_network_port_off
 * Description:
 *      Disable specific networking functionality and disable hardware from send and receive packets.
 * Input:
 *      None
 * Output:
 *      None
 ...
 *      None
 */
void rtk_network_port_off(uint32 unit, uint32 port)
{
...
int sys_htp_print(void)
{
    ...
    for(i=1; i<=entry; i++)
    {
        ...
        if(HTP_ERR_NONE == (int)rsn)
            ...
            printf("HTP Test %4d: Loop test  - port %d(tx port) fail!\n", i, (int)msg);
        else if(HTP_ERR_BREAK == (int)rsn)
            printf("HTP Test %4d: Break by user!\n", i);
        else if(HTP_ERR_FAN_FAIL == (int)rsn)
            printf("HTP Test %4d: Fan Speed - check fail! %u RPM.\n", i, (int)msg);
        else if(HTP_ERR_PHY_REG_FAIL == (int)rsn)
            printf("HTP Test %4d: PHY Reg - port %d read fail!\n", i, (int)msg);
        else
        {
            printf("HTP Test: Get HTP test log fail!\n");
            printf("  ------------ HTP Testlog End! ------------\n");
...
static int __sys_htp_phyLoop_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
    int32 rv;
    uint32 val;
#if defined(CONFIG_PRODUCT_XGS1210)
    if((port==24) || (port==25))
    {
...
int sys_htp_internal_loopback_set(int port, int ena)
{
    uint32 unit = uboot_cmd_unit;

    if (ena)
        __sys_htp_phyLoop_set(unit, port, ENABLED);
    else
        __sys_htp_phyLoop_set(unit, port, DISABLED);
    return 0;
}

int sys_htp_looptest_run_internal(void)
{
    int i, j, recv, count;
    ...
    htpModeIf = 1;
    //not necessary and produce duplicated pkt
    ...
    for(i=0; i<12; i++)
    {
        ...
        udelay(3000000);//enlarge to 3 sec due to slow link up
...
void _port_isolation_setup(void)
{
    uint32          unit = uboot_cmd_unit;
    uint32          port;
    rtk_portmask_t  portmask;
    ...
    {
        osal_memset(&portmask, 0, sizeof(rtk_portmask_t));
        RTK_PORTMASK_PORT_SET(portmask, HWP_CPU_MACID(unit));
        if ((ret = rtk_port_isolation_set(unit, port, &portmask)) != RT_ERR_OK)
            printf("isolation fail(%d): source=%u, destination=%u\n", ret, port, HWP_CPU_MACID(unit));
    }/* end for */
...
int sys_htp_looptest_run_external(void)
{
    int i, j, recv, count;
    int txPort[] = {0, 1, 2, 3, 4, 5, 6, 7, 24, 25, 27};
    int rxPort[] = {1, 0, 3, 2, 5, 4, 7, 6, 25, 24, 27};
    ...
    //set 10g fiber to 10g media
    rtk_10gMedia_fiber10g_set(27);

    /* setup port isolation for each port to talk with cpu only */
    _port_isolation_setup();

    //not necessary and produce duplicated pkt
    //MEM32_WRITE(0xbb00940c, 0); // src port filter close
    NetInit();
    udelay(3000000);
    for(i=0; i<11; i++)//port 26 is not tested
    {
        drv_nic_reset(unit); // clear nic fifo
        recv = 0;

        //only enable MAC_TX on txPort and cpu port
        MEM32_WRITE(0xbb00a490, ~((1<<txPort[i])|(1<<HWP_CPU_MACID(unit))));
        udelay(1000000);
        NetSendPacket(pkt,sizeof(pkt));
        udelay(1000000);

        //restore MAC_TX
        MEM32_WRITE(0xbb00a490, 0);
        ...
        for(count=0; count<0xF; count++)
        {
            ...
            for(j=0; j<sizeof(pkt); j++)
            {
                if(pkt[j] != (uint8)(htpRxPkt[j]))
                {
                    htpFailMsg = (u_long)(txPort[i]);
                    printf("(tx port[%d], rx port[%d]) Received htpRxPkt[%d] = %d, while sent pkt[%d] = %d!\n", txPort[i], rxPort[i], j, (uint8)(htpRxPkt[j]), j, pkt[j]);
                    printf("  ------------  Port Loop Test Fail!  ------------\n");
                    __sys_htp_rx_clear();
                    return HTP_ERR_PORT_PKT_VAL;
                }
        ...
        {
            htpFailMsg = (u_long)(txPort[i]);
            printf("(tx port[%d], rx port[%d]) has not received a packet!\n", txPort[i], rxPort[i]);
            printf("  ------------  Port Loop Test Fail!  ------------\n");
            htpModeIf = 0;
            return HTP_ERR_PORT_PKT_VAL;
        }
        printf("(tx port[%d], rx port[%d]) loop test ok!\n", txPort[i], rxPort[i]);
    }
    printf("  ------------  Port Loop Test Pass!  ------------\n");
    //MEM32_WRITE(0xbb00940c, 1); // src port filter on
    //restore port isolation
    rtk_port_isolation_off();
    __sys_htp_rx_clear();
    return 0;
}
...
#if defined(CONFIG_PRODUCT_XGS1250)
int sys_htp_fan_speed_test(int *speed)
{
    uint32 value, preValue, rpm = 0;
    int i;
    drv_generalCtrlGpio_pinConf_t gpioConfig;
    //uint32  unit = HWP_MY_UNIT_ID();
    uint32 unit = HWP_DEFHWP_UNIT_ID;
    ...
    gpioConfig.ext_gpio.inverter = 0;
    gpioConfig.ext_gpio.direction = 0;
    drv_generalCtrlGPIO_pin_init(unit, GEN_GPIO_DEV_ID0_INTERNAL, FAN_INT_GPIO_PIN, &gpioConfig);
    ...
    {
        drv_generalCtrlGPIO_dataBit_get(unit, GEN_GPIO_DEV_ID0_INTERNAL, FAN_INT_GPIO_PIN, &value);
    ...
    printf("  HTP FAN Speed Test: Speed %u RPM. (valid range: %d ~ %d)\n", rpm, SYS_FAN_MIN_THRESHOLD, SYS_FAN_MAX_THRESHOLD);

    if (SYS_FAN_MIN_THRESHOLD > rpm || SYS_FAN_MAX_THRESHOLD < rpm)
    {
        htpFailMsg = (u_long)rpm;
...
int sys_htp_phy_reg_test(void)
{
    ...
    printf("  ------------  PHY Reg Test Start    ------------\n");

    for (port=24;port<=26;port++)
    {
        if ((rv = phy_reg_mmd_get(unit, port, 7, 2, &id)) != RT_ERR_OK)
        {
            htpFailMsg = (u_long)port;
            return HTP_ERR_PHY_REG_FAIL;
        }

        if ((rv = phy_reg_mmd_get(unit, port, 7, 3, &ver)) != RT_ERR_OK)
        {
            htpFailMsg = (u_long)port;
            return HTP_ERR_PHY_REG_FAIL;
        }

        if (id==0x31c3 && ver==0x1c12)
            printf("  PHY Reg port %d id 0x%04X 0x%04X)\n", port, id, ver);
        else
        {
            htpFailMsg = (u_long)port;
            return HTP_ERR_PHY_REG_FAIL;
...
{
    int i, ret;
    /* for now, one round need 4 min 20 seconds, so do 13 rounds in one hour */
    int round = 13; // run four per hour
...
#if defined(CONFIG_PRODUCT_XGS1250)
        if((ret = sys_htp_phy_reg_test()) != HTP_ERR_NONE)
        {
            if(ret != HTP_ERR_BREAK)
                MEM32_WRITE(0xbb00cc00, 0x01051659); // led off
            _sys_htp_log_set(entry, (u_long)ret, htpFailMsg);
...
#endif
#if defined(CONFIG_PRODUCT_XGS1210)
        if((ret = sys_htp_looptest_run_internal()) != HTP_ERR_NONE)
        {
            if(ret != HTP_ERR_BREAK)
                MEM32_WRITE(0xbb00cc00, 0x01051659); // led off
            _sys_htp_log_set(entry, (u_long)ret, htpFailMsg);
...
#endif
#if defined(CONFIG_PRODUCT_XGS1250)
        if((ret = sys_htp_looptest_run_external()) != HTP_ERR_NONE)
        {
            if(ret != HTP_ERR_BREAK)
                MEM32_WRITE(0xbb00cc00, 0x01051659); // led off
            _sys_htp_log_set(entry, (u_long)ret, htpFailMsg);
...
#endif
#if defined(CONFIG_PRODUCT_XGS1250)
        if((ret = sys_htp_fan_speed_test(&fan_speed)) != HTP_ERR_NONE)
        {
            if(ret != HTP_ERR_BREAK)
                MEM32_WRITE(0xbb00cc00, 0x01051659); // led off
            _sys_htp_log_set(entry, (u_long)ret, fan_speed);
            htpFailMsg = 0;
            _sys_htp_exit_set();
            _sys_htp_info_set(htpModeIf, htpBreakIf, 0, entry, 0);
            printf("============= Factory Test End ! =============\n");
