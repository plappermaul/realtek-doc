    ...
    if (0 == strcmp(argv[1], "network"))
    {
        ...
        if (0 == strcmp(argv[2], "on"))        /* network on */
        {
            ...
            else
            {
                ...
                rtk_network_port_on(uboot_cmd_unit, port_num);
            }
        ...
        else if (0 == strcmp(argv[2], "off"))  /* network off */
        {
            ...
            {
                port_num = simple_strtoul(argv[3], NULL, 10);
                rtk_network_port_off(uboot_cmd_unit, port_num);
            }
...
int do_sys(cmd_tbl_t * cmdtp, int flag, int argc, char * const argv[])
{
    ...
    if(strcmp(argv[1], "fan-test") == 0)
    {
        int speed;
        ret = sys_htp_fan_speed_test(&speed);
        if(HTP_ERR_NONE == ret)
            return 0;
        else
            return 1;
...
#endif
    if(strcmp(argv[1], "internal-loopback-set") == 0)
    {
        uint32 port_num;
        port_num = simple_strtoul(argv[2], NULL, 10);

        ret = sys_htp_internal_loopback_set(port_num, 1);
        return 0;
    }
    if(strcmp(argv[1], "internal-loopback-unset") == 0)
    {
        uint32 port_num;
        port_num = simple_strtoul(argv[2], NULL, 10);

        ret = sys_htp_internal_loopback_set(port_num, 0);
        return 0;
    }
    if(strcmp(argv[1], "internal-loopback-test") == 0)
    {
        ret = sys_htp_looptest_run_internal();
        if(HTP_ERR_NONE == ret)
            return 0;
        else
            return 1;
    }
    if(strcmp(argv[1], "external-loopback-test") == 0)
    {
        ret = sys_htp_looptest_run_external();
        if(HTP_ERR_NONE == ret)
            return 0;
        else
            return 1;
...
U_BOOT_CMD(
    ...
    "sys fan-test\n"
#endif
    "sys internal-loopback-set 1"
    "sys internal-loopback-unset 1"
    "sys internal-loopback-test\n"
    "sys external-loopback-test\n"
);
