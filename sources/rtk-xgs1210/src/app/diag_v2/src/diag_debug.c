 ...
 *
 * $Revision: 98655 $
 * $Date: 2019-07-12 14:48:25 +0800 (Fri, 12 Jul 2019) $
 *
...
cparser_result_t cparser_cmd_debug_get_sw_wd(cparser_context_t *context)
{
    ...
    diag_util_printf("DrainTX OK  WatchDog CNT = %u\n", value);
    ...
    uint32_t *index_ptr)
{
    ...
    switch (*index_ptr)
    {
        ...
        case RTL9310_DBG_CNTR_RX_MAX_FRAME_SIZE:
            diag_util_mprintf("Debug Counter %2d - RX Max Frame Size : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTL9310_DBG_CNTR_EGR_QUEUE:
            diag_util_mprintf("Debug Counter %2d - Egress Queue : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTL9310_DBG_CNTR_STACK_UCAST_NONUCAST_TTL:
            diag_util_mprintf("Debug Counter %2d - Stacking Unicast/NonUnicast TTL : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTL9310_DBG_CNTR_EACL_METER:
            diag_util_mprintf("Debug Counter %2d - EACL Meter (learn) : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTL9310_DBG_CNTR_UC_MC_RPF:
            diag_util_mprintf("Debug Counter %2d - Unicast/Multicast RPF (learn) : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTL9310_DBG_CNTR_TUNNEL_IP_ADDRESS_CHECK:
            diag_util_mprintf("Debug Counter %2d - Tunnel IP Address Check (learn) : %u \n", *index_ptr, cntr);
            break;
        case RTL9310_DBG_CNTR_EACL_DROP:
            diag_util_mprintf("Debug Counter %2d - EACL Drop (learn) : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTL9310_DBG_CNTR_SYSTEM_PORT_LIMIT_LEARN:
            diag_util_mprintf("Debug Counter %2d - System Port Limit Learn : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTL9310_DBG_CNTR_CFI_1:
            diag_util_mprintf("Debug Counter %2d - CFI=1 : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTL9310_DBG_CNTR_IGR_VLAN_CONVERT:
            diag_util_mprintf("Debug Counter %2d - Ingress VLAN Convert : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTL9310_DBG_CNTR_MSTP_SRC_DROP:
            diag_util_mprintf("Debug Counter %2d - MSTP SRC Drop (Learning) (learn) : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTL9310_DBG_CNTR_MTU:
            diag_util_mprintf("Debug Counter %2d - MTU : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTL9310_DBG_CNTR_IP4_IP_OPTION:
            diag_util_mprintf("Debug Counter %2d - IPv4 IP Option : %u \n", *index_ptr, cntr);
            break;
        case RTL9310_DBG_CNTR_IP6_HBH_EXT_HEADER:
            diag_util_mprintf("Debug Counter %2d - IPv6 Hop-by-Hop Extension Header : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTL9310_DBG_CNTR_OAM_MUX:
            diag_util_mprintf("Debug Counter %2d - OAM Mux : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTL9310_DBG_CNTR_L2_UC:
            diag_util_mprintf("Debug Counter %2d - L2 Unicast : %u \n", *index_ptr, cntr);
            break;
        case RTL9310_DBG_CNTR_L2_MC:
            diag_util_mprintf("Debug Counter %2d - L2 Multicast : %u \n", *index_ptr, cntr);
            break;
        case RTL9310_DBG_CNTR_IP4_MC:
            diag_util_mprintf("Debug Counter %2d - IPv4 Multicast : %u \n", *index_ptr, cntr);
            break;
        case RTL9310_DBG_CNTR_IP6_MC:
            diag_util_mprintf("Debug Counter %2d - IPv6 Multicast : %u \n", *index_ptr, cntr);
            break;
        ...
        case RTL9310_DBG_CNTR_UNKNOWN_L2_UC_FLPM:
            diag_util_mprintf("Debug Counter %2d - Unknown L2 Unicast Flooding Port Mask : %u \n", *index_ptr, cntr);
            break;
        case RTL9310_DBG_CNTR_BC_FLPM:
            diag_util_mprintf("Debug Counter %2d - Broadcast Flooding Port Mask : %u \n", *index_ptr, cntr);
            break;
        case RTL9310_DBG_CNTR_VLAN_PRO_UNKNOWN_L2_MC_FLPM:
            diag_util_mprintf("Debug Counter %2d - VLAN Pro Unknown L2 Multicast Flooding Port Mask : %u \n", *index_ptr, cntr);
            break;
        case RTL9310_DBG_CNTR_VLAN_PRO_UNKNOWN_IP4_MC_FLPM:
            diag_util_mprintf("Debug Counter %2d - VLAN Pro Unknown IPv4 Multicast Flooding Port Mask : %u \n", *index_ptr, cntr);
            break;
        case RTL9310_DBG_CNTR_VLAN_PROFILE_UNKNOWN_IP6_MC_FLPM:
            diag_util_mprintf("Debug Counter %2d - VLAN Profile Unknown IPv6 Multicast Flooding Port Mask : %u \n", *index_ptr, cntr);
            break;
        default:
            diag_util_printf("User config: Error!\n");
    }
 ...
 */
cparser_result_t
cparser_cmd_debug_get_memory_base_addr(cparser_context_t *context)
{
    uint32      unit = 0;
    int32       ret = RT_ERR_FAILED;
    ...
    DIAG_UTIL_ERR_CHK(debug_mem_baseAddr_show(unit), ret);
    return CPARSER_OK;
}
#endif
...
cparser_result_t cparser_cmd_debug_get_memory_available(cparser_context_t *context)
{
    ...
    {
        if ((ret = read (fd, &buffer, (BUF_SIZE-1))) > 0)
        {
            buffer[ret] = '\0';
    ...
    system("cat /proc/meminfo > /tmp/mem_log_cur");
...
_cparser_cmd_debug_get_image_info_runtime(cparser_context_t *context)
{
    ...
    if ((n = read(fd, &ih, sizeof(image_header_t))) < 0)
    {
        close(fd);
...
typedef struct debug_pthread {
    void            (*thread_cb)(void *);
    void            (*thread_exit)(void *);
    char            *thread_name;
    void            *thread_arg;
    int             running; /* 1: running, 0:exit */
} debug_pthread_t;

int pthread_sample0_cookie=999;
void _diagShell_pthread_sample0(void *p)
{
    debug_pthread_t *thread_info = (debug_pthread_t *)p;

    while(thread_info->running){
        diag_util_printf("sample pthread 0 (cookie=%d)\n",*(int *)thread_info->thread_arg);
        sleep(3);
    }
    return;
}
void _diagShell_pthread_sample1(void *p)
{
    debug_pthread_t *thread_info = (debug_pthread_t *)p;

    while(thread_info->running){
        diag_util_printf("sample pthread 1\n");
        sleep(2);
    }
    return;
}
...
debug_pthread_t debug_pthread[]=
{
    {
        .thread_cb      = _diagShell_pthread_sample0,
        .thread_exit    = NULL,
        .thread_name    = "sample_pthread 0",
        .thread_arg     = (void *)&pthread_sample0_cookie,
        .running        = 0,
    },
    {
        .thread_cb      = _diagShell_pthread_sample1,
        .thread_exit    = NULL,
        .thread_name    = "sample_pthread 1",
        .thread_arg     = NULL,
        .running        = 0,
    },
};
 ...
 * debug pthread help
 */
cparser_result_t cparser_cmd_debug_pthread_help(cparser_context_t *context)
{
    uint32  i, thread_num_max;


    DIAG_UTIL_PARAM_CHK();

    thread_num_max = (sizeof(debug_pthread)/sizeof(debug_pthread_t));

    diag_util_printf("To create pthread for debug purpose. Should program the thread first.\n Command:\n\n");

    diag_util_printf("  debug pthread create ID  (hint: could create multiple times)\n");
    diag_util_printf("  debug pthread destroy ID (hint: if multiple, will destroy all)\n\n");


    diag_util_printf("  -- ID:\n");
    for(i=0;i<thread_num_max;i++){
        diag_util_printf("     %d: %s\n",i,debug_pthread[i].thread_name);
    }
    diag_util_printf("\n");
 ...
 */
cparser_result_t cparser_cmd_debug_pthread_create_destroy_value(cparser_context_t *context,
    uint32_t *value_ptr)
{
    uint32  thread_num=0;
    uint32  create=1;
    ...
    if(create)
    {
        diag_util_printf("create  pthread (%s) .... ",debug_pthread[thread_num].thread_name);

        debug_pthread[thread_num].running = 1;
        osal_thread_create(debug_pthread[thread_num].thread_name,
                           4096,
                           100,
                           debug_pthread[thread_num].thread_cb,
                           (void *)&debug_pthread[thread_num]);

        diag_util_printf("DONE!\n");
    }
    else
    {
        diag_util_printf("pthread (%s) exit\n",debug_pthread[thread_num].thread_name);
        debug_pthread[thread_num].running = 0;
    }
    return CPARSER_OK;
}
#endif /* CMD_DEBUG_PTHREAD_CREATE */
