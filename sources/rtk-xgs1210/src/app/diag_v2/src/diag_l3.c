 ...
 *
 * $Revision: 97162 $
 * $Date: 2019-05-24 16:12:53 +0800 (Fri, 24 May 2019) $
 *
...
#define DUMP_ECMP_LH()                                                              \
do {                                                                                \
    DIAG_UTIL_MPRINTF(" No. | Nexthop ID |  Interface | MAC address         \n");   \
    DIAG_UTIL_MPRINTF("-----+------------+------------+---------------------\n");   \
} while (0)

#define DUMP_ECMP_ENTRY(__no, __nhId, __intfId, __macAddr)  \
do {                                                        \
    char __ms[32];                                          \
    osal_memset(&__ms, 0, sizeof(__ms));                    \
    diag_util_mac2str(__ms, __macAddr.octet);               \
    DIAG_UTIL_MPRINTF(" %3d | 0x%08X | 0x%08X | %s \n",     \
        __no, __nhId, __intfId, __ms);                      \
} while (0)
...
#ifdef CMD_L3_ADD_VRRP_VID_VID_VRID_VRID
/*
 * l3 add vrrp vid <UINT:vid> vrid <UINT:vrid>
 */
cparser_result_t
cparser_cmd_l3_add_vrrp_vid_vid_vrid_vrid(
    cparser_context_t *context,
    uint32_t *vid_ptr,
    uint32_t *vrid_ptr)
{
    ...
    DIAG_UTIL_FUNC_INIT(unit);
    DIAG_UTIL_ERR_CHK(rtk_l3_vrrp_add(unit, *vid_ptr, *vrid_ptr), ret);

    return CPARSER_OK;
}   /* end of cparser_cmd_l3_add_vrrp_vid_vid_vrid_vrid */
#endif

#ifdef CMD_L3_DEL_VRRP_VID_VID_VRID_VRID
/*
 * l3 del vrrp vid <UINT:vid> vrid <UINT:vrid>
 */
cparser_result_t
cparser_cmd_l3_del_vrrp_vid_vid_vrid_vrid(
    cparser_context_t *context,
    uint32_t *vid_ptr,
    uint32_t *vrid_ptr)
{
    ...
    DIAG_UTIL_FUNC_INIT(unit);
    DIAG_UTIL_ERR_CHK(rtk_l3_vrrp_del(unit, *vid_ptr, *vrid_ptr), ret);

    return CPARSER_OK;
}   /* end of cparser_cmd_l3_del_vrrp_vid_vid_vrid_vrid */
#endif

#ifdef CMD_L3_DEL_VRRP_VID_VID_VRID_ALL
/*
 * l3 del vrrp vid <UINT:vid> vrid all
 */
cparser_result_t
cparser_cmd_l3_del_vrrp_vid_vid_vrid_all(
    cparser_context_t *context,
    uint32_t *vid_ptr)
{
    uint32  unit;
    ...
    DIAG_UTIL_FUNC_INIT(unit);
    DIAG_UTIL_ERR_CHK(rtk_l3_vrrp_delAll(unit, *vid_ptr), ret);

    return CPARSER_OK;
}   /* end of cparser_cmd_l3_del_vrrp_vid_vid_vrid_all */
#endif

#ifdef CMD_L3_GET_VRRP_VID_VID_VRID
/*
 * l3 get vrrp vid <UINT:vid> vrid
 */
cparser_result_t
cparser_cmd_l3_get_vrrp_vid_vid_vrid(
    cparser_context_t *context,
    uint32_t *vid_ptr)
{
    uint32  unit;
    ...
    DIAG_UTIL_ERR_CHK(rtk_l3_vrrp_get(unit, *vid_ptr, L3_VRID_MAX, vrIds, &maxIdRet), ret);
    ...
    uint32_t *count_ptr)
{
    ...
    for (i=0; i<maxIdRet; i++)
    {
        ...
        DUMP_ECMP_ENTRY((i+1), nhIds[i], nh.intf_id, nh.mac_addr);
    }
