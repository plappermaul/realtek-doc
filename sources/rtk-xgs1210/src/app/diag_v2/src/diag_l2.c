 ...
 *
 * $Revision: 98576 $
 * $Date: 2019-07-11 14:45:28 +0800 (Thu, 11 Jul 2019) $
 *
    ...
    char **port_ptr)
{
    ...
    DIAG_UTIL_PORTMASK_SCAN(portlist, port)
    {
...
#endif
        {
            ...
            mac_cnt = limitCnt.un.portTrkCnt.cnt;
        }
        ...
        if(ENABLED == enable)
        {
...
#endif
            {
                ...
                action = limitAction.un.portTrkAct.act;
            }
...
cparser_result_t cparser_cmd_l2_table_get_entry_index_index(cparser_context_t *context, uint32_t *index_ptr)
{
    ...
    {
        osal_memcpy(&l2_data, &entry.un.unicast, sizeof(rtk_l2_ucastAddr_t));
#if defined(CONFIG_SDK_RTL8390) || defined(CONFIG_SDK_RTL8380)
    ...
    {
        osal_memcpy(&mcast_data, &entry.un.l2mcast, sizeof(rtk_l2_mcastAddr_t));
    ...
    {
        osal_memcpy(&ip_mcast_data, &entry.un.ipmcast, sizeof(rtk_l2_ipMcastAddr_t));
        diag_util_lPortMask2str(port_list, &ip_mcast_data.portmask);
    ...
    {
        osal_memcpy(&ip6_mcast_data, &entry.un.ip6mcast, sizeof(rtk_l2_ip6McastAddr_t));
        diag_util_lPortMask2str(port_list, &ip6_mcast_data.portmask);
    ...
    uint32_t *index_ptr)
{
    ...
    if(l2_entry.entry_type  == FLOW_TYPE_UNICAST)
    {
        ...
        if (DIAG_OM_GET_FAMILYID(RTL9300_FAMILY_ID))
        {
            ...
            if(l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_NEXTHOP)
            {
                diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %3d | %4s | %3d |%4d|%4d|%4d| %2d |%3d| %s| %11d|  %2d\n",
                            l2_entry.un.unicast.l2_idx,
                            l2_entry.un.unicast.vid,
                            l2_entry.un.unicast.mac.octet[0],l2_entry.un.unicast.mac.octet[1],l2_entry.un.unicast.mac.octet[2],l2_entry.un.unicast.mac.octet[3],l2_entry.un.unicast.mac.octet[4],l2_entry.un.unicast.mac.octet[5],
                            (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_TRUNK_PORT)?1:0,
                            (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_TRUNK_PORT) ? (l2_entry.un.unicast.devID==1 ? "ntfy" : " - ") : buffer,
                            (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_TRUNK_PORT)?l2_entry.un.unicast.trk_gid:l2_entry.un.unicast.port,
                            (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_SA_BLOCK)?1:0,
                            (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_DA_BLOCK)?1:0,
                            (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_STATIC)?1:0,
                            (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_NEXTHOP)?1:0,
                            (l2_entry.un.unicast.state&RTK_L2_UCAST_STATE_SUSPEND)?1:0,
                            "Not Support",
                            l2_entry.un.unicast.route_idx,
                            l2_entry.un.unicast.age);
            }
            else
            {
                diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %3d | %4s | %3d |%4d|%4d|%4d| %2d |%3d| %10d | %s|  %2d\n",
        ...
        if (DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID))
        {
            ...
            sprintf(buffer, "%d", l2_entry.un.unicast.devID);
            diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %d |%5s|%3d|%4d|%4d|%4d|%2d|%3d|%3d|%4d|%4d| %d |0x%06X|  %2d\n",
                l2_entry.un.unicast.l2_idx,
                l2_entry.un.unicast.vid,
                l2_entry.un.unicast.mac.octet[0],l2_entry.un.unicast.mac.octet[1],l2_entry.un.unicast.mac.octet[2],l2_entry.un.unicast.mac.octet[3],l2_entry.un.unicast.mac.octet[4],l2_entry.un.unicast.mac.octet[5],
                (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_TRUNK_PORT) ? 1 : 0,
                (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_TRUNK_PORT) ? " - " : buffer,
                (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_TRUNK_PORT) ? l2_entry.un.unicast.trk_gid : l2_entry.un.unicast.port,
                (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_SA_BLOCK) ? 1 : 0,
                (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_DA_BLOCK) ? 1 : 0,
                (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_STATIC) ? 1 : 0,
                (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_NEXTHOP) ? 1 : 0,
                (l2_entry.un.unicast.state&RTK_L2_UCAST_STATE_SUSPEND) ? 1 : 0,
                (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_TAG_STS) ? 1 : 0,
                l2_entry.un.unicast.agg_pri,
                l2_entry.un.unicast.agg_vid,
                (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_L2_TUNNEL) ? 1 : 0,
                (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_L2_TUNNEL) ? l2_entry.un.unicast.l2_tunnel_idx : l2_entry.un.unicast.ecid,
                l2_entry.un.unicast.age);
    ...
    else
    {
        ...
        if(DIAG_OM_GET_FAMILYID(RTL9300_FAMILY_ID))
        {
            ...
            osal_memset(port_list, 0, DIAG_UTIL_PORT_MASK_STRING_LEN);
            diag_util_lPortMask2str(port_list, &l2_entry.un.l2mcast.portmask);

            if(l2_entry.un.l2mcast.nextHop)
            {
                diag_util_mprintf("%6d | %4d | %02X:%02X:%02X:%02X:%02X:%02X | %20s | %4d | %4d\n",
                            l2_entry.un.l2mcast.l2_idx,
                            l2_entry.un.l2mcast.rvid, l2_entry.un.l2mcast.mac.octet[0], l2_entry.un.l2mcast.mac.octet[1], l2_entry.un.l2mcast.mac.octet[2],
                            l2_entry.un.l2mcast.mac.octet[3], l2_entry.un.l2mcast.mac.octet[4], l2_entry.un.l2mcast.mac.octet[5], port_list, l2_entry.un.l2mcast.nextHop, l2_entry.un.l2mcast.mac_idx);
            }
            else
            {
                diag_util_mprintf("%6d | %4d | %02X:%02X:%02X:%02X:%02X:%02X | %20s | %4d | %s\n",
                            l2_entry.un.l2mcast.l2_idx,
                            l2_entry.un.l2mcast.rvid, l2_entry.un.l2mcast.mac.octet[0], l2_entry.un.l2mcast.mac.octet[1], l2_entry.un.l2mcast.mac.octet[2],
                            l2_entry.un.l2mcast.mac.octet[3], l2_entry.un.l2mcast.mac.octet[4], l2_entry.un.l2mcast.mac.octet[5], port_list, l2_entry.un.l2mcast.nextHop, "Not Support");
            }
        }
#endif
        ...
        if(DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID))
        {
            ...
            diag_util_mprintf("%6d | %4d | %02X:%02X:%02X:%02X:%02X:%02X | %20s | %2d | 0x%X\n",
                        l2_entry.un.l2mcast.l2_idx,
                        l2_entry.un.l2mcast.rvid, l2_entry.un.l2mcast.mac.octet[0], l2_entry.un.l2mcast.mac.octet[1], l2_entry.un.l2mcast.mac.octet[2],
                        l2_entry.un.l2mcast.mac.octet[3], l2_entry.un.l2mcast.mac.octet[4], l2_entry.un.l2mcast.mac.octet[5], port_list, l2_entry.un.l2mcast.nextHop, l2_entry.un.l2mcast.groupId);
        }
#endif
    }
    ...
    uint32_t *index_ptr)
{
    ...
    if(l2_entry.entry_type  == FLOW_TYPE_UNICAST)
    {
        if(l2_entry.un.unicast.flags & RTK_L2_UCAST_FLAG_NEXTHOP)
        {
            ...
            diag_util_mprintf("%5d | %02X:%02X:%02X:%02X:%02X:%02X | %4d | %4d | %8d | %8d | %7d | %8d | %7d | %5d | %8d\n",
                scanIdx,
                l2_entry.un.unicast.mac.octet[0],l2_entry.un.unicast.mac.octet[1],l2_entry.un.unicast.mac.octet[2],l2_entry.un.unicast.mac.octet[3],l2_entry.un.unicast.mac.octet[4],l2_entry.un.unicast.mac.octet[5],
                l2_entry.un.unicast.vid, l2_entry.un.unicast.port, (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_SA_BLOCK)?1:0, (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_DA_BLOCK)?1:0,
                (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_STATIC)?1:0, (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_NEXTHOP)?1:0,
                (l2_entry.un.unicast.state&RTK_L2_UCAST_STATE_SUSPEND)?1:0, l2_entry.un.unicast.vlan_target, l2_entry.un.unicast.route_idx);
        ...
        else
        {
            ...
            diag_util_mprintf("%5d | %02X:%02X:%02X:%02X:%02X:%02X | %4d | %4d | %8d | %8d | %7d | %8d | %7d | %4d\n",
                ...
                l2_entry.un.unicast.vid, l2_entry.un.unicast.port, (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_SA_BLOCK)?1:0, (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_DA_BLOCK)?1:0,
                (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_STATIC)?1:0, (l2_entry.un.unicast.flags&RTK_L2_UCAST_FLAG_NEXTHOP)?1:0,
                (l2_entry.un.unicast.state&RTK_L2_UCAST_STATE_SUSPEND)?1:0, l2_entry.un.unicast.agg_vid);
        }
    }
    else if(l2_entry.entry_type  == FLOW_TYPE_L2_MULTI)
    {
        ...
        diag_util_mprintf("%5d | %4d | %02X:%02X:%02X:%02X:%02X:%02X | %20s | %4d\n",
                    scanIdx,
                    l2_entry.un.l2mcast.rvid, l2_entry.un.l2mcast.mac.octet[0], l2_entry.un.l2mcast.mac.octet[1], l2_entry.un.l2mcast.mac.octet[2],
                    l2_entry.un.l2mcast.mac.octet[3], l2_entry.un.l2mcast.mac.octet[4], l2_entry.un.l2mcast.mac.octet[5], port_list, l2_entry.un.l2mcast.agg_vid);
    }
    else if(l2_entry.entry_type  == FLOW_TYPE_IP4_MULTI)
    {
        ...
        diag_util_ip2str_format(strBuf1, l2_entry.un.ipmcast.sip, 15);
        diag_util_ip2str_format(strBuf2, l2_entry.un.ipmcast.dip, 15);
        diag_util_mprintf("%5d | %s | %s | %4d | %20s\n", scanIdx, strBuf1, strBuf2, l2_entry.un.ipmcast.rvid, port_list);
    }
    else if(l2_entry.entry_type  == FLOW_TYPE_IP6_MULTI)
    {
        ...
        osal_memset(port_list, 0, DIAG_UTIL_PORT_MASK_STRING_LEN);
        diag_util_lPortMask2str(port_list, &l2_entry.un.ip6mcast.portmask);
        DIAG_UTIL_ERR_CHK(diag_util_ipv62str(ipv6SipStr, l2_entry.un.ip6mcast.sip.octet), ret);
        DIAG_UTIL_ERR_CHK(diag_util_ipv62str(ipv6DipStr, l2_entry.un.ip6mcast.dip.octet), ret);
        diag_util_mprintf("%5d | %20s | %20s | %4d | %20s \n", scanIdx, ipv6SipStr, ipv6DipStr, l2_entry.un.ip6mcast.rvid, port_list);
    }
    else
        return CPARSER_NOT_OK;
...
cparser_result_t cparser_cmd_l2_table_get_limit_learning_global(cparser_context_t *context)
{
    ...
    if (IS_BACKWARD_COMPATIBLE)
    {
        DIAG_UTIL_ERR_CHK(rtk_l2_limitLearningCnt_get(unit, &limitNum.un.glbCnt), ret);
    }
    else
#endif
    {
    ...
    if (IS_BACKWARD_COMPATIBLE)
    {
        ...
        limitAct.un.glbAct = (rtk_action_t) act;
    }
    else
#endif
    {
    ...
    if (limitNum.un.glbCnt == L2_MAC_CST_DISABLE)
    ...
    if (limitAct.un.glbAct == ACTION_FORWARD)
    {
        diag_util_mprintf("Forward\n");
    }
    else if (limitAct.un.glbAct ==ACTION_DROP)
    {
        diag_util_mprintf("Drop\n");
    }
    else if (limitAct.un.glbAct == ACTION_TRAP2CPU)
    {
        diag_util_mprintf("Trap-To-Cpu\n");
    }
    else if (limitAct.un.glbAct == ACTION_COPY2CPU)
    {
        diag_util_mprintf("Copy-To-Cpu\n");
    }
    else if (limitAct.un.glbAct == ACTION_TRAP2MASTERCPU)
    {
       diag_util_mprintf("Trap-To Master Cpu\n");
    }
    else if (limitAct.un.glbAct == ACTION_COPY2MASTERCPU)
    {
       diag_util_mprintf("Copy-To Master Cpu\n");
    }
    else
    ...
    uint32_t *max_count_ptr)
{
    ...
    limitNum.un.glbCnt = *max_count_ptr;
#if defined (CONFIG_SDK_DRIVER_RTK_LEGACY_API)
    if (IS_BACKWARD_COMPATIBLE)
    {
...
cparser_result_t cparser_cmd_l2_table_set_limit_learning_global_action_copy_to_cpu_drop_forward_trap_to_cpu(cparser_context_t *context)
{
    ...
    if ('f' == TOKEN_CHAR(5, 0))
    {
        limitAct.un.glbAct = ACTION_FORWARD;
    }
    else if ('d' == TOKEN_CHAR(5, 0))
    {
        limitAct.un.glbAct = ACTION_DROP;
    }
    else if ('t' == TOKEN_CHAR(5, 0))
    {
        limitAct.un.glbAct = ACTION_TRAP2CPU;
    }
    else if ('c' == TOKEN_CHAR(5, 0))
    {
        limitAct.un.glbAct = ACTION_COPY2CPU;
    }
    else
    {
        diag_util_printf("User config: Error!\n");
    ...
    {
        rtk_l2_limitLearnCntAction_t    act;
        act = (rtk_l2_limitLearnCntAction_t) limitAct.un.glbAct;
...
cparser_result_t cparser_cmd_l2_table_set_limit_learning_global_action_copy_to_master_trap_to_master(cparser_context_t *context)
{
    ...
    if ('c' == TOKEN_CHAR(5, 0))
    {
        limitAct.un.glbAct = ACTION_COPY2MASTERCPU;
    }
    else if ('t' == TOKEN_CHAR(5, 0))
    {
        limitAct.un.glbAct = ACTION_TRAP2MASTERCPU;
    }
    else
    {
        diag_util_printf("User config: Error!\n");
    ...
    char **port_trunk_ptr)
{
    ...
    if ('p' == TOKEN_CHAR(3, 0))
    {
        ...
        DIAG_UTIL_PORTMASK_SCAN(portlist, port)
        {
            ...
            limitCnt.un.portTrkCnt.id = port;
            limitNum.un.portTrkCnt.id = port;
            limitAct.un.portTrkAct.id = port;

#if defined (CONFIG_SDK_DRIVER_RTK_LEGACY_API)
            if (IS_BACKWARD_COMPATIBLE)
            {
                DIAG_UTIL_ERR_CHK(rtk_l2_portLearningCnt_get(unit, port, &limitCnt.un.portTrkCnt.cnt), ret);
            }
            else
#endif
            {
            ...
            if (IS_BACKWARD_COMPATIBLE)
            {
                DIAG_UTIL_ERR_CHK(rtk_l2_portLimitLearningCnt_get(unit, port, &limitNum.un.portTrkCnt.cnt), ret);
            }
            else
#endif
            {
            ...
            if (IS_BACKWARD_COMPATIBLE)
            {
                ...
                limitAct.un.portTrkAct.act = (rtk_action_t)act;
            }
            else
#endif
            {
            ...
            if (limitNum.un.portTrkCnt.cnt == L2_MAC_CST_DISABLE)
                diag_util_mprintf("        Max Mac Count           : Unlimited\n");
            else
                diag_util_mprintf("        Max Mac Count           : %u\n", limitNum.un.portTrkCnt.cnt);

            diag_util_mprintf("        Current Mac Count       : %u\n", limitCnt.un.portTrkCnt.cnt);

            diag_util_mprintf("        Exceed Max Count Action : ");

            if (limitAct.un.portTrkAct.act == ACTION_FORWARD)
            {
                diag_util_mprintf("Forward\n");
            }
            else if (limitAct.un.portTrkAct.act == ACTION_DROP)
            {
                diag_util_mprintf("Drop\n");
            }
            else if (limitAct.un.portTrkAct.act == ACTION_TRAP2CPU)
            {
                diag_util_mprintf("Trap-To-Cpu\n");
            }
            else if (limitAct.un.portTrkAct.act == ACTION_COPY2CPU)
            {
                diag_util_mprintf("Copy-To-Cpu\n");
            }
            else if (limitAct.un.portTrkAct.act == ACTION_TRAP2MASTERCPU)
            {
               diag_util_mprintf("Trap-To Master Cpu\n");
            }
            else if (limitAct.un.portTrkAct.act == ACTION_COPY2MASTERCPU)
            {
               diag_util_mprintf("Copy-To Master Cpu\n");
            }
            else
    ...
    else if ('t' == TOKEN_CHAR(3, 0))
    {
        ...
        DIAG_UTIL_TRKMASK_SCAN(trklist, trunk)
        {
            ...
            limitCnt.un.portTrkCnt.id = trunk;
            limitNum.un.portTrkCnt.id = trunk;
            limitAct.un.portTrkAct.id = trunk;
    ...
    uint32_t *max_count_ptr)
{
    ...
    limitNum.un.portTrkCnt.cnt = *max_count_ptr;
    ...
    if ('p' == TOKEN_CHAR(3, 0))
    {
        ...
        DIAG_UTIL_PORTMASK_SCAN(portlist, port)
        {
            ...
            if (IS_BACKWARD_COMPATIBLE)
            {
                DIAG_UTIL_ERR_CHK(rtk_l2_portLimitLearningCnt_set(unit, port, limitNum.un.portTrkCnt.cnt), ret);
            }
            else
#endif
            {
    ...
    char **port_trunk_ptr)
{
    ...
    if ('f' == TOKEN_CHAR(6, 0))
    {
        limitAct.un.portTrkAct.act = ACTION_FORWARD;
    }
    else if ('d' == TOKEN_CHAR(6, 0))
    {
        limitAct.un.portTrkAct.act = ACTION_DROP;
    }
    else if ('t' == TOKEN_CHAR(6, 0))
    {
        limitAct.un.portTrkAct.act = ACTION_TRAP2CPU;
    }
    else if ('c' == TOKEN_CHAR(6, 0))
    {
        limitAct.un.portTrkAct.act = ACTION_COPY2CPU;
    }
    else
    {
        diag_util_printf("User config: Error!\n");
    ...
    if ('p' == TOKEN_CHAR(3, 0))
    {
        ...
        DIAG_UTIL_PORTMASK_SCAN(portlist, port)
        {
            ...
            if (IS_BACKWARD_COMPATIBLE)
            {
                ...
                act = (rtk_l2_limitLearnCntAction_t)limitAct.un.portTrkAct.act;
    ...
    char **port_trunk_ptr)
{
    ...
    if ('c' == TOKEN_CHAR(6, 0))
    {
        limitAct.un.portTrkAct.act = ACTION_COPY2MASTERCPU;
    }
    else if ('t' == TOKEN_CHAR(6, 0))
    {
        limitAct.un.portTrkAct.act = ACTION_TRAP2MASTERCPU;
    }
    else
    {
        diag_util_printf("User config: Error!\n");
    ...
    uint32_t *index_ptr)
{
    ...
    limitCnt.un.fidCnt.entryId = *index_ptr;
#if defined (CONFIG_SDK_DRIVER_RTK_LEGACY_API)
    if (IS_BACKWARD_COMPATIBLE)
    {
        uint32 val = 0;
    ...
    if (IS_BACKWARD_COMPATIBLE)
    {
        ...
        limitAct.un.fidAct.act = (rtk_action_t)act;
    }
    else
#endif
    {
    ...
    if (limitAct.un.fidAct.act == ACTION_FORWARD)
    {
        diag_util_mprintf("Forward\n");
    }
    else if (limitAct.un.fidAct.act ==ACTION_DROP)
    {
        diag_util_mprintf("Drop\n");
    }
    else if (limitAct.un.fidAct.act == ACTION_TRAP2CPU)
    {
        diag_util_mprintf("Trap-To-Cpu\n");
    }
    else if (limitAct.un.fidAct.act == ACTION_COPY2CPU)
    {
        diag_util_mprintf("Copy-To-Cpu\n");
    }
    else if (limitAct.un.fidAct.act == ACTION_TRAP2MASTERCPU)
    {
       diag_util_mprintf("Trap-To Master Cpu\n");
    }
    else if (limitAct.un.fidAct.act == ACTION_COPY2MASTERCPU)
    {
       diag_util_mprintf("Copy-To Master Cpu\n");
    }
    else
...
cparser_result_t cparser_cmd_l2_table_set_limit_learning_vlan_based_action_copy_to_cpu_drop_forward_trap_to_cpu(cparser_context_t *context)
{
    ...
    if ('f' == TOKEN_CHAR(5, 0))
    {
        limitAct.un.fidAct.act = ACTION_FORWARD;
    }
    else if ('d' == TOKEN_CHAR(5, 0))
    {
        limitAct.un.fidAct.act = ACTION_DROP;
    }
    else if ('t' == TOKEN_CHAR(5, 0))
    {
        limitAct.un.fidAct.act = ACTION_TRAP2CPU;
    }
    else if ('c' == TOKEN_CHAR(5, 0))
    {
        limitAct.un.fidAct.act = ACTION_COPY2CPU;
    }
    else
    {
        diag_util_printf("User config: Error!\n");
    ...
    {
        rtk_l2_limitLearnCntAction_t   act;
        act = (rtk_l2_limitLearnCntAction_t)limitAct.un.fidAct.act;
    ...
    cparser_context_t *context)
{
    ...
    if ('c' == TOKEN_CHAR(5, 0))
    {
        limitAct.un.fidAct.act = ACTION_COPY2MASTERCPU;
    }
    else if ('t' == TOKEN_CHAR(5, 0))
    {
        limitAct.un.fidAct.act = ACTION_TRAP2MASTERCPU;
    }
    else
    {
        diag_util_printf("User config: Error!\n");
    ...
    char **port_ptr)
{
    ...
    DIAG_UTIL_PORTMASK_SCAN(portlist, port)
    {
        ...
        act.un.dynAct.port = port;
#if defined (CONFIG_SDK_DRIVER_RTK_LEGACY_API)
    if (IS_BACKWARD_COMPATIBLE)
    {
        DIAG_UTIL_ERR_CHK(rtk_l2_legalPortMoveAction_get(unit, port, &act.un.dynAct.act), ret);
    }
    else
#endif
    {
        ...
        if (act.un.dynAct.act == ACTION_FORWARD)
        {
            diag_util_mprintf("Forward\n");
        }
        else if (act.un.dynAct.act  == ACTION_DROP)
        {
            diag_util_mprintf("Drop\n");
        }
        else if (act.un.dynAct.act  == ACTION_TRAP2CPU)
        {
            diag_util_mprintf("Trap-To-Cpu\n");
        }
        else if (act.un.dynAct.act  == ACTION_COPY2CPU)
        {
            diag_util_mprintf("Copy-To-Cpu\n");
        }
        else if (act.un.dynAct.act == ACTION_TRAP2MASTERCPU)
        {
           diag_util_mprintf("Trap-To Master Cpu\n");
        }
        else if (act.un.dynAct.act == ACTION_COPY2MASTERCPU)
        {
           diag_util_mprintf("Copy-To Master Cpu\n");
        }
        else
    ...
    char **port_ptr)
{
    ...
    if ('f' == TOKEN_CHAR(7, 0))
    {
        act.un.dynAct.act  = ACTION_FORWARD;
    }
    else if('d' == TOKEN_CHAR(7, 0))
    {
        act.un.dynAct.act  = ACTION_DROP;
    }
    else if ('t' == TOKEN_CHAR(7, 0))
    {
        act.un.dynAct.act  = ACTION_TRAP2CPU;
    }
    else if ('c' == TOKEN_CHAR(7, 0))
    {
        act.un.dynAct.act  = ACTION_COPY2CPU;
    }
    else
    {
        diag_util_printf("User config: Error!\n");
    ...
    if (IS_BACKWARD_COMPATIBLE)
    {
        DIAG_UTIL_ERR_CHK(rtk_l2_legalPortMoveAction_set(unit, port, act.un.dynAct.act), ret);
    }
    else
#endif
    {
    ...
    char **port_ptr)
{
    ...
    if ('c' == TOKEN_CHAR(7, 0))
    {
        act.un.dynAct.act  = ACTION_COPY2MASTERCPU;
    }
    else if ('t' == TOKEN_CHAR(7, 0))
    {
        act.un.dynAct.act  = ACTION_TRAP2MASTERCPU;
    }
    else
    {
        diag_util_printf("User config: Error!\n");
    ...
    char **port_ptr)
{
    ...
    if ('e' == TOKEN_CHAR(8, 0))
    {
        learn.un.dynLrn.enable = ENABLED;
    }
    else if('d' == TOKEN_CHAR(8, 0))
    {
        learn.un.dynLrn.enable = DISABLED;
    }
    else
    {
        diag_util_printf("User config: Error!\n");
...
cparser_result_t cparser_cmd_l2_table_get_port_move_sttc_port_move(cparser_context_t *context)
{
    ...
    diag_util_mprintf("\tAction:              ");
    if (act.un.sttAct.act == ACTION_FORWARD)
    {
        diag_util_mprintf("Forward\n");
    }
    else if (act.un.sttAct.act  == ACTION_DROP)
    {
        diag_util_mprintf("Drop\n");
    }
    else if (act.un.sttAct.act  == ACTION_TRAP2CPU)
    {
        diag_util_mprintf("Trap-To-Cpu\n");
    }
    else if (act.un.sttAct.act  == ACTION_COPY2CPU)
    {
        diag_util_mprintf("Copy-To-Cpu\n");
    }
    else if (act.un.sttAct.act  == ACTION_TRAP2MASTERCPU)
    {
        diag_util_mprintf("Trap-To-Master Cpu\n");
    }
    else if (act.un.sttAct.act  == ACTION_COPY2MASTERCPU)
    {
        diag_util_mprintf("Copy-To-Master Cpu\n");
    }
    else
    ...
    if(DIAG_OM_GET_FAMILYID(RTL9300_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID))
    {
        ...
        diag_util_mprintf("\tLearn:              ");
        if (learn.un.sttLrn.enable == ENABLED)
        {
            diag_util_mprintf("enable-learn\n");
        }
        else if (learn.un.sttLrn.enable == DISABLED)
        {
            diag_util_mprintf("disable-learn\n");
        }
        else
...
cparser_result_t cparser_cmd_l2_table_set_port_move_sttc_port_move_action_copy_to_cpu_drop_forward_trap_to_cpu(cparser_context_t *context)
{
    ...
    if ('f' == TOKEN_CHAR(5, 0))
    {
        act.un.sttAct.act  = ACTION_FORWARD;
    }
    else if('d' == TOKEN_CHAR(5, 0))
    {
        act.un.sttAct.act  = ACTION_DROP;
    }
    else if ('t' == TOKEN_CHAR(5, 0))
    {
        act.un.sttAct.act  = ACTION_TRAP2CPU;
    }
    else if ('c' == TOKEN_CHAR(5, 0))
    {
        act.un.sttAct.act  = ACTION_COPY2CPU;
    }
    else
    {
        diag_util_printf("User config: Error!\n");
...
cparser_result_t cparser_cmd_l2_table_set_port_move_sttc_port_move_action_copy_to_master_trap_to_master(cparser_context_t *context)
{
    ...
    if ('c' == TOKEN_CHAR(5, 0))
    {
        act.un.sttAct.act  = ACTION_COPY2MASTERCPU;
    }
    else if ('t' == TOKEN_CHAR(5, 0))
    {
        act.un.sttAct.act  = ACTION_TRAP2MASTERCPU;
    }
    else
    {
        diag_util_printf("User config: Error!\n");
...
cparser_result_t cparser_cmd_l2_table_set_port_move_sttc_port_move_learn_state_disable_enable(cparser_context_t *context)
{
    ...
    if ('e' == TOKEN_CHAR(6, 0))
    {
        learn.un.sttLrn.enable = ENABLED;
    }
    else if('d' == TOKEN_CHAR(6, 0))
    {
        learn.un.sttLrn.enable = DISABLED;
    }
    else
    {
        diag_util_printf("User config: Error!\n");
    ...
    char **port_trunk_ptr)
{
    ...
    if (IS_BACKWARD_COMPATIBLE)
    {
        DIAG_UTIL_ERR_CHK(rtk_l2_dynamicPortMoveForbidAction_get(unit, &act.un.forbidAct.act), ret);
    }
    else
#endif
    {
    ...
    diag_util_mprintf("\tAction : ");
    if (act.un.forbidAct.act == ACTION_FORWARD)
    {
        diag_util_mprintf("Forward\n");
    }
    else if (act.un.forbidAct.act  == ACTION_DROP)
    {
        diag_util_mprintf("Drop\n");
    }
    else if (act.un.forbidAct.act  == ACTION_TRAP2CPU)
    {
        diag_util_mprintf("Trap-To-Cpu\n");
    }
    else if (act.un.forbidAct.act  == ACTION_COPY2CPU)
    {
        diag_util_mprintf("Copy-To-Cpu\n");
    }
    else if (act.un.forbidAct.act  == ACTION_TRAP2MASTERCPU)
    {
        diag_util_mprintf("Trap-To-Master Cpu\n");
    }
    else if (act.un.forbidAct.act  == ACTION_COPY2MASTERCPU)
    {
        diag_util_mprintf("Copy-To-Master Cpu\n");
    }
    else
    ...
    cparser_context_t *context)
{
    ...
    if ('f' == TOKEN_CHAR(5, 0))
    {
        act.un.forbidAct.act  = ACTION_FORWARD;
    }
    else if('d' == TOKEN_CHAR(5, 0))
    {
        act.un.forbidAct.act  = ACTION_DROP;
    }
    else if ('t' == TOKEN_CHAR(5, 0))
    {
        act.un.forbidAct.act  = ACTION_TRAP2CPU;
    }
    else if ('c' == TOKEN_CHAR(5, 0))
    {
        act.un.forbidAct.act  = ACTION_COPY2CPU;
    }
    else
    {
        diag_util_printf("User config: Error!\n");
    ...
    if (IS_BACKWARD_COMPATIBLE)
    {
        DIAG_UTIL_ERR_CHK(rtk_l2_dynamicPortMoveForbidAction_set(unit, act.un.forbidAct.act), ret);
    }
    else
#endif
    {
    ...
    cparser_context_t *context)
{
    ...
    if ('c' == TOKEN_CHAR(5, 0))
    {
        act.un.forbidAct.act  = ACTION_COPY2MASTERCPU;
    }
    else if ('t' == TOKEN_CHAR(5, 0))
    {
        act.un.forbidAct.act  = ACTION_TRAP2MASTERCPU;
    }
    else
    {
        diag_util_printf("User config: Error!\n");
...
#define MAX_HASH_BUCKET_NUM 8
    ...
    input_l2_entry.un.unicast.vid = *vid_ptr;
    osal_memcpy(input_l2_entry.un.unicast.mac.octet, mac_ptr->octet, ETHER_ADDR_LEN);
    ...
    for (i = 0; i < ret_cnt; i++)
    {
        if (output_l2_entry[i].entry_type == FLOW_TYPE_UNICAST)
        {
            ...
            {
                diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %4d | %8d | %8d | %7d | %8d | %7d |   %4d  |  %2d | %10d | %8d \n",
                                output_l2_entry[i].un.unicast.l2_idx,
                                output_l2_entry[i].un.unicast.vid,
                                output_l2_entry[i].un.unicast.mac.octet[0],output_l2_entry[i].un.unicast.mac.octet[1],output_l2_entry[i].un.unicast.mac.octet[2],output_l2_entry[i].un.unicast.mac.octet[3],output_l2_entry[i].un.unicast.mac.octet[4],output_l2_entry[i].un.unicast.mac.octet[5],
                         output_l2_entry[i].un.unicast.port, (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_SA_BLOCK)?1:0, (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_DA_BLOCK)?1:0,
                         (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_STATIC)?1:0, (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_NEXTHOP)?1:0,
                         (output_l2_entry[i].un.unicast.state&RTK_L2_UCAST_STATE_SUSPEND)?1:0, output_l2_entry[i].un.unicast.agg_vid, output_l2_entry[i].un.unicast.age,
                         ((output_l2_entry[i].un.unicast.flags & RTK_L2_UCAST_FLAG_NEXTHOP) ? output_l2_entry[i].un.unicast.vlan_target : 0),
                         ((output_l2_entry[i].un.unicast.flags & RTK_L2_UCAST_FLAG_NEXTHOP) ? output_l2_entry[i].un.unicast.route_idx : 0));

            }
#endif
#if defined(CONFIG_SDK_RTL9300)
            if(output_l2_entry[i].un.unicast.flags & RTK_L2_UCAST_FLAG_NEXTHOP)
            {
                diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %3d | %4d | %3d |%4d|%4d|%4d| %2d |%3d| %s | %7d\n",
                            output_l2_entry[i].un.unicast.l2_idx,
                            output_l2_entry[i].un.unicast.vid,
                            output_l2_entry[i].un.unicast.mac.octet[0],output_l2_entry[i].un.unicast.mac.octet[1],output_l2_entry[i].un.unicast.mac.octet[2],output_l2_entry[i].un.unicast.mac.octet[3],output_l2_entry[i].un.unicast.mac.octet[4],output_l2_entry[i].un.unicast.mac.octet[5],
                            (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_TRUNK_PORT)?1:0,
                            (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_TRUNK_PORT) ? " - " : buffer,
                            (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_TRUNK_PORT)?output_l2_entry[i].un.unicast.trk_gid:output_l2_entry[i].un.unicast.port,
                            (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_SA_BLOCK)?1:0,
                            (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_DA_BLOCK)?1:0,
                            (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_STATIC)?1:0,
                            (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_NEXTHOP)?1:0,
                            (output_l2_entry[i].un.unicast.state&RTK_L2_UCAST_STATE_SUSPEND)?1:0,
                            "Not Support",
                            output_l2_entry[i].un.unicast.route_idx);
            }
            else
            {
                diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %3d | %4d | %3d |%4d|%4d|%4d| %2d |%3d| %7d | %s\n",
                            ...
                            output_l2_entry[i].un.unicast.mac.octet[0],output_l2_entry[i].un.unicast.mac.octet[1],output_l2_entry[i].un.unicast.mac.octet[2],output_l2_entry[i].un.unicast.mac.octet[3],output_l2_entry[i].un.unicast.mac.octet[4],output_l2_entry[i].un.unicast.mac.octet[5],
                             ...
                             (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_NEXTHOP)?1:0,
                             (output_l2_entry[i].un.unicast.state&RTK_L2_UCAST_STATE_SUSPEND)?1:0,
                             output_l2_entry[i].un.unicast.agg_vid,
                             "Not Support");
            }
#endif
#if defined(CONFIG_SDK_RTL9310)
            if (DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID))
            {
                sprintf(buffer, "%d", output_l2_entry[i].un.unicast.devID);
                diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %d |%5s|%3d|%4d|%4d|%4d|%2d|%3d|%3d|%4d|%4d| %d |0x%06X|  %2d\n",
                                output_l2_entry[i].un.unicast.l2_idx,
                                output_l2_entry[i].un.unicast.vid,
                                output_l2_entry[i].un.unicast.mac.octet[0],output_l2_entry[i].un.unicast.mac.octet[1],output_l2_entry[i].un.unicast.mac.octet[2],output_l2_entry[i].un.unicast.mac.octet[3],output_l2_entry[i].un.unicast.mac.octet[4],output_l2_entry[i].un.unicast.mac.octet[5],
                                ...
                                (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_NEXTHOP) ? 1 : 0,
                                (output_l2_entry[i].un.unicast.state&RTK_L2_UCAST_STATE_SUSPEND) ? 1 : 0,
                                (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_TAG_STS) ? 1 : 0,
                                output_l2_entry[i].un.unicast.agg_pri,
                                output_l2_entry[i].un.unicast.agg_vid,
                                (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_L2_TUNNEL) ? 1 : 0,
                                (output_l2_entry[i].un.unicast.flags&RTK_L2_UCAST_FLAG_L2_TUNNEL) ? output_l2_entry[i].un.unicast.l2_tunnel_idx : output_l2_entry[i].un.unicast.ecid,
                                output_l2_entry[i].un.unicast.age);
            }
#endif
        }
    }
    ...
    for (i = 0; i < ret_cnt; i++)
    {
        ...
        {
            osal_memset(port_list, 0, DIAG_UTIL_PORT_MASK_STRING_LEN);
            diag_util_lPortMask2str(port_list, &output_l2_entry[i].un.l2mcast.portmask);
#if defined(CONFIG_SDK_RTL8380)
            if (DIAG_OM_GET_FAMILYID(RTL8380_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL8330_FAMILY_ID))
            {
                diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %20s | %4d\n",
                            output_l2_entry[i].un.l2mcast.l2_idx,
                            output_l2_entry[i].un.l2mcast.rvid, output_l2_entry[i].un.l2mcast.mac.octet[0], output_l2_entry[i].un.l2mcast.mac.octet[1], output_l2_entry[i].un.l2mcast.mac.octet[2],
                            output_l2_entry[i].un.l2mcast.mac.octet[3], output_l2_entry[i].un.l2mcast.mac.octet[4], output_l2_entry[i].un.l2mcast.mac.octet[5], port_list, output_l2_entry[i].un.l2mcast.agg_vid);
            }
#endif
            ...
            if (DIAG_OM_GET_FAMILYID(RTL8390_FAMILY_ID) || DIAG_OM_GET_FAMILYID(RTL8350_FAMILY_ID))
            {
                diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %20s \n",
                            output_l2_entry[i].un.l2mcast.l2_idx,
                            output_l2_entry[i].un.l2mcast.rvid, output_l2_entry[i].un.l2mcast.mac.octet[0], output_l2_entry[i].un.l2mcast.mac.octet[1], output_l2_entry[i].un.l2mcast.mac.octet[2],
                            output_l2_entry[i].un.l2mcast.mac.octet[3], output_l2_entry[i].un.l2mcast.mac.octet[4], output_l2_entry[i].un.l2mcast.mac.octet[5], port_list);
            }
#endif
#if defined(CONFIG_SDK_RTL9300)
            if (DIAG_OM_GET_FAMILYID(RTL9300_FAMILY_ID))
            {
                diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %20s | %4d | %4d\n",
                            output_l2_entry[i].un.l2mcast.l2_idx,
                            output_l2_entry[i].un.l2mcast.rvid, output_l2_entry[i].un.l2mcast.mac.octet[0], output_l2_entry[i].un.l2mcast.mac.octet[1], output_l2_entry[i].un.l2mcast.mac.octet[2],
                            output_l2_entry[i].un.l2mcast.mac.octet[3], output_l2_entry[i].un.l2mcast.mac.octet[4], output_l2_entry[i].un.l2mcast.mac.octet[5], port_list, output_l2_entry[i].un.l2mcast.nextHop, output_l2_entry[i].un.l2mcast.mac_idx);
            }
#endif
#if defined(CONFIG_SDK_RTL9310)
            if (DIAG_OM_GET_FAMILYID(RTL9310_FAMILY_ID))
            {
                diag_util_mprintf("%6d |%4d | %02X:%02X:%02X:%02X:%02X:%02X | %20s | %2d | 0x%X\n",
                        output_l2_entry[i].un.l2mcast.l2_idx,
                        output_l2_entry[i].un.l2mcast.rvid, output_l2_entry[i].un.l2mcast.mac.octet[0], output_l2_entry[i].un.l2mcast.mac.octet[1], output_l2_entry[i].un.l2mcast.mac.octet[2],
                        output_l2_entry[i].un.l2mcast.mac.octet[3], output_l2_entry[i].un.l2mcast.mac.octet[4], output_l2_entry[i].un.l2mcast.mac.octet[5], port_list, output_l2_entry[i].un.l2mcast.nextHop, output_l2_entry[i].un.l2mcast.groupId);
            }
#endif
        }
    }
    ...
    if (ret_cnt && contain_ipmcst)
    {
        ...
        for (i = 0; i < ret_cnt; i++)
        {
            ...
            {
                osal_memset(port_list, 0, DIAG_UTIL_PORT_MASK_STRING_LEN);
                diag_util_lPortMask2str(port_list, &output_l2_entry[i].un.ipmcast.portmask);
                diag_util_ip2str_format(strBuf1, output_l2_entry[i].un.ipmcast.sip, 15);
                diag_util_ip2str_format(strBuf2, output_l2_entry[i].un.ipmcast.dip, 15);
                diag_util_mprintf("%6d | %s | %s | %4d | %20s\n", output_l2_entry[i].un.ipmcast.l2_idx, strBuf1, strBuf2, output_l2_entry[i].un.ipmcast.rvid, port_list);
            }
        }
    }
    ...
    if (ret_cnt && contain_ip6mcst)
    {
        ...
        for (i = 0; i < ret_cnt; i++)
        {
            ...
            {
                osal_memset(port_list, 0, DIAG_UTIL_PORT_MASK_STRING_LEN);
                diag_util_lPortMask2str(port_list, &output_l2_entry[i].un.ip6mcast.portmask);
                DIAG_UTIL_ERR_CHK(diag_util_ipv62str(strBuf1, output_l2_entry[i].un.ip6mcast.sip.octet), ret);
                DIAG_UTIL_ERR_CHK(diag_util_ipv62str(strBuf2, output_l2_entry[i].un.ip6mcast.dip.octet), ret);
                diag_util_mprintf("%6d | %20s | %20s | %4d | %20s \n", output_l2_entry[i].un.ip6mcast.l2_idx, strBuf1, strBuf2, output_l2_entry[i].un.ip6mcast.rvid, port_list);
            }
        }
    }
#endif
...
#ifdef CMD_L2_TABLE_DEL_MAC_BASE_ENTRY_MAC_MAC
/*
 * l2-table del mac-base-entry mac <MACADDR:mac>
 */
    cparser_result_t cparser_cmd_l2_table_del_mac_base_entry_mac_mac(cparser_context_t *context,
    cparser_macaddr_t *mac_ptr)
{
    uint32              unit = 0;
    int32               ret = RT_ERR_FAILED;
    ...
    DIAG_UTIL_ERR_CHK(rtk_l2_addr_delByMac(unit, (rtk_mac_t *)mac_ptr), ret);
