...
static int32 _dal_longan_l2_addr_delByMac(uint32 unit, rtk_mac_t *pMac, uint32 sram_if);
...
static int32
_dal_longan_l2_addr_delByMac(uint32 unit, rtk_mac_t *pMac, uint32 sram_if)
{
    int32 ret;
 ...
 */
int32
dal_longan_l2_addr_delByMac(uint32 unit, rtk_mac_t *pMac)
{
    int32 ret;
    uint32 value;
    uint32 sram_if = 1;
    ...
    if((ret = _dal_longan_l2_addr_delByMac(unit, pMac, sram_if)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
