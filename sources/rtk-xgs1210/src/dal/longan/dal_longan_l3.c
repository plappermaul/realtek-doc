 ...
 * Maybe delete interface before nexthop, nexthop need refer interface's vid, so if new added interface with another vid,
 * VID in L2 entry will be worong. so can not change interface VID after disable reference.
 */
#define LONGAN_L3_REFCNT_CHK_DEFAULT     (ENABLED)

/* This MACRO configure one intf is mapped to one vlan id.SS-662
*/
#undef L3_VID_INTERFACE_BINDING
...
static uint32 _actIpRouteCtrlIpHdrErr[] = {   /* LONGAN_L3_IP_ROUTE_CTRLr, LONGAN_IP_HDR_ERR_ACTf */
    RTK_L3_ACT_HARD_DROP,                    /*multicast mac-base bridge, unicast drop*/
    RTK_L3_ACT_TRAP2CPU,                /*multicast mac-base bridge + trap*/
    RTK_L3_ACT_TRAP2MASTERCPU,          /*multicast mac-base bridge + trap*/
    RTK_L3_ACT_DROP,                 /*multicast mac-base bridge, unicast drop*/
    };
...
static int32 _dal_longan_l3_routeEntryIdx_hwFast_get(uint32 unit, rtk_l3_route_t *pRoute, int32 *pIndex)
{
    ...
    *pIndex = -1;
    ...
    if (0 == routeEntry.entry_type || 2 == routeEntry.entry_type)  /* IPUC/IP6UC */
    {
...
dal_longan_l3_intf_destroy(uint32 unit, rtk_intf_id_t intfId)
{
    ...
    if (FALSE == _pL3Db[unit]->intf[intfIdx].valid)
    {
        ret = RT_ERR_ENTRY_NOTFOUND;
        goto errEntryNotFound;
    }
...
dal_longan_l3_intf_set(uint32 unit, rtk_l3_intfKeyType_t type, rtk_l3_intf_t *pIntf)
{
    ...
    RT_PARAM_CHK((pIntf->ipv6_mtu > HAL_MAX_NUM_OF_INTF_MTU_VALUE(unit)), RT_ERR_MTU_EXCEED);
    RT_PARAM_CHK((pIntf->ttl > RTK_IP_TTL_MAX), RT_ERR_TTL_EXCEED);
    RT_PARAM_CHK((RTK_DEFAULT_L3_INTF_VRF_ID != pIntf->vrf_id), RT_ERR_INPUT);
    ...
    if (intfChg)
    {
        ...
        {
            /* search a suitalb MAC entry */
            RT_ERR_HDL(_dal_longan_l3_smacEntry_free(unit, smacIdx), errSmacSet, ret);
            RT_ERR_HDL(_dal_longan_l3_smacEntry_alloc(unit, pIntf->mac_addr, &smacIdx), errSmacSet, ret);
 ...
 */
int32
dal_longan_l3_vrrp_add(uint32 unit, rtk_vlan_t vid, uint32 vrId)
{
    int32   ret = RT_ERR_OK;
 ...
 */
int32
dal_longan_l3_vrrp_del(uint32 unit, rtk_vlan_t vid, uint32 vrId)
{
    int32   ret = RT_ERR_OK;
    int32   macIdx;
    rtk_mac_t   vrrpMac;
 ...
 */
int32
dal_longan_l3_vrrp_delAll(uint32 unit, rtk_vlan_t vid)
{
    int32   ret = RT_ERR_OK;
    int32   macIdx;
    rtk_mac_t   vrrpMac;
 ...
 */
int32
dal_longan_l3_vrrp_get(uint32 unit, rtk_vlan_t vid, uint32 vrIdArraySize, uint32 *pVrIdArray, uint32 *pVrIdCount)
{
    int32   ret = RT_ERR_OK;
    int32   macIdx;
    rtk_mac_t   vrrpMac;
...
dal_longan_l3_nextHop_get(uint32 unit, rtk_l3_pathId_t pathId,  rtk_l3_nextHop_t *pNextHop)
{
    ...
    RT_ERR_HDL(_dal_longan_l3_nhDmac_get(unit, (l2Entry.entry_type == L2_MULTICAST)? l2Entry.un.l2mcast.mac_idx : l2Entry.un.unicast.mac_idx, &nh_mac_addr), errL2EntryGet, ret);
