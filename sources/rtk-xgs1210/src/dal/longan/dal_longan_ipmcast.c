...
static inline int32 _dal_longan_ipmc_mtu_get(uint32 unit, uint32 hw_index, dal_longan_ipmc_mtu_info_t *pMtu)
{
    int32 ret = RT_ERR_OK;
...
static inline int32 _dal_longan_ipmc_Ip6mtu_get(uint32 unit, uint32 hw_index, dal_longan_ipmc_mtu_info_t *pMtu)
{
    int32 ret = RT_ERR_OK;
    dal_longan_l3_hostEntry_t   hostEntry;
...
_dal_longan_ipmc_l3EntryStatMont_remove(uint32 unit, dal_longan_ipmc_l3Entry_t *pEntry)
{
    ...
    l3EntryIdx = DAL_LONGAN_L3_ENTRY_IDX_TO_ADDR(pEntry->hw_entry_idx);
...
{
    int32 ret = RT_ERR_OK;
    uint32 tbl, tfIdx, tfL3Oilvalid, tfStk, tfL3Act, valIdx, valL3Oil, valL3Act;
    dal_longan_ipmc_l3Entry_t *pEntry;
    dal_longan_l3_hostEntry_t hostEntry;
    ...
    if (L3_ENTRY_IS_VALID(pEntry))
    {
        ...
        IPMCAST_TABLE_READ_ERR_HDL(unit, tbl, pEntry->hw_entry_idx, hostEntry, "", errHandle, ret);
        IPMCAST_TABLE_FIELD_GET_ERR_HDL(unit, tbl, tfStk, stkPmsk, hostEntry, "", errHandle, ret);
        IPMCAST_TABLE_FIELD_GET_ERR_HDL(unit, tbl, tfStk, stkPmsk, hostEntry, "", errHandle, ret);
        IPMCAST_TABLE_FIELD_GET_ERR_HDL(unit, tbl, tfL3Act, valL3Act, hostEntry, "", errHandle, ret);
...
//    uint32 entryIdx;
    ...
    if (NULL != pEntry)
    {
        ...
        if (pIpmcAddr->ipmc_flags & RTK_IPMC_FLAG_REPLACE)
        {
            if(pIpmcAddr->ipmc_flags & RTK_IPMC_FLAG_IPV6)
            {
                RT_ERR_HDL(_dal_longan_ipmc_Ip6mtu_get(unit, pEntry->hw_entry_idx, &mtu), errHandle, ret);
            }
            else
            {
                RT_ERR_HDL(_dal_longan_ipmc_mtu_get(unit, pEntry->hw_entry_idx, &mtu), errHandle, ret);
            }
