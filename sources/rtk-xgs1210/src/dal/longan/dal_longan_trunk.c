 ...
 *
 * $Revision: 96829 $
 * $Date: 2019-05-13 16:51:52 +0800 (Mon, 13 May 2019) $
 *
 * Purpose :
 *
 * Feature :
...
dal_longan_trunk_port_set(uint32 unit, uint32 trk_gid, rtk_portmask_t *pTrunk_member_portmask)
{
    ...
    HWP_PORT_TRAVS_EXCEPT_CPU(unit, port)
    {
        ...
        {
            devPort.port = port;
            RT_ERR_CHK(dal_longan_trunk_srcPortMap_set(unit, devPort, 0, 0), ret);
        }
        ...
        if (RTK_PORTMASK_IS_PORT_SET(*pTrunk_member_portmask, port))
        {
            ...
            RT_ERR_CHK(dal_longan_trunk_srcPortMap_set(unit, trk_egr_ports.egr_port[trk_egr_ports.num_ports],1, trk_gid), ret);
    ...
    {
        trk_egr_ports.egr_port[portIdx].devID = unit;
        trk_egr_ports.egr_port[portIdx].port = INVALID_TRUNK_MEMBER_PORT;
    }

    RT_ERR_CHK(dal_longan_trunk_egrPort_set (unit, trk_gid, &trk_egr_ports), ret);
...
dal_longan_trunk_srcPortMap_get(uint32 unit,  rtk_dev_port_t devPort, uint32 *pIsTrkMbr, rtk_trk_t *pTrk_gid)
{
    ...
    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_TRUNK), "unit=%d,devPort=%d", unit, devPort);
    ...
    /*set source port to trunk ID mapping table*/
    tbl_idx = devPort.devID*64+devPort.port;
