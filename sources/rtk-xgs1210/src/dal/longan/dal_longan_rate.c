 ...
 *
 * $Revision: 98640 $
 * $Date: 2019-07-12 11:18:26 +0800 (Fri, 12 Jul 2019) $
 *
...
/*LONGAN SS-528*/
static uint32   **rate_db[RTK_MAX_NUM_OF_UNIT][2];    /*unit->rate mode->packet type->rate
                                                        0:pps(correction), 1:bps(original)*/
...
/*LONGAN SS-528*/
#define RATE_PKT_TYPE_COUNT     3   /*0:unicast, 1:multicast, 2:broadcast*/
#define RATE_CNT_MODE_COUNT     2   /*0:pps(correction), 1:bps(original)*/
#define RATE_TO_DIV     1014
#define ROUND_CRCT      507
#define RATE_TO_MUL     1000
#define RATE_DEFAULT    0xFFFFF
#define RATE_COR_IDX    0
#define RATE_ORI_IDX    1
#define RATE_CORRECTION(ro) ((ro / RATE_TO_DIV * RATE_TO_MUL) + ((ro % RATE_TO_DIV) * RATE_TO_MUL + ROUND_CRCT) / RATE_TO_DIV)
...
dal_longan_rate_init(uint32 unit)
{
    ...
    if (HWP_UNIT_VALID_LOCAL(unit))
    {
        ...
        /*LONGAN SS-528*/
        if (NULL == rate_db[unit][0])
        {
            for (rate_idx = 0; rate_idx < RATE_CNT_MODE_COUNT; rate_idx++)
            {
                rate_db[unit][rate_idx] = (uint32 **)osal_alloc(RATE_PKT_TYPE_COUNT * sizeof(uint32*));
                if (NULL == rate_db[unit][rate_idx])
                {
                ...
                {
                    osal_memset(rate_db[unit][rate_idx], 0, sizeof(RATE_PKT_TYPE_COUNT * sizeof(uint32*)));
                }
                for (mode_idx = 0; mode_idx < RATE_PKT_TYPE_COUNT; mode_idx++)
                {
                    rate_db[unit][rate_idx][mode_idx] = (uint32*)osal_alloc((UNITMAP(unit)->hwp_maxMacId + 1) * sizeof(uint32));
                    if (NULL == rate_db[unit][rate_idx][mode_idx])
                    {
                        RT_ERR(RT_ERR_FAILED, (MOD_RATE|MOD_DAL), "memory allocate failed");
                        goto err_free;
                    }
                    else
                    {
                        for (port = 0; port < (UNITMAP(unit)->hwp_maxMacId + 1); port++)
                        {
                            if (0 == rate_idx)
                            {
                                rate_db[unit][rate_idx][mode_idx][port] = RATE_CORRECTION(RATE_DEFAULT);
                            }
                            else
                            {
                                rate_db[unit][rate_idx][mode_idx][port] = RATE_DEFAULT;
                            }
                        }
                    }
                }
        ...
        /*set chip default correction value*/
        for (port = 0; port < (UNITMAP(unit)->hwp_maxMacId + 1); port++)
        {
            /* get value from CHIP */
            ...
            if (0 == value)
                rate_hw = RATE_CORRECTION(RATE_DEFAULT);
            else
                rate_hw = RATE_DEFAULT;

            if ((ret = reg_array_field_write(unit, LONGAN_STORM_PORT_UC_CTRLr, port, REG_ARRAY_INDEX_NONE, LONGAN_RATEf, &rate_hw)) != RT_ERR_OK)
            {
                RATE_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
                return ret;
            }
            if ((ret = reg_array_field_write(unit, LONGAN_STORM_PORT_MC_CTRLr, port, REG_ARRAY_INDEX_NONE, LONGAN_RATEf, &rate_hw)) != RT_ERR_OK)
            {
                RATE_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
                return ret;
            ...
            if ((ret = reg_array_field_write(unit, LONGAN_STORM_PORT_BC_CTRLr, port, REG_ARRAY_INDEX_NONE, LONGAN_RATEf, &rate_hw)) != RT_ERR_OK)
            {
                RATE_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
                return ret;
...
err_free:
    for (rate_idx = 0; rate_idx < RATE_CNT_MODE_COUNT; rate_idx++)
    {
        for (mode_idx = 0; mode_idx < RATE_PKT_TYPE_COUNT; mode_idx++)
        {
            if (NULL != rate_db[unit][rate_idx][mode_idx])
            {
                osal_free(rate_db[unit][rate_idx][mode_idx]);
            }
        }

        if (NULL != rate_db[unit][rate_idx])
            osal_free(rate_db[unit][rate_idx]);
    }
    ...
    uint32                  *pRate)
{
    ...
    *pRate = rate_db[unit][RATE_ORI_IDX][db_idx][port];
    ...
    uint32                  rate)
{
    ...
    RATE_SEM_LOCK(unit);
    rate_db[unit][RATE_COR_IDX][db_idx][port] = RATE_CORRECTION(rate);
    rate_db[unit][RATE_ORI_IDX][db_idx][port] = rate;
    if (BASED_ON_PKT == rate_mode)
    {
        rate_final = rate_db[unit][RATE_COR_IDX][db_idx][port];
    }
    else
    {
        rate_final = rate_db[unit][RATE_ORI_IDX][db_idx][port];
    }
    ...
    uint32                  burst_size)
{
    ...
    {
        RT_ERR_CHK_EHDL(reg_field_read(unit, LONGAN_STORM_LB_PPS_CTRLr,LONGAN_TKNf, &storm_token), ret,
                            RATE_SEM_UNLOCK(unit);RT_ERR(ret, (MOD_DAL|MOD_RATE), ""););
    }
    RATE_SEM_UNLOCK(unit);
    ...
    rtk_rate_storm_rateMode_t   rate_mode)
{
    ...
    int32   ret = RT_ERR_FAILED;
    uint32  burst_size;
    uint32  storm_ctrl_reg[] = {LONGAN_STORM_PORT_UC_CTRLr,
                                LONGAN_STORM_PORT_MC_CTRLr,
                                LONGAN_STORM_PORT_BC_CTRLr};
    uint32  storm_lb_rst_reg[] = {  LONGAN_STORM_PORT_UC_LB_RSTr,
    ...
    value = 1;
    for (i = 0; i < sizeof(storm_ctrl_reg)/sizeof(storm_ctrl_reg[0]); i++)
    {
        RT_ERR_CHK_EHDL(reg_array_field_write(unit, storm_ctrl_reg[i], port, REG_ARRAY_INDEX_NONE, LONGAN_RATEf, &rate_db[unit][rate_idx][i][port]), ret,
                            RATE_SEM_UNLOCK(unit);RT_ERR(ret, (MOD_DAL|MOD_RATE), ""););

        RT_ERR_CHK_EHDL(reg_array_field_write(unit, storm_ctrl_reg[i], port, REG_ARRAY_INDEX_NONE, LONGAN_BURSTf, &burst_size), ret,
                            RATE_SEM_UNLOCK(unit);RT_ERR(ret, (MOD_DAL|MOD_RATE), ""););

        RT_ERR_CHK_EHDL(reg_array_field_write(unit, storm_lb_rst_reg[i], port, REG_ARRAY_INDEX_NONE, LONGAN_RSTf, &value), ret,
                            RATE_SEM_UNLOCK(unit);RT_ERR(ret, (MOD_DAL|MOD_RATE), ""););
    }
