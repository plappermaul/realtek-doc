...
_dal_longan_sds_rxCali_a_start_1d1(uint32 unit, uint32 sdsId, dal_longan_sds_rxCaliInfo_t *pinfo)
{
    ...
    if(RTK_PHYTYPE_SERDES == HWP_SDS_ID2PHYMODEL(unit, sdsId))
    {
        ...
        if(((media >= PORT_10GMEDIA_DAC_50CM) && (media <= PORT_10GMEDIA_DAC_300CM))&&((sdsId == 8)|| (sdsId ==9)))
        {
    ...
    SDS_FIELD_WRITE(unit, sdsId, 0x2e, 0x19, 9, 5, 0x1f);
    SDS_FIELD_WRITE(unit, sdsId, 0x2f, 0x0b, 15, 9, 0x3c);
    SDS_FIELD_WRITE(unit, sdsId, 0x2e, 0x0b, 1, 0, 0x3);
...
{
    uint32 pTap1List[4] = {0};
    uint32 pTap2List[4] = {0};
    uint32 pTap3List[4] = {0};
    uint32 pTap4List[4] = {0};
...
void
_dal_longan_sds_rxCali_a_start_dfe_disable(uint32 unit, uint32 sdsId, dal_longan_sds_rxCaliInfo_t *pinfo)
{    
    uint32 pTap1List[4] = {0};
    uint32 pTap2List[4] = {0};
    uint32 pTap3List[4] = {0};
...
{    
    ...
    if((HWP_SDS_MODE(unit, sdsId) == RTK_MII_10GR) && (HWP_SDS_ID2PHYMODEL(unit, sdsId) == RTK_PHYTYPE_SERDES))
    {
        SDS_FIELD_R(unit, sdsId, 0x1f, 9, 11, 7, &value);
        if(value == 0x1a)
        {
            SDS_FIELD_R(unit, sdsId, 0x4, 1, 2, 2, &sdsLink.latch_sts);
            ...
            {
                _dal_longan_sds_rxCali_a_start_dfe_disable(unit, sdsId, pinfo);
                _dal_longan_sds_rxCali_a_start_1d4(unit, sdsId, pinfo);
                _dal_longan_sds_rxCali_a_start_1d5(unit, sdsId, pinfo);
            }
