 ...
 *
 * $Revision: 85509 $
 * $Date: 2018-01-18 20:13:54 +0800 (Thu, 18 Jan 2018) $
 *
...
do {\
    if (dal_cypress_port_sem_lock(unit) != RT_ERR_OK)\
    {\
...
do {\
    if (dal_cypress_port_sem_unlock(unit) != RT_ERR_OK)\
    {\
...
{
    pMapper->eee_init = dal_cypress_eee_init;
    pMapper->eee_portEnable_get = dal_cypress_eee_portEnable_get;
    pMapper->eee_portEnable_set = dal_cypress_eee_portEnable_set;
    pMapper->eee_portState_get = dal_cypress_eee_portState_get;
    pMapper->eee_portPowerState_get = dal_cypress_eee_portPowerState_get;
    pMapper->eeep_portEnable_get = dal_cypress_eeep_portEnable_get;
    pMapper->eeep_portEnable_set = dal_cypress_eeep_portEnable_set;
...
dal_cypress_eee_init(uint32 unit)
{
    ...
    {
        if((ret = dal_cypress_eee_portEnable_set(unit, mac_id, DISABLED)) != RT_ERR_OK)
        {
            eee_init[unit] = INIT_NOT_COMPLETED;
            RT_ERR(ret, (MOD_DAL|MOD_EEE), "");
            return ret;
        }
        if((ret = dal_cypress_eeep_portEnable_set(unit, mac_id, DISABLED)) != RT_ERR_OK)
        {
            eee_init[unit] = INIT_NOT_COMPLETED;
            RT_ERR(ret, (MOD_DAL|MOD_EEE), "");
            return ret;
...
dal_cypress_eee_portState_get(uint32 unit, rtk_port_t port, rtk_enable_t *pState)
{
    ...
    if ((ret = dal_cypress_port_link_get(unit, port, &link)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_EEE), "");
        return ret;
    }
...
dal_cypress_eee_portPowerState_get(uint32 unit, rtk_port_t port, rtk_eee_direction_t direction, rtk_eee_power_state_t *pState)
{
    ...
    return RT_ERR_OK;
} /* end of dal_cypress_eee_portPowerState_get */
