 ...
 *
 * $Revision: 96868 $
 * $Date: 2019-05-14 18:45:08 +0800 (Tue, 14 May 2019) $
 *
...
dal_cypress_l2_init(uint32 unit)
{
    ...
    dal_cypress_l2_bucketHashAlgo_get(unit, 0, &algoType[unit]);
    ...
    dal_cypress_l2_addr_delAll(unit, TRUE);
#endif/* !defined(__BOOTLOADER__) */
...
dal_cypress_l2_ucastAddr_flush(uint32 unit, rtk_l2_flushCfg_t *pConfig)
{
    ...
    if (FLUSH_ACT_REPLACE == pConfig->act && l2CamEbl)
    {
        ...
        for (cam_index = 0; cam_index < cam_size[unit]; cam_index++)
        {
            ...
            if (isValid)
            {
                if ((entryType == FLUSH_INCLUDE_STTC) || (entryType == FLUSH_DYN_ONLY && pCamTbl[unit][cam_index].un.unicast.is_static == 0))
                {
                    port_hit = 1;
                    vid_hit  = 1;
                    if (pConfig->flushByPort)
                    {
                        if (pCamTbl[unit][cam_index].un.unicast.port == pConfig->port)
                            port_hit = 1;
                        else
                            port_hit = 0;
                    }
                    ...
                    {
                        if (pCamTbl[unit][cam_index].un.unicast.fid == pConfig->vid)
                            vid_hit = 1;
                        else
                            vid_hit = 0;
                    }
                    ...
                    if (port_hit && vid_hit)
                    {
                        pCamTbl[unit][cam_index].un.unicast.port   = pConfig->replacingPort;
                    }
                }
            }
            else
...
dal_cypress_l2_addr_add(uint32 unit, rtk_l2_ucastAddr_t *pL2_addr)
{
    ...
    else
    {
        if (pL2_addr->add_op_flags & RTK_L2_ADD_OP_FLAG_AGE_SET)
            l2_entry.un.unicast.aging = pL2_addr->age;
        else
...
dal_cypress_l2_mcastAddr_get(uint32 unit, rtk_l2_mcastAddr_t *pMcast_addr)
{
    ...
    if (HWP_CHIP_REV(unit) < CHIP_REV_ID_C)
    {
        ...
        RTK_PORTMASK_ASSIGN(pMcast_addr->portmask, mcast_fwdTable_shadow[l2_entry.un.l2mcast.index].portmask);
        L2_SEM_UNLOCK(unit);
    }
    else
    {
        if ((ret = table_read(unit, CYPRESS_MC_PMSKt, l2_entry.un.l2mcast.index, (uint32 *)&mcast_entry)) != RT_ERR_OK)
        {
            L2_SEM_UNLOCK(unit);
            RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "return failed ret value = %x",ret);
            return RT_ERR_FAILED;
...
dal_cypress_l2_mcastAddr_set(uint32 unit, rtk_l2_mcastAddr_t *pMcast_addr)
{
    ...
    if ((ret = table_write(unit, CYPRESS_MC_PMSKt, l2_entry.un.l2mcast.index, (uint32 *)&mcast_entry)) != RT_ERR_OK)
    {
        L2_SEM_UNLOCK(unit);
        RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "return failed ret value = %x",ret);
        return RT_ERR_FAILED;
    ...
    if (HWP_CHIP_REV(unit) < CHIP_REV_ID_C)
    {
        ...
        RTK_PORTMASK_ASSIGN(mcast_fwdTable_shadow[l2_entry.un.l2mcast.index].portmask, pMcast_addr->portmask);
    }
...
dal_cypress_l2_mcastAddr_del(uint32 unit, rtk_vlan_t vid, rtk_mac_t *pMac)
{
    ...
    _dal_cypress_l2_freeMcastIdx(unit, (int32)l2_entry.un.l2mcast.index);
    ...
    rtk_l2_mcastAddr_t  *pL2_data)
{
    ...
    {
        /* get multicast forward from shadow */
        RTK_PORTMASK_ASSIGN(pL2_data->portmask, mcast_fwdTable_shadow[l2_entry.un.l2mcast.index].portmask);
    }
    else
    {
        osal_memset(&mcast_entry, 0, sizeof(mcast_entry));
...
dal_cypress_l2_ipMcastAddr_addByIndex(uint32 unit, rtk_l2_ipMcastAddr_t *pIpMcast_addr)
{
    ...
    if (ret == RT_ERR_L2_NO_EMPTY_ENTRY && (pIpMcast_addr->add_op_flags & RTK_L2_ADD_OP_FLAG_REPLACE_DYNAMIC) != 0)
    {
        ...
        osal_memcpy(&pIpMcast_addr->macInfo.mac, &l2_entry.un.unicast.mac, ETHER_ADDR_LEN);
        pIpMcast_addr->macInfo.port = l2_entry.un.unicast.port;
        pIpMcast_addr->macInfo.vid  = l2_entry.un.unicast.fid;
        for (trk_gid = 0; trk_gid < HAL_MAX_NUM_OF_TRUNK(unit); trk_gid++)
        {
            if (dal_cypress_trunk_port_get(unit, trk_gid, &trunk_portmask) == RT_ERR_OK)
            {
...
dal_cypress_l2_ip6McastAddr_get(uint32 unit, rtk_l2_ip6McastAddr_t *pIpMcast_addr)
{
    ...
    /* search exist or free entry */
    osal_memset(&l2_entry, 0, sizeof(dal_cypress_l2_entry_t));
    _dal_cypress_l2_ip6CareByteConvertToIP(&l2_entry.un.ipmcast_ip_mc_sip.dip, &l2_entry.un.ipmcast_ip_mc_sip.sip,
...
dal_cypress_l2_ip6McastAddr_del(uint32 unit, rtk_ipv6_addr_t sip, rtk_ipv6_addr_t dip, rtk_vlan_t vid)
{
    ...
    /* search exist or free entry */
    osal_memset(&l2_entry, 0, sizeof(dal_cypress_l2_entry_t));
    _dal_cypress_l2_ip6CareByteConvertToIP(&l2_entry.un.ipmcast_ip_mc_sip.dip, &l2_entry.un.ipmcast_ip_mc_sip.sip, &dip, &sip);
...
dal_cypress_l2_nextValidIp6McastAddr_get(uint32 unit, int32 *pScan_idx, rtk_l2_ip6McastAddr_t *pL2_data)
{
    ...
    /*_dal_cypress_l2_ip6CareByteConvertToIP6Addr(&pL2_data->dip, &pL2_data->sip, &l2_entry.un.ipmcast_ip_mc_sip.dip, &l2_entry.un.ipmcast_ip_mc_sip.sip);*/
...
_dal_cypress_l2_setL2HASHEntry(uint32 unit, dal_cypress_l2_entry_t *pL2_entry, dal_cypress_l2_index_t *pL2_index)
{
    ...
    switch (pL2_entry->entry_type)
    {
        case L2_UNICAST: /* L2 unicast */
            ...
            if ((ret = table_field_set(unit, CYPRESS_L2_UCt, CYPRESS_L2_UC_DA_BLKtf, &pL2_entry->un.unicast.dablock, (uint32 *)&l2_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_set(unit, CYPRESS_L2_UCt, CYPRESS_L2_UC_SA_BLKtf, &pL2_entry->un.unicast.sablock, (uint32 *)&l2_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_set(unit, CYPRESS_L2_UCt, CYPRESS_L2_UC_SLPtf, &pL2_entry->un.unicast.port, (uint32 *)&l2_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_set(unit, CYPRESS_L2_UCt, CYPRESS_L2_UC_STATICtf, &pL2_entry->un.unicast.is_static, (uint32 *)&l2_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_set(unit, CYPRESS_L2_UCt, CYPRESS_L2_UC_AGEtf, &pL2_entry->un.unicast.aging, (uint32 *)&l2_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_set(unit, CYPRESS_L2_UCt, CYPRESS_L2_UC_SUSPENDtf, &pL2_entry->un.unicast.suspending, (uint32 *)&l2_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_set(unit, CYPRESS_L2_UCt, CYPRESS_L2_UC_NEXT_HOPtf, &pL2_entry->un.unicast.nh, (uint32 *)&l2_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
            ...
            {
                if ((ret = table_field_set(unit, CYPRESS_L2_NEXT_HOPt, CYPRESS_L2_NEXT_HOP_VLAN_TARGETtf, &pL2_entry->un.unicast.vlan_target, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_set(unit, CYPRESS_L2_NEXT_HOPt, CYPRESS_L2_NEXT_HOP_ROUTE_IDXtf, &pL2_entry->un.unicast.route_idx, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
            ...
            {
                if ((ret = table_field_set(unit, CYPRESS_L2_UCt, CYPRESS_L2_UC_VIDtf, &pL2_entry->un.unicast.agg_vid, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
        ...
        case L2_MULTICAST: /* l2 Multicast */
            ...
            if ((ret = table_field_set(unit, CYPRESS_L2_MCt, CYPRESS_L2_MC_MC_PMSK_IDXtf, &pL2_entry->un.l2mcast.index, (uint32 *)&l2_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
        ...
        case IP4_MULTICAST: /* IP multicast */
            ...
            {
                RT_LOG(LOG_TRACE, (MOD_DAL|MOD_L2), "index=%d, Mac=%x-%x-%x-%x-%x-%x",
                        pL2_entry->un.ipmcast_mc_ip.index, pL2_entry->un.ipmcast_mc_ip.mac, pL2_entry->un.ipmcast_mc_ip.mac.octet[0],
                ...
                if ((ret = table_field_set(unit, CYPRESS_L2_IP_MCt, CYPRESS_L2_IP_MC_MC_PMSK_IDXtf, &pL2_entry->un.ipmcast_mc_ip.index, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return RT_ERR_FAILED;
                }
            ...
            else if (LOOKUP_ON_DIP_AND_SIP == ip4HashFmt[unit])
            {
                ...
                if ((ret = table_field_set(unit, CYPRESS_L2_IP_MC_SIPt, CYPRESS_L2_IP_MC_SIP_GIPtf, &pL2_entry->un.ipmcast_ip_mc_sip.dip, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_set(unit, CYPRESS_L2_IP_MC_SIPt, CYPRESS_L2_IP_MC_SIP_SIPtf, &pL2_entry->un.ipmcast_ip_mc_sip.sip, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_set(unit, CYPRESS_L2_IP_MC_SIPt, CYPRESS_L2_IP_MC_SIP_FID_RVIDtf, &pL2_entry->un.ipmcast_ip_mc_sip.rvid, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_set(unit, CYPRESS_L2_IP_MC_SIPt, CYPRESS_L2_IP_MC_SIP_MC_PMSK_IDXtf, &pL2_entry->un.ipmcast_ip_mc_sip.index, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
            ...
            else if (LOOKUP_ON_DIP_AND_FVID == ip4HashFmt[unit])
            {
                ...
                if ((ret = table_field_set(unit, CYPRESS_L2_IP_MCt, CYPRESS_L2_IP_MC_FID_RVIDtf, &pL2_entry->un.ipmcast_ip_mc.rvid, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_set(unit, CYPRESS_L2_IP_MCt, CYPRESS_L2_IP_MC_GIPtf, &pL2_entry->un.ipmcast_ip_mc.dip, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_set(unit, CYPRESS_L2_IP_MCt, CYPRESS_L2_IP_MC_MC_PMSK_IDXtf, &pL2_entry->un.ipmcast_ip_mc.index, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
...
_dal_cypress_l2_setL2CAMEntry(uint32 unit, dal_cypress_l2_entry_t *pL2_entry, dal_cypress_l2_index_t *pL2_index)
{
    ...
    switch (pL2_entry->entry_type)
    {
        case L2_UNICAST: /* L2 unicast */
            ...
            if ((ret = table_field_set(unit, CYPRESS_L2_CAM_UCt, CYPRESS_L2_CAM_UC_DA_BLKtf, &pL2_entry->un.unicast.dablock, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_set(unit, CYPRESS_L2_CAM_UCt, CYPRESS_L2_CAM_UC_SA_BLKtf, &pL2_entry->un.unicast.sablock, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_set(unit, CYPRESS_L2_CAM_UCt, CYPRESS_L2_CAM_UC_SLPtf, &pL2_entry->un.unicast.port, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_set(unit, CYPRESS_L2_CAM_UCt, CYPRESS_L2_CAM_UC_FID_RVIDtf, &pL2_entry->un.unicast.fid, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_set(unit, CYPRESS_L2_CAM_UCt, CYPRESS_L2_CAM_UC_STATICtf, &pL2_entry->un.unicast.is_static, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_set(unit, CYPRESS_L2_CAM_UCt, CYPRESS_L2_CAM_UC_AGEtf, &pL2_entry->un.unicast.aging, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_set(unit, CYPRESS_L2_CAM_UCt, CYPRESS_L2_CAM_UC_SUSPENDtf, &pL2_entry->un.unicast.suspending, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_set(unit, CYPRESS_L2_CAM_UCt, CYPRESS_L2_CAM_UC_NEXT_HOPtf, &pL2_entry->un.unicast.nh, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_set(unit, CYPRESS_L2_CAM_UCt, CYPRESS_L2_CAM_UC_VIDtf, &pL2_entry->un.unicast.agg_vid, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
        ...
        case L2_MULTICAST: /* l2 Multicast */
            ...
            if ((ret = table_field_set(unit, CYPRESS_L2_CAM_MCt, CYPRESS_L2_CAM_MC_MC_PMSK_IDXtf, &pL2_entry->un.l2mcast.index, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_set(unit, CYPRESS_L2_CAM_MCt, CYPRESS_L2_CAM_MC_FID_RVIDtf, &pL2_entry->un.l2mcast.rvid, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
        ...
        case IP4_MULTICAST: /* IP multicast */
            ...
            {
                RT_LOG(LOG_TRACE, (MOD_DAL|MOD_L2), "index=%d, rvid=%d, Mac=%x-%x-%x-%x-%x-%x",
                        pL2_entry->un.ipmcast_mc_ip.index, pL2_entry->un.ipmcast_mc_ip.rvid, pL2_entry->un.ipmcast_mc_ip.mac, pL2_entry->un.ipmcast_mc_ip.mac.octet[0],
                        pL2_entry->un.ipmcast_mc_ip.mac.octet[1], pL2_entry->un.ipmcast_mc_ip.mac.octet[2], pL2_entry->un.ipmcast_mc_ip.mac.octet[3],
                        pL2_entry->un.ipmcast_mc_ip.mac.octet[4], pL2_entry->un.ipmcast_mc_ip.mac.octet[5]);
                ...
                if ((ret = table_field_set(unit, CYPRESS_L2_CAM_IP_MCt, CYPRESS_L2_CAM_IP_MC_MC_PMSK_IDXtf, &pL2_entry->un.ipmcast_mc_ip.index, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return RT_ERR_FAILED;
                }
            ...
            else if (LOOKUP_ON_DIP_AND_SIP == ip4HashFmt[unit])
            {
                ...
                if ((ret = table_field_set(unit, CYPRESS_L2_CAM_IP_MC_SIPt, CYPRESS_L2_CAM_IP_MC_SIP_GIPtf, &pL2_entry->un.ipmcast_ip_mc_sip.dip, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_set(unit, CYPRESS_L2_CAM_IP_MC_SIPt, CYPRESS_L2_CAM_IP_MC_SIP_SIPtf, &pL2_entry->un.ipmcast_ip_mc_sip.sip, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_set(unit, CYPRESS_L2_CAM_IP_MC_SIPt, CYPRESS_L2_CAM_IP_MC_SIP_FID_RVIDtf, &pL2_entry->un.ipmcast_ip_mc_sip.rvid, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_set(unit, CYPRESS_L2_CAM_IP_MC_SIPt, CYPRESS_L2_CAM_IP_MC_SIP_MC_PMSK_IDXtf, &pL2_entry->un.ipmcast_ip_mc_sip.index, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
            ...
            else if (LOOKUP_ON_DIP_AND_FVID == ip4HashFmt[unit])
            {
                ...
                if ((ret = table_field_set(unit, CYPRESS_L2_IP_MCt, CYPRESS_L2_CAM_IP_MC_FID_RVIDtf, &pL2_entry->un.ipmcast_ip_mc.rvid, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_set(unit, CYPRESS_L2_CAM_IP_MCt, CYPRESS_L2_CAM_IP_MC_GIPtf, &pL2_entry->un.ipmcast_ip_mc.dip, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_set(unit, CYPRESS_L2_CAM_IP_MCt, CYPRESS_L2_CAM_IP_MC_MC_PMSK_IDXtf, &pL2_entry->un.ipmcast_ip_mc.index, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
        ...
        , dal_cypress_l2_index_t *pL2_index)
{
    ...
    RT_LOG(LOG_TRACE, (MOD_DAL|MOD_L2), "unit=%d, entry_type=%d, Mac=%x-%x-%x-%x-%x-%x\
           ...
           pL2_entry->un.unicast.mac.octet[5], get_method, pL2_entry->un.unicast.nh);
...
_dal_cypress_l2_getL2EntryfromHash(uint32 unit, uint32 hashkey, uint32 location, dal_cypress_l2_entry_t *pL2_entry, uint32 *pIsValid)
{
    ...
    switch ((ip6_multi << 2) | (ip_multi << 1) | is_l2mcast)
    {
        case 0x0: /* L2 unicast */
            ...
            if ((ret = table_field_get(unit, CYPRESS_L2_UCt, CYPRESS_L2_UC_SLPtf, &pL2_entry->un.unicast.port, (uint32 *)&l2_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
        ...
        case 0x1: /* l2 Multicast */
            ...
            if (table_field_get(unit, CYPRESS_L2_MCt, CYPRESS_L2_MC_MC_PMSK_IDXtf, &pL2_entry->un.l2mcast.index, (uint32 *)&l2_entry) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return RT_ERR_FAILED;
            }
        ...
        case 0x2: /* IPv4 multicast */
            ...
            {
                if ((ret = table_field_get(unit, CYPRESS_L2_IP_MC_SIPt, CYPRESS_L2_IP_MC_SIP_MC_PMSK_IDXtf, &pL2_entry->un.ipmcast_ip_mc_sip.index, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }

                if ((ret = table_field_get(unit, CYPRESS_L2_IP_MC_SIPt, CYPRESS_L2_IP_MC_SIP_FID_RVIDtf, &pL2_entry->un.ipmcast_ip_mc_sip.rvid, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }

                if ((ret = table_field_get(unit, CYPRESS_L2_IP_MC_SIPt, CYPRESS_L2_IP_MC_SIP_GIPtf, &pL2_entry->un.ipmcast_ip_mc_sip.dip, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }

                if ((ret = table_field_get(unit, CYPRESS_L2_IP_MC_SIPt, CYPRESS_L2_IP_MC_SIP_SIPtf, &pL2_entry->un.ipmcast_ip_mc_sip.sip, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }

                pL2_entry->un.ipmcast_ip_mc_sip.sip &= 0x000fffff;
                _dal_cypress_l2_hashKeyToVid(unit, pL2_entry, hashkey);
            }
            else if (LOOKUP_ON_DIP_AND_FVID == ip4HashFmt[unit])
            {
                if ((ret = table_field_get(unit, CYPRESS_L2_IP_MCt, CYPRESS_L2_IP_MC_MC_PMSK_IDXtf, &pL2_entry->un.ipmcast_ip_mc.index, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }

                if ((ret = table_field_get(unit, CYPRESS_L2_IP_MCt, CYPRESS_L2_IP_MC_FID_RVIDtf, &pL2_entry->un.ipmcast_ip_mc.rvid, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }

                if ((ret = table_field_get(unit, CYPRESS_L2_IP_MCt, CYPRESS_L2_IP_MC_GIPtf, &pL2_entry->un.ipmcast_ip_mc.dip, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                ...
                pL2_entry->un.ipmcast_ip_mc.dip = (0xE << 28) | pL2_entry->un.ipmcast_ip_mc.dip;
                _dal_cypress_l2_hashKeyToVid(unit, pL2_entry, hashkey);
            }
...
_dal_cypress_l2_getL2EntryfromCAM(uint32 unit, uint32 index, dal_cypress_l2_entry_t *pL2_entry, uint32 *pIsValid)
{
    ...
    switch ((ip6_multi << 2) | (ip_multi << 1) | is_l2mcast)
    {
        case 0x00: /* L2 unicast */
            pL2_entry->entry_type = L2_UNICAST;
            if ((ret = table_field_get(unit, CYPRESS_L2_CAM_UCt, CYPRESS_L2_CAM_UC_SLPtf, &pL2_entry->un.unicast.port, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_get(unit, CYPRESS_L2_CAM_UCt, CYPRESS_L2_CAM_UC_FID_RVIDtf, &pL2_entry->un.unicast.fid, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
        ...
        case 0x01: /* l2 Multicast */
            pL2_entry->entry_type = L2_MULTICAST;
            if ((ret = table_field_get(unit, CYPRESS_L2_CAM_MCt, CYPRESS_L2_CAM_MC_MC_PMSK_IDXtf, &pL2_entry->un.l2mcast.index, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }

            if ((ret = table_field_get(unit, CYPRESS_L2_CAM_MCt, CYPRESS_L2_CAM_MC_FID_RVIDtf, &pL2_entry->un.l2mcast.rvid, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
        ...
        case 0x2: /* IPv4 multicast */
            ...
            {
                if ((ret = table_field_get(unit, CYPRESS_L2_CAM_IP_MC_SIPt, CYPRESS_L2_CAM_IP_MC_SIP_MC_PMSK_IDXtf, &pL2_entry->un.ipmcast_ip_mc_sip.index, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_get(unit, CYPRESS_L2_CAM_IP_MC_SIPt, CYPRESS_L2_CAM_IP_MC_SIP_FID_RVIDtf, &pL2_entry->un.ipmcast_ip_mc_sip.rvid, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_get(unit, CYPRESS_L2_CAM_IP_MC_SIPt, CYPRESS_L2_CAM_IP_MC_SIP_SIPtf, &pL2_entry->un.ipmcast_ip_mc_sip.sip, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_get(unit, CYPRESS_L2_CAM_IP_MC_SIPt, CYPRESS_L2_CAM_IP_MC_SIP_GIPtf, &pL2_entry->un.ipmcast_ip_mc_sip.dip, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                ...
                /* add 0xE in MSB four bit in DIP */
                pL2_entry->un.ipmcast_ip_mc_sip.dip = (0xE << 28) | pL2_entry->un.ipmcast_ip_mc_sip.dip;
            }
            else if (LOOKUP_ON_DIP_AND_FVID == ip4HashFmt[unit])
            {
                if ((ret = table_field_get(unit, CYPRESS_L2_CAM_IP_MCt, CYPRESS_L2_CAM_IP_MC_MC_PMSK_IDXtf, &pL2_entry->un.ipmcast_ip_mc.index, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_get(unit, CYPRESS_L2_CAM_IP_MCt, CYPRESS_L2_CAM_IP_MC_FID_RVIDtf, &pL2_entry->un.ipmcast_ip_mc.rvid, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_get(unit, CYPRESS_L2_CAM_IP_MCt, CYPRESS_L2_CAM_IP_MC_GIPtf, &pL2_entry->un.ipmcast_ip_mc.dip, (uint32 *)&l2cam_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
...
_dal_cypress_l2_entryToHashKey(uint32 unit, dal_cypress_l2_entry_t *pL2_entry, uint32 *pKey)
{
    ...
    switch (pL2_entry->entry_type)
    {
        ...
        case L2_MULTICAST:
            ...
            hashSeed = ((uint64)pL2_entry->un.l2mcast.rvid << 48) |
        ...
        case IP4_MULTICAST:
            ...
            {
                hashSeed = ((uint64)pL2_entry->un.l2mcast.rvid << 48) |
                        ((uint64)pL2_entry->un.ipmcast_mc_ip.mac.octet[0] << 40) |
                        ((uint64)pL2_entry->un.ipmcast_mc_ip.mac.octet[1] << 32) |
                        ((uint64)pL2_entry->un.ipmcast_mc_ip.mac.octet[2] << 24) |
                        ((uint64)pL2_entry->un.ipmcast_mc_ip.mac.octet[3] << 16) |
                        ((uint64)pL2_entry->un.ipmcast_mc_ip.mac.octet[4] << 8) |
                        ((uint64)pL2_entry->un.ipmcast_mc_ip.mac.octet[5]);
            }
            else if (LOOKUP_ON_DIP_AND_SIP == ip4HashFmt[unit])
            {
                /* sip+dip(LSB 28bits) */
                hashSeed = (((uint64)pL2_entry->un.ipmcast_ip_mc_sip.dip & 0xf) << 60) | ((uint64)pL2_entry->un.ipmcast_ip_mc_sip.sip << 28) |
                        ((uint64)pL2_entry->un.ipmcast_ip_mc_sip.dip >> 4);
            }
            else if (LOOKUP_ON_DIP_AND_FVID == ip4HashFmt[unit])
            {
                /* GIP[3:0], FID/VID(12-bit), 0(20-bit), GIP[31:4] */
                hashSeed = (((uint64)pL2_entry->un.ipmcast_ip_mc.dip & 0xf) << 60) | ((uint64)pL2_entry->un.ipmcast_ip_mc.rvid << 48) |
                        ((uint64)pL2_entry->un.ipmcast_ip_mc.dip >> 4);
            }
            break;
        case IP6_MULTICAST:
            if (LOOKUP_ON_FVID_AND_MAC == ip6HashFmt[unit])
...
static int32 _dal_cypress_l2_hashKeyToVid(uint32 unit, dal_cypress_l2_entry_t *pL2_entry, uint32 key)
{
    ...
    if (0 == algoType[unit])
    {
        ...
        {
            case L2_UNICAST:
                pL2_entry->un.unicast.fid = ((key & BITMASK_6B) << 6) | (key >> 6);
                _dal_cypress_l2_entryToHashKey(unit, pL2_entry, &partialkey1);
                pL2_entry->un.unicast.fid  = ((partialkey1 & BITMASK_6B) << 6) | (partialkey1 >> 6);
                break;

            case L2_MULTICAST:
                pL2_entry->un.l2mcast.rvid = ((key & BITMASK_6B) << 6) | (key >> 6);
                _dal_cypress_l2_entryToHashKey(unit, pL2_entry, &partialkey1);
                pL2_entry->un.l2mcast.rvid  = ((partialkey1 & BITMASK_6B) << 6) | (partialkey1 >> 6);
                break;
            ...
            case IP4_MULTICAST:
                ...
                {
                    pL2_entry->un.ipmcast_ip_mc_sip.dip = (0xE << 28) | pL2_entry->un.ipmcast_ip_mc_sip.dip;
                    pL2_entry->un.ipmcast_ip_mc_sip.sip = ((((key & BITMASK_6B) << 6) | (key >> 6)) << 20) | pL2_entry->un.ipmcast_ip_mc_sip.sip;
                    _dal_cypress_l2_entryToHashKey(unit, pL2_entry, &partialkey1);
                    pL2_entry->un.ipmcast_ip_mc_sip.sip = ((((partialkey1 & BITMASK_6B) << 6) | (partialkey1 >> 6)) << 20) | (pL2_entry->un.ipmcast_ip_mc_sip.sip & BITMASK_20B);
                }
                else if (LOOKUP_ON_DIP_AND_FVID == ip4HashFmt[unit])
                {
                    pL2_entry->un.ipmcast_ip_mc.rvid = ((key & BITMASK_6B) << 6) | (key >> 6);
                    _dal_cypress_l2_entryToHashKey(unit, pL2_entry, &partialkey1);
                    pL2_entry->un.ipmcast_ip_mc.rvid  = ((partialkey1 & BITMASK_6B) << 6) | (partialkey1 >> 6);
                }
                break;
    ...
    else if (1 == algoType[unit])
    {
        ...
        {
            case L2_UNICAST:
                pL2_entry->un.unicast.fid = key;
                _dal_cypress_l2_entryToHashKey(unit, pL2_entry, &partialkey1);
                pL2_entry->un.unicast.fid  = partialkey1;
                break;

            case L2_MULTICAST:
                pL2_entry->un.l2mcast.rvid = key;
                _dal_cypress_l2_entryToHashKey(unit, pL2_entry, &partialkey1);
                pL2_entry->un.l2mcast.rvid  = partialkey1;
                break;
            ...
            case IP4_MULTICAST:
                ...
                {
                    pL2_entry->un.ipmcast_ip_mc_sip.dip = (0xE << 28) | pL2_entry->un.ipmcast_ip_mc_sip.dip;
                    pL2_entry->un.ipmcast_ip_mc_sip.sip = (key << 20) | pL2_entry->un.ipmcast_ip_mc_sip.sip;
                    _dal_cypress_l2_entryToHashKey(unit, pL2_entry, &partialkey1);
                    pL2_entry->un.ipmcast_ip_mc_sip.sip = (partialkey1 << 20) | (pL2_entry->un.ipmcast_ip_mc_sip.sip & BITMASK_20B);
                }
                else if (LOOKUP_ON_DIP_AND_FVID == ip4HashFmt[unit])
                {
                    pL2_entry->un.ipmcast_ip_mc.rvid = key;
                    _dal_cypress_l2_entryToHashKey(unit, pL2_entry, &partialkey1);
                    pL2_entry->un.ipmcast_ip_mc.rvid  = partialkey1;
                }
                break;
    ...
    if (pL2_entry->valid)
    {
        ...
        switch (pL2_entry->entry_type)
        {
            ...
            case FLOW_TYPE_L2_MULTI:
                ...
                {
                    /* get multicast forward from shadow */
                    RTK_PORTMASK_ASSIGN(pL2_entry->un.l2mcast.portmask, mcast_fwdTable_shadow[l2_entry.un.l2mcast.index].portmask);
                }
                else
                {
                    osal_memset(&mcast_entry, 0, sizeof(mcast_entry));
                    ...
                    if ((ret = table_field_get(unit, CYPRESS_MC_PMSKt, CYPRESS_MC_PMSK_PMSKtf, pL2_entry->un.l2mcast.portmask.bits, (uint32 *) &mcast_entry)) != RT_ERR_OK)
                    {
                        L2_SEM_UNLOCK(unit);
                        RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "");
                        return ret;
            ...
            case FLOW_TYPE_IP4_MULTI:
                ...
                {
                    /* get multicast forward from shadow */
                    RTK_PORTMASK_ASSIGN(pL2_entry->un.ipmcast.portmask, mcast_fwdTable_shadow[pL2_entry->un.ipmcast.fwdIndex].portmask);
                }
                else
                {
                    osal_memset(&mcast_entry, 0, sizeof(mcast_entry));
                    if ((ret = table_read(unit, CYPRESS_MC_PMSKt, pL2_entry->un.ipmcast.fwdIndex, (uint32 *)&mcast_entry)) != RT_ERR_OK)
                    {
                        L2_SEM_UNLOCK(unit);
                        RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "");
                        return ret;
                    }
                    if ((ret = table_field_get(unit, CYPRESS_MC_PMSKt, CYPRESS_MC_PMSK_PMSKtf, pL2_entry->un.ipmcast.portmask.bits, (uint32 *) &mcast_entry)) != RT_ERR_OK)
                    {
                        L2_SEM_UNLOCK(unit);
                        RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "");
                        return ret;
            ...
            case FLOW_TYPE_IP6_MULTI:
                ...
                else if (LOOKUP_ON_DIP_AND_SIP == ip6HashFmt[unit])
                {
                    ...
                    _dal_cypress_l2_ip6CareByteConvertToIP6Addr(&pL2_entry->un.ip6mcast.dip, &pL2_entry->un.ip6mcast.sip, &l2_entry.un.ipmcast_ip_mc_sip.dip, &l2_entry.un.ipmcast_ip_mc_sip.sip);
                }
                else if (LOOKUP_ON_DIP_AND_FVID == ip6HashFmt[unit])
                {
                ...
                {
                    /* get multicast forward from shadow */
                    RTK_PORTMASK_ASSIGN(pL2_entry->un.ip6mcast.portmask, mcast_fwdTable_shadow[pL2_entry->un.ipmcast.fwdIndex].portmask);
                }
                else
                {
                    osal_memset(&mcast_entry, 0, sizeof(mcast_entry));
                    if ((ret = table_read(unit, CYPRESS_MC_PMSKt, pL2_entry->un.ip6mcast.fwdIndex, (uint32 *)&mcast_entry)) != RT_ERR_OK)
                    {
                        L2_SEM_UNLOCK(unit);
                        RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "");
                        return ret;
                    }
                    if ((ret = table_field_get(unit, CYPRESS_MC_PMSKt, CYPRESS_MC_PMSK_PMSKtf, pL2_entry->un.ip6mcast.portmask.bits, (uint32 *) &mcast_entry)) != RT_ERR_OK)
                    {
                        L2_SEM_UNLOCK(unit);
                        RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "");
                        return ret;
            ...
            switch (l2_entry.entry_type)
            {
                ...
                case L2_MULTICAST:
                    ...
                    if (HWP_CHIP_REV(unit) < CHIP_REV_ID_C)
                    {
                        /* get multicast forward from shadow */
                        L2_SEM_LOCK(unit);
                        RTK_PORTMASK_ASSIGN((pCfAddrList + cf_num)->un.l2mcast.portmask, mcast_fwdTable_shadow[l2_entry.un.l2mcast.index].portmask);
                        L2_SEM_UNLOCK(unit);
                    }
                    else
                    {
                        ...
                        if ((ret = table_field_get(unit, CYPRESS_MC_PMSKt, CYPRESS_MC_PMSK_PMSKtf, (pCfAddrList + cf_num)->un.l2mcast.portmask.bits, (uint32 *) &mcast_entry)) != RT_ERR_OK)
                        {
                            RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "");
                            return ret;
                        }
                ...
                case IP4_MULTICAST:
                    ...
                    if (HWP_CHIP_REV(unit) < CHIP_REV_ID_C)
                    {
                        /* get multicast forward from shadow */
                        L2_SEM_LOCK(unit);
                        RTK_PORTMASK_ASSIGN((pCfAddrList + cf_num)->un.ipmcast.portmask, mcast_fwdTable_shadow[l2_entry.un.ipmcast_ip_mc.index].portmask);
                        L2_SEM_UNLOCK(unit);
                    }
                    else
                    {
                        osal_memset(&mcast_entry, 0, sizeof(mcast_entry));
                        if ((ret = table_read(unit, CYPRESS_MC_PMSKt, l2_entry.un.ipmcast_ip_mc.index, (uint32 *)&mcast_entry)) != RT_ERR_OK)
                        {
                            RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "");
                            return ret;
                        }
                        if ((ret = table_field_get(unit, CYPRESS_MC_PMSKt, CYPRESS_MC_PMSK_PMSKtf, (pCfAddrList + cf_num)->un.ipmcast.portmask.bits, (uint32 *) &mcast_entry)) != RT_ERR_OK)
                        {
                            RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "");
                            return ret;
                        }
...
dal_cypress_l2_macLearningCnt_get(uint32 unit, rtk_l2_macLimitType_t type, rtk_l2_macCnt_t *pLimitCnt)
{
    ...
    switch(type)
    {
        case L2_MAC_LIMIT_GLOBAL:
            return dal_cypress_l2_learningCnt_get(unit, &pLimitCnt->un.glbCnt);
        case L2_MAC_LIMIT_PORT:
            return dal_cypress_l2_portLearningCnt_get(unit, pLimitCnt->un.portTrkCnt.id, &pLimitCnt->un.portTrkCnt.cnt);
        case L2_MAC_LIMIT_FID:
            return dal_cypress_l2_fidLearningCnt_get(unit, pLimitCnt->un.fidCnt.entryId, &pLimitCnt->un.fidCnt.cnt);
        default:
            return RT_ERR_INPUT;
    }
}
...
dal_cypress_l2_limitLearningNum_get(uint32 unit, rtk_l2_macLimitType_t type, rtk_l2_macCnt_t *pLimitCnt)
{
    ...
    {
        case L2_MAC_LIMIT_GLOBAL:
            return dal_cypress_l2_limitLearningCnt_get(unit, &pLimitCnt->un.glbCnt);
        case L2_MAC_LIMIT_PORT:
            return dal_cypress_l2_portLimitLearningCnt_get(unit, pLimitCnt->un.portTrkCnt.id, &pLimitCnt->un.portTrkCnt.cnt);
        case L2_MAC_LIMIT_FID:
            if((ret= dal_cypress_l2_fidLimitLearningEntry_get(unit, pLimitCnt->un.fidCnt.entryId, &fidMacLimitEntry)) !=RT_ERR_OK)
                return ret;
...
dal_cypress_l2_limitLearningNum_set(uint32 unit, rtk_l2_macLimitType_t type, rtk_l2_macCnt_t *pLimitCnt)
{
    ...
    switch(type)
    {
        case L2_MAC_LIMIT_GLOBAL:
            ...
            return dal_cypress_l2_limitLearningCnt_set(unit, pLimitCnt->un.glbCnt);
        case L2_MAC_LIMIT_PORT:
            if (pLimitCnt->un.portTrkCnt.cnt == L2_MAC_CST_DISABLE)
                pLimitCnt->un.portTrkCnt.cnt = HAL_L2_LEARN_LIMIT_CNT_DISABLE(unit);
            return dal_cypress_l2_portLimitLearningCnt_set(unit, pLimitCnt->un.portTrkCnt.id, pLimitCnt->un.portTrkCnt.cnt);
        case L2_MAC_LIMIT_FID:
            ...
            return dal_cypress_l2_fidLimitLearningEntry_set(unit, pLimitCnt->un.fidCnt.entryId, &fidMacLimitEntry);
        default:
            return RT_ERR_INPUT;
    }
}
...
dal_cypress_l2_limitLearningAction_get(uint32 unit, rtk_l2_macLimitType_t type, rtk_l2_macLimitAction_t *pAction)
{
    ...
    {
        case L2_MAC_LIMIT_GLOBAL:
            return dal_cypress_l2_limitLearningCntAction_get(unit, (rtk_l2_limitLearnCntAction_t*)&pAction->un.glbAct);
        case L2_MAC_LIMIT_PORT:
            return dal_cypress_l2_portLimitLearningCntAction_get(unit, pAction->un.portTrkAct.id, (rtk_l2_limitLearnCntAction_t*)&pAction->un.portTrkAct.act);
        case L2_MAC_LIMIT_FID:
            return dal_cypress_l2_fidLearningCntAction_get(unit, (rtk_l2_limitLearnCntAction_t*)&pAction->un.fidAct.act);
        default:
            return RT_ERR_INPUT;
    }
}
...
dal_cypress_l2_limitLearningAction_set(uint32 unit, rtk_l2_macLimitType_t type, rtk_l2_macLimitAction_t *pAction)
{
    ...
    {
        case L2_MAC_LIMIT_GLOBAL:
            return dal_cypress_l2_limitLearningCntAction_set(unit, pAction->un.glbAct);
        case L2_MAC_LIMIT_PORT:
            return dal_cypress_l2_portLimitLearningCntAction_set(unit, pAction->un.portTrkAct.id, pAction->un.portTrkAct.act);
        case L2_MAC_LIMIT_FID:
            return dal_cypress_l2_fidLearningCntAction_set(unit, pAction->un.fidAct.act);
        default:
            return RT_ERR_INPUT;
    }
}
    ...
    rtk_l2_portMoveAct_t        *pAction)
{
    ...
    switch(type)
    {
        case L2_PORT_MOVE_DYNAMIC:
            return dal_cypress_l2_portLegalPortMoveAction_get(unit, pAction->un.dynAct.port, &pAction->un.dynAct.act);
        case L2_PORT_MOVE_STATIC:
            return dal_cypress_l2_staticPortMoveAction_get(unit, 0, &pAction->un.sttAct.act);
        case L2_PORT_MOVE_FORBID:
            return dal_cypress_l2_dynamicPortMoveForbidAction_get(unit, &pAction->un.forbidAct.act);
        default:
            return RT_ERR_INPUT;
    }
}
    ...
    rtk_l2_portMoveAct_t        *pAction)
{
    ...
    {
        case L2_PORT_MOVE_DYNAMIC:
            return dal_cypress_l2_portLegalPortMoveAction_set(unit, pAction->un.dynAct.port, pAction->un.dynAct.act);
        case L2_PORT_MOVE_STATIC:
            HWP_PORT_TRAVS_EXCEPT_CPU(unit, port)
            {
                if ((ret = dal_cypress_l2_staticPortMoveAction_set(unit, port, pAction->un.sttAct.act)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
            ...
            return RT_ERR_OK;
        case L2_PORT_MOVE_FORBID:
            return dal_cypress_l2_dynamicPortMoveForbidAction_set(unit, pAction->un.forbidAct.act);
        default:
            return RT_ERR_INPUT;
    }
}
