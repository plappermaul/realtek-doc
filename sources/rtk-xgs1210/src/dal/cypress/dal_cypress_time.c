...
dal_cypress_time_init(uint32 unit)
{
    ...
    if ((ret = dal_cypress_time_portRefTime_set(unit, portmask, timeStamp)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_TIME), "");
        return ret;
    }
 ...
 */
int32
dal_cypress_time_portRefTime_set(uint32 unit, rtk_portmask_t portmask, rtk_time_timeStamp_t timeStamp)
{
    int32 ret;
    uint32 val;
    ...
    ret = dal_phy_portRefTime_set(unit, portmask, timeStamp);
    if ((ret != RT_ERR_OK) && (ret != RT_ERR_CHIP_NOT_SUPPORTED) && (ret != RT_ERR_PORT_NOT_SUPPORTED))
    {
        TIME_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_TIME), "");
 ...
 */
int32
dal_cypress_time_portRefTimeAdjust_set(uint32 unit, rtk_portmask_t portmask, uint32 sign, rtk_time_timeStamp_t timeStamp)
{
    int32 ret;
    uint32 val;
    ...
    ret = dal_phy_portRefTimeAdjust_set(unit, portmask, sign, timeStamp);
    if ((ret != RT_ERR_OK) && (ret != RT_ERR_CHIP_NOT_SUPPORTED) && (ret != RT_ERR_PORT_NOT_SUPPORTED))
    {
        TIME_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_TIME), "");
