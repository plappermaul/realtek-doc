 ...
 *
 * $Revision: 98633 $
 * $Date: 2019-07-11 22:04:43 +0800 (Thu, 11 Jul 2019) $
 *
...
static int32 _dal_mango_l2_addr_delByMac(uint32 unit, rtk_mac_t *pMac, uint32 sram_if);
...
int32 _dal_mango_l2_hashIdxToVid(uint32 unit, dal_mango_l2_entry_t *pL2_entry, dal_mango_l2_index_t *pIndex)
{
    ...
    if (0 == blkId)
    {
        switch (pL2_entry->entry_type)
        {
            case L2_UNICAST:
                if (blkAlgoType[0][unit] == 0)
                    pL2_entry->un.unicast.fid = ((idx & 0x1ff) << 3) | ((idx) >> 9);
                else
                    pL2_entry->un.unicast.fid = idx;
                _dal_mango_l2_entryToHashIdx(unit, pL2_entry, &partialkey1);
                if (blkAlgoType[0][unit] == 0)
                    pL2_entry->un.unicast.fid = ((partialkey1.blk0_hashIdx & 0x1ff) << 3) | ((partialkey1.blk0_hashIdx & 0xfff) >> 9);
                else
                    pL2_entry->un.unicast.fid = partialkey1.blk0_hashIdx;
                break;

            case L2_MULTICAST:
                if (blkAlgoType[0][unit] == 0)
                    pL2_entry->un.l2mcast.fid = ((idx & 0x1ff) << 3) | ((idx) >> 9);
                else
                    pL2_entry->un.l2mcast.fid = idx;
                _dal_mango_l2_entryToHashIdx(unit, pL2_entry, &partialkey1);
                if (blkAlgoType[0][unit] == 0)
                    pL2_entry->un.l2mcast.fid = ((partialkey1.blk0_hashIdx & 0x1ff) << 3) | ((partialkey1.blk0_hashIdx & 0xfff) >> 9);
                else
                    pL2_entry->un.l2mcast.fid = partialkey1.blk0_hashIdx;
                break;
    ...
    else if (1 == blkId)
    {
        ...
        switch (pL2_entry->entry_type)
        {
            case L2_UNICAST:
                if (blkAlgoType[1][unit] == 0)
                    pL2_entry->un.unicast.fid = ((idx & 0x1ff) << 3) | ((idx & 0xfff) >> 9);
                else
                    pL2_entry->un.unicast.fid = idx;
                _dal_mango_l2_entryToHashIdx(unit, pL2_entry, &partialkey1);
                if (blkAlgoType[1][unit] == 0)
                    pL2_entry->un.unicast.fid = ((partialkey1.blk1_hashIdx & 0xff) << 3) | ((partialkey1.blk1_hashIdx & 0xfff) >> 9);
                else
                    pL2_entry->un.unicast.fid = partialkey1.blk1_hashIdx - ((hashTable_size[unit]/2)>>2);
                break;

            case L2_MULTICAST:
                if (blkAlgoType[1][unit] == 0)
                    pL2_entry->un.l2mcast.fid = ((idx & 0x1ff) << 3) | ((idx & 0xfff) >> 9);
                else
                    pL2_entry->un.l2mcast.fid = idx;
                _dal_mango_l2_entryToHashIdx(unit, pL2_entry, &partialkey1);
                if (blkAlgoType[1][unit] == 0)
                    pL2_entry->un.l2mcast.fid = ((partialkey1.blk1_hashIdx & 0xff) << 3) | ((partialkey1.blk1_hashIdx & 0xfff) >> 9);
                else
                    pL2_entry->un.l2mcast.fid = partialkey1.blk1_hashIdx - ((hashTable_size[unit]/2)>>2);
                break;
...
int32 _dal_mango_l2_getL2Entry(uint32 unit, dal_mango_l2_index_t * pIndex, dal_mango_l2_entry_t *pL2_entry)
{
    ...
    if (is_l2mcast)
    {
        if ((ret = table_field_get(unit, MANGO_L2_MCt, MANGO_L2_MC_LOCAL_FWDtf, &pL2_entry->un.l2mcast.local_fwd, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_get(unit, MANGO_L2_MCt, MANGO_L2_MC_REMOTE_FWDtf, &pL2_entry->un.l2mcast.remote_fwd, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        ...
        pL2_entry->un.l2mcast.valid        = TRUE;
        ...
        if ((ret = table_field_get(unit, MANGO_L2_MCt, MANGO_L2_MC_MC_PMSK_IDXtf, &pL2_entry->un.l2mcast.index, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_get(unit, MANGO_L2_MCt, MANGO_L2_MC_L2_TNL_LST_IDXtf, &pL2_entry->un.l2mcast.tunnel_list_idx, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
    ...
    {
        if ((ret = table_field_get(unit, MANGO_L2_UCt, MANGO_L2_UC_L2_TNLtf, &pL2_entry->un.unicast.l2_tnl_if, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        ...
        pL2_entry->un.unicast.valid        = TRUE;
        ...
        if ((ret = table_field_get(unit, MANGO_L2_UCt, MANGO_L2_UC_IS_TRKtf, &pL2_entry->un.unicast.is_trk, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_get(unit, MANGO_L2_UCt, MANGO_L2_UC_SPAtf, &pL2_entry->un.unicast.slp, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_get(unit, MANGO_L2_UCt, MANGO_L2_UC_AGEtf, &pL2_entry->un.unicast.aging, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_get(unit, MANGO_L2_UCt, MANGO_L2_UC_SA_BLKtf, &pL2_entry->un.unicast.sablock, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_get(unit, MANGO_L2_UCt, MANGO_L2_UC_DA_BLKtf, &pL2_entry->un.unicast.dablock, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_get(unit, MANGO_L2_UCt, MANGO_L2_UC_STATICtf, &pL2_entry->un.unicast.is_static, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_get(unit, MANGO_L2_UCt, MANGO_L2_UC_SUSPENDtf, &pL2_entry->un.unicast.suspending, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        ...
        if (pL2_entry->un.unicast.l2_tnl_if)
        {
            /* L2 tunnel index (share the field with AGG_VID) */
            if ((ret = table_field_get(unit, MANGO_L2_UCt, MANGO_L2_UC_AGG_VIDtf, &pL2_entry->un.unicast.l2_tnl_idx, (uint32 *)&l2_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
            ...
            if (nTo1Ebl)
            {
                if ((ret = table_field_get(unit, MANGO_L2_UCt, MANGO_L2_UC_TAGSTStf, &pL2_entry->un.unicast.tagSts, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_get(unit, MANGO_L2_UCt, MANGO_L2_UC_AGG_PRItf, &pL2_entry->un.unicast.agg_pri, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_get(unit, MANGO_L2_UCt, MANGO_L2_UC_AGG_VIDtf, &pL2_entry->un.unicast.agg_vid, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
            ...
            {
                if ((ret = table_field_get(unit, MANGO_L2_CB_UCt, MANGO_L2_CB_UC_ECIDtf, &pL2_entry->un.unicast.ecid, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
...
int32 _dal_mango_l2_setL2Entry(uint32 unit, dal_mango_l2_index_t * pIndex, dal_mango_l2_entry_t *pL2_entry)
{
    ...
    if (pL2_entry->entry_type == L2_UNICAST)
    {
        ...
        {
            if (hash_algo == 0)
                value = (pL2_entry->un.unicast.fid >> 2) & 0x1;
            else
                value = (pL2_entry->un.unicast.fid >> 11) & 0x1;
        ...
        if ((ret = table_field_set(unit, MANGO_L2_UCt, MANGO_L2_UC_FIDtf, &pL2_entry->un.unicast.fid, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        ...
        if ((ret = table_field_set(unit, MANGO_L2_UCt, MANGO_L2_UC_VALIDtf, &pL2_entry->un.unicast.valid, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        ...
        if ((ret = table_field_set(unit, MANGO_L2_UCt, MANGO_L2_UC_IS_TRKtf, &pL2_entry->un.unicast.is_trk, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_set(unit, MANGO_L2_UCt, MANGO_L2_UC_SPAtf, &pL2_entry->un.unicast.slp, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_set(unit, MANGO_L2_UCt, MANGO_L2_UC_AGEtf, &pL2_entry->un.unicast.aging, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_set(unit, MANGO_L2_UCt, MANGO_L2_UC_SA_BLKtf, &pL2_entry->un.unicast.sablock, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_set(unit, MANGO_L2_UCt, MANGO_L2_UC_DA_BLKtf, &pL2_entry->un.unicast.dablock, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_set(unit, MANGO_L2_UCt, MANGO_L2_UC_STATICtf, &pL2_entry->un.unicast.is_static, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_set(unit, MANGO_L2_UCt, MANGO_L2_UC_SUSPENDtf, &pL2_entry->un.unicast.suspending, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_set(unit, MANGO_L2_UCt, MANGO_L2_UC_NEXT_HOPtf, &pL2_entry->un.unicast.nh, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        ...
        {
            /* L2 tunnel bit */
            if ((ret = table_field_set(unit, MANGO_L2_UCt, MANGO_L2_UC_L2_TNLtf, &pL2_entry->un.unicast.l2_tnl_if, (uint32 *)&l2_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
            ...
            /* L2 tunnel index (share the field with AGG_VID) */
            if ((ret = table_field_set(unit, MANGO_L2_UCt, MANGO_L2_UC_AGG_VIDtf, &pL2_entry->un.unicast.l2_tnl_idx, (uint32 *)&l2_entry)) != RT_ERR_OK)
            {
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            }
            ...
            if (nTo1Ebl)
            {
                if ((ret = table_field_set(unit, MANGO_L2_UCt, MANGO_L2_UC_TAGSTStf, &pL2_entry->un.unicast.tagSts, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_set(unit, MANGO_L2_UCt, MANGO_L2_UC_AGG_PRItf, &pL2_entry->un.unicast.agg_pri, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
                if ((ret = table_field_set(unit, MANGO_L2_UCt, MANGO_L2_UC_AGG_VIDtf, &pL2_entry->un.unicast.agg_vid, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
            ...
            {
                if ((ret = table_field_set(unit, MANGO_L2_CB_UCt, MANGO_L2_CB_UC_ECIDtf, &pL2_entry->un.unicast.ecid, (uint32 *)&l2_entry)) != RT_ERR_OK)
                {
                    RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                }
    ...
    else if (pL2_entry->entry_type == L2_MULTICAST)
    {
        ...
        {
            if (hash_algo == 0)
                value = (pL2_entry->un.l2mcast.fid >> 2) & 0x1;
            else
                value = (pL2_entry->un.l2mcast.fid >> 11) & 0x1;
        ...
        if ((ret = table_field_set(unit, MANGO_L2_MCt, MANGO_L2_MC_FIDtf, &pL2_entry->un.l2mcast.fid, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        ...
        if ((ret = table_field_set(unit, MANGO_L2_MCt, MANGO_L2_MC_VALIDtf, &pL2_entry->un.l2mcast.valid, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        ...
        if ((ret = table_field_set(unit, MANGO_L2_MCt, MANGO_L2_MC_LOCAL_FWDtf, &pL2_entry->un.l2mcast.local_fwd, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_set(unit, MANGO_L2_MCt, MANGO_L2_MC_MC_PMSK_IDXtf, &pL2_entry->un.l2mcast.index, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_set(unit, MANGO_L2_MCt, MANGO_L2_MC_NEXT_HOPtf, &pL2_entry->un.l2mcast.nh, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
        if ((ret = table_field_set(unit, MANGO_L2_MCt, MANGO_L2_MC_REMOTE_FWDtf, &pL2_entry->un.l2mcast.remote_fwd, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }

        if ((ret = table_field_set(unit, MANGO_L2_MCt, MANGO_L2_MC_L2_TNL_LST_IDXtf, &pL2_entry->un.l2mcast.tunnel_list_idx, (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        }
...
dal_mango_l2_init(uint32 unit)
{
    ...
    l2Db->pEntry = osal_alloc((hashTable_size[unit] + cam_size[unit]) * sizeof(dal_mango_l2DrvDb_entry_t));
    osal_memset(l2Db->pEntry, 0, (hashTable_size[unit] + cam_size[unit]) * sizeof(dal_mango_l2DrvDb_entry_t));
    for (index = 0; index < (hashTable_size[unit] + cam_size[unit]); index++)
    {
        ...
        {
            l2Db->pEntry[index].mcastNode.l2Idx.index_type = L2_IN_HASH;
            l2Db->pEntry[index].mcastNode.l2Idx.index = (index >> 2);
            l2Db->pEntry[index].mcastNode.l2Idx.hashdepth = (index & 0x3);
        } else {
            l2Db->pEntry[index].mcastNode.l2Idx.index_type = L2_IN_CAM;
            l2Db->pEntry[index].mcastNode.l2Idx.index = (index - hashTable_size[unit]);
            l2Db->pEntry[index].mcastNode.l2Idx.hashdepth = 0;
        }
    }
    ...
    l2_entry.un.unicast.valid = 1;
    l2_entry.un.unicast.fid = 4095;
    l2_entry.un.unicast.mac.octet[0] = 0xff;
    l2_entry.un.unicast.mac.octet[1] = 0xff;
    l2_entry.un.unicast.mac.octet[2] = 0xff;
    l2_entry.un.unicast.mac.octet[3] = 0xff;
    l2_entry.un.unicast.mac.octet[4] = 0xff;
    l2_entry.un.unicast.mac.octet[5] = 0xff;
...
dal_mango_l2_ucastAddr_flush(uint32 unit, rtk_l2_flushCfg_t *pConfig)
{
    ...
    if (pConfig->flushByPort && pConfig->portOrTrunk == ENABLED)
        RT_PARAM_CHK(!HWP_PORT_EXIST(unit, pConfig->port), RT_ERR_PORT_ID);
    ...
    if (pConfig->act == FLUSH_ACT_REPLACE_ALL_UC || pConfig->act == FLUSH_ACT_REPLACE_DYNM_ONLY)
    {
        ...
        RT_PARAM_CHK((ENABLED == pConfig->replacePortOrTrunk && !HWP_PORT_EXIST(unit, pConfig->replacingPort)), RT_ERR_PORT_ID);
...
dal_mango_l2_macLearningCnt_get(uint32 unit, rtk_l2_macLimitType_t type, rtk_l2_macCnt_t *pLimitCnt)
{
    ...
    switch (type)
    {
        ...
        case L2_MAC_LIMIT_TRUNK:
            RT_PARAM_CHK((pLimitCnt->un.portTrkCnt.id >= STANDALONE_TRUNK_ID_MAX), RT_ERR_TRUNK_ID);
            break;
    ...
    switch (type)
    {
        case L2_MAC_LIMIT_GLOBAL:
            if ((ret = reg_field_read(unit, MANGO_L2_LRN_CONSTRT_CNTr, MANGO_LRN_CNTf, &pLimitCnt->un.glbCnt)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            ...
            break;
        case L2_MAC_LIMIT_PORT:
            if ((ret = reg_array_field_read(unit, MANGO_L2_LRN_PORT_CONSTRT_CNTr, pLimitCnt->un.portTrkCnt.id, REG_ARRAY_INDEX_NONE, MANGO_LRN_CNTf, &pLimitCnt->un.portTrkCnt.cnt)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            ...
            break;
        case L2_MAC_LIMIT_TRUNK:
            if ((ret = reg_array_field_read(unit, MANGO_L2_LRN_TRK_CONSTRT_CNTr, REG_ARRAY_INDEX_NONE, pLimitCnt->un.portTrkCnt.id, MANGO_LRN_CNTf, &pLimitCnt->un.portTrkCnt.cnt)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            ...
            break;
        case L2_MAC_LIMIT_FID:
            if ((ret = reg_array_field_read(unit, MANGO_L2_LRN_VLAN_CONSTRT_CNTr, REG_ARRAY_INDEX_NONE, pLimitCnt->un.fidCnt.entryId, MANGO_LRN_CNTf, &pLimitCnt->un.fidCnt.cnt)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
...
dal_mango_l2_limitLearningNum_get(uint32 unit, rtk_l2_macLimitType_t type, rtk_l2_macCnt_t *pLimitCnt)
{
    ...
    {
        case L2_MAC_LIMIT_GLOBAL:
            if ((ret = reg_field_read(unit, MANGO_L2_LRN_CONSTRT_CTRLr, MANGO_CONSTRT_NUMf, &pLimitCnt->un.glbCnt)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            ...
            break;
        case L2_MAC_LIMIT_PORT:
            if ((ret = reg_array_field_read(unit, MANGO_L2_LRN_PORT_CONSTRT_CTRLr, pLimitCnt->un.portTrkCnt.id, REG_ARRAY_INDEX_NONE, MANGO_CONSTRT_NUMf, &pLimitCnt->un.portTrkCnt.cnt)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            ...
            break;
        case L2_MAC_LIMIT_TRUNK:
            if ((ret = reg_array_field_read(unit, MANGO_L2_LRN_TRK_CONSTRT_CTRLr, REG_ARRAY_INDEX_NONE, pLimitCnt->un.portTrkCnt.id, MANGO_CONSTRT_NUMf, &pLimitCnt->un.portTrkCnt.cnt)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            ...
            break;
        case L2_MAC_LIMIT_FID:
            if ((ret = reg_array_field_read(unit, MANGO_L2_LRN_VLAN_CONSTRT_ENTRYr, REG_ARRAY_INDEX_NONE, pLimitCnt->un.fidCnt.entryId, MANGO_CONSTRT_NUMf, &pLimitCnt->un.fidCnt.cnt)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
...
dal_mango_l2_limitLearningNum_set(uint32 unit, rtk_l2_macLimitType_t type, rtk_l2_macCnt_t *pLimitCnt)
{
    ...
    {
        case L2_MAC_LIMIT_GLOBAL:
            if ((ret = reg_field_write(unit, MANGO_L2_LRN_CONSTRT_CTRLr, MANGO_CONSTRT_NUMf, &pLimitCnt->un.glbCnt)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            ...
            break;
        case L2_MAC_LIMIT_PORT:
            if ((ret = reg_array_field_write(unit, MANGO_L2_LRN_PORT_CONSTRT_CTRLr, pLimitCnt->un.portTrkCnt.id, REG_ARRAY_INDEX_NONE, MANGO_CONSTRT_NUMf, &pLimitCnt->un.portTrkCnt.cnt)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            ...
            break;
        case L2_MAC_LIMIT_TRUNK:
            if ((ret = reg_array_field_write(unit, MANGO_L2_LRN_TRK_CONSTRT_CTRLr, REG_ARRAY_INDEX_NONE, pLimitCnt->un.portTrkCnt.id, MANGO_CONSTRT_NUMf, &pLimitCnt->un.portTrkCnt.cnt)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            ...
            break;
        case L2_MAC_LIMIT_FID:
            if ((ret = reg_array_field_write(unit, MANGO_L2_LRN_VLAN_CONSTRT_ENTRYr, REG_ARRAY_INDEX_NONE, pLimitCnt->un.fidCnt.entryId, MANGO_CONSTRT_NUMf, &pLimitCnt->un.fidCnt.cnt)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
...
dal_mango_l2_limitLearningAction_get(uint32 unit, rtk_l2_macLimitType_t type, rtk_l2_macLimitAction_t *pAction)
{
    ...
    {
        case L2_MAC_LIMIT_PORT:
            RT_PARAM_CHK(!HWP_PORT_EXIST(unit, pAction->un.portTrkAct.id), RT_ERR_PORT_ID);
            break;
        case L2_MAC_LIMIT_TRUNK:
            RT_PARAM_CHK((pAction->un.portTrkAct.id >= STANDALONE_TRUNK_ID_MAX), RT_ERR_TRUNK_ID);
            break;
        default:
            break;
    }
    ...
    switch (type)
    {
        case L2_MAC_LIMIT_GLOBAL:
            ...
            break;
        case L2_MAC_LIMIT_PORT:
            if ((ret = reg_array_field_read(unit, MANGO_L2_LRN_PORT_CONSTRT_CTRLr, pAction->un.portTrkAct.id, REG_ARRAY_INDEX_NONE, MANGO_ACTf, &value)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            ...
            break;
        case L2_MAC_LIMIT_TRUNK:
            if ((ret = reg_array_field_read(unit, MANGO_L2_LRN_TRK_CONSTRT_CTRLr, REG_ARRAY_INDEX_NONE, pAction->un.portTrkAct.id, MANGO_ACTf, &value)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
    ...
    {
        case L2_MAC_LIMIT_GLOBAL:
            pAction->un.glbAct = action;
            break;
        case L2_MAC_LIMIT_PORT:
        case L2_MAC_LIMIT_TRUNK:
            pAction->un.portTrkAct.act = action;
            break;
        case L2_MAC_LIMIT_FID:
            pAction->un.fidAct.act = action;
            break;
        default:
            return RT_ERR_INPUT;
    }
...
dal_mango_l2_limitLearningAction_set(uint32 unit, rtk_l2_macLimitType_t type, rtk_l2_macLimitAction_t *pAction)
{
    ...
    switch (type)
    {
        ...
        case L2_MAC_LIMIT_TRUNK:
            RT_PARAM_CHK((pAction->un.portTrkAct.id >= STANDALONE_TRUNK_ID_MAX), RT_ERR_TRUNK_ID);break;
        default:
            break;
    }
    ...
    {
        case L2_MAC_LIMIT_GLOBAL:
            action = pAction->un.glbAct;
            break;
        case L2_MAC_LIMIT_PORT:
        case L2_MAC_LIMIT_TRUNK:
            action= pAction->un.portTrkAct.act;
            break;
        case L2_MAC_LIMIT_FID:
            action = pAction->un.fidAct.act;
            break;
        default:
            return RT_ERR_INPUT;
    }
    ...
    switch (type)
    {
        case L2_MAC_LIMIT_GLOBAL:
            ...
            break;
        case L2_MAC_LIMIT_PORT:
            if ((ret = reg_array_field_write(unit, MANGO_L2_LRN_PORT_CONSTRT_CTRLr, pAction->un.portTrkAct.id, REG_ARRAY_INDEX_NONE, MANGO_ACTf, &value)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            ...
            break;
        case L2_MAC_LIMIT_TRUNK:
            if ((ret = reg_array_field_write(unit, MANGO_L2_LRN_TRK_CONSTRT_CTRLr, REG_ARRAY_INDEX_NONE, pAction->un.portTrkAct.id, MANGO_ACTf, &value)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
...
dal_mango_l2_agingTime_set(uint32 unit, rtk_l2_ageTimeType_t type, uint32 aging_time)
{
    ...
    if (type == L2_AGE_TIME_NORMAL)
    {
        ageUnit = (uint32)((aging_time * 10 + 7)/ 8);
    ...
    else
    {
        ...
        if (aging_time == 0 || ageUnit == 0)
            value = 0;
        else if ((uint32)(aging_time * 10 / ageUnit) == 0)
            value = 0;
        else
            value = (uint32)(aging_time * 10 / ageUnit) - 1;
...
dal_mango_l2_portNewMacOp_set(uint32 unit, rtk_port_t port, rtk_l2_newMacLrnMode_t lrnMode, rtk_action_t fwdAction)
{
    ...
    if ((ret = reg_array_field_write(unit, MANGO_L2_PORT_SALRNr, port, REG_ARRAY_INDEX_NONE, MANGO_SALRNf, &mode)) != RT_ERR_OK)
    {
        L2_SEM_UNLOCK(unit);
        RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "err ret = %d", ret);
        return ret;
...
dal_mango_l2_addr_add(uint32 unit, rtk_l2_ucastAddr_t *pL2_addr)
{
    ...
    else if (ret == RT_ERR_L2_NO_EMPTY_ENTRY && (pL2_addr->add_op_flags & RTK_L2_ADD_OP_FLAG_REPLACE_DYNAMIC) != 0)
    {
        ...
        pL2_addr->macInfo.devID      = l2_entry.un.unicast.slp >> RTL9310_L2ENTRY_DEVI_ID_OFFSET;
        pL2_addr->macInfo.port      = l2_entry.un.unicast.slp & RTL9310_L2ENTRY_PORT_MASK;
        ...
        pL2_addr->macInfo.trk_gid   = l2_entry.un.unicast.is_trk ? (l2_entry.un.unicast.slp & 0xff) : 0;
    }
    ...
    if (pL2_addr->flags & RTK_L2_UCAST_FLAG_TRUNK_PORT)
    {
        ...
        l2_entry.un.unicast.slp = pL2_addr->trk_gid;
    }
    else
        l2_entry.un.unicast.slp = (pL2_addr->devID << RTL9310_L2ENTRY_DEVI_ID_OFFSET) | lPort;
    ...
    l2_entry.un.unicast.tagSts     = (pL2_addr->flags & RTK_L2_UCAST_FLAG_TAG_STS) ? TRUE: FALSE;
    l2_entry.un.unicast.agg_pri    = pL2_addr->agg_pri;
    l2_entry.un.unicast.agg_vid    = pL2_addr->agg_vid;
    l2_entry.un.unicast.ecid       = pL2_addr->ecid;
    l2_entry.un.unicast.l2_tnl_if  = (pL2_addr->flags & RTK_L2_UCAST_FLAG_L2_TUNNEL)? TRUE : FALSE;
    l2_entry.un.unicast.l2_tnl_idx = pL2_addr->l2_tunnel_idx;
...
dal_mango_l2_addr_del(uint32 unit, rtk_vlan_t vid, rtk_mac_t *pMac)
{
    ...
    if (l2_entry.un.unicast.nh)
    {
        ...
        l2_entry.un.unicast.slp        = PORT_DONT_CARE_9310;
        l2_entry.un.unicast.sablock    = 0;
        l2_entry.un.unicast.dablock    = 0;
        l2_entry.un.unicast.is_static  = 0;
        l2_entry.un.unicast.suspending = 0;
        l2_entry.un.unicast.tagSts     = 0;
        l2_entry.un.unicast.agg_pri    = 0;
        l2_entry.un.unicast.agg_vid    = 0;
        l2_entry.un.unicast.ecid       = 0;
        l2_entry.un.unicast.l2_tnl_if  = 0;
        l2_entry.un.unicast.l2_tnl_idx = 0;
    }
    else
    {
        osal_memset(&l2_entry, 0, sizeof(dal_mango_l2_entry_t));
...
dal_mango_l2_addr_get(uint32 unit, rtk_l2_ucastAddr_t *pL2_addr)
{
    ...
    if (l2_entry.un.unicast.is_trk)
    {
        ...
        pL2_addr->trk_gid   = l2_entry.un.unicast.slp & 0xff;
    }
    else
    {
        pL2_addr->devID     = l2_entry.un.unicast.slp >> RTL9310_L2ENTRY_DEVI_ID_OFFSET;
        pL2_addr->port      = l2_entry.un.unicast.slp & RTL9310_L2ENTRY_PORT_MASK;
    }
    ...
    if (l2_entry.un.unicast.aging == 0 && l2_entry.un.unicast.is_trk == 0 && l2_entry.un.unicast.slp == PORT_DONT_CARE_9310)
        pL2_addr->isAged = TRUE;
    else
        pL2_addr->isAged = FALSE;
    if (l2_entry.un.unicast.tagSts)
        pL2_addr->flags    |= RTK_L2_UCAST_FLAG_TAG_STS;
    pL2_addr->agg_pri   = l2_entry.un.unicast.agg_pri;
    pL2_addr->agg_vid   = l2_entry.un.unicast.agg_vid;
    pL2_addr->age       = l2_entry.un.unicast.aging;
    pL2_addr->ecid      = l2_entry.un.unicast.ecid;

    if (l2_entry.un.unicast.l2_tnl_if)
    {
        pL2_addr->flags |= RTK_L2_UCAST_FLAG_L2_TUNNEL;
        pL2_addr->l2_tunnel_idx = l2_entry.un.unicast.l2_tnl_idx;
    } else {
    ...
    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_L2), "vid=%d, unit=%d, port=%d, sa_block=%d, da_block=%d, is_static=%d, nh=%d, suspend=%d, agg_pri=:%d, agg_vid=:%d, ecid=0x%x",
        pL2_addr->vid, pL2_addr->devID, pL2_addr->port, l2_entry.un.unicast.sablock, l2_entry.un.unicast.dablock, l2_entry.un.unicast.is_static,
        l2_entry.un.unicast.nh, l2_entry.un.unicast.suspending, l2_entry.un.unicast.agg_pri, l2_entry.un.unicast.agg_vid,
        l2_entry.un.unicast.ecid);
...
dal_mango_l2_addr_delAll(uint32 unit, uint32 include_static)
{
    ...
    for (index = 0; index < hashTable_size[unit]; index++)
    {
        ...
        if (l2_entry.entry_type == L2_UNICAST && l2_entry.is_entry_valid == 1 && (include_static || l2_entry.un.unicast.is_static == 0))
        {
            if (l2_entry.un.unicast.nh)
            {
                l2_entry.un.unicast.aging      = 0;
...
_dal_mango_l2_nexthop_add(uint32 unit, dal_mango_l2_ucastNhAddr_t *pNexthop_addr)
{
    ...
    else if (ret == RT_ERR_L2_NO_EMPTY_ENTRY && (pNexthop_addr->add_op_flags & RTK_L2_ADD_OP_FLAG_REPLACE_DYNAMIC) != 0)
    {
        ...
        pNexthop_addr->macInfo.devID      = l2_entry.un.unicast.slp >> RTL9310_L2ENTRY_DEVI_ID_OFFSET;
        pNexthop_addr->macInfo.port      = l2_entry.un.unicast.slp & RTL9310_L2ENTRY_PORT_MASK;
        ...
        pNexthop_addr->macInfo.trk_gid   = l2_entry.un.unicast.is_trk ? (l2_entry.un.unicast.slp & 0xff) : 0;
...
_dal_mango_l2_nexthop_del(uint32 unit, dal_mango_l2_ucastNhAddr_t *pNexthop_addr)
{
    ...
    if (l2_entry.un.unicast.nh)
    {
        ...
        {
            l2_entry.un.unicast.nh = 0;
            if (l2_entry.un.unicast.slp == PORT_DONT_CARE_9310 &&
                l2_entry.un.unicast.sablock == 0 &&
                l2_entry.un.unicast.dablock == 0 &&
                l2_entry.un.unicast.is_static == 0)
            {
...
dal_mango_l2_mcastAddr_del(uint32 unit, rtk_vlan_t vid, rtk_mac_t *pMac)
{
    ...
    if (0 == l2Db->pEntry[L2_ENTRY_UNIQUE_IDX(unit, index_entry)].groupId)
    {
        if (l2_entry.un.l2mcast.local_fwd)
            fwdTableIdx = l2_entry.un.l2mcast.index;

        /* fill content */
        if (l2_entry.un.l2mcast.remote_fwd)
        {
            /* only clear normal ports if ECID list is activated*/
            l2_entry.un.l2mcast.local_fwd  = 0;
    ...
    else
    {
        ...
        ret = _dal_mango_mcast_l2mc_unbind(unit, l2Db->pEntry[l2_index].groupId, &l2Db->pEntry[l2_index].mcastNode);
        if (ret != RT_ERR_OK)
        {
            _dal_mango_switch_tblSemaphore_unlock(unit, DAL_MANGO_SWITCH_TBL_L2_MAC);
            L2_SEM_UNLOCK(unit);
        ...
        l2Db->pEntry[l2_index].groupId = 0;
...
dal_mango_l2_mcastAddr_get(uint32 unit, rtk_l2_mcastAddr_t *pMcastAddr)
{
    ...
    osal_memset(&l2_entry, 0, sizeof(dal_mango_l2_entry_t));
    l2_entry.entry_type     = L2_MULTICAST;
    l2_entry.un.l2mcast.fid    = pMcastAddr->rvid;
    osal_memcpy(&l2_entry.un.l2mcast.mac, &pMcastAddr->mac, sizeof(rtk_mac_t));
    ...
    if (l2_entry.un.l2mcast.local_fwd == 0)
    {
        L2_SEM_UNLOCK(unit);
        return RT_ERR_L2_ENTRY_NOTFOUND;
    }
    ...
    /* fill content */
    pMcastAddr->fwdIndex   = l2_entry.un.l2mcast.index;
    pMcastAddr->nextHop    = l2_entry.un.l2mcast.nh;
    ...
    pMcastAddr->groupId = l2Db->pEntry[pMcastAddr->l2_idx].groupId;
...
dal_mango_l2_mcastAddr_set(uint32 unit, rtk_l2_mcastAddr_t *pMcastAddr)
{
    ...
    if (pMcastAddr->nextHop || nh_used_cnt[unit][pMcastAddr->l2_idx] == 0)
        l2_entry.un.l2mcast.nh = pMcastAddr->nextHop;

    /* group process */
    if ((pMcastAddr->groupId != l2Db->pEntry[pMcastAddr->l2_idx].groupId) || (0 == pMcastAddr->groupId))
    {
        /* unbind the old group if it exists */
        if (l2Db->pEntry[pMcastAddr->l2_idx].groupId > 0)
        {
            ret = _dal_mango_mcast_l2mc_unbind(unit, l2Db->pEntry[pMcastAddr->l2_idx].groupId, &l2Db->pEntry[pMcastAddr->l2_idx].mcastNode);
            if (ret != RT_ERR_OK)
            {
                _dal_mango_switch_tblSemaphore_unlock(unit, DAL_MANGO_SWITCH_TBL_L2_MAC);
                L2_SEM_UNLOCK(unit);
        ...
        /* update database */
        ori_group = l2Db->pEntry[pMcastAddr->l2_idx].groupId;
        l2Db->pEntry[pMcastAddr->l2_idx].groupId = pMcastAddr->groupId;
        if (l2Db->pEntry[pMcastAddr->l2_idx].groupId > 0)
        {
            /* bind to the new group id */
            ret = _dal_mango_mcast_l2mc_bind(unit, l2Db->pEntry[pMcastAddr->l2_idx].groupId, &l2Db->pEntry[pMcastAddr->l2_idx].mcastNode, &mcastL2Info);
            if (ret != RT_ERR_OK)
            {
                l2Db->pEntry[pMcastAddr->l2_idx].groupId = ori_group;
                _dal_mango_switch_tblSemaphore_unlock(unit, DAL_MANGO_SWITCH_TBL_L2_MAC);
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
            ...
            {
                l2_entry.un.l2mcast.local_fwd = 1;
                l2_entry.un.l2mcast.index = mcastL2Info.fwdIndex;
            } else {
                l2_entry.un.l2mcast.local_fwd = 0;
                l2_entry.un.l2mcast.index = 0;
            }

            l2_entry.un.l2mcast.remote_fwd = mcastL2Info.l2_tnl_lst_valid;
            l2_entry.un.l2mcast.tunnel_list_idx = mcastL2Info.l2_tnl_lst_idx;
        }
        else
        {
            /* traditional API (groupId = 0) */
            if ((ret = _dal_mango_l2_mcastFwdPortmaskEntry_get(unit, (int32)l2_entry.un.l2mcast.index, &portmask)) != RT_ERR_OK)
            {
                _dal_mango_switch_tblSemaphore_unlock(unit, DAL_MANGO_SWITCH_TBL_L2_MAC);
                L2_SEM_UNLOCK(unit);
                RT_LOG(ret, (MOD_DAL|MOD_L2), "");
            ...
            l2_entry.un.l2mcast.remote_fwd = 0;
            l2_entry.un.l2mcast.tunnel_list_idx = 0;
        }
    }
...
dal_mango_l2_mcastAddr_addByIndex(uint32 unit, rtk_l2_mcastAddr_t *pMcastAddr)
{
    ...
    osal_memset(&l2_entry, 0, sizeof(dal_mango_l2_entry_t));
    l2_entry.entry_type     = L2_MULTICAST;
    l2_entry.un.l2mcast.valid  = TRUE;
    l2_entry.un.l2mcast.fid    = pMcastAddr->rvid;
    osal_memcpy(&l2_entry.un.l2mcast.mac, &(pMcastAddr->mac), sizeof(rtk_mac_t));
    ...
    else if (ret == RT_ERR_L2_NO_EMPTY_ENTRY && (pMcastAddr->add_op_flags & RTK_L2_ADD_OP_FLAG_REPLACE_DYNAMIC) != 0)
    {
        ...
        osal_memcpy(&pMcastAddr->macInfo.mac, &l2_entry.un.unicast.mac, ETHER_ADDR_LEN);
        pMcastAddr->macInfo.devID      = l2_entry.un.unicast.slp >> RTL9310_L2ENTRY_DEVI_ID_OFFSET;
        pMcastAddr->macInfo.port      = l2_entry.un.unicast.slp & RTL9310_L2ENTRY_PORT_MASK;
        pMcastAddr->macInfo.vid       = l2_entry.un.unicast.fid;
        pMcastAddr->macInfo.flags     = l2_entry.un.unicast.is_trk ? RTK_L2_UCAST_FLAG_TRUNK_PORT : 0;
        pMcastAddr->macInfo.state     = l2_entry.un.unicast.suspending ? RTK_L2_UCAST_STATE_SUSPEND : 0;
        pMcastAddr->macInfo.trk_gid   = l2_entry.un.unicast.is_trk ? (l2_entry.un.unicast.slp & 0xff) : 0;
    }
    else if (RT_ERR_OK != ret)
    {
        _dal_mango_switch_tblSemaphore_unlock(unit, DAL_MANGO_SWITCH_TBL_L2_MAC);
    ...
    {
        if (ret == RT_ERR_OK && l2_entry.is_entry_exist && l2_entry.un.l2mcast.local_fwd)
        {
    ...
    pMcastAddr->l2_idx = L2_ENTRY_UNIQUE_IDX(unit, index_entry);
    l2_entry.un.l2mcast.nh = nh_used_cnt[unit][pMcastAddr->l2_idx] ? 1 : 0;

    /* group process */
    if ((l2Db->pEntry[pMcastAddr->l2_idx].groupId != pMcastAddr->groupId) || (0 == pMcastAddr->groupId))
    {
        ...
        if (l2Db->pEntry[pMcastAddr->l2_idx].groupId > 0)
        {
            ...
            /* traditional API (groupId = 0) */
            l2_entry.un.l2mcast.index      = (uint32)pMcastAddr->fwdIndex;
            l2_entry.un.l2mcast.local_fwd  = 1;
            l2_entry.un.l2mcast.remote_fwd = 0;
            l2_entry.un.l2mcast.tunnel_list_idx = 0;
        }
    ...
    rtk_l2_mcastAddr_t  *pMcastAddr)
{
    ...
    osal_memcpy(pMcastAddr, &entry.un.l2mcast, sizeof(rtk_l2_mcastAddr_t));
...
dal_mango_l2_cpuMacAddr_add(uint32 unit, rtk_vlan_t vid, rtk_mac_t *pMac)
{
    ...
    l2_entry.un.unicast.fid        = vid;
    l2_entry.un.unicast.is_trk     = 0;
    l2_entry.un.unicast.slp        = HWP_CPU_MACID(unit);
    l2_entry.un.unicast.aging      = 7;
    ...
    rtk_l2_portMoveAct_t    *pAction)
{
    ...
    switch (type)
    {
        case L2_PORT_MOVE_DYNAMIC:
            if ((ret = reg_array_field_read(unit, MANGO_L2_PORT_DYN_MV_ACTr, pAction->un.dynAct.port, REG_ARRAY_INDEX_NONE, MANGO_ACTf, &value)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
    ...
    {
        case L2_PORT_MOVE_DYNAMIC:
            pAction->un.dynAct.act = action;
            break;
        case L2_PORT_MOVE_STATIC:
            pAction->un.sttAct.act = action;
            break;
        case L2_PORT_MOVE_FORBID:
            pAction->un.forbidAct.act = action;
            break;
        default:
            return RT_ERR_INPUT;
    }
    ...
    rtk_l2_portMoveAct_t        *pAction)
{
    ...
    {
        case L2_PORT_MOVE_DYNAMIC:
            action= pAction->un.dynAct.act;
            break;
        case L2_PORT_MOVE_STATIC:
            action= pAction->un.sttAct.act;
            break;
        case L2_PORT_MOVE_FORBID:
            action= pAction->un.forbidAct.act;
            break;
        default:
            return RT_ERR_INPUT;
    }
    ...
    {
        case L2_PORT_MOVE_DYNAMIC:
            if ((ret = reg_array_field_write(unit, MANGO_L2_PORT_DYN_MV_ACTr, pAction->un.dynAct.port, REG_ARRAY_INDEX_NONE, MANGO_ACTf, &value)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
    ...
    rtk_l2_portMoveLrn_t        *pLearn)
{
    ...
    {
        case L2_PORT_MOVE_DYNAMIC:
            if ((ret = reg_array_field_read(unit, MANGO_L2_PORT_DYN_MV_LRNr, pLearn->un.dynLrn.port, REG_ARRAY_INDEX_NONE, MANGO_LRNf, &value)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
    ...
    rtk_l2_portMoveLrn_t        *pLearn)
{
    ...
    {
        case L2_PORT_MOVE_DYNAMIC:
            lrn = pLearn->un.dynLrn.enable;
            break;
        case L2_PORT_MOVE_STATIC:
            lrn = pLearn->un.sttLrn.enable;
            break;
        default:
            return RT_ERR_INPUT;
    }
    ...
    {
        case L2_PORT_MOVE_DYNAMIC:
            if ((ret = reg_array_field_write(unit, MANGO_L2_PORT_DYN_MV_LRNr, pLearn->un.dynLrn.port, REG_ARRAY_INDEX_NONE, MANGO_LRNf, &value)) != RT_ERR_OK)
            {
                L2_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_L2), "");
                return ret;
 ...
 *          pL2_entry->un.unicast.trk_gid is valid trunk id value.
 */
int32
dal_mango_l2_addrEntry_get(uint32 unit, uint32 index, rtk_l2_entry_t *pL2_entry)
{
    ...
    if (pL2_entry->entry_type == FLOW_TYPE_UNICAST)
    {
        ...
        if (l2_entry.un.unicast.is_trk)
        {
            ...
            pL2_entry->un.unicast.trk_gid  = l2_entry.un.unicast.slp & 0xff;
        }
        else
        {
            pL2_entry->un.unicast.devID    = l2_entry.un.unicast.slp >> RTL9310_L2ENTRY_DEVI_ID_OFFSET;
            pL2_entry->un.unicast.port     = l2_entry.un.unicast.slp & RTL9310_L2ENTRY_PORT_MASK;
        }
        ...
        if (l2_entry.un.unicast.tagSts)
            pL2_entry->un.unicast.flags    |= RTK_L2_UCAST_FLAG_TAG_STS;
        pL2_entry->un.unicast.agg_pri      = l2_entry.un.unicast.agg_pri;
        pL2_entry->un.unicast.agg_vid      = l2_entry.un.unicast.agg_vid;
        pL2_entry->un.unicast.ecid         = l2_entry.un.unicast.ecid;
        if (l2_entry.un.unicast.l2_tnl_if)
            pL2_entry->un.unicast.flags    |= RTK_L2_UCAST_FLAG_L2_TUNNEL;
        pL2_entry->un.unicast.l2_tunnel_idx = l2_entry.un.unicast.l2_tnl_idx;
        pL2_entry->un.unicast.age          = l2_entry.un.unicast.aging;

        pL2_entry->un.unicast.l2_idx = L2_ENTRY_UNIQUE_IDX(unit, index_entry);
    }
    ...
    if (pL2_entry->entry_type == FLOW_TYPE_L2_MULTI)
    {
        ...
        L2_SEM_LOCK(unit);
        if ((ret = _dal_mango_l2_mcastFwdPortmaskEntry_get(unit, pL2_entry->un.l2mcast.fwdIndex, &pL2_entry->un.l2mcast.portmask)) != RT_ERR_OK)
        {
            L2_SEM_UNLOCK(unit);
            RT_LOG(ret, (MOD_DAL|MOD_L2), "");
            return ret;
        ...
        pL2_entry->un.l2mcast.l2_idx = L2_ENTRY_UNIQUE_IDX(unit, index_entry);
    }
        ...
        if (l2_entry.is_entry_valid == 1)
        {
            ...
            if (l2_entry.entry_type == L2_UNICAST)
            {
                ...
                if (l2_entry.un.unicast.is_trk)
                {
                    ...
                    (pCfAddrList + cf_num)->un.unicast.trk_gid = l2_entry.un.unicast.slp & 0xff;
                }
                else
                {
                    (pCfAddrList + cf_num)->un.unicast.devID   = l2_entry.un.unicast.slp >> RTL9310_L2ENTRY_DEVI_ID_OFFSET;
                    (pCfAddrList + cf_num)->un.unicast.port    = l2_entry.un.unicast.slp & RTL9310_L2ENTRY_PORT_MASK;
                }
                ...
                if (l2_entry.un.unicast.tagSts)
                    (pCfAddrList + cf_num)->un.unicast.flags   |= RTK_L2_UCAST_FLAG_TAG_STS;
                (pCfAddrList + cf_num)->un.unicast.agg_pri     = l2_entry.un.unicast.agg_pri;
                (pCfAddrList + cf_num)->un.unicast.agg_vid     = l2_entry.un.unicast.agg_vid;
                (pCfAddrList + cf_num)->un.unicast.ecid        = l2_entry.un.unicast.ecid;
                if (l2_entry.un.unicast.l2_tnl_if)
                    (pCfAddrList + cf_num)->un.unicast.flags   |= RTK_L2_UCAST_FLAG_L2_TUNNEL;
                (pCfAddrList + cf_num)->un.unicast.l2_tunnel_idx = l2_entry.un.unicast.l2_tnl_idx;
            ...
            else if (l2_entry.entry_type == L2_MULTICAST)
            {
                ...
                if ((ret = _dal_mango_l2_mcastFwdPortmaskEntry_get(unit, (pCfAddrList + cf_num)->un.l2mcast.fwdIndex, &(pCfAddrList + cf_num)->un.l2mcast.portmask)) != RT_ERR_OK)
                {
                    L2_SEM_UNLOCK(unit);
                    RT_LOG(ret, (MOD_DAL|MOD_L2), "");
                    return ret;
                ...
                /* group id */
                (pCfAddrList + cf_num)->un.l2mcast.groupId = l2Db->pEntry[((pCfAddrList + cf_num)->un.l2mcast.l2_idx)].groupId;
            }
            else
            {
                L2_SEM_UNLOCK(unit);
    ...
    rtk_l2_entry_t  *pEntry)
{
    ...
    if (type < L2_NEXT_VALID_TYPE_MC)
    {
        ...
        if (l2_entry.un.unicast.is_trk)
        {
            ...
            pEntry->un.unicast.trk_gid   = l2_entry.un.unicast.slp & 0xff;
        }
        else
        {
            pEntry->un.unicast.devID     = l2_entry.un.unicast.slp >> RTL9310_L2ENTRY_DEVI_ID_OFFSET;
            pEntry->un.unicast.port      = l2_entry.un.unicast.slp & RTL9310_L2ENTRY_PORT_MASK;
        }
        if (l2_entry.un.unicast.sablock)
        ...
        if (l2_entry.un.unicast.tagSts)
            pEntry->un.unicast.flags     |= RTK_L2_UCAST_FLAG_TAG_STS;
        pEntry->un.unicast.agg_pri = l2_entry.un.unicast.agg_pri;
        pEntry->un.unicast.agg_vid = l2_entry.un.unicast.agg_vid;
        pEntry->un.unicast.ecid    = l2_entry.un.unicast.ecid;
        if (l2_entry.un.unicast.l2_tnl_if)
            pEntry->un.unicast.flags |= RTK_L2_UCAST_FLAG_L2_TUNNEL;
        pEntry->un.unicast.l2_tunnel_idx = l2_entry.un.unicast.l2_tnl_idx;
    ...
    else
    {
        ...
        pEntry->un.l2mcast.groupId     = l2Db->pEntry[(pEntry->un.l2mcast.l2_idx)].groupId;

        L2_SEM_LOCK(unit);
        if ((ret = _dal_mango_l2_mcastFwdPortmaskEntry_get(unit, pEntry->un.l2mcast.fwdIndex, &pEntry->un.l2mcast.portmask)) != RT_ERR_OK)
        {
            L2_SEM_UNLOCK(unit);
            RT_LOG(ret, (MOD_DAL|MOD_L2), "");
            return ret;
...
static int32
_dal_mango_l2_addr_delByMac(uint32 unit, rtk_mac_t *pMac, uint32 sram_if)
{
    int32 ret;
    ...
    while(scan_idx < tbl_size)
    {
        ...
        if (!osal_memcmp(pMac, &l2_mac, sizeof(rtk_mac_t)) && ((0 == next_hop) || (0 == is_mc)))
        {
            if (1 == next_hop)
            {
 ...
 */
int32
dal_mango_l2_addr_delByMac(uint32 unit, rtk_mac_t *pMac)
{
    int32 ret;
    uint32 value;
    uint32 sram_if = 1;
    ...
    if((ret = _dal_mango_l2_addr_delByMac(unit, pMac, sram_if)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_L2), "");
        return ret;
    }
