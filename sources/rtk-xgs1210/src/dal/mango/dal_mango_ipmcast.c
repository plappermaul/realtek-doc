    ...
    dal_mango_ipmc_l3Entry_t *pEntry)
{
    ...
    {
        RT_ERR_HDL(_dal_mango_l3_routeEntry_get(unit, pEntry->hw_entry_idx, &routeEntry, \
            DAL_MANGO_L3_API_FLAG_NONE), errHandle, ret);
    ...
    dal_mango_ipmc_l3Entry_t *pEntry)
{
    ...
    {
        if ((RT_ERR_OK != _dal_mango_l3_routeEntry_get(unit, pEntry->hw_entry_idx, &routeEntry, \
                                                      DAL_MANGO_L3_API_FLAG_NONE)) || \
...
dal_mango_ipmc_init(uint32 unit)
{
    ...
    {
        MANGO_IPMCAST_DBG_PRINTF(3, "Entry-Node: idx = %u (L3_ENTRY_ADDR_TO_IDX), pNode = %p, pPrev = %p, pNext = %p\n", \
            L3_ENTRY_ADDR_TO_IDX(unit, pEntry), pEntry, pEntry->nodeRef.pPrev, pEntry->nodeRef.pNext);
    }
#endif
...
dal_mango_ipmc_addr_add(uint32 unit, rtk_ipmc_addr_t *pIpmcAddr)
{
    ...
    if (NULL != pEntry)
    {
        ...
        if (pIpmcAddr->ipmc_flags & RTK_IPMC_FLAG_REPLACE)
        {
            ...
            else
            {
                ...
                RT_ERR_HDL(_dal_mango_l3_routeEntry_set(unit, pEntry->hw_entry_idx, &routeEntry, \
                    DAL_MANGO_L3_API_FLAG_NONE), errHandle, ret);
            }
#endif
            /* update entry info */
    ...
    else
    {
        ...
        else
        {
            ...
            RT_ERR_HDL(_dal_mango_l3_routeEntry_alloc(unit, hostAlloc.width, &routeIdx, \
                DAL_MANGO_L3_API_FLAG_NONE), errHandle, ret);
...
errHandle:
    ...
    if (TRUE == routeEntryAllocated)
        IPMCAST_RT_ERR_HDL_DBG(_dal_mango_l3_routeEntry_free(unit, routeIdx, DAL_MANGO_L3_API_FLAG_NONE), "");
#endif
...
dal_mango_ipmc_addr_del(uint32 unit, rtk_ipmc_addr_t *pIpmcAddr)
{
    ...
    else
    {
        ...
        RT_ERR_HDL(_dal_mango_l3_routeEntry_free(unit, pEntry->hw_entry_idx, \
            DAL_MANGO_L3_API_FLAG_NONE), errHandle, ret);
    }
#endif
