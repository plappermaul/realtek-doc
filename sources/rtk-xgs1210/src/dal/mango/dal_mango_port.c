...
{
    int32   ret;
    uint32  val, tx_en, rx_en;
    rtk_port_linkStatus_t mac_link, phy_link;
    ...
    /* Get TX_EN and RX_EN Conguration */
    if ((ret = reg_array_field_read(unit, MANGO_MAC_L2_PORT_CTRLr, port, REG_ARRAY_INDEX_NONE, MANGO_TX_ENf, &tx_en)) != RT_ERR_OK)
    {
        PORT_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_PORT), "");
        return ret;
    }
    if ((ret = reg_array_field_read(unit, MANGO_MAC_L2_PORT_CTRLr, port, REG_ARRAY_INDEX_NONE, MANGO_RX_ENf, &rx_en)) != RT_ERR_OK)
    {
        PORT_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_PORT), "");
        return ret;
    ...
    /* Set force link up */
    val = 0;
    if ((ret = reg_array_field_write(unit, MANGO_MAC_L2_PORT_CTRLr, port, REG_ARRAY_INDEX_NONE, MANGO_TX_ENf, &val)) != RT_ERR_OK)
    {
        PORT_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_PORT), "");
        return ret;
    }
    if ((ret = reg_array_field_write(unit, MANGO_MAC_L2_PORT_CTRLr, port, REG_ARRAY_INDEX_NONE, MANGO_RX_ENf, &val)) != RT_ERR_OK)
    {
        PORT_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_PORT), "");
        return ret;
    ...
    if ((ret = reg_array_field_write(unit, MANGO_MAC_L2_PORT_CTRLr, port, REG_ARRAY_INDEX_NONE, MANGO_TX_ENf, &tx_en)) != RT_ERR_OK)
    {
        PORT_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_PORT), "");
        return ret;
    }
    if ((ret = reg_array_field_write(unit, MANGO_MAC_L2_PORT_CTRLr, port, REG_ARRAY_INDEX_NONE, MANGO_RX_ENf, &rx_en)) != RT_ERR_OK)
    {
        PORT_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_PORT), "");
        return ret;
    ...
    if ((ret = dal_mango_port_link_get(unit, port, &mac_link)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_PORT), "");
        return ret;
    }
    if ((ret = dal_phy_portLinkStatus_get(unit, port, &phy_link)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_PORT), "");
        return ret;
    }
    if ((mac_link == PORT_LINKDOWN) && (phy_link == PORT_LINKUP))
    {
...
dal_mango_port_init(uint32 unit)
{
    ...
    HWP_PHY_TRAVS(unit, phy)
    {
        ...
        {
            RT_ERR_CHK(phy_macIntfSerdesLinkStatus_get(unit, port, &phySdslinkInfo), ret);
        }
        else
            continue;
