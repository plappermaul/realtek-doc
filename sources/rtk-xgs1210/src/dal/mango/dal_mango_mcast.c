...
_dal_mango_mcast_l2mc_update(uint32 unit, dal_mango_l2_index_t l2Idx, int32 l2_fwdIndex, dal_mango_mcast_l2TnlNode_t *pNode)
{
    ...
    {
        if ((ret = table_read(unit, MANGO_L2_MCt, ((l2Idx.index << 2) | (l2Idx.hashdepth)), (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
    ...
    if (l2Idx.index_type == L2_IN_HASH)
    {
        if ((ret = table_write(unit, MANGO_L2_MCt, ((l2Idx.index << 2) | (l2Idx.hashdepth)), (uint32 *)&l2_entry)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_L2), "");
            goto errTableWrite;
        }
    ...
    MANGO_MCAST_DBG_PRINTF(1, "\n-----------------------------------------------------------------\n"
                              ...
                              "-----------------------------------------------------------------\n",
        l2Idx.index, l2_local_fwd_valid, l2_local_fwd_idx, l2_remote_fwd_valid, l2_tunnel_lst_idx);
...
_dal_mango_mcast_l2mc_bind(uint32 unit, rtk_mcast_group_t groupId, dal_mango_mcast_l2Node_t *pNode, dal_mango_mcast_l2Info_t *pL2Info)
{
    ...
    if (RTK_LIST_LENGTH(&pGroup->mac.l2_entry_list) > 0)
    {
...
_dal_mango_mcast_macEgrIfVxlan_add(uint32 unit, dal_mango_mcast_group_t *pGroup, rtk_mcast_egrif_t *pEgrIf)
{
    ...
    {
        MANGO_MCAST_DBG_PRINTF(3, "pGroup's head_node_idx = %u\n", L2TUNNEL_NODE_ADDR_TO_IDX(unit, RTK_LIST_NODE_HEAD(&pGroup->mac.l2_tunnel_list)));
    }
...
_dal_mango_mcast_ipmc_unbind(uint32 unit, rtk_mcast_group_t groupId, uint32 entryIdx)
{
    ...
    MANGO_MCAST_DBG_PRINTF(1, "unbind (%u) failed!\n", entryIdx);
    ...
    case RTK_MCAST_TYPE_IP:
        {
            ...
            {
                MANGO_MCAST_DBG_PRINTF(3, "pOil = %p, oilIdx = %u\n", pOil, OIL_ENTRY_ADDR_TO_IDX(unit, pOil));
