 ...
 *
 * $Revision: 97138 $
 * $Date: 2019-05-23 19:07:36 +0800 (Thu, 23 May 2019) $
 *
    ...
    rtk_oam_cfmInstMember_t *member)
{
    ...
    {
        RT_PARAM_CHK(!HWP_PORT_EXIST(unit, member->member0_port), RT_ERR_PORT_ID);
    ...
    {
        RT_PARAM_CHK(!HWP_PORT_EXIST(unit, member->member1_port), RT_ERR_PORT_ID);
    ...
    rtk_oam_cfmInstMember_t *member)
{
    ...
    if (DISABLED == member->member0_truk_present)
    {
        RT_PARAM_CHK((member->member0_port > RTK_MAX_PORT_ID_PER_UNIT), RT_ERR_PORT_ID);
    }
    else
    {
        RT_PARAM_CHK(member->member0_port >= HAL_MAX_NUM_OF_TRUNK(unit), RT_ERR_LA_TRUNK_ID);
    ...
    if (DISABLED == member->member1_truk_present)
    {
        RT_PARAM_CHK((member->member1_port > RTK_MAX_PORT_ID_PER_UNIT), RT_ERR_PORT_ID);
    }
    else
    {
        RT_PARAM_CHK(member->member1_port >= HAL_MAX_NUM_OF_TRUNK(unit), RT_ERR_LA_TRUNK_ID);
