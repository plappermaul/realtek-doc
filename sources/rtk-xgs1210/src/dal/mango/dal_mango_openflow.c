 ...
 *
 * $Revision: 96821 $
 * $Date: 2019-05-13 16:17:05 +0800 (Mon, 13 May 2019) $
 *
...
_dal_mango_of_igrFTInstruct_get(uint32 unit, rtk_of_flow_id_t entry_idx, dal_mango_of_igrFtIns_t *pIns)
{
    ...
    OF_SEM_LOCK(unit);
    if ((ret = table_read(unit, tblName, entry_idx, (uint32 *)&insEntry)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_OPENFLOW), "");
        return ret;
    }
...
_dal_mango_of_igrFTInstruct_set(uint32 unit, rtk_of_flow_id_t entry_idx, dal_mango_of_igrFtIns_t *pIns)
{
    ...
    OF_SEM_LOCK(unit);
    if ((ret = table_write(unit, tblName, entry_idx, (uint32 *)&insEntry)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_DAL|MOD_OPENFLOW), "");
        return ret;
    }
    ...
    uint32              		*pValid)
{
    ...
    OF_SEM_LOCK(unit);
    if ((ret = table_read(unit, table, phy_entry_idx, (uint32 *) &entry)) != RT_ERR_OK)
    {
        OF_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_OPENFLOW), "");
        return ret;
    ...
    uint32              		valid)
{
    ...
    if ((ret = table_write(unit, table, phy_entry_idx, (uint32 *) &entry)) != RT_ERR_OK)
    {
        OF_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_OPENFLOW), "");
        return ret;
