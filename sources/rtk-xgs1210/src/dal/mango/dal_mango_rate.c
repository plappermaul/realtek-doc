...
static uint32               assure_rate[RTK_MAX_NUM_OF_UNIT][RTK_MAX_NUM_OF_PORTS][RTK_MAX_NUM_OF_QUEUE];
...
_dal_mango_rate_portBurstSize_check(uint32 unit, rtk_port_t port, rtk_rate_module_t type, uint32 burst)
{
    ...
    else if (type == RATE_MODULE_EGR)
    {
        ...
        {
            /* CPU port */
            /* get pps or bps mode from CHIP */
            if ((ret = reg_field_read(unit, MANGO_EGBW_CTRLr, MANGO_RATE_MODE_CPUf, &mode)) != RT_ERR_OK)
            {
                RATE_SEM_UNLOCK(unit);
                RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
                return ret;
            }
            reg = ((mode == 1) ? MANGO_EGBW_LB_CTRLr : MANGO_EGBW_CPU_PPS_LB_CTRLr);
            field = MANGO_TKNf;
        }
        else
        {
...
dal_mango_rate_portEgrBwCtrlRate_get(uint32 unit, rtk_port_t port, uint32 *pRate)
{
    ...
    /* read value from CHIP*/
    if ((ret = reg_array_field_read(unit, MANGO_EGBW_PORT_CTRLr, port, REG_ARRAY_INDEX_NONE, MANGO_RATEf, pRate)) != RT_ERR_OK)
    {
        RATE_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_DAL|MOD_RATE), "");
        return ret;
...
#if 1 /* [MANGO-3416]-START */
    *pRate = assure_rate[unit][port][queue];
#else
    /* get value from CHIP */
    osal_memset(&egrQEntry, 0, sizeof(egr_qBw_entry_t));
...
dal_mango_rate_portEgrQueueAssuredBwCtrlRate_set(uint32 unit, rtk_port_t port, rtk_qid_t queue, uint32 rate)
{
    ...
    /* [MANGO-3416]-START */
    assure_rate[unit][port][queue] = rate;
    ...
    rtk_rate_assuredMode_t    mode)
{
    ...
    /* [MANGO-3416]-START */
    if (mode == ASSURED_MODE_SHARE)
        rate = assure_rate[unit][port][queue] + 1;
    else
        rate = assure_rate[unit][port][queue];
