 ...
 *
 * $Revision: 97830 $
 * $Date: 2019-06-19 18:43:08 +0800 (Wed, 19 Jun 2019) $
 *
...
#define WAIT_LOCAL_TRUNK_REFRESH_DONE(val) \
do{ \
...
int32 dal_mango_trunk_mon_register(uint32 unit, rtk_trunk_mon_callback_t trunkMon_callback)
{
    ...
    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_PORT), "trunkMon_callback=%x",
           trunkMon_callback);
...
dal_mango_trunk_mode_set(uint32 unit, rtk_trunk_mode_t mode)
{
    ...
    WAIT_LOCAL_TRUNK_REFRESH_DONE(val) ;
...
dal_mango_trunk_port_set(uint32 unit, uint32 trk_gid, rtk_portmask_t *pTrunk_member_portmask)
{
    ...
    RT_LOG(LOG_DEBUG, (MOD_DAL|MOD_TRUNK), "unit=%d, trk_gid=%d pTrunk_member_portmask pmsk0=0x%x, pmsk1=0x%x", unit, trk_gid,
         RTK_PORTMASK_WORD_GET(*pTrunk_member_portmask, 0),
         RTK_PORTMASK_WORD_GET(*pTrunk_member_portmask, 1));
    ...
    ret = _dal_mango_trunk_localPort_get (unit, trk_gid, &origin_mbr);
    if(RT_ERR_OK != ret)
    {
        TRUNK_SEM_UNLOCK(unit);
        return ret;
    }
    ret = _dal_mango_trunk_localPort_set (unit, trk_gid, pTrunk_member_portmask);
    if(RT_ERR_OK != ret)
    {
        TRUNK_SEM_UNLOCK(unit);
        return ret;
    ...
    {
        if (RTK_PORTMASK_IS_PORT_SET(origin_mbr, port)) /*remove source port mapping*/
        {
            devPort.port = port;
            ret = _dal_mango_trunk_srcPortMap_set(unit, devPort, 0, 0);
            if(RT_ERR_OK != ret)
            {
                TRUNK_SEM_UNLOCK(unit);
                return ret;
        ...
        if (RTK_PORTMASK_IS_PORT_SET(*pTrunk_member_portmask, port))
        {
            trk_egr_ports.egr_port[trk_egr_ports.num_ports].devID = unit;
            trk_egr_ports.egr_port[trk_egr_ports.num_ports].port = port;
            ret = _dal_mango_trunk_srcPortMap_set(unit, trk_egr_ports.egr_port[trk_egr_ports.num_ports],
                1, trk_gid);
            if(RT_ERR_OK != ret)
            {
                TRUNK_SEM_UNLOCK(unit);
            ...
            trk_egr_ports.num_ports ++;
        }
    }
    for(portIdx = trk_egr_ports.num_ports; portIdx < 8; portIdx ++)
    {
        trk_egr_ports.egr_port[portIdx].devID = HAL_UNIT_TO_DEV_ID(unit);
        trk_egr_ports.egr_port[portIdx].port = INVALID_TRUNK_MEMBER_PORT;
    }

    ret = _dal_mango_trunk_egrPort_set (unit, trk_gid, &trk_egr_ports);
    if(RT_ERR_OK != ret)
    {
        TRUNK_SEM_UNLOCK(unit);
        return ret;
...
dal_mango_trunk_srcPortMap_get(uint32 unit,  rtk_dev_port_t devPort, uint32 *pIsTrkMbr, rtk_trk_t *pTrk_gid)
{
    ...
    if ((ret = table_read(unit, MANGO_SRC_TRK_MAPt, devPort.devID*64+devPort.port, (uint32 *) &lag_src_map_entry)) != RT_ERR_OK)
    {
        TRUNK_SEM_UNLOCK(unit);
        RT_ERR(ret, (MOD_TRUNK|MOD_DAL), "");
        return ret;
...
_dal_mango_trunk_srcPortMap_set(uint32 unit, rtk_dev_port_t devPort, uint32 isTrkMbr, rtk_trk_t trk_gid)
{
    ...
    if ((ret = table_write(unit, MANGO_SRC_TRK_MAPt, devPort.devID*64+devPort.port, (uint32 *) &lag_src_map_entry)) != RT_ERR_OK)
    {
        RT_ERR(ret, (MOD_TRUNK|MOD_DAL), "");
        return ret;
    }
