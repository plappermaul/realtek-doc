 ...
 *
 * $Revision: 92818 $
 * $Date: 2018-10-12 17:11:06 +0800 (Fri, 12 Oct 2018) $
 *
        ...
        else if (HWP_PHY_EXIST(unit, mac_id))
        {
            ...
            switch (HWP_PHY_MODEL_BY_PORT(unit,mac_id))
            {
              ...
              case RTK_PHYTYPE_RTL8214FC:
                ...
                break;
              #endif
              #if (defined(CONFIG_SDK_RTL8208D) || defined(RTK_CONFIG_RTL8208D))
              case RTK_PHYTYPE_RTL8208G:
              case RTK_PHYTYPE_RTL8208D:
              #endif
              #if (defined(RTK_CONFIG_RTL8214))
...
void dal_maple_construct_macConfig_init(uint32 unit)
{
    ...
    switch(HWP_CHIP_ID(unit))
    {
        case RTL8330M_CHIP_ID:
                 ...
                 ioal_mem32_write(unit,0xa1c8, 0x3BCDEB38);
                 ioal_mem32_write(unit,0xa1cc, 0x62081EE );
                 ioal_mem32_write(unit,0xa1d0, 0x230398a4);
                 ioal_mem32_write(unit,0xa1d8, 0x0005a928);
...
static confcode_mac_regval_t rtl8380_serdes_reset[] = {
  {0xe780, 0x00000c00},{0xe980, 0x00000c00},{0xeb80, 0x00000c00},{0xed80, 0x00000c00},
  {0xef80, 0x00000c00},{0xf180, 0x00000c00},{0xe780, 0x00000c03},{0xe980, 0x00000c03},
  {0xeb80, 0x00000c03},{0xed80, 0x00000c03},{0xef80, 0x00000c03},{0xf180, 0x00000c03},
};
...
void dal_maple_construct_serdesConfig_init(uint32 unit)
{
    ...
    /* Serdes reset*/
    for (i=0; i<(sizeof(rtl8380_serdes_reset)/sizeof(confcode_mac_regval_t)); i++)
    {
        ioal_mem32_write(unit, rtl8380_serdes_reset[i].reg,  rtl8380_serdes_reset[i].val);
        osal_time_mdelay(1);
    }
