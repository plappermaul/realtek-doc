...
#endif
void _dal_linkMon_linkChange(uint32 unit, void *pChanged_portmask);
...
int32 dal_linkMon_devInit(uint32 unit)
{
    ...
    if (( ret = drv_intr_link_stat_register(_dal_linkMon_linkChange)) != RT_ERR_OK)
    {
 ...
 *      isr_param           - the changed portmask in this unit
 * Output:
 *      None
 * Return:
 *      None
 ...
 *
 */
void _dal_linkMon_linkChange(uint32 unit, void *isr_param)
{
    ...
    else
    {
        ...
        RTK_PORTMASK_OR(pLinkMon_cb->link_change_portmask[unit], changed_portmask);
...
dal_linkMon_swChgSerdesMode(uint32 unit, rtk_port_t port)
{
    ...
    if (HWP_9300_FAMILY_ID(unit))
    {
        ...
        _dal_longan_port_swAutoDetectSdsMode(unit, port);
    }
    #endif
}
