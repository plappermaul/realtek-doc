 ...
 *
 * $Revision: 98690 $
 * $Date: 2019-07-12 21:44:30 +0800 (Fri, 12 Jul 2019) $
 *
...
_phy_rtl9310_10gr_symErr_get(uint32 unit, uint32 sds, rtk_sds_symErr_t *info)
{
    ...
    switch (media)
    {
        ...
        case PORT_10GMEDIA_FIBER_10G:
            ...
            RT_ERR_CHK(SDS_FIELD_R(unit, aSds, 0x5, 1, 15, 14, &val), ret);
            info->latch_blk_lock = (val >> 1) & 0x1;
            info->latch_hiber = (val & 0x1);

            evenSds = sds - (sds % 2);
            RT_ERR_CHK(drv_rtl9310_sds2AnaSds_get(unit, evenSds, &evenASds), ret);
            RT_ERR_CHK(hal_serdes_reg_set(unit, evenASds, 0x1f, 0x02, 0x35), ret);
            RT_ERR_CHK(SDS_FIELD_R(unit, evenASds, 0x1f, 0x14, 3, 2, &val), ret);
...
_phy_rtl9310_dfe_leq_adapt(uint32 unit, uint32 sds, rtk_port_10gMedia_t media)
{
    ...
    RT_ERR_CHK(_phy_rtl9310_rx_rst(unit, sds), ret);
    phy_osal_time_mdelay(50);

    RT_ERR_CHK(SDS_FIELD_W(unit, asds, 0x2e, 0xd, 7, 7, 0), ret);
    phy_osal_time_mdelay(200);
...
_phy_rtl9310_sdsCustParam_set(uint32 unit, uint32 sds, rtk_port_10gMedia_t media)
{
    ...
    {
        RT_ERR_CHK(_phy_rtl9310_sdsTxParam_set(unit, sds,
                phy_rtl9310_tx_para[unit][sds][tx_para_idx], tx_para_idx), ret);
    }
...
_phy_rtl9310_rxCali(uint32 unit, uint32 sds)
{
    ...
    RT_ERR_CHK(reg_array_field_read(unit, MANGO_MAC_L2_PORT_CTRLr,
            port, REG_ARRAY_INDEX_NONE, MANGO_TX_ENf, &tx_en), ret);

    RT_ERR_CHK(reg_array_field_read(unit, MANGO_MAC_L2_PORT_CTRLr,
            port, REG_ARRAY_INDEX_NONE, MANGO_RX_ENf, &rx_en), ret);

    osal_time_mdelay(50);

    val = 0x0;
    RT_ERR_CHK(reg_array_field_write(unit, MANGO_MAC_L2_PORT_CTRLr,
            port, REG_ARRAY_INDEX_NONE, MANGO_TX_ENf, &val), ret);

    RT_ERR_CHK(reg_array_field_write(unit, MANGO_MAC_L2_PORT_CTRLr,
            port, REG_ARRAY_INDEX_NONE, MANGO_RX_ENf, &val), ret);
...
ERR:
    ...
    reg_array_field_write(unit, MANGO_MAC_L2_PORT_CTRLr,
            port, REG_ARRAY_INDEX_NONE, MANGO_TX_ENf, &tx_en);

    reg_array_field_write(unit, MANGO_MAC_L2_PORT_CTRLr,
            port, REG_ARRAY_INDEX_NONE, MANGO_RX_ENf, &rx_en);
...
_phy_rtl9310_linkUp_process(uint32 unit, rtk_port_t port)
{
    ...
    if (ret != RT_ERR_OK)
    {
        PHY_SDS_DBG("SDS%d Rx chk fail\n", sds);
        BITMAP_CLEAR(waMonRxCalied[unit], sds);
        return ret;
    }

    RT_ERR_CHK(drv_rtl9310_sds2XsgmSds_get(unit, sds, &dSds), ret);
    RT_ERR_CHK(SDS_FIELD_W(unit, dSds, 31, 1, 0, 0, 0x0), ret);

    #ifndef __BOOTLOADER__
    RT_ERR_CHK(dal_mango_stack_linkChange_handler(unit, port), ret); // this should be called when serdes clock is running
    #endif
...
phy_rtl9310_init(uint32 unit, rtk_port_t port)
{
    ...
    if (HWP_UNIT_VALID_LOCAL(unit))
    {
        ...
        RT_ERR_CHK(SDS_FIELD_W(unit, asds, 0x4, 13, 6, 6, 1), ret);
...
phy_rtl9310_fiberUnidirEnable_set(uint32 unit, rtk_port_t port, rtk_enable_t enable)
{
    ...
    //page 2, reg 0, bit 5: Unidirectional enable
...
_phy_rtl9310_pcb_adapt(uint32 unit, uint32 sds)
{
    ...
    for (loop = 0; loop < 3; ++loop)
    {
        ...
        for (link_loop = 0; link_loop < 5; ++link_loop)
        {
            RT_ERR_CHK(_phy_rtl9310_linkSts_get(unit, sds, &macSdsLinkInfo), ret);
            PHY_SDS_DBG("[0] SDS%d, 0x%x 0x%x\n", sds, macSdsLinkInfo.sts, macSdsLinkInfo.sts1);
            if ((macSdsLinkInfo.sts == 0x1FF) && (macSdsLinkInfo.sts1 == 0x1FF))
                break;
    ...
    SDS_FIELD_W(unit, asds, 0x2e, 0xe, 13, 11, 2);
