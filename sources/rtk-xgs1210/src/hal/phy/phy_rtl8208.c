...
phy_8208_init(uint32 unit, rtk_port_t port)
{
    ...
    {
        pPhy_rtctInfo[unit] = (fe_phy_rtct_info_t *)phy_osal_alloc(sizeof(fe_phy_rtct_info_t));
        if (NULL == pPhy_rtctInfo[unit])
        {
            RT_ERR(RT_ERR_FAILED, (MOD_HAL), "memory allocate failed");
            return RT_ERR_FAILED;
        }
        phy_osal_memset(pPhy_rtctInfo[unit], 0, sizeof(fe_phy_rtct_info_t));
    }


    if (NULL == autoNegoEnablePortMask[unit])
    {
        autoNegoEnablePortMask[unit] = (rtk_portmask_t *)phy_osal_alloc(sizeof(rtk_portmask_t));
        if (NULL == autoNegoEnablePortMask[unit])
        {
            RT_ERR(RT_ERR_FAILED, (MOD_HAL), "memory allocate failed");
            return RT_ERR_FAILED;
        }
        phy_osal_memset(autoNegoEnablePortMask[unit], 0, sizeof(rtk_portmask_t));
    }
...
phy_8208d_init(uint32 unit, rtk_port_t port)
{
    ...
    {
        fiber_pmask[unit] = phy_osal_alloc(sizeof(rtk_portmask_t));
        if (fiber_pmask[unit] != NULL)
            phy_osal_memset(fiber_pmask[unit], 0, sizeof(rtk_portmask_t));
        else
