 ...
 *
 * $Revision: 96680 $
 * $Date: 2019-05-07 14:27:40 +0800 (Tue, 07 May 2019) $
 *
...
int32 _phy_8218b_autoCrossOver_EEEwakeupTxTimer_set(uint32 unit, rtk_port_t port)
{
    ...
    for (i = 0; i < portNum; ++i)
    {
        ...
        if ((val & 0x40) == 0)
        {
            phy_osal_printf("port %u patch ready fail %x\n", pid, val);
        }
...
phy_8218b_init(uint32 unit, rtk_port_t port)
{
    ...
    {
        pPhy_info[unit] = (hal_phy_info_t *)phy_osal_alloc(sizeof(hal_phy_info_t));
        if (NULL == pPhy_info[unit])
        {
            RT_ERR(RT_ERR_FAILED, (MOD_HAL), "memory allocate failed");
            return RT_ERR_FAILED;
        }
        phy_osal_memset(pPhy_info[unit], 0, sizeof(hal_phy_info_t));
    ...
    rtk_port_phy_ability_t *pAbility)
{
    ...
    if ((ret = phy_8214fc_autoNegoEnable_get(unit, port, &enable)) != RT_ERR_OK)
        return ret;
    ...
    rtk_enable_t enable)
{
    ...
    if(enable == ENABLED)
    {
        ...
        phy_osal_time_usleep(100 * 1000); /* delay 10mS */
    }
...
phy_8218b_ieeeTestMode_set(uint32 unit, rtk_port_t port, rtk_port_phyTestMode_t *pTestMode)
{
    ...
    phy_osal_printf("RTL8218B/14FC/18FB Test Mode (PHYID: %u)\n", port);
