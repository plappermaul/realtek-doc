 ...
 *
 * $Revision: 96721 $
 * $Date: 2019-05-08 11:51:19 +0800 (Wed, 08 May 2019) $
 *
...
phy_8295_rxCali_start(uint32 unit, uint32 basePort, uint32 sdsId, phy_8295_rxCaliParam_t *param)
{
    ...
    {
        phy_osal_memset(&rxCaliSds, 0, sizeof(phy_8295_rxCaliSds_t));
        rxCaliSds.unit = unit;
        rxCaliSds.port = basePort;
        rxCaliSds.mdxMacId = mdxMacId;
        rxCaliSds.sdsId = sdsId;

        phy_osal_memset(&rxCaliInfo, 0, sizeof(phy_8295_rxCaliInfo_t));
        phy_osal_memcpy(&rxCaliInfo.param, param, sizeof(phy_8295_rxCaliParam_t));
...
phy_8295r_txParam_init(uint32 unit, rtk_port_t port, rtk_port_10gMedia_t media, phy_8295r_txParam_t *tx_param)
{
    ...
    {
        phy_osal_memset(&gPhy8295rPI[unit], 0, sizeof(phy_8295r_patch_info_t));
...
phy_8295_rxCaliParamDefault_get(uint32 sdsId, phy_8295_rxCaliParam_t *param)
{
    ...
    phy_osal_memset(param, 0, sizeof(phy_8295_rxCaliParam_t));
...
phy_8295r_rxCaliConfPort_set(uint32 unit, uint32 port, phy_8295_rxCaliConf_t *conf)
{
    ...
    if (gPhy8295rPI[unit].prxCaliConf[port] == NULL)
    {
        ...
        gPhy8295rPI[unit].prxCaliConf[port] = phy_osal_alloc(sizeof(phy_8295_rxCaliConf_t));
        if (gPhy8295rPI[unit].prxCaliConf[port]  == NULL)
        {
    ...
    phy_osal_memcpy(gPhy8295rPI[unit].prxCaliConf[port], conf, sizeof(phy_8295_rxCaliConf_t));
...
phy_8295r_rxCaliConfPort_get(uint32 unit, uint32 port, phy_8295_rxCaliConf_t *conf)
{
    ...
    phy_osal_memcpy(conf, gPhy8295rPI[unit].prxCaliConf[port], sizeof(phy_8295_rxCaliConf_t));
    return RT_ERR_OK;
}
#endif
...
phy_8214qf_rxCaliConf_set(uint32 unit, rtk_port_t basePort, phy_8295_rxCaliConf_t *conf)
{
    ...
    {
        phy_osal_memset(&gPhy8214qfPI[unit], 0, sizeof(phy_8214qf_patch_info_t));
        phy8214qfPatch_init[unit] = INIT_COMPLETED;
    }

    if (gPhy8214qfPI[unit].prxCaliConf[basePort] == NULL)
    {
        RT_8295_LOG(LOG_INFO, (MOD_HAL|MOD_PORT), "unit=%u,port=%u,malloc", unit, basePort);
        gPhy8214qfPI[unit].prxCaliConf[basePort] = phy_osal_alloc(sizeof(phy_8295_rxCaliConf_t));
        if (gPhy8214qfPI[unit].prxCaliConf[basePort] == NULL)
        {
    ...
    phy_osal_memcpy(&gPhy8214qfPI[unit].prxCaliConf[basePort], conf, sizeof(phy_8295_rxCaliConf_t));
...
phy_8214qf_rxCaliConf_get(uint32 unit, rtk_port_t basePort, phy_8295_rxCaliConf_t *conf)
{
    ...
    phy_osal_memcpy(conf, &gPhy8214qfPI[unit].prxCaliConf[basePort], sizeof(phy_8295_rxCaliConf_t));
    return RT_ERR_OK;
}
#endif
