 ...
 *
 * $Revision: 98211 $
 * $Date: 2019-06-26 16:33:22 +0800 (Wed, 26 Jun 2019) $
 *
    ...
    rtk_port_linkStatus_t sts)
{
    ...
    if (ENABLED == en)
    {
        ...
        RT_ERR_CHK(reg_array_field_write(unit, MANGO_MAC_FORCE_MODE_CTRLr, port,
                REG_ARRAY_INDEX_NONE, MANGO_SMI_FORCE_LINKf, &val), ret);
    ...
    RT_ERR_CHK(reg_array_field_write(unit, MANGO_MAC_FORCE_MODE_CTRLr, port,
            REG_ARRAY_INDEX_NONE, MANGO_SMI_FORCE_LINK_ENf, &val), ret);
...
int32 rtl9310_smiAddr_init(uint32 unit)
{
    ...
    uint32      smiPrvte0_val[DRV_RTL9310_SMI_MAX] = { 0 };
    ...
    uint32      is_10g = 0, addr_reg0 = 0, addr_reg9 = 0, addr_reg10 = 0;
    uint32      value = 1;
        ...
        if ((smi_id = (uint32)HWP_PORT_SMI(unit, mac_id)) != HWP_NONE)
        {
            ...
            switch (HWP_PHY_MODEL_BY_PORT(unit, mac_id))
            {
                case RTK_PHYTYPE_CUST1:
                    is_10g = 1;
                    addr_reg0   = 0x01010000;
                    addr_reg9   = 0x01E7C400;
                    addr_reg10  = 0x01E7E820;
                    smiPrvte0_val[smi_id] = 0;  /* standard register */
                    break;
                default:
                    smiPrvte0_val[smi_id] = 1;  /* proprietary register */
                    break;
            }
        }
    }/* end for */
    ...
    for (smi_id = 0; smi_id < DRV_RTL9310_SMI_MAX; smi_id++)
    {
        ...
        ret = reg_field_write(unit, MANGO_SMI_GLB_CTRL0r, smiPrvte0_field[smi_id], &smiPrvte0_val[smi_id]);
        if (ret != RT_ERR_OK)
        {
            RT_LOG(LOG_DEBUG, (MOD_HAL), "unit %d SMI%u private0 set fail 0x%x", unit, smi_id, ret);
        }

        value = 1;
        ret = reg_field_write(unit, MANGO_SMI_GLB_CTRL0r, smiPark_field[smi_id], &value);
        if (ret != RT_ERR_OK)
        {
            RT_LOG(LOG_DEBUG, (MOD_HAL), "unit %d SMI%u polling park set fail 0x%x", unit, smi_id, ret);
        }
    ...
    if (is_10g == 1)
    {
        ...
        {
            RT_ERR(ret, (MOD_DAL|MOD_PORT), "unit %u set MANGO_SMI_10GPHY_POLLING_SEL2r fail %x", unit, ret);
        }

        if ((ret = reg_write(unit, MANGO_SMI_10GPHY_POLLING_SEL3r, &addr_reg9)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_PORT), "unit %u set MANGO_SMI_10GPHY_POLLING_SEL3r fail %x", unit, ret);
        }

        if ((ret = reg_write(unit, MANGO_SMI_10GPHY_POLLING_SEL4r, &addr_reg10)) != RT_ERR_OK)
        {
            RT_ERR(ret, (MOD_DAL|MOD_PORT), "unit %u set MANGO_SMI_10GPHY_POLLING_SEL4r fail %x", unit, ret);
        }
    }
