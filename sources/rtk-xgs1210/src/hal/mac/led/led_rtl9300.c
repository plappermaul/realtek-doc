...
/* Function Name:
 *      rtl9300_led_8284_config
 * Description:
 *      Initialize LED module of port with 8284.
 * Input:
 *      unit - unit id
 * Output:
 *      None
 ...
 *
 */
void rtl9300_led_8284_config(uint32 unit)
{
    uint32 port = 0;
    uint32 val =0;
    uint32 val_f=0;
    if(!osal_strcmp(HWP_IDENTIFIER_NAME(), "RTL9302C_2xRTL8284_2XGE"))
    {

        for(port =0; port <=27; port++)
        {
            val =1;
            reg_array_field_write(unit, LONGAN_LED_PORT_NUM_CTRLr, port, REG_ARRAY_INDEX_NONE, LONGAN_LED_NUM_SELf, &val);

            if(port < 24){
                val_f = 0;
                reg_array_field_write(unit, LONGAN_LED_PORT_COPR_SET_SEL_CTRLr, port, REG_ARRAY_INDEX_NONE, LONGAN_LED_COPR_SET_PSELf, &val_f);
            }else{
                val_f = 1;
                reg_array_field_write(unit, LONGAN_LED_PORT_COPR_SET_SEL_CTRLr, port, REG_ARRAY_INDEX_NONE, LONGAN_LED_COPR_SET_PSELf, &val_f);
            }
