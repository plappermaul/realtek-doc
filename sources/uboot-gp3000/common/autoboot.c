...
void autoboot_command(const char *s)
{
	...
	if (stored_bootdelay != -1 && s && !abortboot(stored_bootdelay)) {
		...
		/* RTK feature
		*  support multicast upgrade here.
		*/
		extern void multicast_stop(void);
		extern int multi_upgrade_enable(void);
		int size;
		if (multi_upgrade_enable() && (size = net_loop(MULTI_UPGRADE)) > 0){
			multicast_stop();
		}
#endif
