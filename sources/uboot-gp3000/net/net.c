...
extern int ca77xx_eth_initialize(bd_t *bis);
#endif
...
#ifdef CONFIG_REALTEK_10G_PON
extern u8 eth_initialize_flag;
     if(eth_initialize_flag == 0){
		eth_initialize_flag = 1;
	 	eth_initialize();
	 }
#endif
	net_restarted = 0;
...
extern void multicast_start(void);
		case MULTI_UPGRADE:
			multicast_start();
			break;
#endif
...
void net_process_received_packet(uchar *in_packet, int len)
{
	...
	case PROT_IP:
		...
		if ((ntohl(dst_ip.s_addr)&0xF0000000) == 0xE0000000)
		{
			(*udp_packet_handler)((uchar *)ip + IP_UDP_HDR_SIZE, 0, src_ip, 0, 0);
			return;
		}
#endif
