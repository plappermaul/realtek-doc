...
static int ubi_clean_vol(char *volume)
{
	int err = 0;
	struct ubi_volume *vol;
...
int ubi_volume_read(char *volume, char *buf, size_t size)
{
	...
	// add UBI reading size info to ENV
	snprintf(msg, sizeof(msg)-1,"%X", size);
	env_set("ubi_len_read", msg);
	snprintf(msg, sizeof(msg)-1,"%X", ((int)buf)+size+0x100);
	env_set("tftp_base_bk", msg);
