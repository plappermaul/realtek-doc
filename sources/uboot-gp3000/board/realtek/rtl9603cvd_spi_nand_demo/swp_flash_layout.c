...
#ifndef CONFIG_OTTO_FL_TO_MTDPARTS
	#define CONFIG_OTTO_FL_TO_MTDPARTS
#endif
...
/* SPI flash setting */
#ifdef CONFIG_OTTO_SPI_NOR_FLASH
/****** SPI NOR flash ******/
...
#define SWP_CONFIG_SYS_REDUNDAND_ENVIRONMENT 1
#else
#error "Error!!! One of CONFIG_OTTO_SPI_NOR_FLASH or CONFIG_OTTO_SNAF  or CONFIG_OTTO_PARALLEL_NAND_FLASHmust be defined"
#endif
...
#ifdef CONFIG_STATIC_CONFIG
	#define SWP_FL_128MB_CFG_ST_BASE (11776 * 1024)
	#define SWP_FL_128MB_CFG_ST_SIZE (128 * 1024)
#endif
#ifdef CONFIG_STATIC_CONFIG
	#define SWP_FL_128MB_K0_BASE (11776 * 1024 + SWP_FL_128MB_CFG_ST_SIZE)
	#define SWP_FL_128MB_K_SIZE (5 * 1024 * 1024 - (128 * 1024))
#else
	#define SWP_FL_128MB_K0_BASE (11776 * 1024)
	#define SWP_FL_128MB_K_SIZE (5 * 1024 * 1024)
#endif
#define SWP_FL_128MB_R0_BASE (SWP_FL_128MB_K0_BASE + SWP_FL_128MB_K_SIZE)
#define SWP_FL_128MB_R_SIZE (20 * 1024 * 1024)
...
#define SWP_FL_128MB_APP_SIZE (48 * 1024 * 1024)
...
#define SWP_FL_256MB_APP_SIZE (128 * 1024 * 1024)
#define SWP_FL_256MB_APP_BASE (SWP_FL_256MB_R1_BASE + SWP_FL_256MB_R_SIZE)
#if defined(CONFIG_RTK_USE_ONE_UBI_DEVICE)
#ifdef CONFIG_STATIC_CONFIG
...
#endif
#else
	#error "Error!!! One of CONFIG_OTTO_SPI_NOR_FLASH or CONFIG_OTTO_SPI_NAND_FLASH  or CONFIG_OTTO_PARALLEL_NAND_FLASHmust be defined"
#endif
...
#else
//	#error "Error!!! One of CONFIG_OTTO_SNOF or CONFIG_OTTO_SNAF or CONFIG_CMD_ONFI must be defined"
#endif
...
static swp_part_info_t part_info[] = {
	...
	[SWP_FL_ENV2] = {"env2", CONFIG_ENV_OFFSET_REDUND_STATIC, 0}, /* ENV2                                                        */
#else
	[SWP_FL_ENV2] = {"env2", CONFIG_ENV_OFFSET_REDUND, 0}, /* ENV2                                                        */
#endif
#else
	[SWP_FL_ENV2] = {"env2", CONFIG_ENV_OFFSET_REDUND, 0}, /* ENV2                                                        */
#endif
	...
	[SWP_FL_ENV] = {"env", CONFIG_ENV_OFFSET, 0}, /* U-Boot environment partition                                */
	[SWP_FL_APP] = {"app", 0, 0},				  /* app                                                         */
...
#else
#error "Get spi nand page size in not CONFIG_OTTO_SPI_NAND_FLASH"
#endif
	return ret_val;
}
...
void swp_adjust_parts_alignment(swp_part_info_t *input_pi_ptr[], u32 num)
{
	...
	spi_nand_flash_info_t *snaf_info = otto_sc.snaf_info;
#if CONFIG_OTTO_SNAF
	block_len = SNAF_PAGE_SIZE(snaf_info) * SNAF_NUM_OF_PAGE_PER_BLK(snaf_info);
#endif
...
#if CONFIG_ENABLE_BLOCK_ALIGNMEMT
#if defined(CONFIG_OTTO_SNAF) || defined(CONFIG_CMD_ONFI)
...
addSingleUbiDeviceBoot(void)
{
	...
	env_set("setbootargs", "setenv bootargs ${bootargs_base} ${more_args} ${mtdparts}");
...
addOttoDefault(void)
{
	...
	env_set("upb", "tftp ${tftp_base} plr.img && crc32 ${fileaddr} ${filesize} && sf erase 0 +${fl_boot_sz} && sf write ${fileaddr} 0 ${filesize}");
	env_set("upk", "tftp ${tftp_base} uImage && crc32 ${fileaddr} ${filesize} && sf erase ${fl_kernel1} +${fl_kernel1_sz} && sf write ${fileaddr} ${fl_kernel1} ${filesize}");
	env_set("upr", "tftp ${tftp_base} rootfs && crc32 ${fileaddr} ${filesize} && sf erase ${fl_rootfs1} +${fl_rootfs1_sz} && sf write ${fileaddr} ${fl_rootfs1} ${filesize}");
	env_set("yu", YFUNC_INIT "&& sf erase 0 +${fl_boot_sz} && sf write 80000000 0 ${filesize}");
	env_set("yk", YFUNC_INIT "&& sf erase ${fl_kernel1} +${fl_kernel1_sz} && sf write 80000000 ${fl_kernel1} ${filesize}");
	env_set("yr", YFUNC_INIT "&& sf erase ${fl_rootfs1} +${fl_rootfs1_sz} && sf write 80000000 ${fl_rootfs1} ${filesize}");
#ifdef CONFIG_LUNA_MULTI_BOOT
	env_set("upk1", "tftp ${tftp_base} uImage && crc32 ${fileaddr} ${filesize} && sf erase ${fl_kernel2} +${fl_kernel2_sz} && sf write ${fileaddr} ${fl_kernel2} ${filesize}");
	env_set("upr1", "tftp ${tftp_base} rootfs && crc32 ${fileaddr} ${filesize} && sf erase ${fl_rootfs2} +${fl_rootfs2_sz} && sf write ${fileaddr} ${fl_rootfs2} ${filesize}");
	...
	env_set("yu", YFUNC_INIT "&& nand erase 0 ${fl_boot_sz} && nand write.raw 80000000 0 ${filesize}");
	...
	env_set("upk", "tftp ${tftp_base} uImage && crc32 ${fileaddr} ${filesize} && nand erase ${fl_kernel1} ${fl_kernel1_sz} && nand write ${fileaddr} ${fl_kernel1} ${filesize}");
	env_set("upr", "tftp ${tftp_base} rootfs && crc32 ${fileaddr} ${filesize} && nand erase ${fl_rootfs1} ${fl_rootfs1_sz} && nand write ${fileaddr} ${fl_rootfs1} ${filesize}");
	env_set("yu", "loady 80000000 && cp.b 80000000 81000000 ${filesize} && cmp.b 80000000 81000000 ${filesize} && nand erase 0 ${filesize} && nand write.raw 80000000 0 ${filesize}");
#ifdef CONFIG_LUNA_MULTI_BOOT
	env_set("upk1", "tftp ${tftp_base} uImage && crc32 ${fileaddr} ${filesize} && nand erase ${fl_kernel2} ${fl_kernel2_sz} && nand write ${fileaddr} ${fl_kernel2} ${filesize}");
	env_set("upr1", "tftp ${tftp_base} rootfs && crc32 ${fileaddr} ${filesize} && nand erase ${fl_rootfs2} ${fl_rootfs2_sz} && nand write ${fileaddr} ${fl_rootfs2} ${filesize}");
	env_set("bootargs_base", "console=ttyS0,115200");
...
#if defined(CONFIG_OTTO_SPI_NAND_FLASH)
