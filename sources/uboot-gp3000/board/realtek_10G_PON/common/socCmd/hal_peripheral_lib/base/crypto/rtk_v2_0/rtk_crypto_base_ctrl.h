...
#define CIPHER_TYPE_DES_ECB         0x00
/// DES_CBC for Crypto ROM Code self-define index.
#define CIPHER_TYPE_DES_CBC         0x01
/// DES_CFB for Crypto ROM Code self-define index.
#define CIPHER_TYPE_DES_CFB         0x02
/// DES_OFB for Crypto ROM Code self-define index.
#define CIPHER_TYPE_DES_OFB         0x03
/// DES_CTR for Crypto ROM Code self-define index.
#define CIPHER_TYPE_DES_CTR         0x04
...
/// 3DES_CFB for Crypto ROM Code self-define index.
#define CIPHER_TYPE_3DES_CFB        0x12
/// 3DES_OFB for Crypto ROM Code self-define index.
#define CIPHER_TYPE_3DES_OFB        0x13
/// 3DES_CTR for Crypto ROM Code self-define index.
...
typedef struct hal_crypto_adapter_s {
    ...
    uint8_t     isSecurekey;
    uint8_t     isWriteback;
    uint8_t     isSecurekeypad;
    uint8_t     securekeyIndex;
    uint8_t     writebackIndex;
    uint8_t     securekeypadIndex;
    uint8_t     securekey[12];
    uint8_t     securekeypad[12];
    uint8_t     reserved5[2];   //!<  Reserve

    /**
     * 32 bytes aligned buffer that store inner and outer padding data
     */
    volatile uint8_t g_IOPAD[CRYPTO_PADSIZE*2+32] __attribute__((aligned(64)));
    uint8_t     reserved_IOPAD[32];   //!<  Reserve 32

    /**
     * 32 bytes aligned buffer that store initial vector for AES_GCM/Chacha20/Chacha_poly1305
     */
    volatile uint8_t  gcm_iv[32] __attribute__((aligned(64)));
    uint8_t     reserved_gcmiv[32];   //!<  Reserve 32

    /**
     * 32 bytes aligned buffer that store command register data of source descriptor
     */
    volatile uint8_t  cl_buffer[32] __attribute__((aligned(64)));
    uint8_t     reserved_cl_buffer[32];   //!<  Reserve 32

    /**
     * 32 bytes aligned buffer that store digest result of Hash
     */
    volatile uint8_t  digest_result[32] __attribute__((aligned(64)));
    uint8_t     reserved_digest_result[32];   //!<  Reserve 32

    /**
     * 32 bytes aligned buffer that store tag result of HMAC
     */
    volatile uint8_t  tag_result[32] __attribute__((aligned(64)));
    uint8_t     reserved_tag_result[32];   //!<  Reserve 32

    /**
     * 32 bytes aligned buffer that store first cache line of source descriptor
     */
    volatile uint8_t  src_first_cache_line[32] __attribute__((aligned(64)));
    uint8_t     reserved_src_first_cache_line[32];   //!<  Reserve 32

    /**
     * 32 bytes aligned buffer that store last cache line of source descriptor
     */
    volatile uint8_t  src_last_cache_line[32] __attribute__((aligned(64)));
    uint8_t     reserved_src_late_cache_line[32];   //!<  Reserve 32

    /**
     * 32 bytes aligned buffer that store first cache line of destination descriptor
     */
    volatile uint8_t  dst_first_cache_line[32] __attribute__((aligned(64)));
    uint8_t     reserved_dst_first_cache_line[32];   //!<  Reserve 32

    /**
     * 32 bytes aligned buffer that store last cache line of destination descriptor
     */
    volatile uint8_t  dst_last_cache_line[32] __attribute__((aligned(64)));
    uint8_t     reserved_dst_last_cache_line[32];   //!<  Reserve 32

    /**
     * 32 bytes aligned buffer that store 64 bytes sequential buffer data of Hash
     */
    volatile uint8_t  hmac_seq_buf[64] __attribute__((aligned(64)));

    /**
     * 32 bytes aligned buffer that store first cache line of crc_dma modeh
     */
    volatile uint8_t  crc_first_cache_line[32] __attribute__((aligned(64)));
    uint8_t     reserved_crc_first_cache_line[32];   //!<  Reserve 32
...
void hal_crypto_engine_get_result_rtl8277c(hal_crypto_adapter_t *pcrypto_adapter, u8 *pDigest, u8 *pResult,
 ...
 * }
 */
int hal_crypto_cipher_init_rtl8277c(hal_crypto_adapter_t *pcrypto_adapter, IN const u32 cipher_type,
                                    IN const u8 *key, IN const u32 keylen);
...
int hal_crypto_mix_mode_process_rtl8277c(hal_crypto_adapter_t *pcrypto_adapter, IN const u8 mix_mode_type,
                                         ...
                                         OUT u8 *pResult, OUT u8 *pTag);
/*
 * crypto rom code for key storage
 */
...
#endif /* __RTK_CRYPTO_BASE_CTRL_H__ */
