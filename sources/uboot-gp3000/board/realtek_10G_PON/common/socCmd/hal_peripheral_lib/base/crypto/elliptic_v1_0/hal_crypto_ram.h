...
#define HAL_CRYPTO1_WRITE32(addr, value32)  HAL_WRITE32(CRYPTO1_REG_BASE, (addr), (value32))
#define HAL_CRYPTO1_READ32(addr)            HAL_READ32(CRYPTO1_REG_BASE, (addr))
#if 0
...
typedef struct __rtl_crypto_dest_s
{
    ...
    u32 resv2;
#if (IPS_DDLEN_NOW == (3<<24))//IPS_DDLEN_20
    u32 icv[5+12]; // also set with IPSCTR's DDLEN 
#elif (IPS_DDLEN_NOW == (2<<24))//IPS_DDLEN_16
    u32 icv[5+8]; // also set with IPSCTR's DDLEN 
#elif (IPS_DDLEN_NOW == (1<<24))//IPS_DDLEN_12
    u32 icv[5+4]; // also set with IPSCTR's DDLEN 
#elif (IPS_DDLEN_NOW == (0<<24))//IPS_DDLEN_8
    u32 icv[5]; // also set with IPSCTR's DDLEN 
#else
   #error "IPS_DDLEN_NOW error"
#endif
} RTL_CRYPTO_DEST_T;
...
//#define IPS_DDLEN_NOW		IPS_DDLEN_20				/* Destination Descriptor Length : Length is 8*DW : for SHA-1/MD5 */
...
#define PHYSICAL_ADDRESS(x) (x)
#define UNCACHED_ADDRESS(x) (x)
#define CACHED_ADDRESS(x)	(x)
...
#define CIPHER_TYPE_MEMCPY_ECB		0x32
...
#define CIPHER_TYPE_FUNC_MEMCPY		0x30
