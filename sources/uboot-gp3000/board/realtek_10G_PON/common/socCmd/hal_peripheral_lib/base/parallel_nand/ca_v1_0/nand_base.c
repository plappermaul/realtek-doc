 ...
 */
int nand_unlock(struct mtd_info *mtd, loff_t ofs, uint32_t len)
{
	int ret = 0;
	int chipnr;
	struct nand_chip *chip = mtd->priv;
 ...
 * Returns lock status.
 */
int nand_lock(struct mtd_info *mtd, loff_t ofs, uint32_t len)
{
	int ret = 0;
				  ...
				  const uint8_t *buf, int oob_required, int page)
{
	...
	return chip->ecc.write_page_raw(mtd, chip, buf, oob_required, page);
}
#endif
		...
		int oob_required, int page, int cached, int raw)
{
	...
	if (unlikely(raw))
		...
		status = chip->ecc.write_page(mtd, chip, buf, oob_required, page);
#endif
	if (status < 0)
		return status;
...
#else
		chip->erase(mtd, page & chip->pagemask);
#endif
		status = chip->waitfunc(mtd, chip);
...
/* Parse the Extended Parameter Page. */
 int nand_flash_detect_ext_param_page(struct mtd_info *mtd,
 ...
 */
 void nand_onfi_detect_micron(struct nand_chip *chip,
