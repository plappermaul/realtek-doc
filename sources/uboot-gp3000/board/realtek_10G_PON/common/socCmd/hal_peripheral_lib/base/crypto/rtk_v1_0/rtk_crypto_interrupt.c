...
//printf("(%s-%d)\n", __func__, __LINE__);
	...
	if (interrupt == 1) {
		...
		crypto_src_buf[0][0]=0x24000000; /* 0xa4000000 -> disable ownbit */
		...
		crypto_src_buf[1][3]=PHYSICAL_ADDRESS(_sha1_plaintext);

		crypto_src_buf[2][0]=0x80000008;
		crypto_src_buf[2][1]=0x00;
		crypto_src_buf[2][2]=0x00000;
		crypto_src_buf[2][3]=PHYSICAL_ADDRESS(_sha1_padding);
...
//invalidate_dcache_range((u32)crypto_dst_buf, ((u32)crypto_dst_buf)+sizeof(crypto_dst_buf));
	...
	if (interrupt == 2) {
		...
		crypto_src_buf[0][0]=0x84000000; /* 0xa4000000 -> disable FS */
		crypto_src_buf[0][1]=0x38;
		crypto_src_buf[0][2]=0x80000;
		crypto_src_buf[0][3]=0x0;
...
//invalidate_dcache_range((u32)crypto_dst_buf, ((u32)crypto_dst_buf)+sizeof(crypto_dst_buf));
	...
	if (interrupt == 6) {
		...
		crypto_src_buf[4][3]=PHYSICAL_ADDRESS(_sha1_plaintext);

		crypto_src_buf[5][0]=0x80000008;
		crypto_src_buf[5][1]=0x00;
		crypto_src_buf[5][2]=0x00000;
		crypto_src_buf[5][3]=PHYSICAL_ADDRESS(_sha1_padding);
