...
#define I2S_TEST_TIME_SEC 15
//#define I2S_TEST_TIME_SEC (43200)

//#define I2S_CODEC_ALC5621_OR_ALC5651 1
...
#ifndef I2S_CODEC_ALC5621_OR_ALC5651
...
#define I2S_PAGE_TIME	100	// unit 0.1ms, test case 10, 25, 50 100, 200, 400
  ...
  #define I2S_PAGE_SIZE (96*I2S_PAGE_TIME*6/10) // max 96kHz, max 5.1channel, 24bit wordlength
#else
  #define I2S_PAGE_SIZE (192*I2S_PAGE_TIME*2/10) // max 96kHz, max 2channel, 24bit wordlength
#endif
...
#if !defined(I2S_CODEC_ALC5621_OR_ALC5651)
SRAM_BF_DATA_SECTION
...
)
{
    ...
    DBG_8195A_DRIVER("I2sIndex = %d\n", I2sIndex);
    DBG_8195A_DRIVER("REG_I2S_CTL = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_CTL));
    DBG_8195A_DRIVER("REG_I2S_TX_PAGE_PTR = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_TX_PAGE_PTR));
    DBG_8195A_DRIVER("REG_I2S_RX_PAGE_PTR = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_RX_PAGE_PTR));
    DBG_8195A_DRIVER("REG_I2S_SETTING = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_SETTING));
    DBG_8195A_DRIVER("REG_I2S_TX_MASK_INT = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_TX_MASK_INT));
    DBG_8195A_DRIVER("REG_I2S_TX_STATUS_INT = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_TX_STATUS_INT));
    DBG_8195A_DRIVER("REG_I2S_RX_MASK_INT = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_RX_MASK_INT));
    DBG_8195A_DRIVER("REG_I2S_RX_STATUS_INT = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_RX_STATUS_INT));
    DBG_8195A_DRIVER("REG_I2S_TX_PAGE0_OWN = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_TX_PAGE0_OWN));
    DBG_8195A_DRIVER("REG_I2S_TX_PAGE1_OWN = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_TX_PAGE1_OWN));
    DBG_8195A_DRIVER("REG_I2S_TX_PAGE2_OWN = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_TX_PAGE2_OWN));
    DBG_8195A_DRIVER("REG_I2S_TX_PAGE3_OWN = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_TX_PAGE3_OWN));
    DBG_8195A_DRIVER("REG_I2S_RX_PAGE0_OWN = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_RX_PAGE0_OWN));
    DBG_8195A_DRIVER("REG_I2S_RX_PAGE1_OWN = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_RX_PAGE1_OWN));
    DBG_8195A_DRIVER("REG_I2S_RX_PAGE2_OWN = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_RX_PAGE2_OWN));
    DBG_8195A_DRIVER("REG_I2S_RX_PAGE3_OWN = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_RX_PAGE3_OWN));
    DBG_8195A_DRIVER("REG_I2S_VERSION_ID = %x\n", HAL_I2S_READ32(I2sIndex, REG_I2S_VERSION_ID));
#endif
}
	...
	i2s_rx[pI2sVerifyPara->I2sIndex].payload_24bit = (s32 *)(0xbfe00000+(8<<10));	
        ...
        for (i=0 ; i<(print_size*(pI2sVerifyPara->I2sPageNum+1))+16; i++) {
            if ((i%8) == 7)
                ...
                DiagPrintf(" %x ", i2s_tx[pI2sVerifyPara->I2sIndex].payload_24bit[i]);
        }
#endif
	...
	i2s_rx[pI2sVerifyPara->I2sIndex].payload_32bit = (s32 *)(0xbfe00000+(8<<10));	
        ...
        for (i=0 ; i<(print_size*(pI2sVerifyPara->I2sPageNum+1))+16; i++) {
            if ((i%8) == 7)
                ...
                DiagPrintf(" %x ", i2s_tx[pI2sVerifyPara->I2sIndex].payload_32bit[i]);
        }
#endif
	...
	i2s_rx[pI2sVerifyPara->I2sIndex].payload = (s32 *)(0xbfd00000+(32<<10));
        ...
        for (i=0 ; i<(print_size*(pI2sVerifyPara->I2sPageNum+1))*2+32; i++) {
            if ((i%8) == 7)
                ...
                DiagPrintf(" %x ", i2s_tx[pI2sVerifyPara->I2sIndex].payload[i]);
...
)
{
    ...
    HAL_I2S_WRITE32(pI2sVerifyPara->I2sIndex, REG_I2S_CTL, BIT_(31)|control_setting);
    HAL_I2S_WRITE32(pI2sVerifyPara->I2sIndex, REG_I2S_CTL, 0|control_setting);
    HAL_I2S_WRITE32(pI2sVerifyPara->I2sIndex, REG_I2S_CTL, BIT_(31)|control_setting);
    ...
    DBG_8195A_DRIVER("[test count:%d]\n", i2s_cnt++);
  #ifndef I2S_CODEC_ALC5621_OR_ALC5651
    DBG_8195A_DRIVER("test mode:loopback test\n");
  #else
    #ifdef I2S_PLAY_TONE_TEST
    DBG_8195A_DRIVER("test mode: play tone, play_channel=%d\n", play_channel);
    sample_count = 0;
    tone_phase = 0;
      #ifdef I2S_TONEGEN_TABLE
...
#if (I2S_DBG_LVL >= 1)
    DBG_8195A_DRIVER("i2scr = %x\n", BIT_(31) | control_setting | BIT_(0));
#endif
    HAL_I2S_WRITE32(pI2sVerifyPara->I2sIndex, REG_I2S_CTL, BIT_(31) | control_setting | BIT_(0));
...
)
{
	...
	DBG_8195A_DRIVER("test i2s mode =%x\n", pI2sVerifyPara->I2sTestMode);
	DBG_8195A_DRIVER("test i2s WordLength =%x\n", pI2sVerifyPara->I2sWordLength);
	DBG_8195A_DRIVER("test i2s Mode =%x\n", pI2sVerifyPara->I2sMode);
	DBG_8195A_DRIVER("test i2s LRSwap =%x\n", pI2sVerifyPara->I2sLRSwap);
	DBG_8195A_DRIVER("test i2s Index =%x\n", pI2sVerifyPara->I2sIndex);
	DBG_8195A_DRIVER("test i2s CHNum =%x\n", pI2sVerifyPara->I2sCHNum);
	DBG_8195A_DRIVER("test i2s PageSize =%x\n", pI2sVerifyPara->I2sPageSize);
	DBG_8195A_DRIVER("test i2s PageNum =%x\n", pI2sVerifyPara->I2sPageNum);
	DBG_8195A_DRIVER("test i2s SampleRate =%x\n", pI2sVerifyPara->I2sSampleRate);
...
)
{
    ...
    i2s_wordlength = I2sVerifyPere.I2sWordLength;
        ...
        if (i2s_wordlength == 1) {
           flush_dcache_range((unsigned long)(i2s_rx[I2sIndex].payload_24bit + i2s_page_size*pindex)&(~(ARCH_DMA_MINALIGN-1)), (unsigned long)(i2s_rx[I2sIndex].payload_24bit + i2s_page_size*(pindex+1)+7)&(~(ARCH_DMA_MINALIGN-1)));
           invalidate_dcache_range((unsigned long)(i2s_rx[I2sIndex].payload_24bit + i2s_page_size*pindex)&(~(ARCH_DMA_MINALIGN-1)), (unsigned long)(i2s_rx[I2sIndex].payload_24bit + i2s_page_size*(pindex+1)+7)&(~(ARCH_DMA_MINALIGN-1)));
       } else if (i2s_wordlength == 2) {
           flush_dcache_range((unsigned long)(i2s_rx[I2sIndex].payload_32bit + i2s_page_size*pindex)&(~(ARCH_DMA_MINALIGN-1)), (unsigned long)(i2s_rx[I2sIndex].payload_32bit + i2s_page_size*(pindex+1)+7)&(~(ARCH_DMA_MINALIGN-1)));
           invalidate_dcache_range((unsigned long)(i2s_rx[I2sIndex].payload_32bit + i2s_page_size*pindex)&(~(ARCH_DMA_MINALIGN-1)), (unsigned long)(i2s_rx[I2sIndex].payload_32bit + i2s_page_size*(pindex+1) + 7)&(~(ARCH_DMA_MINALIGN-1)));
       } else {
           flush_dcache_range((unsigned long)(i2s_rx[I2sIndex].payload + 2*i2s_page_size*pindex)&(~(ARCH_DMA_MINALIGN-1)), (unsigned long)(i2s_rx[I2sIndex].payload + 2*i2s_page_size*(pindex+1)+63)&(~(ARCH_DMA_MINALIGN-1)));
           invalidate_dcache_range((unsigned long)(i2s_rx[I2sIndex].payload + 2*i2s_page_size*pindex)&(~(ARCH_DMA_MINALIGN-1)), (unsigned long)(i2s_rx[I2sIndex].payload + 2*i2s_page_size*(pindex+1)+63)&(~(ARCH_DMA_MINALIGN-1)));
       }
#ifndef I2S_CODEC_ALC5621_OR_ALC5651
    j=0;
    for (i=0 ; i<i2s_page_size*(i2s_wordlength?1:2) ; i++) {
        ...
        if (i2s_wordlength == 1) {
			...
			// 20141006 jwsyu notes: if page size is not equal = cache line size.
               ...
               if ((i2s_page_size&0x15) && (i > (i2s_page_size-16)))
		...
		i2s_err_cnt++;
           ...
           if (i2s_isr_cnt < (9992/i2s_page_time*I2S_TEST_TIME_SEC-8)) {
			// 20141006 jwsyu notes: if page size is not equal = cache line size.
                        // the page 0 final word and page 1 start word are share cache line.
                        // write the page 0 final word will error. the cache line will dirty.
               if ((i2s_page_size&0x15) && (i > (i2s_page_size*2-32)))
			;//let the this page final word and next page start word cache line clean;
		else
...
)
{
	...
	UnlockLx1Bus();	
#endif
#if defined(I2S_CODEC_ALC5621_OR_ALC5651) && defined(I2S_PLAY_TONE_TEST)
        ...
        if (I2sVerifyPere.I2sWordLength == 1) {
            ...
            for (i=0 ; i<(print_size*I2sPageNum+1)+16; i++) {
                if ((i%8) == 7)
                    ...
                    DiagPrintf(" %x ", i2s_rx[I2sIndex].payload_24bit[i]);
            }
        } else if (I2sVerifyPere.I2sWordLength == 2) {
            u32 print_size;
            print_size = (I2sPageSize>270)? 270: I2sPageSize;
            ...
            for (i=0 ; i<(print_size*I2sPageNum+1)+16; i++) {
                if ((i%8) == 7)
                    ...
                    DiagPrintf(" %x ", i2s_rx[I2sIndex].payload_32bit[i]);
            }
        } else {
            u32 print_size;
            print_size = (I2sPageSize>270)? 270: I2sPageSize;
            ...
            for (i=0 ; i<(print_size*I2sPageNum*2)+16; i++) {
                if ((i%8) == 7)
                    ...
                    DiagPrintf(" %x ", i2s_rx[I2sIndex].payload[i]);
            }
