...
#ifdef CONFIG_FPGA
#define SRAM_STR_BASE 0x20080000	// SRAM space to store golden value
#define SPI_TEST_ADDR 0x04040000
...
)
{
    ...
    // set ctrlr0: TX mode
    HAL_SPI_WRITE32(REG_SPIC_CTRLR0, 
                (HAL_SPI_READ32(REG_SPIC_CTRLR0)& 0xFFF0CFF));
...
)
{
    ...
    HAL_SPI_WRITE32(REG_SPIC_ADDR_LENGTH, BIT_ADDR_PHASE_LENGTH(Info));
...
) 
{
    ...
    switch(CpuClkMode) {
        case 0:
            ...
            pTempSpinInitPara = (PSPIC_INIT_PARA) &SpicInit200MCLK[BaudRate-1];
            break;
        case 1:
            if (BaudRate > 2) {
                BaudRate = 2;
            }
            pTempSpinInitPara = (PSPIC_INIT_PARA) &SpicInit100MCLK[BaudRate-1];            
            break;
        case 2:            
            if (BaudRate > 1) {
                BaudRate = 1;
            }
            pTempSpinInitPara = (PSPIC_INIT_PARA) &SpicInit50MCLK[BaudRate-1];     
            break;
        default:
            break;
    }
    ...
    pSpicInitPara->DelayLine = pTempSpinInitPara->DelayLine;
    pSpicInitPara->RdDummyCyle = pTempSpinInitPara->RdDummyCyle;
