/*
 * Configuration for Cortina-Access VENUS platform. Parts were derived from
 * other ARM configurations.
 *
 * SPDX-License-Identifier:	GPL-2.0+
 */

#ifndef __VENUS_ASIC_H
#define __VENUS_ASIC_H
...
#ifdef CONFIG_RTK_FPGA_VENUS
#	define VENUS_APB_CLK (25000000) /* Use external clock source */
#else
#	define VENUS_APB_CLK (125000000)
#endif
#define CORTINA_PER_IO_FREQ             VENUS_APB_CLK
#define CONFIG_UART_CLOCK (VENUS_APB_CLK)
...
#define NAND_BBL_OFFSET         (0x440000)  /* 4.25MB */
#define NAND_BBL_OFFSET_REDUND  (NAND_BBL_OFFSET + NAND_BBL_SIZE)  /* OFFSET +  256K */
#else
#define NAND_BBL_OFFSET         (0x180000)  /* 1.5MB */
#define NAND_BBL_OFFSET_REDUND  (0x1C0000)  /* 1.5MB +  256K */
#endif /* CONFIG_BL_IMAGE_INC_OPTEED */
    ...
    #define CONFIG_SYS_MAX_NAND_DEVICE 1
...
#ifdef CONFIG_BL_IMAGE_INC_OPTEED
            #define CONFIG_ENV_OFFSET         (0x4C0000)  /* 4.25MB +  256K x 2*/
            #define CONFIG_ENV_OFFSET_REDUND  (0x500000)
#else
            #define CONFIG_ENV_OFFSET         (0x200000)  /* 1.5MB +  256K x 2*/
            #define CONFIG_ENV_OFFSET_REDUND  (0x240000)
#endif /* End CONFIG_BL_IMAGE_INC_OPTEED */
            ...
            #define CONFIG_EXTRA_ENV_SETTINGS \
                        "pfl_env1_r="CONFIG_ENV_RANGE"\0" \
                        "pfl_env2_r="CONFIG_ENV_OFFSET_REDUND"\0" \
                        "pfl_env_size="CONFIG_ENV_SIZE"\0" \
                        "unzip_src=0x5000000\0" \
                        "tftp_base=0x5000000\0" \
                        "upenv=tftpboot ${tftp_base} uboot-env.bin && nand erase " \
                        "${pfl_env1_r} ${pfl_env_size} && nand write ${tftp_base} " \
                        "${pfl_env1_r} ${filesize} && nand erase ${pfl_env2_r} " \
                        "${pfl_env_size} && nand write ${tftp_base} ${pfl_env2_r} " \
                        "${filesize}\0" \
                        "active_port=0\0" \
                        "auto_scan_active_port=0\0"
	...
	#ifndef CONFIG_ENV_OFFSET
        #define CONFIG_ENV_OFFSET         (0x180000)  /* 1.5MB */
        #define CONFIG_ENV_OFFSET_REDUND  (0x1A0000)
        #define CONFIG_ENV_RANGE          (0x20000)
        #define CONFIG_ENV_SIZE           (0x20000) //(512 * 1024)
        #define CONFIG_ENV_SECT_SIZE      (CONFIG_ENV_SIZE)
	#endif
#endif

#ifdef CONFIG_CORTINA_PNAND
...
#	define CONFIG_G3_NAND_HWECC
#	define CONFIG_SYS_NAND_SELF_INIT

#	if CONFIG_CORTINA_PNAND_ECC_LEVEL == 0
 ...
 * I2C support
 */
#ifdef CONFIG_TARGET_RTK_VENUS_FT
#define CONFIG_SYS_I2C
#define CONFIG_SYS_I2C_BUS_MAX      1
#define CONFIG_SYS_I2C_BASE         PER_BIW0_CFG
/* 400Khz is typical speed for most device */
#define CONFIG_SYS_I2C_SPEED        400000
#define CONFIG_SYS_NUM_I2C_BUSES    2
#define CONFIG_SYS_I2C_MAX_HOPS     1
#define CONFIG_SYS_I2C_BUSES    {{0, {I2C_NULL_HOP} }, \
                                {0, {{I2C_MUX_PCA9544, 0x70, 3}}}}
#endif

#endif /* __VENUS_ASIC_H */
