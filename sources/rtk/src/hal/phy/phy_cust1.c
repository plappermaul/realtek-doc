...
_phy_identify_aqGen3(uint32 unit, rtk_port_t port, uint32 reg1d3)
{
    ...
    if (reg1d3 == 0x1C80) /* AQRxxx */
    {
        PHY_AQR_LOG("U%u P%u match AQRxxx", unit, port);
        return RT_ERR_OK;
    }    

    if (reg1d3 == 0x1C82) /* AQRxxx */
    {
        PHY_AQR_LOG("U%u P%u match AQRxxx", unit, port);
        return RT_ERR_OK;
    }    
...
_aqApi_rtctResult_get(AQ_API_Port *paq_port, rtk_rtctResult_t *pRtctResult)
{
    ...
    _aqApi_basicCableDiagResults_to_rtkRtctChannelStatus(&results.pairAResult, &pRtctResult->channels_result.a);
    _aqApi_basicCableDiagResults_to_rtkRtctChannelStatus(&results.pairAResult, &pRtctResult->channels_result.b);
    _aqApi_basicCableDiagResults_to_rtkRtctChannelStatus(&results.pairAResult, &pRtctResult->channels_result.c);
    _aqApi_basicCableDiagResults_to_rtkRtctChannelStatus(&results.pairAResult, &pRtctResult->channels_result.d);
    return RT_ERR_OK;
}
...
)
{
	...
	*/
	...
	{
#ifdef AQ_VERBOSE
		phy_osal_printf("allocate memory failed\n");
#endif
		return AQ_RET_ERROR;
	}
